########################## TCL Event Handlers ##########################
#
#  sinumerik_840D_lathe.tcl - lathe
#
#    This is a 2-Axis Horizontal Lathe Machine.
#
#  Created by Postino @ Fri Mar  9 12:19:44 2018 -0800
...
# 20-Mar-2011 gsl - Enhanced cycle95 handling
# 28-Mar-2011 gsl - Refine PB_CDM_define_cycle95_handlers and call it in
#                   PB_CMD_init_cycle95_output
# 30-Mar-2011 gsl - Refine PB_CMD_init_cycle95_output
# 12-Apr-2011 lili - Add "LBL_END" option into cycle95 output mode
# 15-Apr-2011 lxy - Use Event marker to replace the definition of "MOM_contour_start" &
#                   "MOM_contour_end" in PB_CMD_define_cycle95_handlers
# 27-Oct-2015 szl - PR6801224: Add PB_CMD_uplevel_MOM_generic_cycle and
#                   PB_CMD_handle_generic_cycle_start_motion to enhance cycle95,
#                   update PB_CMD_contour_end and PB_CMD_init_cycle95_output.
#                   PB_CMD_init_cycle95_output is removed from first Approach move.
...
#=============================================================
  ...
  global mom_motion_event
   ...
   if { [PB_CMD__check_block_rapid_move] } {
...
#=============================================================
proc PB_CMD__check_block_rapid_move { } {
#=============================================================
# This custom command should return
#   1 : Output
#   0 : No output
   ...
   global mom_logname
   global mom_spindle_mode mom_feed_cut_unit
   global mom_sys_spindle_mode_code mom_spindle_status

   if { $mom_spindle_mode == "SFM" || $mom_spindle_mode == "SMM" } {
      if { ![string compare $mom_feed_cut_unit "ipm"] || ![string compare $mom_feed_cut_unit "mmpm"] } {
         set mom_sys_spindle_mode_code($mom_spindle_status) "961"
      } else {
         set mom_sys_spindle_mode_code($mom_spindle_status) "96"
      }
   }
...
#=============================================================
   global mom_spindle_mode feed_mode mom_sys_spindle_mode_code
   global mom_motion_event mom_machine_mode mom_spindle_status

   # In turning switch between G96 and G961
   if { $mom_machine_mode == "TURN" } {
      if { ($mom_motion_event == "linear_move" || $mom_motion_event == "circular_move" ) &&\
         ($mom_spindle_mode == "SFM" || $mom_spindle_mode == "SMM") } {
