...
#
#  Created by yyk4wq @ Tuesday, May 17, 2016 5:32:21 PM China Standard Time
...
# which will be output as the parameters of rough turning cycle command.
  if {$mom_sys_cycle_seq_num_on==0} {
     ...
     if {![info exists mom_seqnum]} {
        ...
        set dpp_turn_cycle_seqno_begin [ expr $mom_def_sequence_start+2*$mom_sys_seqnum_incr ]
        set dpp_turn_cycle_seqno_end [ expr $mom_def_sequence_start+3*$mom_sys_seqnum_incr ]

     } else {

        set dpp_turn_cycle_seqno_begin [ expr $mom_seqnum+2*$mom_sys_seqnum_incr ]
        set dpp_turn_cycle_seqno_end [ expr $mom_seqnum+3*$mom_sys_seqnum_incr ]
     }

  } else {

     set dpp_turn_cycle_seqno_begin [ expr $mom_seqnum+2*$mom_sys_seqnum_incr ]
     set dpp_turn_cycle_seqno_end [ expr $mom_seqnum+(1+$dpp_contour_list_length)*$mom_sys_seqnum_incr ]
  }
...
# post is equipped with the ability to output rough turning cycle:
  if { ([info exists mom_machine_control_motion_output] && $mom_machine_control_motion_output == 2) && \
     ...
     # Other scenarios
        ...
        MOM_display_message "$mom_operation_name: The machine cycle motion has not been supported in current operation type so far. \
...
#=============================================================
proc PB_CMD_output_additional_profiling { } {
#=============================================================
# This command is to check whether output additional profiling data or not.
...
# 25-Dec-2015 Shuai - Initial version
  ...
  if { [string match "FACING" $mom_template_subtype] || [string match "*ROUGH*" $mom_template_subtype] } {
     ...
     if { $mom_sys_output_contour_motion == 1 } {
        ...
        if { $mom_finishing_cut_method == 7 } {
           ...
           if { [EQ_is_equal $mom_finish_equidistant_stock $mom_stock_part] && \
              ...
              MOM_display_message "$mom_operation_name: Profile stock should be the same with rough stock. \
           ...
           MOM_display_message "$mom_operation_name: Strategy of path settings should be Finish All. \
                               \nPost processing will be aborted." "Postprocessor Error Message" "E"
           MOM_abort " *** User Abort Post Processing *** "
        }
        ...
        MOM_display_message "$mom_operation_name: Additional Profiling can only be used when Cutter Compensation UDE is set. \
                            \nPost processing will be aborted." "Postprocessor Error Message" "E"
        MOM_abort " *** User Abort Post Processing *** "
     }
     ...
     MOM_display_message "$mom_operation_name: Additional Profiling can only be used in FACING or ROUGH operation. \
                         \nPost processing will be aborted." "Postprocessor Error Message" "E"
     MOM_abort " *** User Abort Post Processing *** "
  }
}
...
#=============================================================
# This command is to output turning cycle command
# depending on the operation template type.
#
...
#=============================================================
# This command is to override rough contour data with corresponding finish when cutter compensation UDE is not set.
...
# 25-Dec-2015 Shuai - Initial version
  ...
  if { $res == 1 } {
     ...
     MOM_display_message "$mom_operation_name: Fail to run the operation $mom_machine_cycle_subroutine_name with extended command MOM_post_oper_path. \
                         \nPlease recheck the parameters. Post processing will be aborted." "Postprocessor Error Message" "E"
     MOM_abort " *** User Abort Post Processing *** "
  }
}
...
# Set rough turning cycle type according to the step angle
  if { [EQ_is_equal $mom_level_step_angle 270] || [EQ_is_equal $mom_level_step_angle 90] } {
    ...
    set dpp_turn_cycle_msg "ROUGH TURN CYCLE"
      ...
      MOM_display_message "$mom_operation_name: Turning cycle type could be set by variable mom_level_step_angle now.\
                          \nPlease recheck the parameters. Post processing will be aborted." "Postprocessor Error Message" "E"
      MOM_abort " *** User Abort Post Processing *** "
  }
}
...
#
# 05-30-2013 levi  - Initial version
# 10-28-2015 shuai - Output return motion NC codes.
# 12-25-2015 shuai - Modify the code structure so as to make logical more clear.
...
# If user adds profile in tool path, some conditions should be checked to fulfill.
  if {[info exists mom_profiling] && ($mom_profiling == 1)} {
     ...
     PB_CMD_output_additional_profiling
  }
  ...
  if {$mom_generic_cycle_status == 1} {
     ...
     if {([info exists mom_from_status] && $mom_from_status == 1) || \
      ...
      ##++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        MOM_display_message "$mom_operation_name: None of from point, start point or approach path is defined before turning cycle in this operation.\
                            \nPlease recheck the parameters. Post processing will be aborted." "Postprocessor Error Message" "E"
        MOM_abort " *** User Abort Post Processing *** "
     }
  }
