...
#
#  Created by ndcep8 @ Tuesday, April 25, 2017 5:22:29 PM China Standard Time
...
# 28-Oct-2015 shuai - Add PB_CMD_uplevel_MOM_generic_cycle to check non-cutting motions.
#                     Output contour data or tracking data based on if UDE (Cutter Compensation)
#                     has been set. Make the return motion output after lathe rough cycle G71/G71.
# 05-Nov-2015 shuai - Output finish operation G70 using previous rough contour data
#                     if the subroutine name is same between them.
# 25-Dec-2015 shuai - Add command PB_CMD_check_additional_profiling, PB_CMD_check_first_buffer_for_return_motion,
#                     PB_CMD_finish_turn_cycle_contour_start, PB_CMD_finish_turn_cycle_contour_end,
#                     PB_CMD_output_turning_cycle_command, PB_CMD_override_rough_contour_data_with_finish,
#                     PB_CMD_set_contour_motion, PB_CMD_calculate_contour_line_number,
#                     PB_CMD_set_turning_cycle_type and PB_CMD_calculate_parameters_for_turning_cycle_block.
# 17-May-2016 Jintao - Add mom_kin_lathe_cycle_workplane_transform to fix PR6763943
# 25-Nov-2016 shuai - Improve the functionality of Lathe Rough Cycle (G70/ G71/G72). The following issues have been enhanced.
#                      - 1. In MOM_generic_cycle event:
#                         - 1.1 Remove the checking condition for G70\G71\G72 that whether a from point has been set or not.
#                         - 1.2 If a finish operation uses the previous corresponding contour of a rough turning operation, it will also need to skip to the next generic event.
#                      - 2. Lathe rough cycle has been initialized in MOM_generic_cycle, so remove the command "PB_CMD_init_rough_turn_cycle_output" from proc "PB_approach_move".
#                      - 3. In PB_CMD_init_rough_turn_cycle_output, additional profiling should be checked to fulfill some conditions when initializing contour data.
#                      - 4. Add a new proc "PB_CMD_check_settings_for_additional_profiling" to check the settings for additional profile.
#                      - 5. Add a new proc "PB_CMD_check_variables_for_turning_rough_cycle" to check some variables for turning rough cycle.
#                      - 6. Adjust the line number according to turning operation type in PB_CMD_calculate_contour_line_number so as to get the correct values for parameters P and Q.
#                      - 7. Exclude the own name of rough turning operation from the operation name list in command PB_CMD_turn_cycle_contour_end
#                           when contour data of rough operation is overridden by finish operation.
...
#                     Delete the original command "PB_CMD_output_spindle". Rename the command "PB_CMD_output_spindle_speed" to "PB_CMD_output_spindle".
#                     Update the command "PB_CMD_calculate_contour_line_number".
# 21-Dec-2016 shuai - Bug fix PR7290203.
#                     Refactor the output logic for lathe rough cycle function
#                     to make the events content between the second MOM_generic_cycle (EXCLUDE) and MOM_contour_end (INCLUDE)
#                     output at the end position of MOM_contour_end event.
#                     1. Update the handlers.
#                        "MOM_linear_move", "MOM_circular_move", "MOM_contour_start" and "MOM_generic_cycle".
#                     2. Update the commands.
#                        "PB_CMD_calculate_parameters_for_turning_cycle_block", "PB_CMD_finish_turn_cycle_contour_start", "PB_CMD_finish_turn_cycle_contour_end",
#                        "PB_CMD_init_rough_turn_cycle_output", "PB_CMD_output_turning_cycle_command", "PB_CMD_override_rough_contour_data_with_finish",
#                        "PB_CMD_turn_cycle_contour_start", "PB_CMD_turn_cycle_contour_end" and "PB_CMD_uplevel_MOM_generic_cycle".
#                     3. Delete the commands.
#                        "PB_CMD__check_block_linear_for_rough_turn_cycle", "PB_CMD__check_block_circular_move_for_rough_turn_cycle", "PB_CMD__check_block_skip_for_rough_turn_cycle",
#                        "PB_CMD__check_block_rapid_move_for_return_motion", "PB_CMD_check_first_buffer_for_return_motion" and "PB_CMD_calculate_contour_line_number".
#                     5. Delete the blocks.
#                        "circular_move_rough_turn_cycle" and "linear_move_rough_turn_cycle".
#                     6. Enhance the MOM_abort command with "catch" to make it not output error message.
#                     7. Enhance the temp file name with clock time to make it unique in current output folder.
# 25-April-2017 lili - Update implementation of lathe rough cycle function. All related commands please see PB_CMD_init_turn_cycle_setting, PB_CMD_uplevel_MOM_generic_cycle,
#                      PB_CMD_turn_cycle_contour_start and PB_CMD_turn_cycle_contour_end.
#                      Add PB_CMD_init_turn_cycle_setting at "start of program".
}
  ...
  set mom_sys_alt_unit_post_name                "fanuc__MM.pui"
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_lathe_cycle_workplane_transform   "TRUE"
...
#=============================================================
   ...
   PB_CMD_init_turn_cycle_setting
...
# 28-Oct-2015 shuai - Output contour data on condition that UDE (Cutter Compensation)
#                     has been set, otherwise output tracking data.
# 05-Nov-2015 shuai - Output finish operation G70 using previous rough contour data
#                     if the subroutine name is same between them.
# 25-Dec-2015 shuai - Modify the code structure so as to make logical more clear.
# 25-Nov-2016 shuai - Add the variable dpp_fanuc_turning_cycle_type.
# 03-Jan-2017 shuai - Bug fix PR7290203.
# 19-Jan-2017 lili  - Refine code, enhance comments

# ==> This command should be called in the MOM_generic_cycle event for rough and
#     finish turning operations with Machine output mode set to cycle for the
#     G70/G71/G72 call.
#
#-------------------------------------------------------------------------------------
# This command gathers information to determine if NX/Post should
# process contour data and produce subprogram & G70/G71/G72 instructions.
#
# The state of variable "mom_machine_control_motion_output" will indicate when turning
# processor will produce contour data whereas the variable "mom_sys_output_cycle95"
# indicates that post is equipped with the ability to output turning cycle.
#
# Rough turning cycle G71/G72 output can be activated in the event below:
#
#   "Program & Tool Path"
#      -> "Program"
#          -> "Canned Cycles" -> "Lathe Roughing"
#
#*************************************************************************************
# If contour data is ready, MOM events list of tool path will be
#
# MOM_start_of_path
# MOM_initial_move
# MOM_generic_cycle
...
#
#**************************************************************************************
# Sequence output mode for contour codes can be swtiched in "PB_CMD_init_turn_cycle_setting"
# G70/G71/G72 and countour output is addressed from "MOM_contour_end"
#
# This proc is used to determine output contour motion type
# "mom_sys_output_contour_motion" can be set [ 0 | 1 | 2 ].
#  0: No contour output
#  1: Part contour            - need cutcom UDE, G41 or G42 will be outputed.
#  2: Tracking path contour   - without cutcom UDE, contour is trackiong point tool path
#
# This proc is also used to check if "G70" will be output following "G71/G72" in two situations
# "dpp_G70_output" can be set ["Profile" | "Finish"].
#  "Profile" : Rough turning operation with additional profiling.
#  "Finish"  : The name of finish turning operation is same as subroutine name in rough turning operation,
#              And the finish turning operation is set to machine cycle output and has same subroutine name.
#
#  *NOTE* End users need to make sure they have same countour, post will not do contour match
#     checking.
#
#  Output:
#         mom_sys_output_contour_motion
#         dpp_G70_output
#***************************************************************************************
#
...
return
     ...
     if {![string match "FACING" $mom_template_subtype] && ![string match "*ROUGH*" $mom_template_subtype] && \
         ![string match "*FINISH*" $mom_template_subtype]} {
return
     }
...
return
     ...
     if {![EQ_is_equal $mom_level_step_angle 270] && ![EQ_is_equal $mom_level_step_angle 90] &&\
         ![EQ_is_equal $mom_level_step_angle 180] && ![EQ_is_equal $mom_level_step_angle 0]} {
        MOM_output_to_listing_device "$mom_operation_name: Level Angle can only be 0, 90, 180 or 270 for G70/G71/G72 cycles"
        set result "User Error"
     }
     ...
     if {([string match "FACING" $mom_template_subtype] || [string match "*ROUGH*" $mom_template_subtype])} {
        ...
        #-----------------------------------------
        # Check rough operations with profiling
        #-----------------------------------------
        if {[info exists ::mom_profiling] && $::mom_profiling == 1} {
           ...
           # Strategy of path settings must be Finish All
           ...
           if {![EQ_is_equal $::mom_finish_equidistant_stock $::mom_stock_part] || \
              ...
              MOM_output_to_listing_device "$::mom_operation_name: Additional Profiling can only be used when profile stock is the same as rough stock."
              set result "User Error"
           }

           if {[info exists result] && $result == "User Error"} {
              MOM_output_to_listing_device "Switch output mode from Machine-Cycle to Non-Machine-Cycle."
   return
           } else {

              # Set G70 output mode for rough operation with profiling

              set dpp_G70_output($mom_operation_name) "Profile"
           }
        } else {
           ...
           ##############################################################################
           if { [info exists mom_operation_name_list] && [info exists mom_machine_cycle_subroutine_name] && \
              ...
              set finish_temp_file [file native "$mom_output_file_directory/${mom_machine_cycle_subroutine_name}_[clock clicks]"]
              MOM_post_oper_path $mom_machine_cycle_subroutine_name ${finish_temp_file}.ptp
              catch {file delete -force ${finish_temp_file}.ptp}

              if {[file exists ${finish_temp_file}.tcl]} {

                 # get dpp_turn_cycle(finish_contour_data)
                 source ${finish_temp_file}.tcl
                 catch {file delete -force ${finish_temp_file}.tcl}

                 if {![info exists mom_cutcom_status]} {set mom_cutcom_status "UNDEFINED"}
                 if {($dpp_turn_cycle(finish_cutcom_mode) == "UNDEFINED" || $dpp_turn_cycle(finish_cutcom_mode) == "OFF") || \
                     ($mom_cutcom_status == "UNDEFINED" || $mom_cutcom_status == "OFF")} {
                    set dpp_turn_cycle(finish_cutcom_mode) $mom_cutcom_status
                 }

                 if {[info exists mom_cutcom_status] && $dpp_turn_cycle(finish_cutcom_mode) != $mom_cutcom_status} {
                    MOM_output_to_listing_device "$mom_cutcom_status $dpp_turn_cycle(finish_cutcom_mode)"
                    MOM_output_to_listing_device "Cutcom mode in $mom_operation_name and $mom_machine_cycle_subroutine_name don't match"
                 } else {
                    set dpp_G70_output($mom_machine_cycle_subroutine_name) "Finish"
                 }
...
#=============================================================
proc PB_CMD_init_turn_cycle_setting { } {
#=============================================================
# This proc is used to set G70/G71/G72 subprog sequence number mode.
...
# Otherwise all the sequence numbers will be output.
  ...
  set ::dpp_turn_cycle_finish_code 70
...
# 05-17-2016 ljt - PR6763943, add mom_kin_lathe_cycle_work_plane_transform

  global mom_lathe_spindle_axis
  global mom_cycle_spindle_axis
  global mom_cycle_rapid_to_pos mom_cycle_feed_to_pos mom_cycle_retract_to_pos

   if { [info exists mom_lathe_spindle_axis] &&\
        ( ![info exists ::mom_kin_lathe_cycle_workplane_transform] ||\
          $::mom_kin_lathe_cycle_workplane_transform != "TRUE" ) } {

      if { $mom_lathe_spindle_axis == "MCSX" } {
      ...
      } elseif { $mom_lathe_spindle_axis == "MCSY" } {

         set rapid_to $mom_cycle_rapid_to_pos(1)
         set mom_cycle_rapid_to_pos(1) $mom_cycle_rapid_to_pos(2)
         set mom_cycle_rapid_to_pos(2) $rapid_to

         set feed_to $mom_cycle_feed_to_pos(1)
         set mom_cycle_feed_to_pos(1) $mom_cycle_feed_to_pos(2)
         set mom_cycle_feed_to_pos(2) $feed_to

         set retract_to $mom_cycle_retract_to_pos(1)
         set mom_cycle_retract_to_pos(1) $mom_cycle_retract_to_pos(2)
         set mom_cycle_retract_to_pos(2) $retract_to
      }
   }
...
#=============================================================
proc PB_CMD_set_parameters_for_turning_cycle_block { } {
#=============================================================
...
# 03-Jan-2017 Shuai - Bug fix PR7290203.
#                     Remove the output of CUTCOM in this command.
# 19-Jan-2017 lili  - Add unset dpp_save_cutcom_status
...
# 21-Dec-2016 shuai - (PR7290203) Refactor the output logic for lathe rough cycle function.
#
  global mom_sys_output_contour_motion
  global mom_cutcom_status
  global mom_template_subtype
  ...
  MOM_set_seq_on
  set dpp_turn_cycle(contour_seq_end) $mom_seqnum

  if {![info exists dpp_G70_output($mom_operation_name)] || $dpp_G70_output($mom_operation_name) != "Finish"} {
     MOM_do_template turn_cycle_end_tag
  }


  if {[info exists ::dpp_sequence_output_status_save]} {

     if {![string compare "on" $::dpp_sequence_output_status_save] } {
        MOM_set_seq_on
     } else {
        MOM_set_seq_off
     }
  ...
  MOM_close_output_file $dpp_turn_cycle(contour_file_name)

  set ::dpp_turn_cycle_seqno_begin $dpp_turn_cycle(contour_seq_start)
  set ::dpp_turn_cycle_seqno_end $dpp_turn_cycle(contour_seq_end)


  set contour [open $dpp_turn_cycle(contour_file_name) "r"]
  set c_lines [split [read $contour] "\n"]
  close $contour
  catch {file delete -force $dpp_turn_cycle(contour_file_name)}

  MOM_open_output_file $ptp_file_name
  set ::mom_sys_ptp_output "ON"

  if {[string match "FACING" $mom_template_subtype] || [string match "*ROUGH*" $mom_template_subtype]} {
     MOM_output_text  "[lindex $c_lines 0]"
     set pq [MOM_do_template lathe_roughing_seq CREATE]
     regsub $dpp_turn_cycle(seq_temp) [lindex $c_lines 1] $pq rough_cycle_template

     MOM_output_text "$rough_cycle_template"

     if {[info exists dpp_G70_output($mom_machine_cycle_subroutine_name)] && $dpp_G70_output($mom_machine_cycle_subroutine_name) == "Finish"} {
        set dpp_turn_cycle(finish_save_pq) $pq
     }
  }

  if {[info exists dpp_G70_output($mom_operation_name)]  && $dpp_G70_output($mom_operation_name) == "Profile" } {

      set pq [MOM_do_template lathe_roughing_seq CREATE]
      regsub $dpp_turn_cycle(seq_temp) [lindex $c_lines 2] $pq finish_cycle_template
      MOM_output_text "$finish_cycle_template"
  } elseif { [string match "*FINISH*" $mom_template_subtype]} {
      if {![info exists dpp_G70_output($mom_operation_name)] || $dpp_G70_output($mom_operation_name) != "Finish"} {
         set pq [MOM_do_template lathe_roughing_seq CREATE]
      } else {
         set pq $dpp_turn_cycle(finish_save_pq)
      }
      regsub $dpp_turn_cycle(seq_temp) [lindex $c_lines 0] $pq finish_cycle_template
      MOM_output_text "$finish_cycle_template"
  }


  foreach line $c_lines {
     if {![string match "*G70*" $line] && ![string match "*G71*" $line] && ![string match "*G72*" $line] } {
...
#
# 30-May-2013 levi  - Initial version.
# 28-Oct-2015 shuai - Add flag dpp_return_motion_start to indicate return motion end.
# 25-Dec-2015 shuai - Modify the code structure so as to make logical more clear.
# 21-Dec-2016 shuai - (PR7290203) Refactor the output logic for lathe rough cycle function.
# 09-Jan-2017 lili  - Move "set turning cycle type" into MOM_generic_cycle.

  global dpp_turn_cycle
  global mom_seqnum
  global mom_machine_cycle_subroutine_name
  global dpp_G70_output
  global dpp_sequence_output_status_save
  global mom_sys_cycle_seq_num_on
  global mom_operation_name

  set dpp_sequence_output_status_save [MOM_set_seq_on]
  set dpp_turn_cycle(contour_seq_start) $mom_seqnum

  if {![info exists dpp_G70_output($mom_operation_name)] || $dpp_G70_output($mom_operation_name) != "Finish"} {
     MOM_do_template turn_cycle_start_tag
  }

  # set seq number output rule by option
  ...
  # Force output some addresses when contour start.
  MOM_force Once G_motion X Z I K


  if {[info exists dpp_G70_output($::mom_operation_name)] && $dpp_G70_output($::mom_operation_name) == "Finish"} {

      MOM_skip_handler_to_event contour_end

  } elseif {[info exists dpp_G70_output($mom_machine_cycle_subroutine_name)] && $dpp_G70_output($mom_machine_cycle_subroutine_name) == "Finish"} {

      set contour_lines [split $dpp_turn_cycle(finish_contour_data) "\n"]
      foreach line $contour_lines {
...
#
# <lili  27-Jun-2013> Initial version.
# <shuai 25-Oct-2015> Modify for fanuc G70/G71/G72.
# <shuai 25-Dec-2015> Modify the code structure so as to make logical more clear.
# <shuai 21-Dec-2016> (PR7290203) Refactor the output logic for lathe rough cycle function.
#
...
#=============================================================
# This proc is used to initialize turning cycle
  ...
  global mom_sys_output_contour_motion
  global dpp_fanuc_turning_cycle_type
  global mom_post_oper_path
  global mom_output_file_directory
  global dpp_turn_cycle
  global mom_template_subtype
  ...
  if {[info exists mom_generic_cycle_status] && $mom_generic_cycle_status == 1} {
     ...
     # Setup mom_sys_output_contour_motion and do initialization for rougth turning cycle operation.
     PB_CMD_init_rough_turn_cycle_output

     if {[info exists mom_sys_output_contour_motion] && $mom_sys_output_contour_motion != 0 } {

         if {$mom_sys_output_contour_motion == 1 && [info exists mom_cutcom_status]} {
            set dpp_save_cutcom_status $mom_cutcom_status

            # Don't output cutcom until rough turning cycle has been called.
            MOM_disable_address G_cutcom
         }

         if {![info exists mom_post_oper_path] || $mom_post_oper_path != 1} {
            # Skip all events output until next MOM_generic_cycle
            MOM_skip_handler_to_event generic_cycle
         } else {
            #--------------------------------------------------------------------------------------------------------------
            # Because all contour motions and MOM_contour_start, MOM_contour_end are not generated with MOM_post_oper_path.
            # Motions between MOM_generic_cycle in finish operation are used as contour data.
            # A tcl file is generated to store the contour data info.
            # -------------------------------------------------------------------------------------------------------------
            # Output spindle in sub post run of finish operation into ptp file.
            # To avoid spindle output in contour file.
            PB_CMD_output_spindle
            MOM_close_output_file "$ptp_file_name"

            set contour_data_file [file rootname $ptp_file_name].tcl

            MOM_open_output_file $contour_data_file

            MOM_set_seq_off

            if {[info exists mom_cutcom_status]} {
               MOM_output_literal "set dpp_turn_cycle(finish_cutcom_mode) $mom_cutcom_status"
            } else {
               MOM_output_literal "set dpp_turn_cycle(finish_cutcom_mode) UNDEFINED"
            }

            MOM_output_literal "set dpp_turn_cycle(finish_contour_data) \{"
         }
  ...
  if {[info exists mom_generic_cycle_status] && $mom_generic_cycle_status == 0} {
     ...
     if {[info exists mom_sys_output_contour_motion] && $mom_sys_output_contour_motion != 0} {
        # Output spindle before G70/G71/G72
        PB_CMD_output_spindle

        if {[info exists dpp_save_cutcom_status]} {
           MOM_endable_address G_cutcom
           MOM_do_template cutcom_on
        }
        ...
        if {![info exists mom_post_oper_path] || $mom_post_oper_path != 1} {
           ...
           set dpp_turn_cycle(contour_file_name) [file native "${mom_output_file_directory}/Temp_cycle_contour_[clock clicks]"]
           if {[file exists $dpp_turn_cycle(contour_file_name)]} { catch {file delete -force $dpp_turn_cycle(contour_file_name)}}
           MOM_open_output_file $dpp_turn_cycle(contour_file_name)

           PB_CMD_set_parameters_for_turning_cycle_block
           set ::dpp_turn_cycle_seqno_begin 0
           set ::dpp_turn_cycle_seqno_end 0
           set ::dpp_turn_cycle(seq_temp) [MOM_do_template lathe_roughing_seq CREATE]
           ...
           MOM_output_literal "\}"
           catch {MOM_close_output_file [file rootname $::ptp_file_name].tcl}
           MOM_open_output_file $ptp_file_name
       }
