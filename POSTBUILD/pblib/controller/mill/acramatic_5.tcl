########################## TCL Event Handlers ##########################
#
#  acramatic_5.tcl - 3_axis_mill
#
#    
#
#  Created by Postino @ Tuesday, January 31, 2017 12:34:29 PM Pacific Standard Time
...
#
  ...
  set mom_sys_alt_unit_post_name                "acramatic_5__MM.pui"
...
#=============================================================
  ...
  global mom_motion_event
   ...
   set rapid_traverse_blk {G_motion G_mode P_cutcom X Q_cutcom Y Z S M_spindle}
   set rapid_traverse_xy_blk {G_motion G_mode P_cutcom X Q_cutcom Y S M_spindle}
   set rapid_traverse_yz_blk {G_motion G_mode P_cutcom Q_cutcom Y Z S M_spindle}
   set rapid_traverse_xz_blk {G_motion G_mode P_cutcom X Q_cutcom Z S M_spindle}
...
if {[info exists mom_sys_cutcom_type] && ![string compare "PQ" $mom_sys_cutcom_type] } {

  if { ![string compare "PQ" $mom_sys_cutcom_type] && [string compare "OFF" $mom_sys_cutcom_status] } {
    if {$mom_nxt_event_count != 0} {
      for {set i 0} {$i < $mom_nxt_event_count} {incr i 1} {
    if { ![string compare "cutcom_off" $mom_nxt_event($i)] } {set mom_sys_cutcom_status "END"}
      }
    }
    if { ![string compare "linear_move" $mom_motion_event] || ![string compare "rapid_move" $mom_nxt_motion_event] } {
      VEC3_sub mom_prev_mcs_goto mom_mcs_goto tmp_vec
      VEC3_unitize tmp_vec cur_vec
    } elseif { ![string compare "circular_move" $mom_motion_event] } {
      VEC3_sub mom_mcs_goto mom_arc_center cur_vec
      VEC3_unitize cur_vec tmp_vec
      VEC3_cross mom_arc_axis tmp_vec cur_vec
    }
    if { ![string compare "linear_move" $mom_nxt_motion_event] || ![string compare "rapid_move" $mom_nxt_motion_event] } {
      VEC3_sub mom_nxt_mcs_goto mom_mcs_goto tmp_vec
      VEC3_unitize tmp_vec nxt_vec
    } elseif { ![string compare "circular_move" $mom_nxt_motion_event] } {
      VEC3_sub mom_nxt_arc_center mom_mcs_goto nxt_vec
      VEC3_unitize nxt_vec tmp_vec
      VEC3_cross mom_nxt_arc_axis tmp_vec nxt_vec
    }

    if { [string compare "END" $mom_sys_cutcom_status] && [string compare "START" $mom_sys_cutcom_status] } {
    ...
    if { ![string compare "START" $mom_sys_cutcom_status] } {
      if { ![string compare "NORMAL" $mom_sys_cutcom_on_type] } {
        set cur_vec(0) [expr -$nxt_vec(0)]
        set cur_vec(1) [expr -$nxt_vec(1)]
      } elseif { ![string compare "TANGENT" $mom_sys_cutcom_on_type] } {
    ...
    } elseif { ![string compare "END" $mom_sys_cutcom_status] } {
      if { ![string compare "NORMAL" $mom_sys_cutcom_off_type] } {
        set nxt_vec(0) [expr -$cur_vec(0)]
        set nxt_vec(1) [expr -$cur_vec(1)]
      } elseif { ![string compare "TANGENT" $mom_sys_cutcom_off_type] } {
    ...
    if { ![string compare "RIGHT" $mom_cutcom_status] } {
      set pval [expr -$pval]
      set qval [expr -$qval]
    }
  }
