########################## TCL Event Handlers ##########################
#
#  sinumerik_840D_basic.tcl - 5_axis_dual_head
#
#    This is a 5-Axis Milling Machine With
#     Dual Rotary Heads.
#
#  Created by Postino @ Fri Mar  9 12:21:51 2018 -0800
...
#
# 26-Feb-2009 gsl   - Initial version.
# 05-May-2011 lili  - Remove PB_CMD_define_feed_varialble_value from initial move and first move.
#                     Add it into PB_CMD_MOM_sinumerik_840D.
#                     Move feedrate define in variable from PB_CMD_before_motion to PB_CMD_FEEDRATE_SET.
# 15-Oct-2013 Allen - PR6944859 fixed: Modify PB_CMD_choose_output_mode.
...
# 20-May-2014 lili  - Remove mom_kin_arc_output_mode and mom_kin_helical_arc_output_mode from DPP_GE_RESTORE_KINEMATICS.
#                     They affect compressor mode. PB_CMD__check_block_reset_cycle832 is updated to restore them.
# 04-Jun-2014 Allen - PR7143300 fix: check if variable mom_next_oper_has_tool_change and mom_current_oper_is_last_oper_in_program exists
#                     for PB_CMD__check_block_return_to_reference_point.
# 08-Jul-2014 Allen - Enhance new tap cycle: Add PB_CMD__check_block_tap_type_powerline, PB_CMD__check_block_tap_type_solutionline,
#                     PB_CMD__check_block_tap_float_solutionline, PB_CMD__check_block_tap_float_powerline and PB_CMD_cycle_tap_parameter_setting.
# 14-Jul-2014 Lili  - Fix clearance plane position issue in PB_CMD_rotate_cycle_coordinate.
# 30-Apr-2015 ljt   - Add PB_CMD_spindle_orient and add PB__CMD_check_block_cycle_retract to fix PR7162261 and PR7281995.
# 31-Jul-2015 szl   - PR6988503: remove mom_kin_helical_arc_output_mode from DPP_GE_RESTORE_KINEMATICS.
...
# 21-Aug-2015 szl   - Enhance the warning message when users set wrong pitch and wrong spindle speed, fix PR7463004.
# 16-Sep-2015 szl   - DPP_GE_RESTORE_KINEMATICS is fixed with save_kin_machine_type exist checking, fix PR7383025.
...
# 18-Sep-2015 ljt   - 1. Fix lock axis issues: replace obsolete variables with new iks variables in UNLOCK_AXIS and LOCK_AXIS.
#                     2. Fix PR6961328 in PB_CMD_MOM_lock_axis, comment out reload mom_pos in LOCK_AXIS_MOTION and lock mom_prev_pos in LINEARIZE_LOCK_MOTION.
# 22-Dec-2015 szl   - Add PB_CMD_spindle_off to enhance back counter sinking and deep hole drilling.
# 22-Dec-2015 ljt   - Updated PB_CMD_spindle_orient. Remove global declaration and account the rotation of feature reference vector for 3axis machine.
# 21-Jul-2016 szl   - Add DPP_GE_UNSET_KINEMATICS: used to unset saved kinematics variables if needed.
# 26-Jul-2016 shuai - Updated PB_CMD_detect_operation_type: set coordinate system offset to value 0 in 5-axis operation. Fix PR7700495.
# 09-Aug-2016 lili  - Fix ZYX rotation angle calculation issue in DPP_GE_CALCULATE_COOR_ROT_ANGLE. Zero resolution was too high.
# 15-Aug-2016 szl   - Update PB_CMD_MOM_rotate: remove original contents and call proc PB_CMD_kin__MOM_rotate.
# 20-Jan-2017 shuai - Replace "MOM_abort" with "MOM_abort_program" to make the error message not display in NX.
# 22-Jan-2017 shuai - CYCLE832 enhancement.
#                     1. Add the new macro "CYCLE832_v47".
#                     2. Add the new options "Top Surface Smooth" and "Rotary Tolerance" into UDE "Sinumerik 840D".
#                     3. Add the new command PB_CMD_check_settings_for_cycle832, PB_CMD_reset_arc_compressor_mode, PB_CMD_detect_5axis_tool_path and PB_CMD_detect_hole_cutting_operation.
...
#                     5. Adjust some codes format in this TCL file to make the logic and description more clear.
# 14-Feb-2017 szl   - 5-axis ncm for drilling operation.
#                     Add proc DPP_GE_GET_NCM_WORK_PLANE_CHANGE_MODE and PB_CMD_switch_output_mode_in_operation, update PB_CMD_before_motion, PB_CMD_detect_operation_type,
#                     PB_CMD_rotate_rapid_coordinate and PB_CMD_rotate_cycle_coordinate.
# 04-Apr-2017 Shuai - PR7788954: Add the command PB_CMD_set_principal_axis into cycle motion to set the principal axis.
# 31-Aug-2017 Toon VanderKooi PR8958320 add Generic Feature Operation (PB_CMD_detect_hole_cutting_operation & DPP_GE_DETECT_HOLE_CUTTING_OPERATION)
# 30-Oct-2017 cj    - Fix PR9001883: Add tool tip in calculation of mom_pos in function DPP_GE_COOR_ROT_AUTO3D.
# 02-Apr-2018 ljt   - PR9080157, take account of rotary axis direction in DPP_GE_COOR_ROT_AUTO3D
# 20-Apr-2018 zhx   - PR9063633, PB_CMD_set_helix_move: when feed_distance = mom_helix_turn_number * abs(mom_helix_pitch), then mom_helix_turn_number should be decremented to 1
...
# 21-Jan-2019 gsl - Update PB_CMD__choose_preferred_solution & add PB_CMD__convert_point
# 30-Jul-2019 hxz - Switch to alternate solution when primary is out of limit in PB_CMD__check_block_rotation_axes
#
}
  ...
  set mom_sys_alt_unit_post_name                "sinumerik_840D_basic__IN.pui"
...
return
   ...
   puts $tmp_file "      F\[\$mom_sinumerik_feed\]\\opt"
   puts $tmp_file "  \}"
   puts $tmp_file ""
   puts $tmp_file "  BLOCK_TEMPLATE tap_deep"
   puts $tmp_file "  \{"
   puts $tmp_file "      X\[\$mom_pos(0)\]"
   puts $tmp_file "      Y\[\$mom_pos(1)\]"
   puts $tmp_file "      fifth_axis\[\$mom_out_angle_pos(1)\]"
   puts $tmp_file "      fourth_axis\[\$mom_out_angle_pos(0)\]"
   puts $tmp_file "      A3\[\$mom_tool_axis(0)\]"
   puts $tmp_file "      B3\[\$mom_tool_axis(1)\]"
   puts $tmp_file "      C3\[\$mom_tool_axis(2)\]"
   puts $tmp_file "  \}"
   puts $tmp_file ""
   puts $tmp_file "  BLOCK_TEMPLATE rapid_move"
   puts $tmp_file "  \{"
   puts $tmp_file "      G_motion\[\$mom_sys_rapid_code\]"
   puts $tmp_file "      G_mode\[\$mom_sys_output_code(\$mom_output_mode)\]\\opt"
   puts $tmp_file "      X\[\$mom_mcs_goto(0)\]"
   ...
   puts $tmp_file "  \}"
   puts $tmp_file ""
   puts $tmp_file "  BLOCK_TEMPLATE linear_move_2"
   puts $tmp_file "  \{"
   puts $tmp_file "      G_cutcom\[\$mom_sys_cutcom_code(\$mom_cutcom_status)\]\\opt"
   puts $tmp_file "      G_feed\[\$mom_sys_feed_rate_mode_code(\$feed_mode)\]\\opt"
   puts $tmp_file "      G_motion\[\$mom_sys_linear_code\]"
   ...
   puts $tmp_file "      A3\[\$mom_tool_axis(0)\]\\opt"
   puts $tmp_file "      B3\[\$mom_tool_axis(1)\]\\opt"
   puts $tmp_file "      C3\[\$mom_tool_axis(2)\]\\opt"
   puts $tmp_file "  \}"
   puts $tmp_file ""
   puts $tmp_file "  BLOCK_TEMPLATE cycle_rapid"
   puts $tmp_file "  \{"
   puts $tmp_file "      G_motion\[\$mom_sys_rapid_code\]"
   puts $tmp_file "      X\[\$mom_cycle_rapid_to_pos(0)\]"
   puts $tmp_file "      Y\[\$mom_cycle_rapid_to_pos(1)\]"
...
#=============================================================
   ...
   global mom_ude_siemens_csf_group
   ...
   global mom_ude_siemens_csf_group_end
...
#
  ...
  # The options (compressor, smoothing and feed foward.) only have effects on V6 and V7.
  global mom_siemens_compressor
  global mom_siemens_smoothing
  global mom_siemens_feedforward

  if {![string compare "V6" $sinumerik_version] || ![string compare "V7" $sinumerik_version] } {
     ...
     if {[string match "ON" $mom_siemens_compressor]} {
        ...
        switch $sinumerik_version {
           ...
           "V6"   {set mom_siemens_compressor "COMPCURV"}
...
#
  ...
  if {[string compare "Point to Point" $mom_operation_type] && \
     ...
     PB_CMD_reset_arc_compressor_mode
...
#   0 : No output
#
# 30-Jul-2019 hxz - Switch to alternate solution when primary is out of limit.
  ...
  global mom_out_angle_pos
   ...
   if { [info exists sinumerik_version] && ([string match "V7" $sinumerik_version] ||\
                                            [string match "from V26 to V44" $sinumerik_version] ||\
                                            [string match "V45" $sinumerik_version] ||\
                                            [string match "V47" $sinumerik_version]) &&\
        [info exists mom_siemens_ori_def] && ![string match "*ROTARY*" $mom_siemens_ori_def] } {
      ...
      if { $mom_siemens_coord_rotation == 1 } {
         ...
         if { [info exists mom_init_pos(3)] } {
            set mom_out_angle_pos(0) [ROTSET $mom_init_pos(3) $mom_prev_out_angle_pos(0)\
                                             $mom_kin_4th_axis_direction $mom_kin_4th_axis_leader\
         ...
         if { [info exists mom_init_pos(4)] } {
            set mom_out_angle_pos(1) [ROTSET $mom_init_pos(4) $mom_prev_out_angle_pos(1)\
                                             $mom_kin_5th_axis_direction $mom_kin_5th_axis_leader\
         ...
         if { [string match "5_axis_*" $::mom_kin_machine_type] &&\
              [info exists mom_init_pos(3)] && [info exists mom_init_pos(4)] } {

            set alt_solution_status FALSE
            
            if { [ANGLE_CHECK mom_init_pos(3) 4] == -1 || [ANGLE_CHECK mom_init_pos(4) 5] == -1 } {
               # Current solution has at least one rotary axis angle out of limit, switch to alternative solution

               if { [ANGLE_CHECK mom_init_alt_pos(3) 4] == -1 || [ANGLE_CHECK mom_init_alt_pos(4) 5] == -1 } {
                  # Alternative solution is also out of limit, pop up warning

                  CATCH_WARNING "Invalid rotary axis angle. Axis value is under minimum or over maximum."
                  return 0

               } else {

                  set alt_solution_status TRUE
               }
            }
           
            if { [string match "TRUE" $alt_solution_status] } {
               set mom_out_angle_pos(0) [ROTSET $mom_init_alt_pos(3) $mom_prev_out_angle_pos(0)\
                                                $mom_kin_4th_axis_direction $mom_kin_4th_axis_leader\
                                                mom_sys_leader(fourth_axis) $mom_kin_4th_axis_min_limit $mom_kin_4th_axis_max_limit]           
               set mom_out_angle_pos(1) [ROTSET $mom_init_alt_pos(4) $mom_prev_out_angle_pos(1)\
                                                $mom_kin_5th_axis_direction $mom_kin_5th_axis_leader\
                                                mom_sys_leader(fifth_axis) $mom_kin_5th_axis_min_limit $mom_kin_5th_axis_max_limit]               
            }
         }
...
#Only process output mode swtich inside of Hole making operations for 5-axis machine.
  global mom_kin_machine_type mom_siemens_coord_rotation mom_current_motion
  if {[string match "*5_axis*" $mom_kin_machine_type] && \
      ...
      [string compare $mom_current_motion "initial_move"] && [string compare $mom_current_motion "first_move"]} {
...
#
   ...
   # <07-Apr-2017 shuai> PR7788954: Enhancement for cycle motion according to the value of mom_cycle_spindle_axis.
...
#
# 16-Jan-2017 shuai - Initial version.
# 31-Aug-2017 Toon VanderKooi PR8958320 add Generic Feature Operation
#
...
# mom_siemens_coord_rotation "0"/"1"/"2"
  ...
  if {$dpp_ge(toolpath_axis_num)==5} {
    # set 5-axis simultaneous operation type
     ...
     # <26-Jul-2016 Shuai Set coordinate system offset to value 0 in 5-axis operation.>
     # <Fix PR7700495>
     ...
     if {![string compare "TRAFOOF" $mom_siemens_5axis_mode]} {
        if {![string match "*dual_table*" $mom_kin_machine_type]} {
           set dpp_ge(coord_rot) [DPP_GE_COOR_ROT "XYZ" rot_angle offset pos]
           if {$dpp_ge(coord_rot) == "AUTO_3D"} {
              MOM_display_message "$mom_operation_name: Shouldn't set the output mode to TRAFOOF with the UDE if the machine isn't a dual_table machine." \
                                                         "Postprocessor error message" "E"
              MOM_abort_program
           }
        }
     } else {
        ...
        # machine kinematics is reloaded to dual-table machine.
        set dpp_ge(coord_rot) [DPP_GE_COOR_ROT "XYZ" rot_angle offset pos]

        # set coordinate linear offset value
        VMOV 3 offset coord_offset

        # set coordinate rotation angle
...
#=============================================================
proc PB_CMD_reset_arc_compressor_mode { } {
#=============================================================
# This command is used to reset circular and helical mode when compressor is off.
...
return
  ...
  if {[info exists dpp_ge(current_output_mode)] && $dpp_ge(current_output_mode) == "5axis"} {
return
  }
...
#
  ...
  # Set the default settings for smoothing.
...
# set sinumerik_version "V47"
  ...
  if {![string compare "V6" $sinumerik_version] || \
...
# <lili Jan-09-2017> initial version
  global dpp_ge
  global mom_siemens_5axis_output_mode mom_siemens_5axis_mode
  global mom_siemens_coord_rotation
  global coord_angle coord_ang_A coord_ang_B coord_ang_C
  global mom_pos

  # Check if 3plus2 operation output default is TRAFOOF, if yes, then skip this function.
  if {![string compare "TRAFOOF" $mom_siemens_5axis_mode]} {
return
  }

  # Detect current motion output type
   DPP_GE_GET_NCM_WORK_PLANE_CHANGE_MODE

   switch -- $dpp_ge(ncm_work_plane_change_mode) {
      "direct_change"  -
      "end_change"     {
         set dpp_ge(coord_rot) [DPP_GE_COOR_ROT "XYZ" rot_angle offset pos]

         if {$dpp_ge(coord_rot) == "AUTO_3D"} {
            set dpp_ge(current_output_mode) "3plus2"
         } else {
            set dpp_ge(current_output_mode) "3axis"
         }

      }
      "start_change"   -
      "in_change"      {set dpp_ge(current_output_mode) "5axis"}
      default          {set dpp_ge(current_output_mode) "None"}

   }

# Get prev motion output type
   if {$mom_siemens_5axis_output_mode == 1 && $mom_siemens_coord_rotation !=2} {
      set dpp_ge(prev_output_mode) "5axis"
   } elseif {$mom_siemens_coord_rotation !=0} {
      set dpp_ge(prev_output_mode) "3plus2"
   } else {
      set dpp_ge(prev_output_mode) "3axis"
   }
 #MOM_output_literal "___$dpp_ge(current_output_mode) $dpp_ge(prev_output_mode)"
# switch output mode
   if {[string compare $dpp_ge(current_output_mode) "None"] && [string compare $dpp_ge(prev_output_mode) $dpp_ge(current_output_mode)]} {

      #cancel last output mode
      if {$dpp_ge(prev_output_mode) == "5axis"} {

         MOM_output_literal "TRAFOOF"
         set mom_siemens_5axis_output_mode 0

      } elseif {$dpp_ge(prev_output_mode) == "3plus2"} {
          ...
          if { [string compare "SWIVELING" $mom_siemens_5axis_mode] } {
             ...
             set coord_angle(0) 0; set coord_angle(1) 0; set coord_angle(2) 0
             set coord_ang_A    0; set coord_ang_B    0; set coord_ang_C 0
   ...
   } elseif {![string compare $dpp_ge(current_output_mode) "3plus2"] && ![VEC3_is_equal rot_angle coord_angle]} {
...
# no need to rework on mom_pos for cycle, it is handled in MOM_cycle_plane_change and cycle_move.
}
