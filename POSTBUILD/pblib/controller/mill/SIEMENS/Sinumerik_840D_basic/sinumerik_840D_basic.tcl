...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_post_builder_version              "9.0.3"
...
# 05-20-14 Lili  - Remove mom_kin_arc_output_mode and mom_kin_helical_arc_output_mode from DPP_GE_RESTORE_KINEMATICS. They affect compressor mode. PB_CMD__check_block_reset_cycle832 is updated to restore them.
# 06-04-14 Allen - PR7143300 fix: check if variable mom_next_oper_has_tool_change and mom_current_oper_is_last_oper_in_program exists for PB_CMD__check_block_return_to_reference_point
# 07-08-14 Allen - Enhance new tap cycle:Add PB_CMD__check_block_tap_type_powerline,PB_CMD__check_block_tap_type_solutionline,PB_CMD__check_block_tap_float_solutionline,PB_CMD__check_block_tap_float_powerline and PB_CMD_cycle_tap_parameter_setting.
...
return
   ...
   if { ![info exists mom_retract_spindle] || $mom_retract_spindle == 0} {
      set mom_siemens_cycle_sst1 $mom_spindle_speed
   } else {
...
# mom_siemens_coord_rotation "0"/"1"/"2"
  ...
  if { $mom_siemens_milling_setting == "Default"} {
     if {$dpp_ge(toolpath_axis_num)==5} {
       # set 5-axis simutaneous operation type
  ...
  if {$mom_siemens_5axis_mode == "TRAORI" || $mom_siemens_5axis_mode == "TRAORI2" } {
     ...
     set mom_siemens_5axis_output "TRAFOOF"
     if {$dpp_ge(toolpath_axis_num) == 5} {
        MOM_output_to_listing_device " $mom_operation_name is 5 axis simutaneous operation, \
  ...
  if {$dpp_ge(toolpath_axis_num)==5} {
    ...
    # set coord_offset(0) 0; set coord_offset(1) 0; set coord_offset(2) 0
     ...
     if {[info exists mom_kin_coordinate_system_type] && ![string compare "CSYS" $mom_kin_coordinate_system_type]} {
         MOM_output_to_listing_device "$mom_operation_name : LOCAL CSYS Rotated MCS should not work with 5-axis simutaneous operation!"
     }
     global mom_kin_arc_output_mode save_mom_kin_arc_output_mode
     global mom_kin_helical_arc_output_mode save_mom_kin_helical_arc_output_mode
     if { ![info exists save_mom_kin_arc_output_mode] } {
...
#=============================================================
  ...
  if {![info exists mom_cycle_tool_axis_change] || $mom_cycle_tool_axis_change == 0} {
     ...
     if {[info exists mom_cycle_clearance_plane_change] && $mom_cycle_clearance_plane_change>0 } {
        ...
        if {[info exists mom_cycle_retract_mode] && [string match "AUTO" $mom_cycle_retract_mode] } {
           set mom_cycle_clearance_pos(2) [MOM_ask_address_value Z]
        }
...
return
  ...
  global mom_sys_lock_status
   ...
   if { [string match "ON" $mom_sys_lock_status] } {
     ...
     #<11-20-2013 gsl> Skip RAPID (& non-motion) events???
