...
#
#  Created by Postino @ Fri Mar  9 12:22:05 2018 -0800
...
# 02-Nov-2017 zhx   - Fix PR7694440: Update function PB_CMD_cycle_tap.
# 02-Apr-2018 ljt   - PR9080157, take account of rotary axis direction in auto_3d
...
# 03-Apr-2018 Jintao - Add rot_dir_4th and rot_dir_5th
...
#-------------------------------------------------------
  ...
  if { ![string match "tap*_move" $mom_motion_event] } { return }
...
#
  ...
  #======================================================
   if { ([info exists mom_machine_control_motion_output] && $mom_machine_control_motion_output == 2) && \
     ...
     #<2016-12-06 szl> Check if the operation is supported by CYCLE95; otherwise,
     #                 switch off Machine-Cycle output mode and list an warning message.
     #
      if { [string match "*ROUGH*"  $mom_template_subtype] || \
        ...
        #<2016-11-28 szl> Check if Profile stock is the same as Rough stock when "Additional Profile" option is on;
        #                 otherwise, switch off Machine-Cycle output mode.
        #
         if { [info exists ::mom_profiling] && ($::mom_profiling == 1) } {
            ...
            if { !([EQ_is_equal $::mom_finish_equidistant_stock $::mom_stock_part] && \
               ...
               MOM_output_to_listing_device "$mom_operation_name: Additional Profiling can only be used\
                                             when profile stock is the same with rough stock.\n\
                                             So it will not be output in current operation.\
        ...
        #----------------------------------------
         ...
         MOM_output_to_listing_device "$mom_operation_name: The Machine Cycle motion output has not been supported\
                                       for current operation type so far.\n\
                                       Switch output mode from Machine-Cycle to Non-Machine-Cycle."
      }
