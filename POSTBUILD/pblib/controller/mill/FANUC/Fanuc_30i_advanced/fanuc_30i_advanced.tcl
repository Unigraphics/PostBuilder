...
#
#  Created by Postino @ Fri Mar  9 12:20:51 2018 -0800
...
# 08-Apr-2019 gsl - Fixed 8553652 & 9385427 in DPP_GE_COOR_ROT_AUTO3D & DPP_GE_COOR_ROT_AUTO3D_WCS_ROTATION
#
}
  ...
  # Should output rotary axis before when multi-axis mode output or G68 output.
   if { $dpp_ge(toolpath_axis_num) == "3" && $dpp_ge(sys_coord_rotation_output_type) == "SWIVELING" } {
     ...
     # To avoid the can't save bug of pb, initialize the local variable. Just for sim05 vnc!
      ...
      if { ![info exists g68_first_vec] || ![info exists g68_second_vec] ||\
           ![info exists g68_coord_rotation] || ![info exists offset] || ![info exists pos] } {
        return 0
      }
     ...
     # Pop up warning message when user wants to output G68 under local csys on a machine with table.
      if { $dpp_ge(coord_rot) == "LOCAL" && [string match "*table*" $save_mom_kin_machine_type] } {
         MOM_display_message "Don't use local csys to output G68 on a machine with table. Please attach your operation to fixture offset coordinate system.\
...
#=============================================================
  ...
  # Preserve mom_pos & mom_prev_pos
   ...
   if { [string match "*5_axis*" $mom_kin_machine_type] && \
        ...
        [string compare "first_move" $mom_current_motion] } {
...
#
  ...
  if {[info exists mom_nxt_pos] && [info exists mom_nxt_motion_type]} {
    ...
    if { ($mom_motion_type == "CUT" && $mom_nxt_motion_type == "CUT") ||\
         ($mom_motion_type == "FIRSTCUT" && $mom_nxt_motion_type == "FIRSTCUT") ||\
         ($mom_motion_type == "STEPOVER" && $mom_nxt_motion_type == "STEPOVER") } {

      if { [EQ_is_equal $PX $X] && [EQ_is_equal $PY $Y] && [EQ_is_equal $PZ $Z] &&\
          ![EQ_is_equal $P4 $R4] && [EQ_is_equal $P5 $R5] &&\
           [EQ_is_equal $NX $X] && [EQ_is_equal $NY $Y] && [EQ_is_equal $NZ $Z] &&\
          ![EQ_is_equal $N4 $R4] && [EQ_is_equal $N5 $R5] && $dpp_ge(combine_mode) != "5" && $switch_dir_4th == "NO" } {
      ...
      } elseif { [EQ_is_equal $PX $X] && [EQ_is_equal $PY $Y] && [EQ_is_equal $PZ $Z] &&\
                 [EQ_is_equal $P4 $R4] && ![EQ_is_equal $P5 $R5] &&\
                 [EQ_is_equal $NX $X] && [EQ_is_equal $NY $Y] && [EQ_is_equal $NZ $Z] &&\
                 [EQ_is_equal $N4 $R4] && ![EQ_is_equal $N5 $R5] && $dpp_ge(combine_mode) != "4" && $switch_dir_5th == "NO" } {
...
# Do customization here to get different output
  ...
  # Variable below will be changed to "TCP_FIX_TABLE" in postprocessor, if dpp_ge(sys_tcp_tool_axis_output_mode) is set to "VECTOR".
...
#<Jul-06-2018 gsl>
## Syntax:   MOM_enable_post_options <option1, option2, ...>
##
##  This is a general command to choose multiple options for posting output.
##  At least, one option should be specified.
##
##  Following options are supported:
##
##    RESOLUTION_ROUND    - Round off coordinates (mom_pos) before other transformation
##    NX6_DIST_CAL        - Use NX6's method for distance calculation
##    PRE_NX8_INCR_OUTPUT - Use pre-NX8's method for calculating incremental output
##    PRE_NX9_ROUND_OFF   - Use pre-NX9's round-off algorithm

  # MOM_enable_post_options PRE_NX9_ROUND_OFF ;# PRE_NX8_INCR_OUTPUT ;# RESOLUTION_ROUND
}
...
#                   setting working plane as XY here; it may output G18/G19 in cycle.
...
#
# Rotation matrix calculated by current rotary axes angle and 4th & 5th axis vectors.
#
# Output:
...
#
#   Machine's kinematics will be reloaded to 5_axis_dual_table
#
# Return:
#   1 - operation is a 3+2 operation without Local CSYS rotation coordinate system.
#   0 - operation is not a 3+2 operation without Local CSYS rotation coordinate system.
#
#
# Revisions:
#-----------
...
# 2018-04-02 ljt  - 9080157, account for reversed direction.
# 2018-11-28 gsl  - 8553652, fixed random rotation issue.
# 2019-04-08 gsl  - 9385427, relax condition to identify auto 3D.
   ...
   if { [info exists mom_kin_coordinate_system_type] && [string match "CSYS" $mom_kin_coordinate_system_type] } {
      ...
      # Fixed 9385427 - Removed condition of ![VEC3_is_equal mom_kin_5th_axis_vector Z] -
      if { (![EQ_is_zero $mom_out_angle_pos(0)] && ![VEC3_is_equal mom_kin_4th_axis_vector Z]) ||\
           (![EQ_is_zero $mom_out_angle_pos(1)]) } {
      } else {
         return 0
      }
   }
...
#=============================================================
# This procedure is used to detect if an operation is 3+2 operation without Local CSYS rotation coordinate system,
# and calculate the parameters for G68 instruction.
#
# Output:
...
#
   ...
   # For a head-table machine, recalculate the vectors and angles to just output G68 once
   if { [string match "5_axis_head_table" $save_mom_kin_machine_type] } {
   ...
   # For a dual_head machine, recalculate the vectors and angles to output G68 around the rotary axis vectors
...
#
  ...
  #<Nov-27-2018 gsl> Not to show this cmd name
   set level [expr [info level] - int(abs($up_level)) - 1]
