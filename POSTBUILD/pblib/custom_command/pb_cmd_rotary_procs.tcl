...
uplevel #0 {
  ...
  set mom_sys_spindle_axis(2)                    1.0
   ...
   set mom_kin_rotary_reengage_feedrate           10
   set mom_kin_rotary_retract_feedrate            400
   set mom_sys_rotary_retract                     "OFF"
...
#=============================================================
  ...
  if {$mom_clamp_axis == "AUTO"} {
    if {$mom_clamp_axis == "ON"} {
      set mom_sys_auto_clamp "ON"
    } elseif {$mom_clamp_axis == "OFF"} {
  ...
  switch $mom_lock_axis {
    ...
    XAXIS {
      ...
      switch $mom_lock_axis_plane {
        ...
        NONE {
          if {$mtype == 5} {
            set mom_warning_info "Must specify plane for 5 axis machine"
            return "error"
          } else {
          set v [LOCK_AXIS_SUB $a]
          }
...
#
   ...
   global mom_kin_rotary_reengage_feedrate mom_kin_rotary_retract_feedrate
...
# Retract from part
  ...
  if {[info exists mom_kin_rotary_retract_feedrate]} {
    set mom_feed_rate $mom_kin_rotary_retract_feedrate
    FEEDRATE_SET
  }
...
#
    ...
    MOM_suppress once fourth_axis fifth_axis
    if {[info exists mom_kin_rotary_reengage_feedrate]} {
      set mom_feed_rate $mom_kin_rotary_reengage_feedrate
      FEEDRATE_SET
    }
    PB_CMD_linear_move
...
if {[info exists mom_sys_rotary_error] && $mom_sys_rotary_retract != "OFF"} {ROTARY_AXIS_RETRACT}
}
...
#=============================================================
#
#  This procedure must be placed in the PB_CMD_before_motion in order
#  to lock axes.  The UDE lock_axis must be specified in the tool path
