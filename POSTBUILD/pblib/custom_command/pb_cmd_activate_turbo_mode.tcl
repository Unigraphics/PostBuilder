...
#=============================================================================
#
# Custom commands in this file allow you to activate turbo mode for a post of
#
#  1. Fanuc style controllers
#  2. iTNC (Heidenhain Conversational) controllers
#  3. Sinumerik (840D) controllers
#
# You will import one of the 3 custom commands in this package to your post
# and, when necessary, edit the definitions to fit your requirements; then
# attach the command to the Start-of-Program event to activate the turbo mode.
#
#=============================================================================
## HEADER BLOCK END ##
...
#  will be roughly 15 to 20 % of the time required in regular mode.
#
...
#  This command will only modify / enhance the contents of the turbo blocks.
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#
#  Each rapid motion is output in a single block.
#
...
#  modify the parameters of Formats involved in the turbo output.
#
#  Any custom commands in the linear move, circular move, rapid move
#  and MOM_before_motion events will be ignored.
#
#
#  By default, rapid motions are processed in turbo mode where
#  each motion is output in a single block.
#  => MOM_set_turbo_rapid "FALSE" can be issued to process
#     rapid motions in regular mode instead, of which the customization
...
#
  ...
  # Define command below when turbo mode is run in earlier version of NX/Post.
...
# set ::mom_kin_is_turbo_output FALSE  ;# To post in standard mode
   ...
   if { [info exists ::mom_kin_is_turbo_output] } {
      ...
      INFO "Disable advanced turbo mode : $::mom_kin_is_turbo_output"
      MOM_set_turbo_mode FALSE ADVANCED
      MOM_set_turbo_mode $::mom_kin_is_turbo_output
...
return
  ...
  #+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  # Skip this command entirely when running Advanced Turbo mode.
   if { [info exists ::mom_sys_advanced_turbo_output] &&\
                    $::mom_sys_advanced_turbo_output } {

      INFO "Skip legacy turbo mode within an Advanced Turbo post."
...
return
  ...
  #+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  # Activate turbo mode
  # => Turbo mode may be set outside of this command or forced "TRUE" here.
  # => Subsequent conditions will determine if thrbo mode can be activated or not.
  #+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  # When desired, always force turbo output mode here,
  # set turbo_mode  "TRUE"

   MOM_set_turbo_mode $turbo_mode



  #++++++++++++++++++++++++++++++++++++++++++++++++++++++
  # Handle RAPID motions in regular mode, when desired.
  # => By default, rapids are also handled in turbo mode.
  # => This option can also be configured per operation.
  #++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  # => This option is not available with turbo mode prior to NX10.0.3.
  #+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  # Skip multi-axis turbo mode for pre-NX7
   if { ![string match "3_axis_mill" $::mom_kin_machine_type] } {
      ...
      if { $ugii_version < 7 } {
         ...
         CATCH_WARNING "Multi-axis turbo mode is not available prior to NX7."
  ...
  # Load turbo blocks
   ...
   if { [catch { MOM_load_definition_file  "$def_file_name" } res] } {
     ...
     # If some addresses are not defined, post processing may be broken, a popped up message is needed
      MOM_display_message "Failed to load turbo blocks definition. Please check if addresses and turbo symbols are correct."\
                          "Postprocessor error message" "W"
...
#
#  This custom command will activate the turbo process mode for
#  the posts of iTNC controllers. The execution time will be
#  roughly 15 to 20 % of the time required in regular mode.
#
#  This command may only be used with Post Builder 3.2.1 or later
#  and NX2 or newer.
#
...
return
  ...
  # This factor will be applied in feed output IPM (inch/min)
...
return
  ...
  # Open a scratch file for turbo blocks definition
   global mom_logname
...
return
  ...
  # Load turbo blocks
   ...
   if { [catch { MOM_load_definition_file  "$def_file_name" } res] } {
     ...
     # If some addresses is not defined, post processing maybe broken, a popped up message is needed
      MOM_display_message "Failed to load turbo blocks definition. Please check if addresses and turbo symbols are correct."\
                          "Postprocessor error message" "W"
...
#
#  This custom command will activate the turbo process mode for
#  the posts of Sinumerik controllers. The execution time will be
#  roughly 15 to 20 % of the time required in regular mode.
#
#  This command may only be used with Post Builder 3.2.1 or later
#  and NX2 or newer.
