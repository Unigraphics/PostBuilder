...
# 02Aug00       gsl         v17p15 submission.
# 07Aug00       gsl         Modified logic in OPEN_files for out file extension.
# 14Aug00       gsl         Added FRN to feed_mode.
# 29Aug00       gsl   v17p16 submission
# 11Sep00       MKG 4133654 check _status UNDEFINED, f_rn, d_pm in FEEDRATE_SET, sequence no. overflow.
# 26Sep00       MKG 4183560 use group_name_lowercase instead of changing case of mom_group_name.
# 11Oct00       MKG 4193610 mom_sys_list_output ON=create lpt, mom_sys_commentary_output ON=include commentary.
# 12Oct00       MKG 1066943 fixed MTX3_is_equal.
# 16Oct00       MKG 4143900 incorporate tape_bytes for tape length.
# 31Oct00       MKG 4202839 fix TOOL_SET to set vars at proper events.
# 30Nov00       whb         fix OPSKIP
# 09Mar2001     whb 4255997 allow PPRINT, DISPLY and INSERT as MOM_text.
# 06Apr2001     whb         don't output warnings to listing file unless warning file active.
# 13Apr2001     mjr         Add delete markers
# 18Apr2001     whb         Update with latest from PB including:
#               gsl         Adjust feedrate mode per Post output unit in FEEDRATE_SET.
#                           Set mom_spindle_speed if it doesn't exist in SPINDLE_SET.
#               whb         insure we get travers moves for no xy but rotary only moves.
#                           add PB_catch_warning.
# 23Apr2001     whb         add pb_base.tcl rename PB_catch_warning to pb_catch_warning.
# 24Apr2001     whb         pb_catch_warning should be PB_catch_warning as previously.
#
# $HISTORY$
#
######################################################################
...
#_______________________________________________________________________________
  ...
  if {[hiset mom_feed_rate_number]} {
    ...
    if {[EQ_is_zero $delta_distance] || [EQ_is_zero $mom_feed_rate]} {
      ...
      set f_rn [expr $mom_feed_rate / $delta_distance] ; return $f_rn
    }
  }
}
      ...
      if {![hiset isinit_group($mom_group_name)] && $group_level == 1} {
        ...
        if {$mom_sys_warning_output == "ON"} {
          ...
          if {[string length $grp_warn_name] > 30} {
            ...
            if {[string length $grp_warn_name] > 30} { set grp_warn_name ${group_name_lowercase}.out }
...
proc  LIST_FILE {} {
	 global mom_o_buffer
         ...
         if {$mom_sys_list_output == "OFF"} { return }
#*****************ASSUMPTION*************************
         global mom_pos
...
#<11-02-00 gsl>
global mom_sys_commentary_list
if [info exists mom_sys_commentary_list] \
{
#The mom_sys varaible "mom_sys_commentary_list"
