...
proc  LIST_FILE_HEADER {} {
  ...
  switch $mom_kin_machine_type {
   ...
   if {[string match "x " $mom_sys_commentary_list] >= 0} {
     set commentary_out(x) "1"
   } else {
     set commentary_out(x) "0"
   }

   if {[string match "y " $mom_sys_commentary_list] >= 0 && $sys_axis != 2} {
     set commentary_out(y) "1"
   } else {
     set commentary_out(y) "0"
   }

   if {[string match "z " $mom_sys_commentary_list] >= 0} {
     set commentary_out(z) "1"
   } else {
     set commentary_out(z) "0"
   }    

   if {[string match "4axis" $mom_sys_commentary_list] >= 0 && $sys_axis >= 4} {
     set commentary_out(4axis) "1"
   } else {
     set commentary_out(4axis) "0"
   }    

   if {[string match "5axis" $mom_sys_commentary_list] >= 0 && $sys_axis == 5} {
     set commentary_out(5axis) "1"
   } else {
     set commentary_out(5axis) "0"
   }  

   if {[string match "feed" $mom_sys_commentary_list] >= 0} {
     set commentary_out(feed) "1"
   } else {
     set commentary_out(feed) "0"
   } 

   if {[string match "speed" $mom_sys_commentary_list] >= 0} {
