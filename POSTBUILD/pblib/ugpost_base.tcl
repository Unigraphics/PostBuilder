...
# DESCRIPTION:
#
#   This is a special version of ugpost_base.tcl used by Post Builder.
#   Do not subsitude this with or for the one in the MACH kit.
#
###############################################################################


global gPB
if { [info exists gPB(ugpost_base_sourced)]  &&  $gPB(ugpost_base_sourced) } {
   return
}
...
#=============================================================
  ...
  if { [MOM_ask_env_var "UGII_CAM_POST_DISABLE_INCR_MODE_FIX"] != 1 } {
     global mom_kin_4th_axis_incr_switch mom_kin_5th_axis_incr_switch
     global mom_kin_4th_axis_direction mom_kin_5th_axis_direction
     global mom_kin_4th_axis_direction_save mom_kin_5th_axis_direction_save

     if { [info exists mom_kin_4th_axis_incr_switch] && \
          [string match "ON" $mom_kin_4th_axis_incr_switch] } {

        MOM_incremental $isincr fourth_axis

        if ![info exists mom_kin_4th_axis_direction_save] {
           set mom_kin_4th_axis_direction_save $mom_kin_4th_axis_direction
        }

        if [string match "ON" $isincr] {
           set mom_kin_4th_axis_direction "MAGNITUDE_DETERMINES_DIRECTION"
        } else {
           set mom_kin_4th_axis_direction $mom_kin_4th_axis_direction_save
        }

        MOM_reload_kinematics
     }

     if { [info exists mom_kin_5th_axis_incr_switch] && \
          [string match "ON" $mom_kin_5th_axis_incr_switch] } {

        MOM_incremental $isincr fifth_axis

        if ![info exists mom_kin_5th_axis_direction_save] {
           set mom_kin_5th_axis_direction_save $mom_kin_5th_axis_direction
        }

        if [string match "ON" $isincr] {
           set mom_kin_5th_axis_direction "MAGNITUDE_DETERMINES_DIRECTION"
        } else {
           set mom_kin_5th_axis_direction $mom_kin_5th_axis_direction_save
        }
...
#-------------------------------------------------------------------------------
   ...
   } elseif { [string match "LIST" [string toupper $mom_sys_warning_output]] } {

    # <NX10.0.2> Warning messages may be directed to listing device
    # - mom_sys_warning_output can be set to "LIST" prior to MOM_start_of_program to enable this behavior.
    #
