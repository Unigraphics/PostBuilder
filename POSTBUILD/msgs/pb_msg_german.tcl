...
::msgcat::mcset $gPB(LANG) "MC(listing,tran_path,check,Label)"            "Ausgabeübergangsweg"

::msgcat::mcset $gPB(LANG) "MC(listing,tran_path,check,Context)"          "Mit diesem Umschalter wird aktiviert, dass NX/Post die sich aus dem Übergangsweg zwischen Operationen ergebenden Ereignisse verarbeitet und ausgibt."
...
::msgcat::mcset $gPB(LANG) "MC(block,def_popup,Label)"                    "Definition"

::msgcat::mcset $gPB(LANG) "MC(msg,info,obj_undef)"                       "Informationen zu diesem Objekt noch nicht konfiguriert."

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_cmd_by_pb)"                   "Von Post Builder erstellter VNC-Befehl zum Übertragen der Einstellungen eines Postprozessors an seine VNC-Steuerung."

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_util)"                        "In PB_CMD_vnc__define_misc_procs definierte VNC-Dienstprogrammbefehle."

::msgcat::mcset $gPB(LANG) "MC(msg,info,sim_cmd)"                         "SIM-Befehle zur Verwendung in einer Simulationssteuerung."

::msgcat::mcset $gPB(LANG) "MC(msg,info,pb_vnc_cmd)"                      "Von Post Builder erstellte Befehle zum Koppeln von Schlüssel-Postprozessor-Ereignissen mit einer VNC-Steuerung."

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_broker_cmd)"                  "In einer VNC-Steuerung verwendeter Broker-Befehl zum Ausführen anderer Befehle."

# Confirmation when user is about to delete an event
::msgcat::mcset $gPB(LANG) "MC(msg,event,delete_confirm)"                 "Möchten Sie das Ereignis wirklich löschen?"
...
::msgcat::mcset $gPB(LANG) "MC(event,misc,user_limit_action,name)"        "Benutzerdefinierte Achsenbegrenzungsaktion"

# Skip output of NC code when rotary limit violation has occurred.
::msgcat::mcset $gPB(LANG) "MC(machine,axis,violation,omit,Label)"        "Ausgabe auslassen"

# Add/edit notes for a set of custom commands to be exported.
::msgcat::mcset $gPB(LANG) "MC(func,edit_note,Label)"                     "Hinweise bearbeiten"
...
::msgcat::mcset $gPB(LANG) "MC(event,misc,region_start_marker,name)"      "Anfang der Bereichsmarkierung"
::msgcat::mcset $gPB(LANG) "MC(event,misc,region_end_marker,name)"        "Ende der Bereichsmarkierung"

#=============================================================================
# pb11.01
#=============================================================================
# A comment of Tcl language
::msgcat::mcset $gPB(LANG) "MC(event,combo,tcl_line,Label)"               "Tcl-Kommentar"

#=============================================================================
# pb10.03
#=============================================================================
::msgcat::mcset $gPB(LANG) "MC(msg,filename_with_spec_char)"              "Dateiname mit Sonderzeichen wird nicht unterstützt."
...
::msgcat::mcset $gPB(LANG) "MC(new,main_post,warning_2,msg)"         "Der Haupt-Postprozessor muss in Post Builder Version 8.0\noder einer neuen Version erzeugt oder gespeichert werden."
...
::msgcat::mcset $gPB(LANG) "MC(main,file,exit,Balloon)"             "Post Builder beenden."
::msgcat::mcset $gPB(LANG) "MC(main,file,exit,Context)"             "Post Builder beenden."
...
::msgcat::mcset $gPB(LANG) "MC(nav_button,filter,Context)"          "Mit dieser Schaltfläche wird der Verzeichnisfilter angewendet und es werden Dateien aufgelistet, die die Bedingung erfüllen."
...
::msgcat::mcset $gPB(LANG) "MC(machine,axis,plane,Context)"             "XY, YZ, ZX usw. als Rotationsebene auswählen. Die Option \\\"Andere\\\" ermöglicht das Festlegen eines beliebigen Vektors."
...
### Program End
::msgcat::mcset $gPB(LANG) "MC(event,end_prog,name)"                "Programmende"
...
::msgcat::mcset $gPB(LANG) "MC(udc,drill,simulate,INFO)"            "Dieses Ereignis ist ein simulierter Zyklus."
