...
::msgcat::mcset $gPB(LANG) "MC(event,misc,pattern_csys_start,name)"       "Start of Pattern Set Coordinate System"
::msgcat::mcset $gPB(LANG) "MC(event,misc,pattern_csys_end,name)"         "End of Pattern Set Coordinate System"

# Toggle to signal post being able to process and output subprograms
::msgcat::mcset $gPB(LANG) "MC(listing,subprog_out,check,Label)"          "Enable Subprogram Output"

::msgcat::mcset $gPB(LANG) "MC(listing,subprog_out,check,Context)"        "This toggle will notify NX/Post core processor that this post\
                                                                           is equipped with the ability to produce subprogram output."

::msgcat::mcset $gPB(LANG) "MC(address,leader,err_msg)"                   "Leader of Address X, Y, Z, fourth_axis, fifth_axis and N\
...
::msgcat::mcset $gPB(LANG) "MC(event,misc,before_contour_end,name)"       "Before Contour End"
...
::msgcat::mcset $gPB(LANG) "MC(listing,tran_path,check,Label)"            "Output Transition Path"

::msgcat::mcset $gPB(LANG) "MC(listing,tran_path,check,Context)"          "This toggle will enable NX/Post to process and output\
...
::msgcat::mcset $gPB(LANG) "MC(msg,info,obj_undef)"                       "Information about this object not yet configured."

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_cmd_by_pb)"                   "VNC command generated by Post Builder\
                                                                           to convey the settings of a post to its VNC driver."

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_util)"                        "VNC utility commands defined in PB_CMD_vnc__define_misc_procs."

::msgcat::mcset $gPB(LANG) "MC(msg,info,sim_cmd)"                         "SIM Commands to be used in a simulation driver."

::msgcat::mcset $gPB(LANG) "MC(msg,info,pb_vnc_cmd)"                      "Commands generated by Post Builder to interface\
                                                                           key post events with a VNC driver."

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_broker_cmd)"                  "Broker command used in a VNC driver to execute other commands."

# Confirmation when user is about to delete an event
::msgcat::mcset $gPB(LANG) "MC(msg,event,delete_confirm)"                 "Do you really want to delete event"
...
::msgcat::mcset $gPB(LANG) "MC(event,misc,user_limit_action,name)"        "User Defined Axis Limit Action"

# Skip output of NC code when rotary limit violation has occurred.
::msgcat::mcset $gPB(LANG) "MC(machine,axis,violation,omit,Label)"        "Omit Output"

# Add/edit notes for a set of custom commands to be exported.
::msgcat::mcset $gPB(LANG) "MC(func,edit_note,Label)"                     "Edit Notes"
...
::msgcat::mcset $gPB(LANG) "MC(event,misc,region_start_marker,name)"      "Start of Region Marker"
::msgcat::mcset $gPB(LANG) "MC(event,misc,region_end_marker,name)"        "End of Region Marker"

#=============================================================================
# pb11.01
#=============================================================================
# A comment of Tcl language
::msgcat::mcset $gPB(LANG) "MC(event,combo,tcl_line,Label)"               "Tcl Comment"
...
::msgcat::mcset $gPB(LANG) "MC(udc,drill,simulate,INFO)"            "This event is a simulated cycle."
