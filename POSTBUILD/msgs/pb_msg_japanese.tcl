...
::msgcat::mcset $gPB(LANG) "MC(listing,tran_path,check,Label)"            "出力トランジションパス"

::msgcat::mcset $gPB(LANG) "MC(listing,tran_path,check,Context)"          "このトグルは、NX/Postがオペレーション間のトランジションパスから生じるイベントを処理して出力することを可能にします。"
...
::msgcat::mcset $gPB(LANG) "MC(block,def_popup,Label)"                    "定義"

::msgcat::mcset $gPB(LANG) "MC(msg,info,obj_undef)"                       "このオブジェクトについての情報はまだ構成されていません。"

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_cmd_by_pb)"                   "ポストの設定をVNCドライバに運ぶためにポストビルダにより生成されたVNCコマンドです。"

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_util)"                        "PB_CMD_vnc__define_misc_procsにおいて定義されたVNCユーティリティコマンドです。"

::msgcat::mcset $gPB(LANG) "MC(msg,info,sim_cmd)"                         "シミュレーションドライバにおいて使用されるSIMコマンドです。"

::msgcat::mcset $gPB(LANG) "MC(msg,info,pb_vnc_cmd)"                      "キーポストイベントとVNCのインタフェースとするためにポストビルドにより生成されたコマンドです。"

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_broker_cmd)"                  "他のコマンドを実行するVNCドライバにおいて使用されるブローカーコマンドです。"

# Confirmation when user is about to delete an event
::msgcat::mcset $gPB(LANG) "MC(msg,event,delete_confirm)"                 "本当にイベントを削除しますか"
...
::msgcat::mcset $gPB(LANG) "MC(event,misc,user_limit_action,name)"        "ユーザ定義軸制限アクション"

# Skip output of NC code when rotary limit violation has occurred.
::msgcat::mcset $gPB(LANG) "MC(machine,axis,violation,omit,Label)"        "出力を省略"

# Add/edit notes for a set of custom commands to be exported.
::msgcat::mcset $gPB(LANG) "MC(func,edit_note,Label)"                     "注記を編集"

#=============================================================================
# pb12.00
#=============================================================================
# Short for "information about ..."
::msgcat::mcset $gPB(LANG) "MC(block,info_popup,Label)"                   "情報"

#=============================================================================
# pb11.02
#=============================================================================
# Markers of events taking place at the start & end of cut regions.
::msgcat::mcset $gPB(LANG) "MC(event,misc,region_start_marker,name)"      "Region Markerの開始"
::msgcat::mcset $gPB(LANG) "MC(event,misc,region_end_marker,name)"        "Region Markerの最後"

#=============================================================================
# pb11.01
#=============================================================================
# A comment of Tcl language
::msgcat::mcset $gPB(LANG) "MC(event,combo,tcl_line,Label)"               "Tclコメント"
...
::msgcat::mcset $gPB(LANG) "MC(ude,import,up,Label)"              "アップ"
::msgcat::mcset $gPB(LANG) "MC(ude,import,down,Label)"            "ダウン"
...
::msgcat::mcset $gPB(LANG) "MC(main,help,manual,Balloon)"           "オンラインヘルプです"
...
## Tool Bar
##
::msgcat::mcset $gPB(LANG) "MC(tool,new,Label)"                     "新規作成"
...
::msgcat::mcset $gPB(LANG) "MC(tool,manual,Context)"                "オンラインヘルプです"
...
::msgcat::mcset $gPB(LANG) "MC(nav_button,yes,Label)"               "はい"
::msgcat::mcset $gPB(LANG) "MC(nav_button,yes,Context)"             "はい"
::msgcat::mcset $gPB(LANG) "MC(nav_button,no,Label)"                "いいえ"
::msgcat::mcset $gPB(LANG) "MC(nav_button,no,Context)"              "いいえ"
...
::msgcat::mcset $gPB(LANG) "MC(wseq,popup_delete,Label)"            "削除"
...
::msgcat::mcset $gPB(LANG) "MC(link_post,delete,Label)"             "削除"
...
::msgcat::mcset $gPB(LANG) "MC(block,delete_popup,Label)"           "削除"
...
::msgcat::mcset $gPB(LANG) "MC(ude,editor,popup,DELETE)"            "削除"
...
::msgcat::mcset $gPB(LANG) "MC(udc,drill,simulate,INFO)"            "このイベントはシミュレーションサイクルです。"
...
::msgcat::mcset $gPB(LANG) "MC(isv,spec_command,delete,Label)"      "削除"
