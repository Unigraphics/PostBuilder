...
::msgcat::mcset $gPB(LANG) "MC(listing,tran_path,check,Label)"            "Выходная траектория перехода"

::msgcat::mcset $gPB(LANG) "MC(listing,tran_path,check,Context)"          "Этот переключатель позволит NX/постпроцессору обрабатывать и выводить события, возникшие в результате перехода между операциями."
...
::msgcat::mcset $gPB(LANG) "MC(block,def_popup,Label)"                    "Определение"

::msgcat::mcset $gPB(LANG) "MC(msg,info,obj_undef)"                       "Сведения об этом объекте не сконфигурированы."

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_cmd_by_pb)"                   "Команда Виртуальный цифровой контроллер сгенерирована в Генераторе постпроцессоров для передачи настроек постпроцессора в драйвер виртуального цифрового контроллера."

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_util)"                        "Команды утилиты виртуального цифрового контроллера заданы в PB_CMD_vnc__define_misc_procs."

::msgcat::mcset $gPB(LANG) "MC(msg,info,sim_cmd)"                         "Команды SIM, который будут использования в симуляции."

::msgcat::mcset $gPB(LANG) "MC(msg,info,pb_vnc_cmd)"                      "Команды, сгенерированные в Постпроцессоре, для обеспечения взаимодействия ключевых событий постпроцессора с драйвером виртуального цифрового контроллера."

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_broker_cmd)"                  "Команда Брокер используется в драйвере виртуального цифрового контроллера для выполнения других команд."

# Confirmation when user is about to delete an event
::msgcat::mcset $gPB(LANG) "MC(msg,event,delete_confirm)"                 "Вы действительно хотите удалить событие"
...
::msgcat::mcset $gPB(LANG) "MC(event,misc,user_limit_action,name)"        "Задаваемое пользователем действие ограничения по осям"

# Skip output of NC code when rotary limit violation has occurred.
::msgcat::mcset $gPB(LANG) "MC(machine,axis,violation,omit,Label)"        "Пропустить вывод"

# Add/edit notes for a set of custom commands to be exported.
::msgcat::mcset $gPB(LANG) "MC(func,edit_note,Label)"                     "Изменить замечания"

#=============================================================================
# pb12.00
#=============================================================================
# Short for "information about ..."
::msgcat::mcset $gPB(LANG) "MC(block,info_popup,Label)"                   "Информация"

#=============================================================================
# pb11.02
#=============================================================================
# Markers of events taking place at the start & end of cut regions.
::msgcat::mcset $gPB(LANG) "MC(event,misc,region_start_marker,name)"      "Начало маркера области"
::msgcat::mcset $gPB(LANG) "MC(event,misc,region_end_marker,name)"        "Конец маркера области"

#=============================================================================
# pb11.01
#=============================================================================
# A comment of Tcl language
::msgcat::mcset $gPB(LANG) "MC(event,combo,tcl_line,Label)"               "Комментарий Tcl"
...
::msgcat::mcset $gPB(LANG) "MC(block,text_combo,Label)"             "Элемент текста"
...
::msgcat::mcset $gPB(LANG) "MC(definition,oper_txt,Label)"          "Сообщение-заглушка BLOCK"
...
::msgcat::mcset $gPB(LANG) "MC(udc,drill,simulate,INFO)"            "Это событие в цикле симуляции."
