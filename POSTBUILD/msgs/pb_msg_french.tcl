...
::msgcat::mcset $gPB(LANG) "MC(listing,tran_path,check,Label)"            "Ecrire le chemin de transition"

::msgcat::mcset $gPB(LANG) "MC(listing,tran_path,check,Context)"          "Cette bascule permet à NX/Post de traiter et écrire les événements résultant du chemin de transition entre les opérations."
...
::msgcat::mcset $gPB(LANG) "MC(block,def_popup,Label)"                    "Définition"

::msgcat::mcset $gPB(LANG) "MC(msg,info,obj_undef)"                       "Informations sur ce projet pas encore configurées."

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_cmd_by_pb)"                   "Commande VNC générée par Post Builder pour transmettre les paramètres d'un Post vers son pilote VNC."

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_util)"                        "Commandes de l'utilitaire VNC définies dans PB_CMD_vnc__define_misc_procs."

::msgcat::mcset $gPB(LANG) "MC(msg,info,sim_cmd)"                         "Commandes SIM à utiliser dans un pilote de simulation."

::msgcat::mcset $gPB(LANG) "MC(msg,info,pb_vnc_cmd)"                      "Commandes générées par Post Builder pour interfacer des événements Post clés avec un pilote VNC."

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_broker_cmd)"                  "Commande Broker utilisée dans un pilote VNC pour exécuter d'autres commandes."

# Confirmation when user is about to delete an event
::msgcat::mcset $gPB(LANG) "MC(msg,event,delete_confirm)"                 "Confirmez-vous de détruire l'événement?"
...
::msgcat::mcset $gPB(LANG) "MC(event,misc,user_limit_action,name)"        "Action Limite d'axe définie par l'utilisateur"

# Skip output of NC code when rotary limit violation has occurred.
::msgcat::mcset $gPB(LANG) "MC(machine,axis,violation,omit,Label)"        "Omettre la sortie"

# Add/edit notes for a set of custom commands to be exported.
::msgcat::mcset $gPB(LANG) "MC(func,edit_note,Label)"                     "Modifier les notes"

#=============================================================================
# pb12.00
#=============================================================================
# Short for "information about ..."
::msgcat::mcset $gPB(LANG) "MC(block,info_popup,Label)"                   "Infos"

#=============================================================================
# pb11.02
#=============================================================================
# Markers of events taking place at the start & end of cut regions.
::msgcat::mcset $gPB(LANG) "MC(event,misc,region_start_marker,name)"      "Début de marqueur de région"
::msgcat::mcset $gPB(LANG) "MC(event,misc,region_end_marker,name)"        "Fin de marqueur de région"

#=============================================================================
# pb11.01
#=============================================================================
# A comment of Tcl language
::msgcat::mcset $gPB(LANG) "MC(event,combo,tcl_line,Label)"               "Commentaire TCL"
...
::msgcat::mcset $gPB(LANG) "MC(address,modal_drop,off,Label)"       "Désactivé"
...
::msgcat::mcset $gPB(LANG) "MC(event,spindle_off,dir_m_code,off)"   "Désactivé"
...
::msgcat::mcset $gPB(LANG) "MC(event,coolant_off,m_code,off)"       "Désactivé"
...
::msgcat::mcset $gPB(LANG) "MC(udc,drill,simulate,INFO)"            "Cet événement est un cycle simulé."
