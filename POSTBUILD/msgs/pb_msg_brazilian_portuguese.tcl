...
::msgcat::mcset $gPB(LANG) "MC(listing,tran_path,check,Label)"            "Caminho de transição da saída"

::msgcat::mcset $gPB(LANG) "MC(listing,tran_path,check,Context)"          "Essa alternância habilitará o NX/Post no processamento e saída dos eventos resultantes do caminho de transição entre operações. "
...
::msgcat::mcset $gPB(LANG) "MC(block,def_popup,Label)"                    "Definição"

::msgcat::mcset $gPB(LANG) "MC(msg,info,obj_undef)"                       "Informações sobre este objeto ainda não configuradas."

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_cmd_by_pb)"                   "Comando VNC gerado pelo Post Builder para transmitir as configurações de uma coluna para seu driver VNC."

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_util)"                        "Comandos do utilitário VNC definidos em PB_CMD_vnc__define_misc_procs."

::msgcat::mcset $gPB(LANG) "MC(msg,info,sim_cmd)"                         "Comandos SIM a serem usados em um driver de simulação."

::msgcat::mcset $gPB(LANG) "MC(msg,info,pb_vnc_cmd)"                      "Comandos gerados pelo Post Builder para interatuar com os  principais eventos de coluna com um driver VNC."

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_broker_cmd)"                  "Comando do agente usado em um driver VNC para executar outros comandos."

# Confirmation when user is about to delete an event
::msgcat::mcset $gPB(LANG) "MC(msg,event,delete_confirm)"                 "Você deseja mesmo excluir o evento"
...
::msgcat::mcset $gPB(LANG) "MC(event,misc,user_limit_action,name)"        "Ação de Limite do Eixo Definido pelo Usuário"

# Skip output of NC code when rotary limit violation has occurred.
::msgcat::mcset $gPB(LANG) "MC(machine,axis,violation,omit,Label)"        "Omitir Saída"

# Add/edit notes for a set of custom commands to be exported.
::msgcat::mcset $gPB(LANG) "MC(func,edit_note,Label)"                     "Editar Notas"

#=============================================================================
# pb12.00
#=============================================================================
# Short for "information about ..."
::msgcat::mcset $gPB(LANG) "MC(block,info_popup,Label)"                   "Informações"

#=============================================================================
# pb11.02
#=============================================================================
# Markers of events taking place at the start & end of cut regions.
::msgcat::mcset $gPB(LANG) "MC(event,misc,region_start_marker,name)"      "Início do Marcador de Região"
::msgcat::mcset $gPB(LANG) "MC(event,misc,region_end_marker,name)"        "Final do Marcador de Região"

#=============================================================================
# pb11.01
#=============================================================================
# A comment of Tcl language
::msgcat::mcset $gPB(LANG) "MC(event,combo,tcl_line,Label)"               "Comentário Tcl"
...
::msgcat::mcset $gPB(LANG) "MC(definition,oper_txt,Label)"          "BLOCO Mensagem Fictícia"
...
::msgcat::mcset $gPB(LANG) "MC(udc,drill,simulate,INFO)"            "Este evento é um ciclo simulado."
