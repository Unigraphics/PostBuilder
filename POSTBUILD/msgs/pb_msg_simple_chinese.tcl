...
::msgcat::mcset pb_msg_simple_chinese "MC(msg,no_license_title)"               "UG/Post Builder 非许可使用"
::msgcat::mcset pb_msg_simple_chinese "MC(msg,no_license_dialog)"              "您被允许在不具备适当许可证的情况下\n使用 UG/Post Builder。但您\n不能保存随后的操作。"
...
::msgcat::mcset pb_msg_simple_chinese "MC(inposts,title,Label)"                "安装后处理"
...
::msgcat::mcset pb_msg_simple_chinese "MC(machine,gen,step_size,Label)"            "线性运动转数"
::msgcat::mcset pb_msg_simple_chinese "MC(machine,gen,step_size,min,Label)"        "最小值"
::msgcat::mcset pb_msg_simple_chinese "MC(machine,gen,step_size,min,Context)"      "最小转数"
...
::msgcat::mcset pb_msg_simple_chinese "MC(machine,gen,lathe_post,select_name,Context)" "选择简单车铣后处理要使用的车后处理名称。假设此后处理存在于 UG/Post 运行时的 \\\$UGII_CAM_POST_DIR 目录中，否则将使用铣后处理所在目录中相同名称的后处理。"
...
::msgcat::mcset pb_msg_simple_chinese "MC(machine,axis,rotary_res,Label)"          "旋转运动转数（度）"
::msgcat::mcset pb_msg_simple_chinese "MC(machine,axis,rotary_res,Context)"        "指定旋转运动转数（度）。"
...
::msgcat::mcset pb_msg_simple_chinese "MC(link_post,head,Context)"             "复杂机床可以在多种加工模式中使用不同运动学执行加工操作。每组运动学可以作为 UG/Post 的独立机头。需要用特定机头执行的加工操作将作为一组放在机床或加工方法视图中。\\\"机头\\\" UDE 随后将被指派到该组，以指定此机头的名称。"
...
::msgcat::mcset pb_msg_simple_chinese "MC(link_post,select_name,Context)"      "选择要指派到机头的后处理名称。假设该后处理存在于主后处理位于 UG/Post 运行时的目录中，否则将使用 \\\$UGII_CAM_POST_DIR 目录中名称相同的后处理。"
...
::msgcat::mcset pb_msg_simple_chinese "MC(block,msg,min_word)"                 "一个块必须至少包含一个文字。"
...
::msgcat::mcset pb_msg_simple_chinese "MC(event,cycle,customize,Context)"      "此开关允许您定制一个循环。\n\n默认情况下，每个循环的基本构造由“常用参数”的设置定义。每个循环中的这些常用单元修改受限。\n\n打开此开关允许您获得对循环配置的完全控制。对“常用参数”所作的更改将不再影响定制的循环。"
...
::msgcat::mcset pb_msg_simple_chinese "MC(event,cycle,com_param,name)"         "常用参数"
