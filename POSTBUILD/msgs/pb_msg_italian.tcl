...
::msgcat::mcset $gPB(LANG) "MC(event,misc,pattern_csys_start,name)"       "Sistema di coordinate inizio gruppo di pattern"
::msgcat::mcset $gPB(LANG) "MC(event,misc,pattern_csys_end,name)"         "Sistema di coordinate fine gruppo di pattern"

# Toggle to signal post being able to process and output subprograms
::msgcat::mcset $gPB(LANG) "MC(listing,subprog_out,check,Label)"          "Attiva output di sottoprogramma"

::msgcat::mcset $gPB(LANG) "MC(listing,subprog_out,check,Context)"        "Se attivata, l'opzione notifica al processore base di NX/Post che il post è in grado di produrre output di sottoprogramma."

::msgcat::mcset $gPB(LANG) "MC(address,leader,err_msg)"                   "La direttrice di indirizzo X, Y, Z, fourth_axis, fifth_axis e N non deve essere configurata con un'espressione contenente variabili. \n\nÈ possibile disattivare questo controllo impostando gPB(ALLOW_VAR_SYS_LEADER) su \"1\" nel file ui_pb_user_resource.tcl."

#=============================================================================
# pb1872
#=============================================================================
# Event triggered, during lathe roughing cycle, before the last segment of contour geometry is processed.
::msgcat::mcset $gPB(LANG) "MC(event,misc,before_contour_end,name)"       "Prima della fine del contorno"
...
::msgcat::mcset $gPB(LANG) "MC(listing,tran_path,check,Label)"            "Percorso di transizione output"

::msgcat::mcset $gPB(LANG) "MC(listing,tran_path,check,Context)"          "L'attivazione/disattivazione consente l'abilitazione NX/Post all'elaborazione e all'output degli eventi derivanti dal percorso di transizione tra operazioni."
...
::msgcat::mcset $gPB(LANG) "MC(block,def_popup,Label)"                    "Definizione"

::msgcat::mcset $gPB(LANG) "MC(msg,info,obj_undef)"                       "L'informazione sull'oggetto non è ancora stata configurata."

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_cmd_by_pb)"                   "Comando VNC generato da Post Builder per trasmettere le impostazioni di un post al relativo driver VNC."

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_util)"                        "Comandi dell'utilità VNC definiti in PB_CMD_vnc__define_misc_procs."

::msgcat::mcset $gPB(LANG) "MC(msg,info,sim_cmd)"                         "Comandi SIM da utilizzare in un driver di simulazione."

::msgcat::mcset $gPB(LANG) "MC(msg,info,pb_vnc_cmd)"                      "Comandi generati da Post Builder per interfacciare eventi post chiave con un driver VNC."

::msgcat::mcset $gPB(LANG) "MC(msg,info,vnc_broker_cmd)"                  "Comandi del broker utilizzati in un driver VNC per eseguire altri comandi."

# Confirmation when user is about to delete an event
::msgcat::mcset $gPB(LANG) "MC(msg,event,delete_confirm)"                 "Eliminare l'evento"
...
::msgcat::mcset $gPB(LANG) "MC(event,misc,user_limit_action,name)"        "Azione definita dall'utente per il limite dell'asse"

# Skip output of NC code when rotary limit violation has occurred.
::msgcat::mcset $gPB(LANG) "MC(machine,axis,violation,omit,Label)"        "Ometti output"

# Add/edit notes for a set of custom commands to be exported.
::msgcat::mcset $gPB(LANG) "MC(func,edit_note,Label)"                     "Modifica note"

#=============================================================================
# pb12.00
#=============================================================================
# Short for "information about ..."
::msgcat::mcset $gPB(LANG) "MC(block,info_popup,Label)"                   "Informazioni"

#=============================================================================
# pb11.02
#=============================================================================
# Markers of events taking place at the start & end of cut regions.
::msgcat::mcset $gPB(LANG) "MC(event,misc,region_start_marker,name)"      "Contrassegno di inizio regione"
::msgcat::mcset $gPB(LANG) "MC(event,misc,region_end_marker,name)"        "Contrassegno di fine regione"

#=============================================================================
# pb11.01
#=============================================================================
# A comment of Tcl language
::msgcat::mcset $gPB(LANG) "MC(event,combo,tcl_line,Label)"               "Commento Tcl"
...
# UDE Inclusion
::msgcat::mcset $gPB(LANG) "MC(other,ude,Label)"                    "Eventi definiti dall'utente"
...
::msgcat::mcset $gPB(LANG) "MC(event,circular,clockwise)"           "Senso orario (CLW)"
...
::msgcat::mcset $gPB(LANG) "MC(ude,prev,tab,Label)"                 "Eventi definiti dall'utente"
...
::msgcat::mcset $gPB(LANG) "MC(udc,drill,simulate,INFO)"            "L'evento è un ciclo simulato."
...
::msgcat::mcset $gPB(LANG) "MC(isv,setup,scale,Label)"              "Scala"
