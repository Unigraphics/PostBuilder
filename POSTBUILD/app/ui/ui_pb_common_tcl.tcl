  ...
  foreach row_elem_obj $event::($event_obj,evt_elem_list) \
  {
   foreach elem_obj $row_elem_obj \
   {
     ...
     } elseif { $block::($block_obj,blk_type) == "comment" } \
    {
     ...
     set blk_nc_width [font measure $font $func_text]
     set evt_nc_height [expr $evt_nc_height + \
     [font metrics $font -linespace]]
...
#=======================================================================
proc UI_PB_com_DisableSpecialChars { widget key {minus_flag 0}} {
   set disable_flag 0
   switch $key \
   {
    "percent"       -
   ...
   if { $minus_flag == 0 && ![string compare $key "minus"] } {
    set disable_flag 1
   }
   ...
   switch -- [winfo class $child] {
    Toplevel {
     if { [info exists LanMapTable($child)] } {
      ...
      $gPB(main,title,Version) $ver$ext"
