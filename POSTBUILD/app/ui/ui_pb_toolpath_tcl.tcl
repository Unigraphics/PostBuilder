...
#=======================================================================
proc __path_DrawConnectLine { page_obj rect_obj } {
  return
...
proc UI_PB_tpth_DefaultCallBack { page_obj event_obj } {
  ...
  foreach evt_elem_obj $event::($event_obj,evt_elem_list) \
  {
   ...
   foreach blk_elem $block::($block_obj,active_blk_elem_list) \
   {
    block_element::readvalue $blk_elem blk_elem_attr
    if 0 {
     set addr $address::($blk_elem_attr(0),add_name)
     UI_PB_debug_ForceMsg "\n ++++++ Block elem of $block::($block_obj,block_name) \"$block::($block_obj,blk_owner)\" \
     $addr: [array get blk_elem_attr] owner: $block_element::($blk_elem,owner) \n"
    }
   ...
   foreach evt_elem_obj $event::($event_obj,evt_elem_list) \
   {
    ...
    foreach blk_elem_obj $block::($block_obj,elem_addr_list) \
    {
     if { [string match "Cycle Parameters" $def_evt_obj_attr(0)] } {
      ...
      if { ![string match "Cycle Parameters" $block_element::($blk_elem_obj,owner)] && \
       ![string match "post"             $block_element::($blk_elem_obj,owner)] } {
       array set blk_elem_attr $block_element::($blk_elem_obj,def_value)
       } else {
       if { [lsearch $blk_elem_addr_list $blk_elem_obj] < 0 } {
        set idx [lsearch $blk_obj_attr(2) $blk_elem_obj]
