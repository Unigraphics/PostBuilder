...
set gPB(REV_07FEB2017_FIX_GRID_CRASH) 1
...
proc UI_PB_tpth_LinkedPostEvent { SEQ_PAGE_OBJ SEQ_OBJ EVENT_OBJ } {
  ...
  if { [llength $item_obj_list ] > 0 } \
  {
   ...
   {
    if [info exists ::gPB_GEOM($win)] {
     set geom $::gPB_GEOM($win)
     } else {
     set geom "800x400+200+200"
    }
  ...
  if 1 { ;# It doesn't seem to be needed???
   if { ![info exists event::($event_obj,applied)] || ![info exists Page::($page_obj,rest_evt_mom_var)] } {
...
proc UI_PB_tpth_ToolPath { SEQ_PAGE_OBJ SEQ_OBJ EVENT_OBJ } {
  ...
  set Page::($page_obj,blk_WordNameList) $comb_box_elems
  UI_PB_tpth_CreateEventBlockTemplates page_obj event_obj
  if { [info exists event::($event_obj,regen_elem)] && $event::($event_obj,regen_elem) } {
   UI_PB_tpth_RestoreCallBack $page_obj $event_obj
   unset event::($event_obj,regen_elem)
   UI_PB_debug_ForceMsg "\n%%%%% Event >$event::($event_obj,event_name)< elements regenerated. %%%%%\n"
  }
...
proc UI_PB_tpth_CreateHorzPane { PAGE_OBJ EVENT_OBJ } {
  ...
  if { $canvas_flag } \
  {
   set pane [tixPanedWindow $page_id.pane -orient vertical -handlebg orange]
  ...
  if { $Page::($page_obj,add_blk) != 0 } \
  {
    ...
    } elseif { $sel_base_addr == "Command" } \
   {
    if { $new_elem_mom_var == "Custom Command" } \
    {
     ...
     } elseif { $new_elem_mom_var == "IF" } \
    {
     set blk_flag "command"
     set new_elem_mom_var "$::gPB(condition_cmd_prefix)command"
     PB_int_CreateNewCmdBlkElem new_elem_mom_var new_elem_obj
    } else \
    {
     set blk_flag ""
    ...
    set block_element::($new_elem_obj,owner) $event::($event_obj,event_name)
    } elseif { $sel_base_addr == "Condition" } \
   {
   ...
   } else \
   {
    ...
    set block_element::($new_elem_obj,owner) \
    $event::($event_obj,event_name)
    UI_PB_debug_ForceMsg "sel_base_addr = $sel_base_addr event_name = $event::($event_obj,event_name)\
    block_element,parent_name = $block_element::($new_elem_obj,parent_name)\n"
    set blk_flag ""
   }
    ...
    } elseif { $blk_flag == "command"} \
   {
    ...
    new_blk_obj New
    } elseif { $blk_flag == "condition"} \
   {
    set new_blk_obj $event_element::($add_to_evt_elem_obj,block_obj)
    UI_PB_tpth_BringCmdPage page_obj event_obj add_to_evt_elem_obj \
    new_blk_obj "Condition"
...
proc UI_PB_tpth_AddBlkElemTopOrBottom { PAGE_OBJ EVENT_OBJ ADD_TO_EVT_ELEM_OBJ \
  ...
  {
   if { $::gPB(comment_is_tcl) || [string match "$::gPB(tcl_line_blk)*" $block::($add_to_blk_obj,block_name)] } {}
   set new_block_name "$::gPB(comment_blk)"
   if { $::gPB(comment_is_tcl) } {
    set new_block_name "$::gPB(tcl_line_blk)"
   }
...
proc UI_PB_tpth_HighLightCellDividers { PAGE_OBJ EVT_ELEM_OBJ xx yy } {
  ...
  set __is_rapid_element 0
  if { [string match "rapid_traverse" $block::($block_obj,block_name)] } {
   set __is_rapid_element 1
  }
  foreach blk_elem_obj $block::($block_obj,active_blk_elem_list) \
  {
   ...
   {
    if { $__is_rapid_element } {
     set rap_add_obj $block_element::($blk_elem_obj,elem_add_obj)
     if { [string match "rapid2" $address::($rap_add_obj,add_name)] || \
      [string match "rapid3" $address::($rap_add_obj,add_name)] } {
      break
     }
    }
...
#=======================================================================
proc __tpth_DrawConnectLine { page_obj rect_obj } {
  return
  if { [info exists $rect_obj] } {
   set bot_canvas $Page::($page_obj,bot_canvas)
   set coords [$bot_canvas coords $rect_obj]
   set rect_coord(0) [lindex $coords 0]
   set rect_coord(1) [lindex $coords 1]
   set rect_coord(2) [lindex $coords 2]
   set rect_coord(3) [lindex $coords 3]
...
proc UI_PB_tpth_HighlightSeperators { page_obj event_obj sel_addr x y } {
  ...
  foreach elem_obj $event_elem_list \
  {
   ...
   if $post_flag \
   {
    ...
    [string match "post" $block::($block_obj,blk_owner)] } \
    {
     set special_flag 1
    }
   }
   if $special_flag \
   {
    ...
    $x < [expr $blk_width + [expr 6 * $rect_region]] } \
    {
     ...
     if { $post_flag == 1 && \
     [info exists ::mom_sys_arr(\$cycle_start_blk)] && $::mom_sys_arr(\$cycle_start_blk) } \
     {
      if { [info exists ::mom_sys_arr(post_startblk)] && ![string match "" $::mom_sys_arr(post_startblk)] } \
      {
       if { [string match "PB_CMD_config_cycle_start" $block::($block_obj,block_name)] } \
       {
        set Page::($page_obj,add_blk) 0
        continue
       }
    ...
    $x < [expr $blk_width + [expr 6 * $rect_region]] } \
    {
     if { [info exists ::mom_sys_arr(\$cycle_start_blk)] && $::mom_sys_arr(\$cycle_start_blk) } \
     {
      if { [string match "just_mcall_sinumerik" $block::($block_obj,block_name)] } \
      {
       set Page::($page_obj,add_blk) 0
...
#=======================================================================
proc UI_PB_tpth_UnsetTpthEvtBlkAttr { EVENT_OBJ {page_obj 0} } {
  upvar $EVENT_OBJ event_obj
  if { [llength $event::($event_obj,evt_elem_list)] > 0 } \
  {
   foreach elem_obj $event::($event_obj,evt_elem_list) \
   {
    ...
    if { $block::($block_obj,sep_id) != "" } \
    {
     if { $page_obj > 0 } {
      set c $Page::($page_obj,bot_canvas)
      $c delete [lindex $block::($block_obj,sep_id) 0]
      $c delete [lindex $block::($block_obj,sep_id) 1]
     }
...
#=======================================================================
proc __tpth_ValidateData { page_obj } {
  global mom_kin_var
  global mom_sys_arr
...
#=======================================================================
proc __tpth_PruneEventElemList { event_obj } {
  if { ![info exists event::($event_obj,evt_elem_list)] } {
   return
  }
  foreach evt_elem $event::($event_obj,evt_elem_list) {
   if { ![info exists event_element::($evt_elem,block_obj)] ||\
    ( [info exists event_element::($evt_elem,block_obj)] &&\
    ![PB_com_HasObjectExisted $event_element::($evt_elem,block_obj)] ) } {
    set idx [lsearch $event::($event_obj,evt_elem_list) $evt_elem]
    if { $idx >= 0 } {
     set event::($event_obj,evt_elem_list) [lreplace $event::($event_obj,evt_elem_list) $idx $idx]
    }
   }
  }
 }
...
proc UI_PB_tpth_ApplyCallBack { page_obj seq_page_obj seq_obj event_obj } {
  ...
  global gPB
  if 0 {
   if { ![UI_PB_com_ReturnEventNcOutAttr event_obj str_size ver_height evt_nc_output] } {
    return 0
   }
  }
  __tpth_PruneEventElemList $event_obj
  if { ![UI_PB_com_CheckIFCondition event_obj] } {
   return 0
  }
  if { [__tpth_ValidateData $page_obj] } {
   tk_messageBox -parent [UI_PB_com_AskActiveWindow] \
   -message "$gPB(machine,empty_entry_err,msg)" \
   -icon error
   return 0
  ...
  set event::($event_obj,block_nof_rows) [llength $event::($event_obj,evt_elem_list)]
  ...
  if { $seq_obj > 0 } {
   PB_int_RetCycleComAndSharedEvts cycle_com_evt cycle_shared_evts
   if { [string match "$event::($event_obj,event_name)" $cycle_com_evt] } {
    set evt_obj_list $sequence::($seq_obj,evt_obj_list)
    foreach evt $evt_obj_list {
     if { $evt > 0 && $evt != $event_obj } {
      if { [lsearch $cycle_shared_evts $event::($evt,event_name)] >= 0 } {
       set event::($evt,regen_elem) 1
       UI_PB_debug_ForceMsg "\n ++++++ cycle event $event::($evt,event_name) set regen +++++ \n"
      }
     }
    }
    set evt_elem_names ""
    foreach evt_elem $event::($event_obj,evt_elem_list) {
     ...
     lappend evt_elem_names "$block::($blk_obj,block_name)"
     if { [string match "post_startblk" $block::($blk_obj,block_name)] } {
      set ::mom_sys_arr(post_startblk) "post_startblk"
     }
    }
    set Post::($::post_object,cyc_com_evt_elem_names) $evt_elem_names
   }
  }
  ...
  set event::($event_obj,applied) 1
  update idletasks
  UI_PB_debug_ForceMsg "\n ++++++ Apply event >$event::($event_obj,event_name)< +++++ \n"
  foreach elem_obj $event::($event_obj,evt_elem_list) \
  {
   set blk_obj $event_element::($elem_obj,block_obj)
   infoObj_ForceMsg $blk_obj
  }
...
proc UI_PB_tpth_OkCallBack { page_obj seq_page_obj seq_obj event_obj } {
  ...
  if { [string match "Cycle Parameters" $event::($event_obj,event_name)] } {
   UI_PB_tpth_RestoreCallBack $page_obj $event_obj
  }
...
proc UI_PB_tpth_CancelCallBack { seq_page_obj page_obj event_obj } {
  ...
  set evt_name $event::($event_obj,event_name)
  PB_int_RetCycleComAndSharedEvts cycle_com_evt cycle_shared_evts
  UI_PB_debug_ForceMsg "\n ++++++ Cancel event >$evt_name< +++++ \n"
  lappend cycle_shared_evts $cycle_com_evt
  if { [lsearch $cycle_shared_evts $evt_name] >= 0 } {
   if { [string match $cycle_com_evt $evt_name] } {
    UI_PB_debug_ForceMsg "\n ++++++ cycle_com_evt = >$cycle_com_evt< +++++ \n"
    PB_int_RetPostBlocks post_blk_list
    set blk_name post_cycle_set
    PB_com_RetObjFrmName blk_name post_blk_list blk_obj
    if { $blk_obj > 0 } {
     array set blk_attr $block::($blk_obj,rest_value)
     block::setvalue $blk_obj blk_attr
     set block::($blk_obj,active_blk_elem_list) $block::($blk_obj,elem_addr_list)
     infoObj_ForceMsg $blk_obj
     } else {
     UI_PB_debug_ForceMsg "\n ++++++ Block post_cycle_set not found in ($post_blk_list) +++++ \n"
    }
    set seq_obj [PB_com_FindSeqObjOfEvent $event_obj]
    if { $seq_obj > 0 } {
     set evt_obj_list $sequence::($seq_obj,evt_obj_list)
     foreach evt $evt_obj_list {
      if { $evt > 0 && $evt != $event_obj } {
    ...
    if 0 { ;# Logic below doesn't work!
     foreach cyc $cycle_shared_evts {
      if { ![string match $cyc $cycle_com_evt] } {
       set cyc_evt [PB_com_FindObjFrmName $cyc cycle_event]
       UI_PB_debug_ForceMsg "\n ++++++ cycle event $cyc : $cyc_evt +++++ \n"
       set cyc_evt [PB_com_FindObjFrmName $cyc event]
       UI_PB_debug_ForceMsg "\n ++++++ cycle event $cyc : $cyc_evt +++++ \n"
       if { $cyc_evt > 0 } {
        set event::($cyc_evt,regen_elem) 1
       }
      }
     }
    }
  ...
  set seq_obj $Page::($seq_page_obj,active_seq)
  __tpth_UpdateSequenceEvent seq_page_obj seq_obj event_obj
  if 0 {
   foreach evt_elem_obj $event::($event_obj,evt_elem_list) \
   {
...
proc UI_PB_tpth_DeleteTpthEventBlkAttr { PAGE_OBJ EVENT_OBJ } {
  ...
  if { [string compare $event::($event_obj,evt_elem_list) ""] != 0 } \
  {
   ...
   {
    set block_obj $event_element::($evt_elem_obj,block_obj)
    UI_PB_debug_ForceMsg "\n +++++ Deleting block $block::($block_obj,block_name) +++\n"
...
proc UI_PB_tpth_DeleteChangeEvtElems { EVENT_OBJ } {
  ...
  {
   set block_obj $event_element::($elem_obj,block_obj)
   if { ![info exists rest_evt_obj_attr(2)] } { set rest_evt_obj_attr(2) [list] }
...
proc UI_PB_tpth_UpdateBlkList { ACTIVE_ELEM_LIST CHANGE_TO_ELEM_LIST } {
  ...
  {
   if { ![info exists block::($blk_obj,blk_type)] } {
    continue
   }
...
proc UI_PB_tpth_DefaultCallBack { page_obj event_obj } {
  ...
  global mom_kin_var
  UI_PB_tpth_DeleteTpthEventBlkAttr page_obj event_obj
  PB_int_RetCycleComAndSharedEvts cyc_com_evt cyc_shared_evts
  if 0 {
   if { [lsearch $cyc_shared_evts $event::($event_obj,event_name)] >= 0 } {
    set seq_obj [PB_com_FindSeqObjOfEvent $event_obj]
    if { $seq_obj > 0 && [string match "Cycles" $sequence::($seq_obj,seq_name)] } {
     set evt_obj_list $sequence::($seq_obj,evt_obj_list)
     PB_com_RetObjFrmName cyc_com_evt evt_obj_list com_evt_obj
     UI_PB_tpth_UpdateDefsOfCycleEvent com_evt_obj event_obj
    }
   }
  }
  array set def_evt_obj_attr $event::($event_obj,def_value)
  foreach evt_elem_obj $event::($event_obj,evt_elem_list) \
  {
   if { [string match "$cyc_com_evt" $event::($event_obj,event_name)] } {
    UI_PB_debug_ForceMsg "\n ++++++ Skip re-configuration for Default state of $cyc_com_evt \n"
    break
   }
   set block_obj $event_element::($evt_elem_obj,block_obj)
   block::DeleteFromEventElemList $block_obj evt_elem_obj
   set blk_elem_addr_arr($evt_elem_obj) [list]
   foreach blk_elem $block::($block_obj,active_blk_elem_list) \
   {
    block_element::readvalue $blk_elem blk_elem_attr
    if { ![info exists block_element::($blk_elem,owner)] } {
     UI_PB_debug_ForceMsg "\n ++++++ Block elem owner of $block::($block_obj,block_name) no found. \n"
     infoObj_ForceMsg $blk_elem
     set block_element::($blk_elem,owner) "Cycle Parameters"
    }
    if { [string match "Cycle Parameters" $block_element::($blk_elem,owner)] || \
     [string match "post"             $block_element::($blk_elem,owner)] } {
     if { ![string match "Cycle Parameters" $def_evt_obj_attr(0)] } {
      lappend blk_elem_addr_arr($evt_elem_obj) $blk_elem_attr(0)
     }
     } else {
     address::DeleteFromBlkElemList $blk_elem_attr(0) blk_elem
    }
  ...
  set active_elem_list $event::($event_obj,evt_elem_list)
  set def_elem_list $def_evt_obj_attr(2) ;# List of evt elem
  ...
  if { [string compare $event::($event_obj,evt_elem_list) ""] != 0 } \
  {
   foreach evt_elem_obj $event::($event_obj,evt_elem_list) \
   {
    if { [info exists blk_elem_addr_arr($evt_elem_obj)] && [llength $blk_elem_addr_arr($evt_elem_obj)] } {
     array set evt_elem_obj_attr $event_element::($evt_elem_obj,rest_value)
     array set def_evt_elem_attr $event_element::($evt_elem_obj,def_value)
     set idx 0
     foreach addr $evt_elem_obj_attr(4) {
      if { [lsearch $blk_elem_addr_arr($evt_elem_obj) $addr] < 0 } {
       set evt_elem_obj_attr(4) [lreplace $evt_elem_obj_attr(4) $idx $idx]
       } else {
       incr idx
      }
     }
     set evt_elem_obj_attr(4) [lmerge $evt_elem_obj_attr(4) $def_evt_elem_attr(4)]
     if [info exists def_evt_elem_attr(5)] {
      set evt_elem_obj_attr(5) $def_evt_elem_attr(5)
     }
     } else {
    ...
    UI_PB_debug_ForceMsg "\n ++++++ def_evt_elem_attr of $block::($block_obj,block_name): $event_element::($evt_elem_obj,def_value)\n"
    UI_PB_debug_ForceMsg "\n ++++++ res_evt_elem_attr of $block::($block_obj,block_name): $event_element::($evt_elem_obj,rest_value)\n"
    set blk_elem_addr_list $block::($block_obj,elem_addr_list)  ;# Current elem list
    array set blk_obj_attr $block::($block_obj,def_value)
    UI_PB_debug_ForceMsg "\n ++++++ Current elem list of $block::($block_obj,block_name) : $blk_elem_addr_list\n"
    UI_PB_debug_ForceMsg "\n ++++++ Default elem list of $block::($block_obj,block_name) : $blk_obj_attr(2)\n"
    set blk_elem 0
    foreach blk_elem $block::($block_obj,elem_addr_list) \
    {
     if { [string match "Cycle Parameters" $def_evt_obj_attr(0)] } {
      if { [info exists block_element::($blk_elem,def_value)] } {
       array set blk_elem_attr $block_element::($blk_elem,def_value)
       set blk_elem 0 ;# To prevent error later in this function.
      }
      } else {
      if { ![PB_com_HasObjectExisted $blk_elem] } {
       UI_PB_debug_ForceMsg "\n ++++++ Block elem obj ($blk_elem) of $block::($block_obj,block_name) not defined. \n"
       continue
      }
      if { ![info exists block_element::($blk_elem,owner)] } {
       infoObj_ForceMsg $blk_elem
      ...
      if { ![string match "Cycle Parameters" $block_element::($blk_elem,owner)] && \
       ![string match "post"             $block_element::($blk_elem,owner)] } {
       array set blk_elem_attr $block_element::($blk_elem,def_value)
       UI_PB_debug_ForceMsg "\n ++++++ Fetch def_blk_elem_attr: $block_element::($blk_elem,def_value)\n"
       } else {
       if { [lsearch $blk_elem_addr_list $blk_elem] < 0 } {
        set idx [lsearch $blk_obj_attr(2) $blk_elem]
        set blk_obj_attr(2) [lreplace $blk_obj_attr(2) $idx $idx]
        set blk_elem 0
        } else {
        array set blk_elem_attr $block_element::($blk_elem,rest_value)
        UI_PB_debug_ForceMsg "\n ++++++ Fetch res_blk_elem_attr: $block_element::($blk_elem,rest_value)\n"
       }
      }
     }
     if { $blk_elem != 0 } {
      if [info exists blk_elem_attr(0)] {
       block_element::setvalue   $blk_elem blk_elem_attr
       address::AddToBlkElemList $blk_elem_attr(0) blk_elem
      }
      PB_com_unset_var blk_elem_attr
     }
    } ;# foreach blk_elem
  ...
  for { set count 0 } { $count < $no_vars } { incr count } \
  {
   ...
   } else \
   {
    ...
    {
     set data_type [UI_PB_com_RetSysVarDataType mom_var]
     set mom_sys_arr($mom_var) [UI_PB_com_RetValByDataType $def_val_mom_var($mom_var) $data_type]
    } else \
    {
...
#=======================================================================
proc UI_PB_tpth_RefreshEventDisplay { page_obj } {
  ...
  {
   UI_PB_tpth_SetPageAttributes page_obj
   UI_PB_tpth_RefreshCmdBlkElements page_obj event_obj
  }
 }
...
proc UI_PB_tpth_RestoreCallBack { page_obj event_obj args } {
  ...
  if $event::($event_obj,canvas_flag) \
  {
   ...
   UI_PB_debug_ForceMsg "\n 1.++++++ evt_elem_list num : >[llength $event::($event_obj,evt_elem_list)]< >$event::($event_obj,evt_elem_list)< ++++++ \n"
   foreach evt_elem_obj $event::($event_obj,evt_elem_list) \
   {
    set block_obj $event_element::($evt_elem_obj,block_obj)
    block::DeleteFromEventElemList $block_obj evt_elem_obj
   ...
   array set rest_evt_obj_attr $event::($event_obj,rest_value)
   if { [info exists rest_evt_obj_attr(2)] } {
    set active_elem_list $event::($event_obj,evt_elem_list)
    set rest_elem_list $rest_evt_obj_attr(2)
    UI_PB_tpth_UpdateBlkList active_elem_list rest_elem_list
    UI_PB_tpth_DeleteChangeEvtElems event_obj
   ...
   if [llength $event::($event_obj,evt_elem_list)] \
   {
    UI_PB_debug_ForceMsg "\n 2.++++++ evt_elem_list num : >[llength $event::($event_obj,evt_elem_list)]< >$event::($event_obj,evt_elem_list)< ++++++ \n"
    foreach elem_obj $event::($event_obj,evt_elem_list) \
    {
  ...
  if { [info exists Page::($page_obj,rest_evt_mom_var)] } \
  {
   ...
   if 0 {
    __Pause "5\n\
    rest_evt_mom_var = $Page::($page_obj,rest_evt_mom_var)\n\
    evt_mom_var_list = $Page::($page_obj,evt_mom_var_list)\n"
   }
  }
  if $redraw_flag \
  {
...
proc UI_PB_tpth_CycleRapidBlks { page_obj event_obj member_obj args } {
  ...
  if 0 { ;# May not be needed; due to the change in CancelCallBack.
   array set blk_attr $block::($blk_obj,rest_value)
   UI_PB_debug_ForceMsg "\n ===> Block   : $block::($blk_obj,block_name) restore point \n"
   UI_PB_debug_ForceMsg_no_trace "\n$block::($blk_obj,rest_value)\n"
   infoObj_ForceMsg $blk_obj
  }
  ...
  set event::($event_obj,cycle_rapid_to) $mom_sys_arr(\$cycle_rapto_opt)
  if 0 { ;# May not be needed; due to the change in CancelCallBack.
   set block::($blk_obj,rest_value) [array get blk_attr]
  }
...
proc UI_PB_tpth_CycleStartBlks { page_obj event_obj member_obj args } {
  ...
  UI_PB_debug_ForceMsg "\n cycle_blk_name = $cycle_blk_name \n"
  PB_com_unset_var event::($event_obj,mcall_index)
  PB_com_unset_var event::($event_obj,startblk_index)
  set elem_index 0
  foreach evt_elem_obj $event::($event_obj,evt_elem_list) \
  {
   set block_obj $event_element::($evt_elem_obj,block_obj)
   UI_PB_debug_ForceMsg "\n block_obj $block::($block_obj,block_name) \n"
   if { [string match "just_mcall_sinumerik" $block::($block_obj,block_name)] } {
    set event::($event_obj,mcall_index) $elem_index
    UI_PB_debug_ForceMsg "\n Event $event_obj $event::($event_obj,event_name) found MCALL index = $event::($event_obj,mcall_index) \n"
   }
   if { $block::($block_obj,block_name) == "$cycle_blk_name" } \
   {
    set event::($event_obj,startblk_index) $elem_index
    set strt_blk_flag 1
    break
   }
   incr elem_index
...
proc UI_PB_tpth_AdjustPreviousBlkForCycleStartBlk { page_obj event_obj } {
  ...
  if $flag \
  {
   ...
   if { ![llength $block::($block_obj,elem_addr_list)] } \
   {
    ...
    PB_com_DeleteObject $block_obj
    UI_PB_tpth_AddVirtualMCallBlock mcall_blk_obj mcall_blk_elem
    ...
    UI_PB_debug_ForceMsg "[info level 0] - 1\n"
   } else \
   {
    set i 0
    ...
    UI_PB_debug_ForceMsg "[info level 0] - 2\n"
    if 0 {
     UI_PB_tpth_AddVirtualMCallBlock mcall_blk_obj mcall_blk_elem
     set block_element::($mcall_blk_elem,owner) "post"
     set block::($mcall_blk_obj,blk_owner) $event::($event_obj,event_name)
  ...
  {
   UI_PB_debug_ForceMsg "[info level 0] - 3\n"
   if 0 {
    set i 0
    foreach evt_elem_obj $evt_elem_list \
    {
...
#=======================================================================
proc UI_PB_tpth_AddVirtualMCallBlock { BLK_OBJ BLK_ELEM } {
...
#=======================================================================
proc __tpth_DeleteCycleEvtElems { CYC_EVT_OBJ COM_EVT_NAME } {
...
proc UI_PB_tpth_CreateEvtElemFrmEvtObj { EVT_ELEM_OBJ NEW_EVT_ELEM_OBJ \
  ...
  if { [info exists event_element::($evt_elem_obj,indent)] && \
   $event_element::($evt_elem_obj,indent) > 0 } {
   set evt_elem_attr(5) $event_element::($evt_elem_obj,indent)
  }
...
proc UI_PB_tpth_UpdateDefsOfCycleEvent { COM_EVENT_OBJ CYCLE_EVT_OBJ } {
  ...
  set evt_elem_names ""
  foreach evt_elem $def_cyc_evt_attr(2) {
   set blk_obj $event_element::($evt_elem,block_obj)
   if { [info exists block::($blk_obj,block_name)] } {
    set blk_name $block::($blk_obj,block_name)
    } else {
    set blk_name $event_element::($evt_elem,evt_elem_name)
   }
   lappend evt_elem_names "$blk_name"
  }
  UI_PB_debug_ForceMsg          "\n @@@@@ Default elems of event >$cycle_evt_name< before = >$def_cyc_evt_attr(2)< @@@@@\n"
  UI_PB_debug_ForceMsg_no_trace "\n @@@@@ Default elems of event >$cycle_evt_name<        = >$evt_elem_names< @@@@@\n"
  set special_flag 0
  if { [PB_com_IsSiemensController    $::gPB(post_controller_type) 1] || \
  [PB_com_IsHeidenhainController $::gPB(post_controller_type)] } \
  {
  ...
  UI_PB_debug_ForceMsg "\n +++++ cycle $cycle_evt_name evt_elem_list = $event::($cycle_evt_obj,evt_elem_list)\n"
  foreach evt_elem_obj $event::($cycle_evt_obj,evt_elem_list) \
  {
   ...
   UI_PB_debug_ForceMsg "\n +++++ cycle block: $block::($evt_blk,block_name) owner: $block::($evt_blk,blk_owner) ret: $ret\n"
    ...
    foreach cyc_evt_elem $def_cyc_evt_attr(2) \
    {
     if [info exists event_element::($cyc_evt_elem,block_obj)] \
     {
      ...
      if { $ret_code } \
      {
       ...
       } elseif { [info exists block::($cyc_blk_obj,blk_owner)] && $block::($cyc_blk_obj,blk_owner) == "$cycle_evt_name"} \
      {
       lappend def_cyc_evt_elem $cyc_evt_elem
      }
     }
  ...
  set evt_elem_names ""
  foreach evt_elem $def_cyc_evt_elem {
   set blk_obj $event_element::($evt_elem,block_obj)
   lappend evt_elem_names "$block::($blk_obj,block_name)"
  }
  UI_PB_debug_ForceMsg          "\n @@@@@ Default elems of event >$cycle_evt_name< collected = >$def_cyc_evt_elem< @@@@@"
  UI_PB_debug_ForceMsg_no_trace "\n @@@@@ Default elems of event >$cycle_evt_name<           = >$evt_elem_names< @@@@@\n"
  set def_cyc_evt_elem_blks ""
  foreach evt_elem $def_cyc_evt_elem {
   if [info exists event_element::($evt_elem,block_obj)] {
    set evt_blk $event_element::($evt_elem,block_obj)
    lappend def_cyc_evt_elem_blks $evt_elem $block::($evt_blk,block_name)
   }
  }
  UI_PB_debug_ForceMsg "\n @@@@@ Default elems of event >$cycle_evt_name< new = >$def_cyc_evt_elem_blks< @@@@@\n"
  if 0 {
   foreach evt_elem $def_cyc_evt_attr(2) {
    if [info exists event_element::($evt_elem,block_obj)] {
     set evt_blk $event_element::($evt_elem,block_obj)
     set idx [lsearch $def_cyc_evt_elem_blks $block::($evt_blk,block_name)]
     UI_PB_debug_ForceMsg "\n @@@@@ Default elems of event >$cycle_evt_name< block = >$block::($evt_blk,block_name)< $idx @@@@@\n"
     if { $idx > 0 } {
      set new_evt_elem [lindex $def_cyc_evt_elem_blks [expr $idx - 1]]
      set idy [lsearch $def_cyc_evt_attr(2) $evt_elem]
      UI_PB_debug_ForceMsg "\n @@@@@ Default elems of event >$cycle_evt_name< exists = >$new_evt_elem $block::($evt_blk,block_name)< $idx $idy @@@@@\n"
      set def_cyc_evt_attr(2) [lreplace $def_cyc_evt_attr(2) $idy $idy $new_evt_elem]
     }
    }
   }
  }
  if 1 {
   foreach evt_elem $def_cyc_evt_attr(2) {
    infoObj_ForceMsg $evt_elem
    if [info exists event_element::($evt_elem,evt_elem_name)] {
     set idx [lsearch $def_cyc_evt_elem_blks $event_element::($evt_elem,evt_elem_name)]
     UI_PB_debug_ForceMsg "\n @@@@@ Default elems of event >$cycle_evt_name< = >$event_element::($evt_elem,evt_elem_name)< $idx @@@@@\n"
     if { $idx > 0 } {
      set new_evt_elem [lindex $def_cyc_evt_elem_blks [expr $idx - 1]]
      set idy [lsearch $def_cyc_evt_attr(2) $evt_elem]
      UI_PB_debug_ForceMsg "\n @@@@@ Default elems of event >$cycle_evt_name< exists = >$new_evt_elem $event_element::($evt_elem,evt_elem_name)< $idx $idy @@@@@\n"
      set def_cyc_evt_attr(2) [lreplace $def_cyc_evt_attr(2) $idy $idy $new_evt_elem]
     }
    }
   }
   if 0 {
    foreach evt_elem $def_cyc_evt_elem {
     if { [lsearch $def_cyc_evt_attr(2) $evt_elem] < 0 } {
      lappend def_cyc_evt_attr(2) $evt_elem
     }
    }
   }
   __tpth_OrderDefaultEventElems $cycle_evt_obj def_cyc_evt_attr

#=======================================================================
proc __ladd { list index elmt_list args } {
  if [info exists list] \
  {
   set new_list $elmt_list
   if { [llength $args] && [lindex $args 0] == "no_dup" } \
   {
    set elem [lindex $list $index]
    set new_list [ltidy $elmt_list]
    foreach e $list \
    {
  ...
  {
   foreach e $new_list {
    set list [linsert $list $index $e]
    incr index
   }
  }
  return $list
 }
 set post_startblk "PB_CMD_config_cycle_start"
 if { [info exists ::mom_sys_arr(post_startblk)] } {
  set post_startblk $::mom_sys_arr(post_startblk)
 }
 set elem_list_merged 0
 set idx 0
 foreach evt_elem $def_cyc_evt_attr(2) {
  if { ![PB_com_HasObjectExisted $evt_elem] } {
   set def_cyc_evt_attr(2) [lreplace $def_cyc_evt_attr(2) $idx $idx]
   continue
  }
  if { [string match "$post_startblk" $event_element::($evt_elem,evt_elem_name)] } {
   set def_cyc_evt_attr(2) [lreplace $def_cyc_evt_attr(2) $idx $idx]
   set idx2 0
   foreach evt_elem2 $def_cyc_evt_elem {
    if { [string match "$post_startblk" $event_element::($evt_elem2,evt_elem_name)] } {
     set def_cyc_evt_elem [__ladd $def_cyc_evt_elem $idx2 $def_cyc_evt_attr(2) "no_dup"]
     set elem_list_merged 1
     UI_PB_debug_ForceMsg          "\n @@@@@ Default elems of event >$cycle_evt_name< merged @$idx2 = >$def_cyc_evt_elem< @@@@@"
     break
    }
    incr idx2
   }
 ...
 set cycle_set_blk "post_cycle_set"
 if { [PB_com_IsSiemensController    $::gPB(post_controller_type) 1] || \
  [PB_com_IsHeidenhainController $::gPB(post_controller_type)] } {
  set cycle_set_blk "just_mcall_sinumerik"
 }
 if { !$elem_list_merged } {
  set iset [lsearch $Post::($::post_object,cyc_com_evt_elem_names) "$cycle_set_blk"]
  if { $iset >= 0 } {
   set def_cyc_evt_elem [__ladd $def_cyc_evt_elem $iset $def_cyc_evt_attr(2) "no_dup"]
   set elem_list_merged 1
   UI_PB_debug_ForceMsg          "\n @@@@@ Default elems of event >$cycle_evt_name< merged @$cycle_set_blk = >$def_cyc_evt_elem< @@@@@"
  }
 }
 if { !$elem_list_merged } {
  set def_cyc_evt_elem [__ladd $def_cyc_evt_elem end $def_cyc_evt_attr(2) "no_dup"]
  UI_PB_debug_ForceMsg          "\n @@@@@ Default elems of event >$cycle_evt_name< merged @end = >$def_cyc_evt_elem< @@@@@"
 }
 set def_cyc_evt_attr(2) $def_cyc_evt_elem
 set evt_elem_names ""
 foreach ee $def_cyc_evt_attr(2) {
  lappend evt_elem_names "$event_element::($ee,evt_elem_name)"
 }
 UI_PB_debug_ForceMsg_no_trace "\n @@@@@ Default elems of event >$cycle_evt_name<      = >$evt_elem_names< @@@@@\n"
 foreach evt_elem $def_cyc_evt_attr(2) {
  if { ![PB_com_HasObjectExisted $evt_elem] } {
   set idx [lsearch $def_cyc_evt_attr(2) $evt_elem]
   set def_cyc_evt_attr(2) [lreplace $def_cyc_evt_attr(2) $idx $idx]
   continue
  }
  if { [string match "$post_startblk" $event_element::($evt_elem,evt_elem_name)] } {
   if { ![info exists ::mom_sys_arr(\$cycle_start_blk)] || !$::mom_sys_arr(\$cycle_start_blk) } {
    set idx [lsearch $def_cyc_evt_attr(2) $evt_elem]
    set def_cyc_evt_attr(2) [lreplace $def_cyc_evt_attr(2) $idx $idx]
    UI_PB_debug_ForceMsg "\n @@@@@ Remove PB_CMD_config_cycle_start from Default elems of event >$cycle_evt_name< @@@@@\n"
    break
   }
  }
 }
...
set def_cyc_evt_attr(1) [llength $def_cyc_evt_attr(2)]
set def_cyc_evt_attr(1) [llength $def_cyc_evt_attr(2)]
set evt_elem_names ""
foreach evt_elem $def_cyc_evt_attr(2) {
 lappend evt_elem_names "$event_element::($evt_elem,evt_elem_name)"
}
UI_PB_debug_ForceMsg          "\n @@@@@ Default elems of event >$cycle_evt_name< after  = >$def_cyc_evt_attr(2)< @@@@@\n"
UI_PB_debug_ForceMsg_no_trace "\n @@@@@ Default elems of event >$cycle_evt_name<        = >$evt_elem_names< @@@@@\n"
event::DefaultValue $cycle_evt_obj def_cyc_evt_attr
}

#=======================================================================
proc __tpth_OrderDefaultEventElems { cyc_evt_obj DEF_CYC_EVT_ATTR } {
  upvar $DEF_CYC_EVT_ATTR def_cyc_evt_attr
  PB_int_RetCycleComAndSharedEvts cyc_com_evt cyc_shared_evts
  set cycle_evt_name $event::($cyc_evt_obj,event_name)
  if { [string match $cyc_com_evt $cycle_evt_name] } {
   return
  }
  set evt_elem_names ""
  foreach evt_elem $def_cyc_evt_attr(2) {
  ...
  if [info exists Post::($::post_object,cyc_com_evt_elem_names)] {
   UI_PB_debug_ForceMsg ">$cycle_evt_name<\n$Post::($::post_object,cyc_com_evt_elem_names)\n\n$evt_elem_names"
   set idx 0
   foreach evt_elem $def_cyc_evt_attr(2) {
    set blk_obj $event_element::($evt_elem,block_obj)
    set evt_elem_name $event_element::($evt_elem,evt_elem_name)
    if { [lsearch $Post::($::post_object,cyc_com_evt_elem_names) $evt_elem_name] < 0 } {
     if { [info exists block::($blk_obj,blk_owner)] && $block::($blk_obj,blk_owner) != "$cycle_evt_name"} {
      set def_cyc_evt_attr(2) [lreplace $def_cyc_evt_attr(2) $idx $idx]
      set evt_elem_names      [lreplace $evt_elem_names $idx $idx]
      incr idx -1
      UI_PB_debug_ForceMsg "\n @@@@@ Default elems of event >$cycle_evt_name< remove elem : $evt_elem_name @@@@@\n"
     }
    }
    incr idx
   }
   set icon [lsearch $Post::($::post_object,cyc_com_evt_elem_names) "PB_CMD_config_cycle_start"]
   set icon -1
   if { $icon >= 0 } { ;# Get next elem from master list
    set elem [lindex $Post::($::post_object,cyc_com_evt_elem_names) [expr $icon +1]]
    set inxt [lsearch $evt_elem_names "$elem"]
    if { $inxt > 0 } { ;# Stick PB_CMD_config_cycle_start into list before inxt.
     set icon [lsearch $evt_elem_names "PB_CMD_config_cycle_start"]
     if { $icon >= $inxt } {
      set con_elem [lindex $def_cyc_evt_attr(2) $icon]
      set def_cyc_evt_attr(2) [lreplace $def_cyc_evt_attr(2) $icon $icon]
      set def_cyc_evt_attr(2) [linsert  $def_cyc_evt_attr(2) $inxt $con_elem]
      UI_PB_debug_ForceMsg "\n @@@@@ Moved PB_CMD_config_cycle_start of Default elems of event >$cycle_evt_name< from $icon to $inxt @@@@@\n"
     }
    }
   }
   set def_cyc_evt_attr(1) [llength $def_cyc_evt_attr(2)]
...
#=======================================================================
proc __tpth_OrderDefaultEventElems-X { cyc_evt_obj DEF_CYC_EVT_ATTR } {
  upvar $DEF_CYC_EVT_ATTR def_cyc_evt_attr
  set cycle_evt_name $event::($cyc_evt_obj,event_name)
  set evt_elem_names ""
  foreach evt_elem $def_cyc_evt_attr(2) {
...
#=======================================================================
proc __tpth_GetCycleGCodeVar { cycle_event_name } {
  switch $cycle_event_name {
   ...
   "Tap Float" {
    set gcode "\$mom_sys_cycle_tap_float_code"
   }
   "Tap Deep" {
    set gcode "\$mom_sys_cycle_tap_deep_code"
   }
   "Tap Break Chip" {
    set gcode "\$mom_sys_cycle_tap_break_chip_code"
   }
   "Tap" {
...
#=======================================================================
proc UI_PB_tpth_IsCycleCustom-b4_pb1003 { cycle_event_name } {
  global mom_sys_arr
  set custom_cycle 0
  switch $cycle_event_name {
   "Drill" {
   ...
   "Tap Float" {
    if [info exists mom_sys_arr(\$mom_sys_sim_cycle_tap_float)] {
     if $mom_sys_arr(\$mom_sys_sim_cycle_tap_float) { set custom_cycle 1 }
    }
   }
   "Tap Deep" {
    if [info exists mom_sys_arr(\$mom_sys_sim_cycle_tap_deep)] {
     if $mom_sys_arr(\$mom_sys_sim_cycle_tap_deep) { set custom_cycle 1 }
    }
   }
   "Tap Break Chip" {
    if [info exists mom_sys_arr(\$mom_sys_sim_cycle_tap_break_chip)] {
     if $mom_sys_arr(\$mom_sys_sim_cycle_tap_break_chip) { set custom_cycle 1 }
    }
   }
   "Bore" {
...
proc UI_PB_tpth_IsCycleCustom { cycle_event_name } {
  ...
  switch $cycle_event_name {
   "Drill" {
    if { [info exists mom_sys_arr(\$mom_sys_sim_cycle_drill)] &&\
     $mom_sys_arr(\$mom_sys_sim_cycle_drill) == 0 } { set custom_cycle 0 }
   }
   "Drill Dwell" {
    if { [info exists mom_sys_arr(\$mom_sys_sim_cycle_drill_dwell)] &&\
     $mom_sys_arr(\$mom_sys_sim_cycle_drill_dwell) == 0 } { set custom_cycle 0 }
   }
   "Drill Deep" {
    if { [info exists mom_sys_arr(\$mom_sys_sim_cycle_drill_deep)] &&\
     $mom_sys_arr(\$mom_sys_sim_cycle_drill_deep) == 0 } { set custom_cycle 0 }
   }
   "Drill Break Chip" {
    if { [info exists mom_sys_arr(\$mom_sys_sim_cycle_drill_break_chip)] &&\
     $mom_sys_arr(\$mom_sys_sim_cycle_drill_break_chip) == 0 } { set custom_cycle 0 }
   }
   "Tap" {
    if { [info exists mom_sys_arr(\$mom_sys_sim_cycle_tap)] &&\
     $mom_sys_arr(\$mom_sys_sim_cycle_tap) == 0 } { set custom_cycle 0 }
   }
   "Tap Float" {
    if { [info exists mom_sys_arr(\$mom_sys_sim_cycle_tap_float)] } {
     if { $mom_sys_arr(\$mom_sys_sim_cycle_tap_float) == 0 } { set custom_cycle 0 }
     } else {
     if { [info exists mom_sys_arr(\$mom_sys_sim_cycle_tap)] &&\
      $mom_sys_arr(\$mom_sys_sim_cycle_tap) == 0 } { set custom_cycle 0 }
    }
   ...
   "Tap Deep" {
    if { [info exists mom_sys_arr(\$mom_sys_sim_cycle_tap_deep)] } {
     if { $mom_sys_arr(\$mom_sys_sim_cycle_tap_deep) == 0 } { set custom_cycle 0 }
     } else {
     if { [info exists mom_sys_arr(\$mom_sys_sim_cycle_tap)] &&\
      $mom_sys_arr(\$mom_sys_sim_cycle_tap) == 0 } { set custom_cycle 0 }
    }
   ...
   "Tap Break Chip" {
    if { [info exists mom_sys_arr(\$mom_sys_sim_cycle_tap_break_chip)] } {
     if { $mom_sys_arr(\$mom_sys_sim_cycle_tap_break_chip) == 0 } { set custom_cycle 0 }
     } else {
     if { [info exists mom_sys_arr(\$mom_sys_sim_cycle_tap)] &&\
      $mom_sys_arr(\$mom_sys_sim_cycle_tap) == 0 } { set custom_cycle 0 }
    }
   ...
   "Bore" {
    if { [info exists mom_sys_arr(\$mom_sys_sim_cycle_bore)] &&\
     $mom_sys_arr(\$mom_sys_sim_cycle_bore) == 0 } { set custom_cycle 0 }
   }
   "Bore Drag" {
    if { [info exists mom_sys_arr(\$mom_sys_sim_cycle_bore_drag)] &&\
     $mom_sys_arr(\$mom_sys_sim_cycle_bore_drag) == 0 } { set custom_cycle 0 }
   }
   "Bore No Drag" {
    if { [info exists mom_sys_arr(\$mom_sys_sim_cycle_bore_nodrag)] &&\
     $mom_sys_arr(\$mom_sys_sim_cycle_bore_nodrag) == 0 } { set custom_cycle 0 }
   }
   "Bore Manual" {
    if { [info exists mom_sys_arr(\$mom_sys_sim_cycle_bore_manual)] &&\
     $mom_sys_arr(\$mom_sys_sim_cycle_bore_manual) == 0 } { set custom_cycle 0 }
   }
   "Bore Dwell" {
    if { [info exists mom_sys_arr(\$mom_sys_sim_cycle_bore_dwell)] &&\
     $mom_sys_arr(\$mom_sys_sim_cycle_bore_dwell) == 0 } { set custom_cycle 0 }
   }
   "Bore Manual Dwell" {
    if { [info exists mom_sys_arr(\$mom_sys_sim_cycle_bore_manual_dwell)] &&\
     $mom_sys_arr(\$mom_sys_sim_cycle_bore_manual_dwell) == 0 } { set custom_cycle 0 }
   }
   "Bore Back" {
    if { [info exists mom_sys_arr(\$mom_sys_sim_cycle_bore_back)] &&\
     $mom_sys_arr(\$mom_sys_sim_cycle_bore_back) == 0 } { set custom_cycle 0 }
   }
   default {
    set var mom_sys_sim_cycle_[join [string tolower $cycle_event_name] _]
    if { [info exists mom_sys_arr(\$$var)] &&\
     $mom_sys_arr(\$$var) == 0 } { set custom_cycle 0 }
   }
  }
  return $custom_cycle
 }
...
proc UI_PB_tpth_UpdateCycleEvent { EVENT_OBJ SEQ_OBJ } {
  ...
  {
   set n_evt_elems [llength $event::($event_obj,evt_elem_list)]
   if $n_evt_elems \
   {
    foreach cycle_event_name $cycle_shared_evts \
    {
     ...
     if { [info exists cycle_evt_obj] && $cycle_evt_obj > 0 } {
   ...
   foreach evt_elem_obj $evt_elem_obj_list {
    ...
    if { $blk_obj } {
     ...
     switch $block_type {
      ...
      "comment" {
       set cmd_blk_elem [lindex $block::($blk_obj,elem_addr_list) 0]
       if { $::gPB(comment_is_tcl) || [string match "$::gPB(tcl_line_blk)*" $block::($blk_obj,block_name)] } {}
   ...
   } else \
   {
    ...
    if [info exists event::($cycle_evt_obj,custom_def)] \
    {
     ...
     foreach blk_elem_obj $blk_elem_obj_list {
      ...
      $block_element::($blk_elem_obj,elem_mom_variable)] {
       set block_element::($blk_elem_obj,elem_mom_variable) \
       [__tpth_GetCycleGCodeVar $cycle_event_name]
    ...
    } else \
    {
     ...
     foreach cyc_evt_elem $event::($cycle_evt_obj,evt_elem_list) \
     {
      ...
      UI_PB_debug_ForceMsg "\n +++++ cycle event block: $block::($cyc_evt_blk,block_name) ret_code: $ret_code\n"
      if { $ret_code == 1 } \
      {
       ...
       if 0 {
        ...
        [lmerge $event_element::($evt_elem_obj,force_addr_list) $event_element::($cyc_evt_elem,force_addr_list)]
        } else {
        set com_addr_objs ""
        foreach blk_elem $block::($com_evt_blk,elem_addr_list) {
         lappend com_addr_objs $block_element::($blk_elem,elem_add_obj)
        }
        set com_force_attrs $event_element::($evt_elem_obj,force_addr_list)
        set cyc_force_attrs $event_element::($cyc_evt_elem,force_addr_list)
        UI_PB_debug_ForceMsg "\n +++++ cycle block bef: $block::($cyc_evt_blk,block_name) force attr: $cyc_force_attrs\n"
        set idx 0
        foreach addr $cyc_force_attrs {
         if { [lsearch $com_addr_objs $addr] >= 0 } {
          set cyc_force_attrs [lreplace $cyc_force_attrs $idx $idx]
          } else {
          incr idx
         }
        }
        set event_element::($cyc_evt_elem,force_addr_list) [lmerge $cyc_force_attrs $com_force_attrs]
        UI_PB_debug_ForceMsg "\n +++++ cycle block aft: $block::($cyc_evt_blk,block_name) \
        force attr: $event_element::($cyc_evt_elem,force_addr_list)\n"
        if { [info exists event_element::($evt_elem_obj,indent)] } {
         set event_element::($cyc_evt_elem,indent) \
         $event_element::($evt_elem_obj,indent)
        }
       } ;# if 0 else
  ...
  if { ![info exists event::($cycle_evt_obj,custom_def)] } \
  {
   foreach evt_elem_obj $event::($com_evt_obj,evt_elem_list) \
   {
    ...
    $event_element::($evt_elem_obj,indent) > 0 } \
    {
     ...
     {
      set cyc_evt_blk $event_element::($cyc_evt_elem,block_obj)
      if [string match "$block::($cyc_evt_blk,block_name)" $block::($com_evt_blk,block_name)] {
       UI_PB_debug_ForceMsg "\n +++++ matched cycle block: $block::($com_evt_blk,block_name) indent: $event_element::($evt_elem_obj,indent)\n"
       set event_element::($cyc_evt_elem,indent) \
       $event_element::($evt_elem_obj,indent)
      }
     }
...
proc UI_PB_tpth_CreateDividers { PAGE_OBJ BLOCK_OBJ } {
  ...
  set x0 [expr $x_orig - 3]
  set x1 [expr $x0 + 6 + [expr [expr $w_divi + $w_cell] * $n_comp] + \
  $w_divi * 3]
...
#=======================================================================
proc UI_PB_tpth_RefreshCmdBlkElements { PAGE_OBJ EVENT_OBJ } {
  upvar $PAGE_OBJ page_obj
  upvar $EVENT_OBJ event_obj
  set event_name $event::($event_obj,event_name)
  if { ![info exists event::($event_obj,canvas_flag)] || \
  !$event::($event_obj,canvas_flag) } \
  {
   UI_PB_debug_ForceMsg "\n+++ Short reconstruction for event >$event_name< <<<\n"
   return
  }
  foreach event_elem $event::($event_obj,evt_elem_list) \
  {
   if { ![info exists event_element::($event_elem,block_obj)] } {
    continue
   }
   set block_obj $event_element::($event_elem,block_obj)
   if { $block::($block_obj,blk_type) == "command" } {
    set blk_elem [lindex $block::($block_obj,elem_addr_list) 0]
    set cmd $block_element::($blk_elem,elem_mom_variable)
    if { $cmd != "ELSE" && $cmd != "END" && ![string match "MOM_*" $cmd] } {
     UI_PB_debug_ForceMsg "\n+++ Rename block name >$block::($block_obj,block_name)< to command name >$command::($cmd,name)< <<<\n"
     set cmd_name $command::($cmd,name)
     set block::($block_obj,block_name) "$cmd_name"
     foreach ee $block::($block_obj,evt_elem_list) {
      set event_element::($ee,evt_elem_name) "$cmd_name"
     }
    }
    UI_PB_blk_DeleteCellsIcons page_obj block_obj
    UI_PB_blk_CreateBlockImages page_obj block_obj
    ...
    UI_PB_blk_CreateBlockCells page_obj block_obj active_blk_elem_obj event_name $event_elem
   }
   set Page::($page_obj,y_orig) [expr $Page::($page_obj,y_orig) + $Page::($page_obj,blk_blkdist)]
  }
  UI_PB_tpth_IconBindProcs page_obj event_obj
 }
...
proc UI_PB_tpth_CreateBlkElements { PAGE_OBJ EVENT_OBJ } {
  ...
  !$event::($event_obj,canvas_flag) } \
  {
   UI_PB_debug_ForceMsg "\n+++ Shorting block elem reconstruction, no output for event >event_name< <<<\n"
   return
  }
  UI_PB_debug_ForceMsg "\n Event $event_obj $event::($event_obj,event_name) \n"
  if 0 {
   if { [info exist event::($event_obj,mcall_index)] } {
    UI_PB_debug_ForceMsg "\n MCALL index = $event::($event_obj,mcall_index) \n"
    set mcall_index 0
    foreach event_elem $event::($event_obj,evt_elem_list) \
    {
     if { ![info exists event_element::($event_elem,block_obj)] } {
     ...
     set block_obj $event_element::($event_elem,block_obj)
     if { [string match "just_mcall_sinumerik" $block::($block_obj,block_name)] } {
      if { $mcall_index != $event::($event_obj,mcall_index) } {
       set event::($event_obj,evt_elem_list) [lreplace $event::($event_obj,evt_elem_list) $mcall_index $mcall_index]
       set event::($event_obj,evt_elem_list) \
       [linsert $event::($event_obj,evt_elem_list) $event::($event_obj,mcall_index) $event_elem]
       UI_PB_debug_ForceMsg "\n+++ MCALL found @ $mcall_index should be @ $event::($event_obj,mcall_index) <<<\n"
       break
      }
     }
     incr mcall_index
  ...
  UI_PB_debug_ForceMsg "\n mom_sys_arr(post_startblk) is cycle_blk_name = >$cycle_blk_name< \n"
  if { $cycle_blk_name != "" && [info exists event::($event_obj,startblk_index)] } {
   UI_PB_debug_ForceMsg "\n STARTBLK index = $event::($event_obj,startblk_index) cycle_blk_name = $cycle_blk_name \n"
   set stblk_index 0
   foreach event_elem $event::($event_obj,evt_elem_list) \
   {
    if { ![info exists event_element::($event_elem,block_obj)] } {
     incr stblk_index
     UI_PB_debug_ForceMsg "\n block_obj of $event_element::($event_elem,evt_elem_name) not defined for event_element >$event_elem< stblk_index = $stblk_index \n"
     continue
    }
    set block_obj $event_element::($event_elem,block_obj)
    UI_PB_debug_ForceMsg "\n block name : >$block::($block_obj,block_name)< stblk_index = $stblk_index \n"
    if { [string match "$cycle_blk_name" $block::($block_obj,block_name)] } {
     if { $stblk_index != $event::($event_obj,startblk_index) } {
      set event::($event_obj,evt_elem_list) [lreplace $event::($event_obj,evt_elem_list) $stblk_index $stblk_index]
      set event::($event_obj,evt_elem_list) \
      [linsert $event::($event_obj,evt_elem_list) $event::($event_obj,startblk_index) $event_elem]
      UI_PB_debug_ForceMsg "\n+++ STARTBLK found @ $stblk_index should be @ $event::($event_obj,startblk_index) <<<\n"
      break
     }
    }
    incr stblk_index
  ...
  {
   if { ![info exists event_element::($event_elem,block_obj)] } {
    UI_PB_debug_ForceMsg "\n+++ <[info level 0]> block_obj of event_element >$event_elem< doesn't exist! +++\n"
    incr ievt
    continue
   }
   set block_obj $event_element::($event_elem,block_obj)
   UI_PB_debug_ForceMsg "\n+++ <[info level 0]> block name = >$block::($block_obj,block_name)< +++\n"
   if [string match "post_startblk" $block::($block_obj,block_name)] {
    set mom_sys_arr(post_startblk) "post_startblk"
   }
   if 0 { ;# Didn't seem to matter!
    if { [string match "just_mcall_sinumerik" $block::($block_obj,block_name)] } {
     set event::($event_obj,mcall_index) $ievt
    }
   }
   if { [string match "*PB_CMD_config_cycle_start" $block::($block_obj,block_name)] } {
    set cmd_obj [PB_com_FindObjFrmName "PB_CMD_config_cycle_start" command]
    if { $cmd_obj > 0 } {
     UI_PB_debug_ForceMsg "\n+++ Recreate PB_CMD_config_cycle_start +++\n"
     set new_elem_mom_var "PB_CMD_config_cycle_start"
     PB_int_CreateNewCmdBlkElem new_elem_mom_var blk_elem
     set block_element::($blk_elem,elem_mom_variable) $cmd_obj
     set block::($block_obj,elem_addr_list)       [list $blk_elem]
     set block::($block_obj,block_nof_elements)   1
     set block::($block_obj,block_name)           "PB_CMD_config_cycle_start"
     set block::($block_obj,blk_type)             "command"
     set block::($block_obj,active_blk_elem_list) [list $blk_elem]
     set block::($block_obj,owner)                "post"
     set block_element::($blk_elem,owner)         "post"
     if 0 {
      Post::GetObjList  $::post_object block blk_obj_list
      if { [lsearch $blk_obj_list $block_obj] < 0 } {
...
proc UI_PB_tpth_CreateExecAttrSymbols { PAGE_OBJ EVENT_ELEM } {
  ...
  {
   set x0 $block::($block_obj,rect_x0)
...
proc UI_PB_tpth_WholeBlkFocusOn { page_obj event_obj x y } {
  ...
  set display_tip 0
  if [info exists event_element::($act_evt_elem,block_obj)] {
   set block_obj $event_element::($act_evt_elem,block_obj)
   if { $block::($block_obj,blk_type) != "comment" && $block::($block_obj,blk_type) != "command" } {
    set display_tip 1
   }
  }
  if { $display_tip && $::gPB_help_tips(state) } \
  {
   ...
   if { $tpth_item_focus_on == 0 } \
   {
    ...
    set c $Page::($page_obj,bot_canvas)
    PB_enable_balloon $c
    set ::gPB_help_tips($c) "$event_element::($act_evt_elem,evt_elem_name)"
...
proc UI_PB_tpth_WholeBlkFocusOff { page_obj } {
  ...
  if { $::tpth_item_focus_on && $::gPB_help_tips(state) } \
  {
...
proc UI_PB_tpth_WholeBlkStartDrag { page_obj event_obj x y } {
  if 0 { ;# Not working!
   if { [llength [info commands backup__UI_PB_tpth_EndDragBlk]] == 0 } {
    rename UI_PB_tpth_EndDragBlk backup__UI_PB_tpth_EndDragBlk
    } else {
    rename UI_PB_tpth_EndDragBlk ""
   }
   uplevel #0 {
...
UI_PB_tpth_GetEvtElemByPos page_obj event_obj act_evt_elem $x $y
if { $act_evt_elem == 0 } { return }
UI_PB_ScrollCanvasWin $page_obj $x $y
  ...
  switch $blk_type {
   ...
   comment  {
    ...
    if { [string match "$::gPB(tcl_line_blk)*" $block::($block_obj,block_name)] } {
     set blk_nc_code "# $block_element::($elem_obj,elem_mom_variable)"
    }
   }
   command  {
    ...
    if { [string match "MOM_*" $cmd_obj] ||\
     ...
     UI_PB_debug_ForceMsg          "\n+++++ Condition command's block_name : >$block::($block_obj,block_name)< +++++\n"
     UI_PB_debug_ForceMsg_no_trace "\n+++++ Condition command's cmd name   : >$command::($cmd_obj,name)< +++++\n"
...
proc UI_PB_tpth_WholeBlkEndDrag { page_obj event_obj } {
  ...
  if { $Page::($page_obj,trash_flag) } {
   ...
   set __RestoreAnimate 0
   if { [info exists ::gPB(animation_control)] &&  $::gPB(animation_control) } {
    if { [info exists ::gPB(animation)]  &&  $::gPB(animation) } {
     set __RestoreAnimate 1
     set __AnimateStatus $::gPB(animation)
     set ::gPB(animation) 0
    }
   }
   $Page::($page_obj,bot_canvas) config -state disabled
   set Page::($page_obj,in_focus_evt_elem) $src_evt_elem  ;# Ref in UI_PB_blk_DeleteBlockElement
   set blk_obj $event_element::($src_evt_elem,block_obj)
   if { $block::($blk_obj,blk_type) == "normal" } {
    CB__DeleteOneRow $page_obj $event_obj $src_evt_elem
    } else {
    UI_PB_tpth_EndDragBlk $page_obj $event_obj
   }
   if { $__RestoreAnimate } {
    set ::gPB(animation) $__AnimateStatus
   }
   $Page::($page_obj,bot_canvas) config -state normal
   $bot_canvas bind whole_block <3> "UI_PB_tpth_WholeBlkRightButton  $page_obj $event_obj %x %y %X %Y"
   return
  }
...
proc UI_PB_tpth_WholeBlkRightButton { page_obj event_obj x y winX winY } {
  ...
  infoObj_ForceMsg $block_obj
  UI_PB_debug_ForceMsg "\n+++++ comment_blk_list: $Post::($::post_object,comment_blk_list) +++++\n"
...
proc UI_PB_tpth_AddPopupForWholeBlk { page_obj event_obj evt_elem \
  ...
  if { $exec_attr_flag } {
   ...
   foreach cmd_obj $cmd_obj_list {
    if { [string match "$gPB(check_block_cmd_prefix)*" $command::($cmd_obj,name)] == 0 } {
     continue
    }
    set cmd_name $command::($cmd_obj,name)
    set state "normal"
  ...
  if { [string match "comment" $blk_type] } {
   set block_obj $event_element::($evt_elem,block_obj)
   if { ![string match "$::gPB(tcl_line_blk)*" $block::($block_obj,block_name)] } {
    set state "normal"
    set popupvar(0) $event_element::($evt_elem,suppress_flag)
  ...
  if { [string match "event" $page_type] } {
   ...
   if { $::gPB(ENABLE_TCL_INDENTATION) } {
    set block_obj $event_element::($evt_elem,block_obj)
    set block_name $block::($block_obj,block_name)
    if { !( [string match "comment" $blk_type] && [string match "$::gPB(tcl_line_blk)*" $block_name] ) } {}
    if 0 {
     if { ![string compare $event::($event_obj,event_name) "Cycle Parameters"] } {
      $blockpopup add command -label "$::gPB(event,elem,indent,popup,Label)..." -state normal \
      -command "CB__SetEvtElemIndentation $page_obj $evt_elem"
      $blockpopup add separator
      } elseif { ![PB_evt_CheckCycleRefWord block_obj 1] &&\
      [string compare $block::($block_obj,blk_owner) "Cycle Parameters"] &&\
      [string compare $block::($block_obj,blk_owner) "post"] &&\
      [string compare $block::($block_obj,blk_owner) "NONE"] } {
      $blockpopup add command -label "$::gPB(event,elem,indent,popup,Label)..." -state normal \
      -command "CB__SetEvtElemIndentation $page_obj $evt_elem"
      $blockpopup add separator
     }
    ...
    if { ![string compare $event::($event_obj,event_name) "Cycle Parameters"] } {
     ...
     -command "CB__SetEvtElemIndentation $page_obj $evt_elem"
     $blockpopup add separator
     } elseif { ![string compare $event::($event_obj,event_name) $block::($block_obj,blk_owner)] &&\
     ![PB_evt_CheckCycleRefWord block_obj 1] } {
     $blockpopup add command -label "$::gPB(event,elem,indent,popup,Label)..." -state normal \
     -command "CB__SetEvtElemIndentation $page_obj $evt_elem"
     $blockpopup add separator
    }
   }
   if { [PB_com_is_true event_element::($evt_elem,block_obj)] } {
    set block_obj $event_element::($evt_elem,block_obj)
   }
   if { [PB_com_is_true block_obj] && [PB_com_object_is_external $block_obj] } {
...
#=======================================================================
proc CB__SetEvtElemIndentation { page_obj evt_elem_obj } {
  global gPB
  set canvas_frame $Page::($page_obj,canvas_frame)
  set win [toplevel $canvas_frame.indent]
  set toplevel_index [llength $gPB(toplevel_list)]
  set win_index [expr $toplevel_index + 1]
  set gPB(toplevel_disable_$win_index) 1
  set block_obj $event_element::($evt_elem_obj,block_obj)
  set win_close_cb ""
  UI_PB_com_CreateTransientWindow $win \
  "$gPB(event,elem,indent,dialog,title)" "AT_CURSOR" "" \
  "UI_PB_blk_DisableBlkPageWidgets  $page_obj" "$win_close_cb" \
  "UI_PB_blk_ActivateBlkPageWidgets $page_obj $win_index"
  frame $win.top; pack $win.top
  global gpb_indentation
  if [info exists event_element::($evt_elem_obj,indent)] {
   set gpb_indentation $event_element::($evt_elem_obj,indent)
   } else {
   set gpb_indentation 0
  }
  tixControl $win.top.indent -integer true -min 0 \
  -selectmode immediate \
  -variable gpb_indentation \
  -options {
   entry.width 6
   label.anchor e
  }
  $win.top.indent.frame config -relief sunken -bd 1
  $win.top.indent.frame.entry config -relief flat
  set ent_widget [$win.top.indent subwidget entry]
  bind $ent_widget <KeyPress> "UI_PB_com_ValidateDataOfEntry %W %K i"
  bind $ent_widget <KeyRelease> {%W config -state normal }
  label $win.top.label -text " $gPB(event,elem,indent,dialog,blanks)"
  pack $win.top.indent -side left  -padx 5 -pady 4
  pack $win.top.label  -side right -padx 5 -pady 4
  ...
  set cb_arr(gPB(nav_button,ok,Label))     "CB__SetEvtElemIndentationOk $win $page_obj $evt_elem_obj"
  bind $ent_widget <Return>                "CB__SetEvtElemIndentationOk $win $page_obj $evt_elem_obj"
  UI_PB_com_CreateButtonBox $box1_frm label_list cb_arr
  UI_PB_com_PositionWindow $win
 }

#=======================================================================
proc CB__SetEvtElemIndentationCancel { win page_obj event_obj evt_elem_obj \
  blk_elem_obj } {
...
#=======================================================================
proc CB__SetEvtElemIndentationOk { win page_obj evt_elem_obj } {
  global gpb_indentation
  global gPB
  set event_element::($evt_elem_obj,indent) $gpb_indentation
  destroy $win
 }
...
proc UI_PB_tpth_StartDragBlk { page_obj event_obj x y } {
  ...
  if { [info exists cur_img_tag] && \
  [string compare $cur_img_tag "movable"] == 0 } \
  {
   ...
   if [string match "Comment" $block_element::($focus_blk_elem,elem_add_obj)] \
   {
    set blk_obj $event_element::($focus_evt_elem,block_obj)
    if [string match "$::gPB(tcl_line_blk)*" $block::($blk_obj,block_name)] {
     set blk_elem_text "# $blk_elem_mom_var"
     set ::gPB(comment_is_tcl) 1
    }
   }
   if [string match "Command" $block_element::($focus_blk_elem,elem_add_obj)] \
   {
    set blk_obj $event_element::($focus_evt_elem,block_obj)
    if [string match "$::gPB(condition_cmd_prefix)*" $block::($blk_obj,block_name)] {
     set blk_elem_text "IF($block::($blk_obj,block_name))"
    }
   }
...
proc UI_PB_tpth_DragBlk { page_obj event_obj x y } {
  ...
  if [expr $focus_blk_elem > 0] {
   ...
   UI_PB_tpth_HighlightSeperators $page_obj $event_obj $sel_addr_name $xc $yc
  }
  set canvas_x [winfo rootx $top_canvas]
  set canvas_y [winfo rooty $top_canvas]
  set canvas_height [expr {[winfo height $bot_canvas] + [winfo height $top_canvas]}]
...
proc UI_PB_tpth_SwapBlockElements { PAGE_OBJ EVENT_OBJ } {
  ...
  } else \
  {
   ...
   if { [string match "MOM_*" $cmd_elem] ||\
    [string match "ELSE"  $cmd_elem] ||\
   [string match "END"  $cmd_elem] } \
   {
...
proc UI_PB_tpth_DeleteARow { PAGE_OBJ EVENT_OBJ EVT_ELEM_OBJ } {
  ...
  for { set count $elem_index } { $count < $no_elems } { incr count } \
  {
   ...
   if { [info exists event_element::($temp_elem_obj,block_obj)] } {
  ...
  if { $elem_index } \
  {
   ...
   if { $active_elem_obj != "" } {
    ...
    set active_blk_elem_obj [lindex $block::($active_blk_obj,active_blk_elem_list) end]
    set im [$Page::($page_obj,bot_canvas) itemconfigure $block_element::($active_blk_elem_obj,icon_id) -image]
    [lindex $im end] configure -relief sunken -bg $paOption(sunken_bg)
...
proc UI_PB_tpth_BringCommentPage { PAGE_OBJ EVENT_OBJ EVT_ELEM_OBJ \
  ...
  set block_name $block::($block_obj,block_name)
  if { [string match "$::gPB(tcl_line_blk)*" $block_name] } {
   set dlg_title "$::gPB(event,combo,tcl_line,Label)"
   } else {
   set dlg_title "$gPB(tool,oper_trans,title,Label)"
  }
  UI_PB_com_CreateTransientWindow $win \
  $dlg_title "" \
  "" "UI_PB_blk_DisableBlkPageWidgets $page_obj" "$win_close_cb" \
  ...
  if { $comment_mode == "New" } \
  {
   ...
   $evt_elem_obj $blk_elem_obj"
   set cb_arr(gPB(nav_button,ok,Label)) \
   "UI_PB_cmd_tpthNewCommentOk_CB $win $page_obj $evt_elem_obj \
   $blk_elem_obj"
   bind $win.top.ent <Return>  "UI_PB_cmd_tpthNewCommentOk_CB $win $page_obj \
   $evt_elem_obj $blk_elem_obj"
  } else \
  {
   set cb_arr(gPB(nav_button,cancel,Label)) "destroy $win"
...
proc UI_PB_tpth_BringCmdPage { PAGE_OBJ EVENT_OBJ EVT_ELEM_OBJ BLOCK_OBJ \
  ...
  if { [string match "Condition" $cmd_mode] } \
  {
   ...
   set cmd_obj 0
   set cmd_obj_attr(0) "$gPB(check_block_cmd_prefix)"
  ...
  if { [string match "$::gPB(condition_cmd_prefix)*" $block::($block_obj,block_name)] } {
   set dlg_title "Condition Command"
  }
  ...
  } else \
  {
   ...
   $blk_elem_obj $cmd_page_obj"
   if 0 { ;# How come this scheme doesn't work for condition command??>
    set cb_arr(gPB(nav_button,ok,Label)) \
...
proc UI_PB_tpth_BringFuncPage { PAGE_OBJ EVENT_OBJ EVT_ELEM_OBJ BLOCK_OBJ mode } {
  ...
  UI_PB_debug_ForceMsg "\n%%%%% win >$win< %%%%%\n"
  if 0 {
   if { [info exists Page::($page_obj,blockpopup)] } {
    UI_PB_debug_ForceMsg "\n%%%%% blockpopup : $Page::($page_obj,blockpopup) %%%%%\n"
    $Page::($page_obj,blockpopup) delete 0 end
   }
   if { [winfo exists $Page::($page_obj,bot_canvas).pop] } {
    UI_PB_debug_ForceMsg "\n%%%%% pop : $Page::($page_obj,bot_canvas).pop %%%%%\n"
   }
   UI_PB_debug_ForceMsg "\n%%%%% popup menu : [$Page::($page_obj,bot_canvas).pop config] %%%%%\n"
   for { set idx 0 } { $idx <= [$Page::($page_obj,bot_canvas).pop index last] } { incr idx } {
    UI_PB_debug_ForceMsg "\n%%%%% popup $idx : [$Page::($page_obj,bot_canvas).pop entryconfig $idx] %%%%%\n"
   }
  }
  set toplevel_index [llength $gPB(toplevel_list)]
  set win_index [expr $toplevel_index + 1]
  ...
  if { [info exists win_grab] && $win_grab != "" } {

#=======================================================================
proc __WinClose_CB { win func_page_obj page_obj \
  event_obj evt_elem_obj block_obj blk_elem_obj mode win_grab } {
  UI_PB_func_tpthCancelCallBack $win $func_page_obj $page_obj \
  $event_obj $evt_elem_obj $block_obj $blk_elem_obj $mode
  grab release [grab current]; grab $win_grab"
 }
 set win_close_cb "__WinClose_CB $win $func_page_obj $page_obj \
 $event_obj $evt_elem_obj $block_obj $blk_elem_obj $mode $win_grab"
 } else {
...
if [info exists ::gPB_GEOM($win)] {
 ...
 set geom "850x750+100+100"
}
UI_PB_com_CreateTransientWindow $win $function::($func_obj,id) "$geom" \
"" "UI_PB_blk_DisableBlkPageWidgets $page_obj" "$win_close_cb" \
"UI_PB_blk_ActivateBlkPageWidgets $page_obj $win_index"
set Page::($func_page_obj,page_id)      $win
set Page::($func_page_obj,canvas_frame) $win
set box_frm [frame $win.total_box]
pack $box_frm -side bottom -fill x -padx 3 -pady 3 ;#<07-16-09 wbh> add -padx 3
...
proc UI_PB_func_DisplayFunctionText { page_obj } {
  ...
  set word_seq "$Page::($page_obj,Word_Separator)"
  ...
  set frm_width [winfo reqwidth $cntl_text]
  ...
  {
   set temp_str " "
   set temp_str [PB_string_repeat $temp_str [expr $start_index + 1]]
  }
  ...
  UI_PB_debug_ForceMsg "\n +++ disptext : >$disptext< >$frm_width<  >$text_width< +++ \n"
  update idletasks
 }
   ...
   if { $frac } \
   {
    ...
    { ;# remove the decimal
     ...
     if [expr $::tcl_version > 8.0] {
      ...
      set s1 [string range $val_text 0 [expr $dec_index - 1]]
      set s2 [string range $val_text [expr $dec_index + 1] end]
      set val_text $s1$s2
     }
    }
   } else \
...
proc UI_PB_func_SimpleFormat { page_obj w area x1 y1 x2 y2 } {
  ...
  if { [info exists Page::($page_obj,btn_right)] && $Page::($page_obj,btn_right) != 0 } \
  {
...
proc UI_PB_func_DeleteOneRow { page_obj grid row } {
  ...
  if { $::gPB(REV_07FEB2017_FIX_GRID_CRASH) == 0 } {
   $grid delete row $row
...
proc UI_PB_func_ClearAllGridRows { PAGE_OBJ } {
  upvar $PAGE_OBJ page_obj
  set grid $Page::($page_obj,grid)
  if { $::gPB(REV_07FEB2017_FIX_GRID_CRASH) == 1 } {
   UI_PB_func_ReCreateGridWidgets $page_obj
   } else {
   if { $Page::($page_obj,btn_right) } \
   {
...
proc UI_PB_func__EnterParamEntry { page_obj w } {
  ...
  if { ![info exists gf_leave_flag] || $gf_leave_flag != 2 } {
...
proc UI_PB_func__LeaveParamEntry { page_obj col w } {
  ...
  if { ![UI_PB_com_ValidateObjectName $new_text 1] } \
  {
   set gf_leave_flag 2
   set err_msg "\"$new_text\": Object name may not start with numeral preceded by underscore!"
   if { [tk_messageBox -icon error -parent [UI_PB_com_AskActiveWindow] -message $err_msg] == "ok" } {
  ...
  foreach param_obj $param_list \
  {
   ...
   {
    set gf_leave_flag 2
    set err_msg "\"$new_text\" $gPB(msg,name_exists)"
    if { [tk_messageBox -icon error -parent [UI_PB_com_AskActiveWindow] -message $err_msg] == "ok" } {
     UI_PB_func__EnterParamEntry $page_obj $w
     focus $w
     grab $w
...
proc UI_PB_func__CreateOneRow { page_obj param_obj grid no_row } {
  ...
  if { ![winfo exists $name_frm] } {
   ...
   bind $n_ent <KeyPress>      "+UI_PB_com_DisableSpecialChars %W %K 0 1"
   bind $n_ent <Control-Key-v> " UI_PB_com_Validate_Control_V  %W %K %A"
   bind $n_ent <KeyRelease>    "+UI_PB_com_Validate_Control_V_Release %W"
  ...
  if { ![winfo exists $int_frm] } \
  {
   ...
   set gPB(c_help,[$int_frm.con subwidget entry]) "format,data,num,integer"
  }
...
#=======================================================================
proc UI_PB_func_ApplyCallBack { page_obj {block_obj 0} } {
  global gf_disp_flag gf_apply_flag
  ...
  if $gf_edit_name_flag \
  {
   ...
   set pre_func_name $function::($func_obj,id)
   set function::($func_obj,id) $gf_edit_name
   if { [info exists ::gPB__new_elem_mom_var] } {
    if { ($pre_func_name == "$::gPB__new_elem_mom_var") &&\
     ($pre_func_name != "$gf_edit_name") } {
     UI_PB_debug_ForceMsg "\n pre_func_name >$pre_func_name< is selected. %%%%%\n"
...
proc UI_PB_func_SaveFuncInfo { page_obj func_obj } {
  ...
  set function::($func_obj,separator) $Page::($page_obj,Word_Separator)
...
proc UI_PB_func_SetFunctionSpecialAttr { PAGE_OBJ FUNC_ATTR_ARR } {
  ...
  UI_PB_func__GetValuesFrmSpecChar func_attr_arr(3) separator_val sep_opts_list
  set gf_name_arr(Word_Separator,label) $separator_val
  set gf_name_arr(Word_Separator,char) $func_attr_arr(3)
...
proc UI_PB_func_CreateFuncBlkPage { PAGE_OBJ func_mode } {
  ...
  {
   $win config -pady 0 ;#<07-15-09 gsl>
  }
  set top_frm [frame $win.top]
  pack $top_frm -side top -fill x
  set pane [tixPanedWindow $win.pane -orient vertical -handlebg orange -paneborderwidth 2]
  ...
  if { ![PB_com_is_true ::gPB(DISABLE_MIN_MAX_BUTTONS)] } {
   ...
   bind $bot_frm <Enter> "bind $w <Map> {}; bind $w <Unmap> {}"
   bind $bot_frm <Leave> "bind $w <Map> [list $map_cb]; bind $w <Unmap> [list $unmap_cb]"
  }
  update
...
proc UI_PB_func_AddWidgetsToTopFrame { page_obj frm mode } {
  ...
  if $mode \
  {
   ...
   if { [info exists gPB(VIEW_ADDRESS)] && $gPB(VIEW_ADDRESS)} {
    $fch_entry config -state disabled
    } else {
  ...
  set disp_text [tixScrolledText $frm.cntl_ent -height 90 -scrollbar x]
...
proc UI_PB_func_ShowHelpInfo { page_obj } {
  ...
  wm title $func_w "$gPB(cmd,desc_dlg,title): $function::($func_obj,disp_name)"
  if [info exists ::gPB_GEOM($func_w)] {
   set geom $::gPB_GEOM($func_w)
   } else {
   set geom "800x650+200+200"
  }
  wm geometry $func_w "$geom"
  set pane [tixPanedWindow $func_w.pane -orient vertical -handlebg orange -paneborderwidth 2]
  pack $pane -expand yes -fill both
  set txtSize 650
...
proc UI_PB_func_AddWidgetsToMidFrame { page_obj pane_frm func_mode } {
  ...
  set label_list {"Word_Separator" "Comment_Start"  "Comment_End"}
  set count 0
...
proc UI_PB_func_ReCreateGridWidgets { page_obj } {
  global gPB
  set gPB(Disable_SaveWindowGeometry) 1
  ...
  UI_PB_func_AddWidgetsToBtmFrame $page_obj $bot_frm
  set gPB(Disable_SaveWindowGeometry) 0
 }
...
proc UI_PB_func_EditFuncID { page_obj index } {
  ...
  __func_RestoreNamePopup $new_frm.ent $page_obj
...
proc UI_PB_func_ValidateFuncName { cur_func_obj cur_func_name ERR_MSG } {
  ...
  if { $len == 0 } \
  {
   ...
   } elseif { ![UI_PB_com_ValidateObjectName $cur_func_name 1] } \
  {
   set err_flag 1
   set err_msg "\"$cur_func_name\": Object name may not start with numeral(s)!"
  } else \
  {
...
proc UI_PB_func_SeqNewFuncBlkOk_CB { win page_obj seq_obj elem_obj \
  ...
  set elem_text_id [$bot_canvas create text [expr $elem_xc + $t_shift]  $elem_yc -text $elem_text -font $font -justify left  -tag blk_movable]
