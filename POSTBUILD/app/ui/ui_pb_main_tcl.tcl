...
proc __main_set_patch_revisions { } {
  UI_PB_AddPatchMsg "2008.0.3" "<Aug-06-2015> Fixed issues on Unix platforms"
...
proc __IsLanguageDefined { msg_file_name } {
 ...
 set gPB(Postbuilder_Version)       "2008.0.3"
 set release_date                   "9/30/2014"
 set release_date                   "10/9/2015"
 set phase                          ".1"
 set phase                          ".2"
...
proc __main_AddOptionsMenu { } {
  ...
  if { [info exist gPB(PB_POST_DEBUG)] && [string length [string trim $::gPB(PB_POST_DEBUG)]] > 0 } {
   if 0 {
    set gPB(PB_POST_DEBUG_command) [lindex $gPB(PB_POST_DEBUG) 0]
    if { [lindex $gPB(PB_POST_DEBUG) 1] } {
     set gPB(PB_POST_DEBUG) $gPB(PB_POST_DEBUG_command)
     } else {
     set gPB(PB_POST_DEBUG) ""
    }
   }
   set gPB(PB_POST_DEBUG_command) $gPB(PB_POST_DEBUG)
   set gPB(PB_POST_DEBUG) ""
   $gPB(option_menu) add separator
   $gPB(option_menu) add checkbutton -label "Debug Post" \
