#12
if { [info exists env(PB_ENABLE_POST_CONFIGURATION)] && [string trim $env(PB_ENABLE_POST_CONFIGURATION)] != "" } {
 set gPB(ENABLE_POST_CONFIGURATION) $env(PB_ENABLE_POST_CONFIGURATION)
 } else {
 set gPB(ENABLE_POST_CONFIGURATION) 0
}

#=======================================================================
proc __main_set_patch_revisions { } {
  UI_PB_AddPatchMsg "2009.0.2" "<08-04-2015> Fixed error with ::disable_license_win_title"
  UI_PB_AddPatchMsg "2005.0.0" "<06-18-08> Fixed file select dialog problem when opening a post"
  UI_PB_AddPatchMsg "2005.0.0" "<06-18-08> Added VMOV command in mill_base.tcl"
 }
 global env gPB

#=======================================================================
proc TraceVar { VAR idx op } {
  upvar $VAR var
  if { $idx != "" } {
   set val $var($idx)
   } else {
  ...
  if { $idx != "" } {
   UI_PB_debug_ForceMsg "TRACE >${VAR}($idx)< >$op< : >$val<"
   } else {
   UI_PB_debug_ForceMsg "TRACE >${VAR}< >$op< : >$val<"
  }
 }
 set trace_vars_list ""
 if { [info exists env(PB_TRACE_TCL_VARS)] && [llength $env(PB_TRACE_TCL_VARS)] } {
  set trace_vars_list [join $env(PB_TRACE_TCL_VARS)]
  } elseif { [info exists gPB(TRACE_TCL_VARS)] && [llength $gPB(TRACE_TCL_VARS)] } {
  set trace_vars_list [join $gPB(TRACE_TCL_VARS)]
 }
 if { [llength $trace_vars_list] } {
  foreach var $trace_vars_list {
   trace var [set var] r TraceVar
   trace var [set var] w TraceVar
  }
 }
 if { ![info exists gPB(DISABLE_COMP_BLK)] } {
  if { [info exists env(PB_DISABLE_COMP_BLK)] && [string trim $env(PB_DISABLE_COMP_BLK)] != "" } {
   set gPB(DISABLE_COMP_BLK) $env(PB_DISABLE_COMP_BLK)
  }
 }
 if { ![info exists gPB(DISABLE_COMP_BLK)] } {
  set gPB(DISABLE_COMP_BLK) 1
 }
 set gPB(DISABLE_COMP_BLK_default) $gPB(DISABLE_COMP_BLK)
 if { [info exists env(HOMEDRIVE)] && [info exists env(HOMEPATH)] } {
  regsub -all {\\} $env(HOMEDRIVE)$env(HOMEPATH) {/} env(LIC_HOME)
  } else {
  set env(LIC_HOME) $env(HOME)
 }
 if { [info exists gPB(USERNAME)] && [string trim $gPB(USERNAME)] != "" } {
  set env(USERNAME) $gPB(USERNAME)
  } else {
  if { [info exists env(PB_USERNAME)] && [string trim $env(PB_USERNAME)] != "" } {
   set env(USERNAME) $env(PB_USERNAME)
  }
 }
 set disable_license_win_title 0
 if { ![info exists gPB(PB_POST_DEBUG)] } {
  if { [info exists env(PB_POST_DEBUG)] && $env(PB_POST_DEBUG) } {
   set gPB(PB_POST_DEBUG) 1
  }
 }
 set gPB(ENABLE_TCL_STATEMENT) 1
 set gPB(ENABLE_IF_CONDITION)  1
 if { 0 && $::gPB(PB_LICENSE) == "UG_POST_ADV_BLD" } {
  if { ![info exists gPB(ENABLE_TCL_STATEMENT)] } {  ;# Resource file has the last word
   set gPB(ENABLE_TCL_STATEMENT) 0
   if { [info exists env(PB_ENABLE_TCL_STATEMENT)] && [string trim $env(PB_ENABLE_TCL_STATEMENT)] != "" } {
    set gPB(ENABLE_TCL_STATEMENT) $env(PB_ENABLE_TCL_STATEMENT)
   }
  }
  if { $gPB(ENABLE_TCL_STATEMENT) } {
   if { ![info exists gPB(ENABLE_TCL_INDENTATION)] } {  ;# From resource file
    set gPB(ENABLE_TCL_INDENTATION) 0
    if { [info exists env(PB_ENABLE_TCL_INDENTATION)] && [string trim $env(PB_ENABLE_TCL_INDENTATION)] != "" } {
     set gPB(ENABLE_TCL_INDENTATION) $env(PB_ENABLE_TCL_INDENTATION)
    }
   }
  }
 }
 if 1 {
  ...
  if { 0 && $gPB(ENABLE_TCL_STATEMENT) } {
   set gPB(ENABLE_TCL_INDENTATION) 1
  }
 }
 if { ![info exists gPB(ENABLE_TCL_STATEMENT)] } {
  set gPB(ENABLE_TCL_STATEMENT)   0
 }
 if { ![info exists gPB(ENABLE_TCL_INDENTATION)] } {
  set gPB(ENABLE_TCL_INDENTATION) 0
 }
 set gPB(tcl_label)        "Tcl Comment" ;#<Aug-10-2016 gsl> "Tcl Statement"
 set gPB(tcl_line_icon)    "pb_tcl_line"
 set gPB(tcl_line_blk)     "tcl_line_blk"
 set gPB(comment_blk)      "comment_blk"
 set gPB(comment_is_tcl_selected) 0
 set gPB(comment_is_tcl)          0
 if { ![info exists gPB(ENABLE_IF_CONDITION)] } {  ;# Resource file has the last word
  set gPB(ENABLE_IF_CONDITION) 0
  if { [info exists env(PB_ENABLE_IF_CONDITION)] && [string trim $env(PB_ENABLE_IF_CONDITION)] != "" } {
   set gPB(ENABLE_IF_CONDITION) $env(PB_ENABLE_IF_CONDITION)
  }
 }
 ...
 set gPB(check_block_cmd_prefix)  "PB_CMD__check_block_"
 set gPB(condition_cmd_prefix)    $gPB(check_block_cmd_prefix)
 set gPB(condition_indent)        3
 set gPB(condition_cmd_icon)      "pb_diamond"
 set gPB(ATP_cmd_prefix)           "PB_CMD__AdvTurboPost"
 set gPB(ATP,customize_post)       "$gPB(ATP_cmd_prefix)__Customize_Post"
 set gPB(ATP,config_output)        "$gPB(ATP_cmd_prefix)__config_output"
 set gPB(ATP,config_before_motion) "$gPB(ATP_cmd_prefix)__config_before_motion"
 set gPB(ATP,config_end_of_path)   "$gPB(ATP_cmd_prefix)__config_end_of_path"
 set gPB(Edit_Post_Object)         1

#=======================================================================
proc PB_is_probe_post { } {
  if { [string match "mill_probe" [PB2TCL_read_fly_var mom_fly_machine_type]] } {
   return 1
   } elseif { [string match "lathe_probe" [PB2TCL_read_fly_var mom_fly_machine_type]] } {
   return 2
   } else {
   return 0
  }
...
#=======================================================================
proc PB_source { file_str } {
  set ::source_file ${file_str}
  if { [catch {
    uplevel #0 {
     source $::source_file
    }
    } err] } {
   tk_messageBox -message "[info script]\n[PB_trace]\nSourcing $file_str\n$err"
   set file_ok 0
   } else {
   set file_ok 1
  }
 ...
 set disable_internal_user_lc 0 ;#<11-11-11 gsl> Back to 0. #<07-22-11 gsl> was 0.
 if [info exists gPB(PB_SITE_ID)] {
  if { [string match "__UGPB_ISV_internal_use"        $gPB(PB_SITE_ID)] ||\
   [string match "For Internal Siemens PLM Use*"  $gPB(PB_SITE_ID)] ||\
   [string match "For Internal UGS PLM Use Only"  $gPB(PB_SITE_ID)] ||\
   [string match "CLIENTDEMO - Siemens PLM*"      $gPB(PB_SITE_ID)] } {
...
proc __MapEnvToPBvar { env_var } {
  ...
  switch -regexp $env_var {
   ^cs_.* {
    set pbvar pb_msg_czech
   }
   ^pl_.* {
    set pbvar pb_msg_polish
   }
   ^hu_.* {
    set pbvar pb_msg_hungarian
   }
   ^pt_.* {
    if [string match pt_br $env_var] {
     set pbvar pb_msg_brazilian_portuguese
    }
   }
   ^en_.* {
   ...
   ^zh_.* {
    if [string match zh_cn $env_var] {
     set pbvar pb_msg_simplified_chinese
     } else {
...
proc __IsLanguageDefined { msg_file_name } {
  ...
  if [file exists $env(PB_HOME)/msgs/${msg_file_name}.tcl] {
   set gPB(LANG_TEST) 1
   set is_defined [PB_source "$env(PB_HOME)/msgs/${msg_file_name}.tcl"]
   unset gPB(LANG_TEST)
   } else {
   set is_defined 0
  }
 ...
 if { $::tcl_version >= 8.6 } {
  PB_source [file join $env(TCL_LIBRARY) msgcat msgcat.tcl]
  } else {
  PB_source [file join $env(TCL_LIBRARY) msgcat.tcl]
 }
 ...
 PB_source $env(PB_HOME)/app/ui/ui_pb_language.tcl
 PB_source $env(PB_HOME)/app/ui/ui_pb_fonts.tcl
 set gPB(Postbuilder_Version)       "2011.0.2"
 set gPB(Postbuilder_Version)       "2012.0.0"
 set gPB(Postbuilder_Version)       "2012.0.1"
 set release_date                   "12/13/2019"
 if { [info exists ::gPB(Postbuilder_Release_Date)] } {
  set release_date $::gPB(Postbuilder_Release_Date)
 }
 if { [info exists ::gPB(Revision_Date)] && $::gPB(Revision_Date) != "" } {
  set release_date $::gPB(Revision_Date)
 }
 set phase                          ".1"
 set phase                          ".0"
 set gPB(Postbuilder_PUI_Version)   "$gPB(Postbuilder_Version)$phase"
 set gPB(Postbuilder_Release_MP)    "" ;# "Beta"
 ...
 if 0 {
  if { info exists ::env(UGII_MAJOR_VERSION)] } {
   set gPB(Postbuilder_Release_Version)  "$::env(UGII_MAJOR_VERSION).$::env(UGII_MINOR_VERSION).$::env(UGII_SUBMINOR_VERSION)"
   } else {
 ...
 if { ![info exists gPB(Postbuilder_Release_Version)] } {
  set gPB(Postbuilder_Release_Version)  "(Final Release)"
 }
 set gPB(Postbuilder_Release)       \
 "$gPB(main,title,UG)/$gPB(main,title,Post_Builder) $gPB(main,title,Version) $gPB(Postbuilder_Release_Version)"
 ...
 set gPB(nc_code_font)         $tixOption(fixed_font_sm)
...
set paOption(unit_fg)             black ;#<11-03-10 lxy> #EE3333
...
proc __CreateStyle { } {
  uplevel #0 {
   switch $::tix_version {
    8.4 {
     set gPB(font_style_normal_external) [tixDisplayStyle imagetext \
     -fg $paOption(tree_fg) -bg $paOption(tree_bg) -padx 4 -pady 1 -font $gPB(italic_font_normal) \
   ...
   set gPB(sys_font_style) [list $gPB(font_style_normal) $gPB(font_style_bold) \
   $gPB(font_style_normal_gray) $gPB(font_style_bold_gray)]
  }
 }
 if ![info exists gPB(master_pid)] {
  __CreateStyle
...
proc UI_PB_log_msg { args } {
  ...
  if $gPB(LOG_MSG_ENABLED) {
   if { [info exists gPB(autoqc_log_file_id)] } {
    set fid $gPB(autoqc_log_file_id)
    if { ![info exists gPB(log_msg_index)] } {
...
proc PB_is_old_v { ver } {
  ...
  set old_ver $gPB(Old_PUI_Version)
  if { [string compare $old_ver "$ver"] >= 0 } {
   return 1
   } else {
   return 0
  }
...
proc UI_PB_main { w } {
  ...
  __DisplaySplash
  after 4000 "__DestroySplash"
  if { ![info exists gPB(PB_Session_Start)] } {
   set gPB(PB_Session_Start) [PB_com_GetTimeString]
  }
  ...
  if 0 {
   wm transient $w .
   wm geometry  . 0x0-100+0
   wm protocol  . WM_DELETE_WINDOW ""
   wm iconbitmap . "$::env(PB_HOME)/images/pb_setup.xpm"
  }
  if 0 {
   if { [string match "windows" $::tcl_platform(platform)] &&\
    $::tix_version == "8.4" } {
  ...
  UI_PB_debug_ForceMsg "\n [join [array get ::tcl_platform] \n] \n"
  UI_PB_debug_ForceMsg_no_trace "\n screenwidth . = >[winfo screenwidth .]< >[winfo id .]< >[wm maxsize .]< \n] \n"
  if 0 {
   set gPB(WIN_X_OFF) 0
   if { [string match "windows" $::tcl_platform(platform)] &&\
    [info exists ::tcl_platform(osVersion)] && $::tcl_platform(osVersion) > 6.1 } {
    set gPB(WIN_X_OFF) -9
   }
  }
  wm withdraw  $w
  ...
  set yc 0
  set gPB(TOP_WIN_WI)  1200
  set gPB(TOP_WIN_HI)  107
  set gPB(MAIN_WIN_HI) 800
  if 0 {
   if { [info exists ::gPB_GEOM($w)] } {
    set geom "$::gPB_GEOM($w)"
    } else {
    set geom "$gPB(TOP_WIN_WI)x$gPB(TOP_WIN_HI)+$xc+$yc"
   }
  }
  set geom "$gPB(TOP_WIN_WI)x$gPB(TOP_WIN_HI)+$xc+$yc"
  wm geometry  $w "$geom"
  ...
  if { [string match "windows" $::tcl_platform(platform)] &&\
   ...
   set gPB(WIN_X_OFF) [expr $xc - $twx]
  }
  if { [string match "windows" $::tcl_platform(platform)] &&\
   [info exists ::tcl_platform(osVersion)] && $::tcl_platform(osVersion) > 6.1 } {
   set gPB(TOP_WIN_WI) [expr $gPB(TOP_WIN_WI) -2]
  }
  if { [PB_com_is_true gPB(DISABLE_MIN_MAX_BUTTONS)] } {
   if { [string match "windows" $::tcl_platform(platform)] &&\
    $::tix_version == 8.4 } {
    wm attributes $gPB(top_window) -toolwindow 1
    } else {
    set gPB(DISABLE_MIN_MAX_BUTTONS) 0
   }
   } else {
   set gPB(DISABLE_MIN_MAX_BUTTONS) 0
  }
  if { ![string match "windows" $::tcl_platform(platform)] } {
   set gPB(DISABLE_MIN_MAX_BUTTONS) 1
  }
  ...
  if 0 {
   $gPB(file_menu) add command -label "$gPB(main,file,properties,Label)" -underline 5 \
   -command "UI_PB_DisplayPostProperty" \
   -accelerator ""
  }
  set label_text $gPB(main,file,properties,Label)
  ...
  set gPB(menu_index,file,property) 9
  set gPB(menu_index,file,history) 10 ;# 10 was 9 (set in pb.c)
  ...
  $mb.file.m entryconfigure $gPB(menu_index,file,property)   -state disabled
  ...
  set gPB(WIN_X_MAIN) [expr $xxc - $xc] ;# Border width
  set gPB(WIN_Y_MAIN) [expr $yyc - $yc] ;# Border plus title width
  set gPB(WIN_X) [expr $xxc - $xc] ;# Border width
  set gPB(WIN_Y) [expr $yyc - $yc] ;# Border plus title width
  set gPB(win_max_width)  [expr $gPB(screen_width)  - $gPB(WIN_X_MAIN) - $gPB(WIN_X_MAIN)]
  set gPB(win_max_height) [expr $gPB(screen_height) - $gPB(WIN_Y_MAIN) - $gPB(WIN_X_MAIN)]
...
proc UI_PB_OpenCmdLinePost { } {
  ...
  if { ![info exists gPB(CMD_LINE_PUI)] || [string trim $gPB(CMD_LINE_PUI)] == "" } {
   ...
   if { $gPB(CMD_LINE_PUI) == "autoqc" } {
    ...
    if { ![string compare [string trim $env(PB_AUTOQC_POST_DIR)] [string trim $env(PB_AUTOQC_SAVE_DIR)]] } {
     set run_autoqc 0
     tk_messageBox -message "Target directory >$env(PB_AUTOQC_SAVE_DIR)< should be different from\n\
     the source >$env(PB_AUTOQC_POST_DIR)< for an AutoQC run!"
   ...
   if $run_autoqc {
    ...
    set log_fid [open $save_dir/__pb_autoqc_[expr abs([clock clicks])].log w+]
    set gPB(autoqc_log_file_id) $log_fid
   }
   if { [info exists gPB(autoqc_log_file_id)] } {
   ...
   foreach pui $post_list save_pui $save_post_list {
    ...
    if { [lsearch -exact [package names] "stooop"] < 0 } {
     if [PB_source $env(PB_HOME)/tcl/exe/stooop.tcl] {
...
proc __main_AddOptionsMenu { } {
  ...
  if { $::tix_version == 8.4 && $::tcl_platform(platform) == "windows" } {
   ...
   if [llength $msg_files] {
    foreach one [lsort $msg_files] {
     ...
     if [string match "pb_msg_braz_portuguese" $fn] {
      continue
     }
     ...
     if { $gPB(localize_lan_menu_item) == 0 } {
      switch $fn {
       ...
       pb_msg_braz_portuguese -
       pb_msg_brazilian_portuguese {
        set lbl "Portugu\u00EAs (Brasil)" ;#"Portugu___s (Brasil)" ;#"Portugueses Brasileiros"
       }
       pb_msg_english {
        set lbl English
       }
       ...
       pb_msg_simplified_chinese -
       pb_msg_simple_chinese -
       pb_msg_simpl_chinese {
        set lbl \u4E2D\u6587(\u7B80\u4F53)
       }
       pb_msg_spanish {
        set lbl Espa\u00F1ol
  ...
  if { [string match "windows" $tcl_platform(platform)] } {}
  $gPB(option_menu) config -postcommand [list __EnableOptionMenu $gPB(option_menu)]
  if 0 {
   set gPB(ADV_TURBO_POST) 0
   $gPB(option_menu) add separator
   $gPB(option_menu) add checkbutton -label "Enable Advanced Turbo Post Output" \
   -variable gPB(ADV_TURBO_POST) -state disabled
   set gPB(ADV_TURBO_POST_menu_index) [$::gPB(option_menu) index end]
  }
  if 0 {
  ...
  if { ![info exists gPB(PB_POST_DEBUG_command)] } {
   set gPB(PB_POST_DEBUG_command) "PB_POST__debug_msg"
  }
  if { ![info exists gPB(PB_POST_DEBUG_define)] || \
   [string length [string trim $gPB(PB_POST_DEBUG_define)]] == 0 } {
   set gPB(PB_POST_DEBUG_define)   "PB_CMD__define_debug_msg"
  }
  if { ![info exists gPB(PB_POST_DEBUG_exclude)] } {
   set gPB(PB_POST_DEBUG_exclude) [list "$gPB(PB_POST_DEBUG_command)" "$gPB(PB_POST_DEBUG_define)"       \
   "OPERATOR_MSG" "VMOV" "UNSET_VARS" "ARCTAN" "TRACE" "STR_MATCH"  \
  ...
  if { [lsearch $gPB(PB_POST_DEBUG_exclude) $gPB(PB_POST_DEBUG_command)] < 0 } {
   lappend gPB(PB_POST_DEBUG_exclude) $gPB(PB_POST_DEBUG_command)
  }
  if { [lsearch $gPB(PB_POST_DEBUG_exclude) $gPB(PB_POST_DEBUG_define)] < 0 } {
   lappend gPB(PB_POST_DEBUG_exclude) $gPB(PB_POST_DEBUG_define)
  }
  if { [info exist gPB(PB_POST_DEBUG)] } {
   set __debug [string trim $gPB(PB_POST_DEBUG)]
   if { [string length $__debug] > 0  &&  $__debug } {
    set gPB(PB_POST_DEBUG) ""
    $gPB(option_menu) add separator
    $gPB(option_menu) add checkbutton -label "$gPB(main,options,debug,Label)" \
    -variable gPB(PB_POST_DEBUG) -onvalue $gPB(PB_POST_DEBUG_command) -offvalue "" \
    -command "__DisplayDebugOptionTip"
    if 0 {
     PB_enable_balloon $gPB(option_menu) "%W index 7"
     set gPB_help_tips($gPB(option_menu),7) {$gPB(main,options,Label)}
     PB_enable_balloon $gPB(option_menu) "%W index 8"
     set gPB_help_tips($gPB(option_menu),8) {$gPB(main,options,Label)}
     PB_enable_balloon $gPB(option_menu) "%W index 9"
     set gPB_help_tips($gPB(option_menu),9) {$gPB(main,options,Label)}
    }
   }
  }
 }
...
#=======================================================================
proc __DisplayDebugOptionTip { args } {
  if { $::gPB(PB_POST_DEBUG) != "" } {
   set font_org              $::gPB(c_help,font)
   set ::gPB(c_help,font)    $::tixOption(font)
   UI_PB_chelp__display_msg "$::gPB(main,options,debug,Label)"\
   "Special statements will be embedded throughout the post to display information about\
   event handlers & commands being executed, as Operator Messages, during a NX/Post run.\
   \n\nThis option should be disabled upon the release of the post to production.\
   \n\nYou may import custom command PB_CMD__define_debug_msg from\
   \"postbuild/pblib/custom_command/pb_cmd_debug_utils.tcl\" to enahnce\
   the messages with more detailed information for debugging purpose.\
   \n\nOnce you have saved the post with debug option enabled, Do Not edit the post's files manually!"
...
proc __main_AddLicenseControlManagement { } {
  ...
  if 1 {
   if { [info exists gPB(PB_SITE_ID)] && ![string compare $gPB(PB_LICENSE) "UG_POST_AUTHOR"] } {
    if { [info exists gPB(lic_list)]  &&  [llength $gPB(lic_list)] > 0 } {
     if { [string match "*EXPIRATION *" [lindex $gPB(lic_list) 0] ] } {
      ...
      if { ![__file_is_SiteID_UG_internal $gPB(PB_SITE_ID)] } {
      ...
      catch { UI_PB_encrypt_post $env(LIC_HOME)/pb_license_tcl.txt $tmp_list TRUE }
      unset expiration
      unset tmp_list
     }
     if { [__file_is_SiteID_UG_internal $gPB(PB_SITE_ID)] } {  ;# <== What about the end users?
      if { [lsearch $gPB(lic_list) "CAM_SME_1300 *"] < 0 } {
       set idx [lsearch $gPB(lic_list) "CAM_SME_1301 *"]
       if { $idx >= 0 } {
        set gPB(lic_list) [linsert $gPB(lic_list) $idx {CAM_SME_1300 1300}]
       }
      }
     }
    }
  ...
  if { ![info exists ::__ADD_NULL_LICENSE] || $::__ADD_NULL_LICENSE == 0 } {
   if { ![info exists gPB(lic_list)]  ||  [llength $gPB(lic_list)] == 0 } {
    set ::disable_license_win_title 1
    return
   }
  }
  wm title $gPB(top_window) "$gPB(Postbuilder_Release) - $gPB(main,title,license_control)"
...
proc UI_PB_rest_license_option {} {
  ...
  if { [info exists gPB(license_authorize)] && $gPB(license_authorize) == 1 } {
   ...
   PB_com_unset_var gPB(license_select)
  }
 }
...
proc UI_PB_browse_licenses {} {
  ...
  if { ![info exists gPB(post_licenses)] } {
   set gPB(post_licenses) [list]
   set gPB(lic_list)      [lsort $gPB(lic_list)]
   foreach one $gPB(lic_list) {
    set lic_name [lindex $one 0]
    if { [string match "CAM_KIT_POST_*"    $lic_name] ||\
     [string match "CAM_EX_KIT_POST_*" $lic_name] ||\
     ([string match "CAM_SME_*"         $lic_name] && ![string match "CAM_SME_MASTER" $lic_name]) } {
     lappend gPB(post_licenses) $lic_name
    }
   }
  }
  if { [info exists gPB(post_licenses)] } {
   foreach one $gPB(post_licenses) {
    $lsb insert end $one
   }
  }
...
#=======================================================================
proc UI_PB_select_license_for_post { {force_select_lic 0} } {
  ...
  if { [string match "UG_POST_AUTHOR" $gPB(PB_LICENSE)] } {
   if { [info exists gPB(lic_list)] } {
    if { $gPB(session) == "NEW" || $force_select_lic } {
     set is_needed 1
     } else {
     if { [info exists LicInfo(is_encrypted_file)] && $LicInfo(is_encrypted_file) == 1 } {
      if { ![string compare $gPB(PB_SITE_ID) $LicInfo(post_site_id)] } { ;#<07-21-11 gsl> "==" was used.
  ...
  if { [info exists gPB(post_licenses)] } {
   foreach one $gPB(post_licenses) {
    $lst insert end $one  ;#<07-20-11 gsl> Bug - lst was lsb.
   }
  }
  if 0 {
...
proc UI_PB_select_license_OK { win } {
  ...
  if { $gPB(license_authorize) == 1 } {
   ...
   if { $idx >= 0 } {
    ...
    if { ![string match "CAM_SME_*" $gPB(license_select)] } {
...
proc __EnableOptionMenu { m } {
  ...
  if { [info exists env(MULTI_INTERP)] && $env(MULTI_INTERP) == 1 } {
   ...
   if { [info exists ::post_object] } {
    ...
    if [info exists ::gPB(ADV_TURBO_POST)] {
     set mach_type [PB2TCL_read_fly_var mom_fly_machine_type]
     if { [string match "3_axis_mill"  $mach_type] ||\
      [string match "4_axis_head"  $mach_type] ||\
      [string match "4_axis_table" $mach_type] ||\
      [string match "5_axis*"      $mach_type] } {
      $m entryconfigure $::gPB(ADV_TURBO_POST_menu_index) -state normal
      } else {
      $m entryconfigure $::gPB(ADV_TURBO_POST_menu_index) -state disabled
     }
    }
    } else {
...
proc __main_EditTPD { w } {
  ...
  if { ![winfo exists $w] } {
   ...
   PB_source "$gPB(edit_template_posts_data)"
  }
...
proc __main_BrowseMOMVar { w } {
  ...
  if { ![winfo exists $w] } {
   ...
   PB_source "$gPB(mom_vars_browser)"
  }
  _mom_chm_reader $w
  after 5 "raise $w"
  if $flag {
...
#=======================================================================
proc UI_PB_main_ViewPostFiles { } {
  if 0 {
   if { ![info exists ::gPB(VIEW_POST_FILES)] || $::gPB(VIEW_POST_FILES) == 0 } {
    return
   }
  }
  if { ! [string match "windows" $::tcl_platform(platform)] } {
  ...
  set ::gPB(util_menu_view_files) $::gPB(util_menu).view
  set m $::gPB(util_menu_view_files)
  if { [winfo exists $::gPB(util_menu_view_files)] } \
  {
   catch { $::gPB(util_menu_view_files) delete 0 end }
   if { $::gPB(session) == "NEW" } {
    set menu_index [expr [$::gPB(util_menu) index end] - 1]
    $::gPB(util_menu) delete $menu_index
    $::gPB(util_menu) delete $menu_index
    destroy $::gPB(util_menu_view_files)
   }
   update
  }
  if { $::gPB(session) == "NEW" } {
  ...
  {
   $::gPB(util_menu) add separator
   $::gPB(util_menu) add cascade -label "View Post Files" -menu $m
   menu $m -tearoff 0
  }
  if [file exists $tcl_file] {
   $m add command -label "[file tail $tcl_file]" -command "__main_ViewTempFile \"$tcl_file\""
   $m add command -label "[file tail $def_file]" -command "__main_ViewTempFile \"$def_file\""
  }
  if { [file exists $cdl_file] && $::env(PB_UDE_ENABLED) } {
   $m add command -label "[file tail $cdl_file]" -command "__main_ViewTempFile \"$cdl_file\""
  }
  if { [file exists $vnc_file] && $::mom_sys_arr(Output_VNC) } {
   $m add command -label "[file tail $vnc_file]" -command "__main_ViewTempFile \"$vnc_file\""
  }
 }

#=======================================================================
proc __main_ViewTempFile { f } {
  if { ![info exists ::env(EDITOR)] || \
   [string match "" [string trim $::env(EDITOR)]] || \
   ![file exists $::env(EDITOR)] } {
   UI_PB_chelp__display_msg "$::gPB(msg,external_editor)" "$::gPB(msg,set_ext_editor)"
   return
  }
  set temp_file "$::env(TEMP)/$::env(LOGNAME)_PB_temp_[expr abs( [clock clicks] )]__[file tail $f]"
  file copy -force -- "$f" $temp_file
  if [catch { exec $::env(EDITOR) $temp_file & } result] {
...
proc __main_AddMoreHelpMenuItems {} {
  ...
  if 0 {
   if { ![info exists gPB(release_notes)] || ![file exists $gPB(release_notes)] } {
    if { [info exists env(LANG)] } {
     set gPB(release_notes)  "$env(UGII_BASE_DIR)/rel_info/release_notes/$env(LANG)/index.html"
    }
   }
   if { [info exists gPB(release_notes)] && [file exists $gPB(release_notes)] } {
  ...
  if { [info exists gPB(release_notes)] && [string length $gPB(release_notes)] > 0 } {
...
#=======================================================================
proc UI_PB_DisplayPostProperty { } {
  global gPB
  set win [UI_PB_com_AskActiveWindow].property
  toplevel $win
  set toplevel_index [llength $gPB(toplevel_list)]
  set win_index [expr $toplevel_index + 1]
  set gPB(toplevel_disable_$win_index) 1
  Post::ReadPostOutputFiles $::post_object dir pui_file def_file tcl_file
  set pui_file [file rootname $pui_file]
  UI_PB_com_CreateTransientWindow $win "$gPB(main,file,properties,Label)  -  $pui_file" "800x750+120+140" \
  "" "UI_PB_PostPropertyDisableWidgets" "UI_PB_PostPropertyClose $win" \
  "UI_PB_PostPropertyActivateWidgets $win_index"
  set pp [frame $win.info -relief flat -bd 0]
  pack $pp -fill both -expand 1
  UI_PB_mach_DisplayPostInfo "" $pp
...
#=======================================================================
proc UI_PB_PostPropertyDisableWidgets { } {
  global gPB
  $gPB(file_menu) entryconfigure $gPB(menu_index,file,property) -state disabled
  if { ![string compare $::tix_version "8.4"] } {
   bind all <Enter> {}
  }
 }
...
#=======================================================================
proc UI_PB_PostPropertyClose { win } {
  global gPB
  ...
  for { set i 1 } { $i < $len } { incr i 3 } {
   ...
   lappend gPB(post_description) $line
  }
  destroy $win
 }

#=======================================================================
proc UI_PB_PostPropertyActivateWidgets { win_index } {
  global gPB
  $gPB(file_menu) entryconfigure $gPB(menu_index,file,property) -state normal
...
proc UI_PB_AddVisitedPosts_mod { } {
  ...
  if { [info exists gPB(file_menu_posts_list)] } \
  {
   ...
   catch { $gPB(file_menu) delete $gPB(menu_index,file,history) }
  }
...
proc __DisplayAboutPostBuilder {} {
  ...
  set title "$gPB(main,help,about,Label) - $gPB(PB_Session_Start)"
  global env
  if 0 {
   set text " $gPB(Postbuilder_Release_About)\n\n\
   UGII_BASE_DIR \t $env(UGII_BASE_DIR)\n\
   POSTBUILD \t $env(PB_HOME)\n\
   HOME \t\t [file nativename $env(HOME)]"
  }
  set text " $gPB(main,title,UG)/$gPB(main,title,Post_Builder) $gPB(main,title,Version)\
  $gPB(Postbuilder_Release_Version)$gPB(Postbuilder_Version_Phase) $gPB(Postbuilder_Release_MP)\
  ($gPB(Postbuilder_Release_Date))\n\n\
  $gPB(Postbuilder_Release_About)\n\n\
  UGII_BASE_DIR \t $env(UGII_BASE_DIR)\n\
  POSTBUILD \t $env(PB_HOME)\n\
  HOME \t\t [file nativename $env(HOME)]\n\
  ...
  if { [info exists ::gPB_GEOM($win)] } {
   set ix [string first "+" $::gPB_GEOM($win)]
   if { $ix >= 0 } {
    set geom [string range $::gPB_GEOM($win) $ix end]
    } else {
    set ix [string first "-" $::gPB_GEOM($win)]
    if { $ix >= 0 } {
     set geom [string range $::gPB_GEOM($win) $ix end]
  ...
  UI_PB_com_CreateTransientWindow $win "$title" "$geom" "" "" \
...
proc __HelpItemCmd { doc_type } {
   ...
   "RELEASE_NOTES" {
    if 0 {
     switch $gPB(LANG) {
      ...
      pb_msg_trad_chinese -
      pb_msg_traditional_chinese {
       set loc trad_chinese
      }
      default {
       set loc english
      }
     }
     if [file exists "$env(UGII_BASE_DIR)/rel_info/release_notes/$loc/index.html"] {
      set gPB(release_notes)  "$env(UGII_BASE_DIR)/rel_info/release_notes/$loc/index.html"
     }
    }
  ...
  if { ![string match "USER_MANUAL" $doc_type] && ![string match "RELEASE_NOTES" $doc_type] } {
...
proc HelpCmd_ack {} {
  ...
  {
   set msg "Project team of NX/Post\ Builder\
...
proc UI_PB_ExitPost { args } {
  ...
  if 0 {
   if { [winfo exists $gPB(main_window)] } {
    set ::gPB_GEOM($gPB(main_window)) [winfo geometry $gPB(main_window)]
   }
  }
  if { [info exists ::env(MULTI_INTERP)] && $::env(MULTI_INTERP) == 1 } {
   global PID
...
proc UI_PB_save_SessionLog { args } {
  ...
  if { $fid != "" } \
  {
   ...
   if { [info exists ::gPB_GEOM] && [array exists ::gPB_GEOM] } {
    puts $fid " "
    puts $fid "\#------------------"
    puts $fid "\# Windows Geometry"
    puts $fid "\#------------------"
    foreach { w g } [array get ::gPB_GEOM] {
     puts $fid "set ::gPB_GEOM($w)  \"$g\""
    }
   }
   if { [info exists gPB(open_files_list)]  &&  [llength $gPB(open_files_list)] } \
   {
...
proc UI_PB_save_a_post { args } {
  ...
  if { [info exists mom_sys_arr(Output_VNC)] && $mom_sys_arr(Output_VNC) } {
   ...
   set gPB(non_override_mom_sim) 1
  }
  ...
  if [info exists cmd_obj_list] {
   ...
   foreach cc $cmd_obj_list {
    ...
    set msg ""
    if { ![string match "PB_CMD_kin_*" $attr(0)] } {
     if { [info exists ::CMD_BCK] } {
      if { [info exists ::CMD_BCK($attr(0))] } {
       if { 0 == [string compare $attr(1) $::CMD_BCK($attr(0))] } {
        set to_validate 0
        } else {
        set to_validate 1
       }
    ...
    if { ![string match "PB_CMD_*" $attr(0)] } {
     set to_validate 0
    }
    if { $to_validate == 1 } {
    ...
    if { $to_validate == 1 } {
     if { $proc_state == 1 } {
      set ::CMD_BCK($attr(0)) $attr(1)
      } else {
      if [info exists ::CMD_BCK($attr(0))] {
       unset ::CMD_BCK($attr(0))
      }
     }
    }
    if { $proc_state <= 0 && $msg != "" } {
  ...
  set time_string [PB_com_GetTimeString]
  set out_file [file rootname [file tail $gPB(work_file)]]
  if { [string match "NEW" $gPB(session)] } {
   if { [info exists gPB(post_saved_from)] } {
    if { [string compare $gPB(post_saved_from) $out_file] != 0 } {
     set action "Created"
     } else {
     set action "Saved"
    }
    unset gPB(post_saved_from)
    } else {
    set action "Created"
   }
   } else {
  ...
  set out_file [string trim $out_file]
  if { $out_file != "" } {
   set out_file "$out_file "
  }
  lappend gPB(post_history) "$action ${out_file}with $gPB(Postbuilder_Release)$gPB(Postbuilder_Version_Phase)\
  ...
  if [string match "NEW" $gPB(session)] {
   set gPB(PB_SITE_ID) $gPB(DEFAULT_PB_SITE_ID)
  }
  PB_CreatePostFiles
  ...
  if [string length $license] {
   ...
   if { $gPB(PB_LICENSE) == "UG_POST_AUTHOR" } {
    if { $::disable_internal_user_lc || $::disable_license_win_title } {
     ...
     set title "$gPB(Postbuilder_Release) - $gPB(main,title,license_control)"
    }
   }
  }
...
#=======================================================================
proc UI_PB_ChangeTopWinTitle { title } {
  global gPB
  if [info exists gPB(master_pid)] {
...
proc __CheckFilePermissions {args} {
  ...
  set cdl_file [file rootname $gpb_tcl_file].cdl
  if { [file exists "$cur_dir/$cdl_file"] && \
   ![file writable "$cur_dir/$cdl_file"] } {
   set err_msg "$err_msg\n  [file nativename $cur_dir/$cdl_file]"
   if { ![__AddWritePermission "$cur_dir/$cdl_file"] } {
    set enable_write_permission 0
    set deny_cdl 1
    } else {
    set deny_cdl 0
  ...
  if { ![string match "" $err_msg] } {
   if { !$enable_write_permission} {
    ...
    if $restore_file_protection {
     ...
     if { $deny_cdl == 0 } {
      __RemoveWritePermission "$cur_dir/$cdl_file"
     }
     if [info exists gPB(post_in_progress)] {
      unset gPB(post_in_progress)
     }
...
proc UI_PB_ClosePost { args } {
  ...
  if { [info exists env(MULTI_INTERP)] && $env(MULTI_INTERP) == 1 } {
   global gPB
   if [info exists gPB(master_pid)] {
    set choice [UI_PB_ClosePost_mod $args]
    } else {
    global PID
  ...
  if { [info exists ::gPB__new_elem_mom_var] } {
   PB_com_unset_var ::gPB__new_elem_mom_var
   PB_com_unset_var ::gPB__comb_var
   PB_com_unset_var ::gPB__base_addr
  }
  return $choice
 }
...
proc UI_PB_ClosePost_mod { args } {
  ...
  if { [info exists gPB(PB_LICENSE)] } {
   if { $gPB(PB_LICENSE) != "UG_POST_NO_LICENSE" } {
    if [PB_file_DiffPostObjects] {
     ...
     if 0 {
      set choice [tk_dialog [UI_PB_com_AskActiveWindow].save "" "$gPB(msg,save)" \
      question 1 "Save As" "Save" No Cancel]
     }
  ...
  if { [string compare "yes" $choice] == 0 } {
   if { [UI_PB_file_CheckMachResolution "Pend"] == 0 } {
    set choice "cancel"
    } else {
    set gPB(suppress_check_resolution) 1
   }
  }
  if { $choice == "yes" } \
  {
  ...
  {
   set ::do_close 1
   set ::validation_result ok
   UI_PB_SavePost
   if { [info exists ::validation_result] } {
    if { ![string match $::validation_result "ok"] } {
     unset ::do_close
   ...
   PB_com_unset_var ::do_close
   UI_PB_com_SetStatusbar "$gPB(main,default,Status)" ;# "$prev_mssg"
  ...
  if { $choice == "yes"  ||  $choice == "no" } \
  {
   ...
   __ClearLicRelatedInfo
   if [info exists ::CMD_BCK] {
    PB_com_unset_var ::CMD_BCK
   }
   if [info exists ::ListObjectAttr] {
    PB_com_unset_var ::ListObjectAttr
   }
   PB_com_unset_var ::gPB(machine_axis_type)
  }
  if { $choice == "yes" || $choice == "no" } {
...
proc __ClearLicRelatedInfo { } {
  ...
  if { $::disable_internal_user_lc || $::disable_license_win_title } {
   set title "$gPB(Postbuilder_Release)"
   } elseif { $gPB(PB_LICENSE) == "UG_POST_AUTHOR" } {
   set title "$gPB(Postbuilder_Release) - $gPB(main,title,license_control)"
  }
  UI_PB_ChangeTopWinTitle $title
...
proc __DeletePostWidgets { } {
  ...
  if [info exists gPB(tix_grid)] \
  {
   ...
   tixDestroy $gPB(tix_grid)
   PB_com_unset_var gPB(tix_grid)
  }
...
proc UI_PB_main_window {args} {
  ...
  set xc [expr $twx - $gPB(WIN_X_MAIN)]
  set yc [expr $twy + $gPB(TOP_WIN_HI) + $gPB(WIN_X_MAIN)]
  set yc [expr $yc + $gPB(WIN_X_OFF)]
  set mw [toplevel $gPB(top_window).main]
  if 0 {
   if { [info exists ::gPB_GEOM($mw)] } {
    set geom $::gPB_GEOM($mw)
    } else {
    set geom "$gPB(TOP_WIN_WI)x$gPB(MAIN_WIN_HI)+$xc+$yc"
   }
  }
  UI_PB_debug_ForceMsg "\n TTTTT ($xc , $yc) ($twx , $twy)  >$gPB(WIN_X_MAIN)< TTTTT \n"
  set geom "$gPB(TOP_WIN_WI)x$gPB(MAIN_WIN_HI)+$xc+$yc"
  UI_PB_com_CreateTransientWindow $mw "Post Processor" "$geom" \
  ...
  if { ![PB_is_probe_post] } {
   ...
   Book::CreatePage $gPB(book) mac "" "" "" ""
   $Book::($gPB(book),book_id) pageconfigure mac -state disabled
  }
  ...
  if { ![PB_file_is_JE_POST_DEV] && ![PB_is_probe_post] } {
  ...
  if { [PB_is_probe_post] } {
   $Book::($gPB(book),book_id) raise pro
  }
  update idletasks
  ...
  set gPB(WIN_Y) [expr $gPB(WIN_Y) - $gPB(WIN_X_OFF)]
...
#=======================================================================
proc UI_PB_main_window_unit_sub {args} {
  global gPB
  global paOption
  if { [info exists gPB(main_window)] } {
   unset gPB(main_window)
  ...
  set yc [expr $yc + $gPB(WIN_X_OFF)]
  set mw [toplevel $gPB(top_window).main]
  UI_PB_com_CreateTransientWindow $mw "Post Processor" "$gPB(TOP_WIN_WI)x$gPB(MAIN_WIN_HI)+$xc+$yc" \
  "" "" "UI_PB_ClosePost" "__DeletePostWidgets" 0
  set gPB(main_window) $mw
  UI_PB_com_SetWindowTitle
  set w [tixNoteBook $mw.nb -ipadx 3 -ipady 3]
  ...
  bind [$w subwidget nbframe] <Button-1> "UI_PB_NoteBookTabBinding $w %x %y"
  set gPB(book) [new Book w]
  Book::CreatePage $gPB(book) mac "$gPB(unit_related_param,tab,Label)" "pb_mach_kin" \
  UI_PB_MachineTool UI_PB_MachToolTab
  Book::CreatePage $gPB(book) adr "$gPB(address,tab,Label)" "pb_address" \
  UI_PB_Def_Address UI_PB_Def_AddressTab
  Book::CreatePage $gPB(book) fmt "$gPB(format,tab,Label)" "pb_format" \
...
proc UI_PB_ValidateCommandPage { cmd_page CC_ERR_MSG} {
  ...
  if { [info exists Page::($cmd_page,active_cmd_obj)] } \
  {
   ...
   if { $reg == 1} {
    ...
    set cur_cmd_name $command::($cmd_obj,name)
   }
...
proc UI_PB_ValidatePrevBookData { BOOK_OBJ } {
  ...
  set blk_page_flag 0
  set add_page_flag 0
  set fmt_page_flag_detail 0
  set add_page_flag_msg ""
  ...
  switch -exact -- $Book::($book_obj,current_tab) \
  {
   ...
   1 { ;# Program & Tool Path
    ...
    if [info exists Page::($page_obj,book_obj)] \
    {
     ...
     {
      0 {
       UI_PB_debug_ForceMsg "\n %%% Validate Sequence output %%% \n"
       set prog_tab $Book::($page_book_obj,current_tab)
       set prog_page_obj [lindex $Book::($page_book_obj,page_obj_list) $prog_tab]
       if { ![UI_PB_evt_progItemSelection $prog_page_obj 99] } {
        set raise_page 1
       }
      }
      3 { ;# Addresses Summary page
       ...
       set leader_err [UI_PB_ads_ErrorWithAddressLeaders]
       if { $leader_err } {
        set raise_page 1
       }
      }
      ...
      7 { ;#<06-03-09 wbh> Macro page
       set func_page [lindex $Book::($page_book_obj,page_obj_list) 7]
       set func_err_msg ""
       set raise_page [UI_PB_func_CheckFuncFormat $func_page func_err_msg]
       if { !$raise_page && [info exists Page::($func_page,active_func_obj)] } {
     ...
     switch -- $Book::($page_book_obj,current_tab) \
     {
      0 { ;# Blocks
       ...
       if { [info exists Page::($blk_page,active_blk_obj)] } \
       {
        ...
        if { !$raise_page && [UI_PB_file_CheckNameForOldVersion "block" first_long_name 0] } {
         set raise_page 1
         set blk_page_flag 101
        }
       }
      }
      1 { ;# Addresses
       ...
       if { [info exists Page::($addr_page,act_addr_obj)] } \
       {
        ...
        if { [UI_PB_add_ValidateFormat $addr_page add_page_flag_msg] } {
         set add_page_flag 5
        }
        if { ![UI_PB_addr_ValidateAddrLeader $gPB_address_name $::ADDRESSOBJATTR(8)] } {
         set add_page_flag 6
        }
        if { $add_page_flag } \
        {
         set raise_page 1
         } elseif { [string length $gPB_address_name] > 32 } \
        {
         set add_page_flag 100
         set raise_page 1
        } elseif [UI_PB_file_CheckNameForOldVersion "address" first_long_name 0] \
        {
         set add_page_flag 101
         set raise_page 1
        }
       }
      }
   ...
   4 { ;# Virtual N/C Controller
    ...
    switch -- $Book::($page_book_obj,current_tab) {
     0 { ;# VNC definition Page
      ...
      set bottom_page_obj [lindex $Book::($page_book_obj,page_obj_list) 0]
...
set cmd_proc [$book_id pagecget $prev_page_name -raisecmd]
 ...
 switch -exact -- $Book::($book_obj,current_tab) \
 {
  0 { ;#<02-03-03 gsl> Machine Tool
   if { [info exists empty_ent_validate] && $empty_ent_validate == 1 } {
    UI_PB_mach_SignalEmptyEntry $page_obj
    unset empty_ent_validate
    } else {
    tk_messageBox -parent [UI_PB_com_AskActiveWindow] \
    -type ok -icon error \
   ...
   switch -- $Book::($page_book_obj,current_tab) \
   {
    3 { ;#<11-27-01 gsl> Addresses Summary page
     if { [info exists addrsum_err] && $addrsum_err } {
      ...
      set msg "$::gPB(address,leader,err_msg)"
      tk_messageBox -parent [UI_PB_com_AskActiveWindow] \
      -type ok -icon error -message "$msg"
     }
    }
    5 {
    ...
    7 { ;#<06-03-09 wbh> Macro page
     tk_messageBox -parent [UI_PB_com_AskActiveWindow] \
     -type ok -icon error -message $func_err_msg
    }
   }
  }
  2 { ;# N/C Data Definitions
   ...
   switch -- $Book::($page_book_obj,current_tab) \
   {
    ...
    1 {
     if { $add_page_flag < 4 } {
      ...
      } elseif { $add_page_flag == 100 } {
      tk_messageBox -parent [UI_PB_com_AskActiveWindow] \
      -type ok -icon error \
      -message "$gPB(address,maximum_name_msg)"
      } elseif { $add_page_flag == 101 } {
      tk_messageBox -parent [UI_PB_com_AskActiveWindow] \
      -type ok -icon error \
      -message "$first_long_name : $gPB(address,maximum_name_msg)"
      } elseif { $add_page_flag == 4 } {
      tk_messageBox -parent [UI_PB_com_AskActiveWindow] \
      -type ok -icon error \
      -message "$gPB(msg,add_max1) N $gPB(msg,add_max2) $max."
      } elseif { $add_page_flag == 5 } {
      UI_PB_mthd_DisplayErrMsg "$add_page_flag_msg"
      } elseif { $add_page_flag == 6 } {
      set msg "$::gPB(address,leader,err_msg)"
      tk_messageBox -parent [UI_PB_com_AskActiveWindow] \
      -type ok -icon error -message "$msg"
     }
...
#=======================================================================
proc UI_PB_ApplyCustCommand { PAGE_OBJ } {
...
proc UI_PB_UpdateProgTpthBook { PRG_BOOK } {
  ...
  switch -exact -- $Book::($prg_book,current_tab) \
  {
   ...
   5 {
    ...
    set ret_flag [UI_PB_ApplyCustCommand page_obj]
   }
...
#=======================================================================
proc PB_is_newer_v { ver } {
  global gPB
  if { ![info exists gPB(Postbuilder_Release_Version)] } {
   return 0
  }
  set src_list [split $ver .]
  set cur_list [split $gPB(Postbuilder_Release_Version) .]
  set src_len [llength $src_list]
  set cur_len [llength $cur_list]
  set count [expr { $cur_len < $src_len ? $cur_len : $src_len }]
  set ret_code 0
  for { set i 0 } { $i < $count } { incr i } {
   set src_v [lindex $src_list $i]
   set cur_v [lindex $cur_list $i]
   if { $cur_v > $src_v } {
    return 1
    } elseif { $cur_v < $src_v } {
    return 0
