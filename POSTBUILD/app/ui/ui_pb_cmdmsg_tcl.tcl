 ...
 if { !$mom_vars_csv_found } {
  ...
  if [array exists ::gPB_mom_var_info] {
   foreach { var desc } [array get ::gPB_mom_var_info] {
    ...
    if { ![info exists __hello_var_names($var)] } {
     set __hello_var_names($var)   "$var"
    }
    if { ![info exists __hello_class_names($var)] } {
     set __hello_class_names($var) "General"
    }
    if { ![info exists __hello_value_names($var)] } {
     set __hello_value_names($var) ""
    }
    if { ![info exists __hello_type_names($var)] } {
     set __hello_type_names($var)  ""
    }
    if { ![info exists __hello_def_names($var)] } {
     set __hello_def_names($var)   ""
    }
   }
  }
 } ;# $mom_vars_csv_found
