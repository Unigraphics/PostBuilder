...
#=======================================================================
proc __cmd_EditBlock { page_obj block_obj \
...
label $top_frm.prc -text " proc    " -fg $fg -bg $bg -font $ft -anchor e
 ...
 -relief flat -bd 0 -justify left
 $top_frm.ent config -state disabled
 label $top_frm.brc -text "   \{ [info args $pb_cmd_procname] \}   \{" -fg $fg -bg $bg -font $ft
  pack $top_frm.prc $top_frm.ent $top_frm.brc -side left -pady 10 -fill x
  tixScrolledText $mid_frm.scr
  [$mid_frm.scr subwidget hsb] config -troughcolor $paOption(trough_bg) \
  -width $paOption(trough_wd)
...
$Page::($page_obj,canvas_frame).top.prc config -text " proc    "
 set pb_cmd_procname [string range $command::($proc_obj,name) $len end]
...
set cb_arr(gPB(nav_button,cancel,Label)) \
"__cmd_tpthViewCmdBlkCancel_CB $win $page_obj $cmd_page_obj"
if { [string match "proc" $obj_type] } {
...
set box_frm [frame $box.frm]
pack $box_frm -fill x
set label_list { "gPB(nav_button,cancel,Label)" }
UI_PB_com_CreateButtonBox $box_frm label_list cb_arr
UI_PB_com_PositionWindow $win
return $win
}
...
proc __cmd_InfoObject { page_obj obj_type obj_name obj_id } {
  ...
  if { ![info exists __hello_des_names($des_elem)] } {
   if { [string match "VNC_load_post_definitions" $obj_name] } {
    set __hello_des_names($des_elem) "VNC command generated by Post Builder\
    to convey the settings of a post to its VNC driver."
    } elseif { [string match "VNC_*" $obj_name] } {
    set __hello_des_names($des_elem) "VNC utility command defined in PB_CMD_vnc__define_misc_procs."
    } elseif { [string match "SIM_*" $obj_name] } {
    set __hello_des_names($des_elem) "Commands to be used in simulation driver."
    } elseif { [string match "PB_SIM_call" $obj_name] } {
    set __hello_des_names($des_elem) "Broker command used in VNC driver."
   }
  }
  if { [array exists __hello_des_names] && [info exists __hello_des_names($des_elem)] } {
   ...
   -message "Information about $obj_name not yet configured." -title "$obj_type : $obj_name"
  }
 }
...
proc __cmd_EditObject { page_obj obj_type obj_id } {
     ...
     foreach evt_elem_obj $block::($current_blk_obj,evt_elem_list) {
      ...
      foreach obj $Post::($::post_object,seq_obj_list) {
       if { [lsearch $sequence::($obj,evt_obj_list) $evt_obj] >= 0 } {
        set seq_obj $obj
        UI_PB_debug_ForceMsg "\n+++ Edit Block $block::($current_blk_obj,block_name) $block::($current_blk_obj,evt_elem_list) of Sequence >$sequence::($seq_obj,seq_name)< +++\n"
  ...
  if { $obj_type == "Macro" } \
  {
   __cmd_EditMacroFunc $page_obj $current_blk_obj
   return
  }
  if { $obj_type == "Command" } \
  {
   __cmd_EditCommand $page_obj $current_blk_obj
   return
  }
  if { $block::($current_blk_obj,blk_type) == "normal" } \
  {
   ...
   set rename_mode 0
   __cmd_EditBlock $page_obj $current_blk_obj \
   ...
   $page_obj $seq_obj $evt_elem_obj $current_blk_obj"
   set cb_arr(gPB(nav_button,ok,Label)) \
...
if { $obj_type != "" && $obj_id > 0 } {
 ...
 set opt "edit"
 if { $obj_type == "Command" } {
  set gCmd $::pb_cmd_procname
  set ::pb_cmd_procname [string range $obj_name 12 end]
  if { [__cmd_IsSystemVNCCmd] } {
   set opt "view"
  }
  ...
  set ::pb_cmd_procname $gCmd
 }
 if { $obj_type == "proc" } {
  if { [string match "MOM_*"    $obj_name] || \
   [string match "SIM_*"    $obj_name] || \
   [string match "VNC_*"    $obj_name] || \
   [string match "PB_SIM_*" $obj_name] } {
   set opt "info"
   set obj_type "Command"
   } else {
   set opt "view"
 ...
 switch $opt {
  edit {
   ...
   -command "__cmd_EditObject $page_obj $obj_type $obj_id"
  }
  view {
   $menu add command -label "$gPB(block,view_popup,Label) $obj_type $obj_name" \
   -command "__cmd_ViewObject $page_obj $obj_type $obj_name $obj_id"
  }
  default {
   $menu add command -label "$gPB(block,info_popup,Label) $obj_type $obj_name" \
   -command "__cmd_InfoObject $page_obj $obj_type $obj_name $obj_id"
  }
 }
}
}
...
if { !$tag } {
 ...
 if { [UI_PB_cmd_CheckCmdState "PB_CMD_$pb_cmd_procname" "rename"] == 0 || \
  [string match "before_motion" $pb_cmd_procname] || \
 [string match "kin*" $pb_cmd_procname] } \
 {
  bind $ent_wdg <Return> ""
