...
proc UI_PB_cmd_CreateTreePopupElements { page_obj X Y x y } {
  ...
  $active_index == $cursor_entry } \
  {
   ...
   if { $disabled_rename_flag || \
    [string match "PB_CMD_before_motion" $indx_string] || \
...
proc UI_PB_cmd_CutACmdBlock { page_obj } {
   ...
   } elseif {$command::($active_cmd_obj,blk_elem_list) != ""} \
  {
   ...
   } elseif { [string match "$gPB(condition_cmd_prefix)*" $command::($active_cmd_obj,name)] && \
...
#=======================================================================
label $top_frm.prc -text " proc    $gPB(condition_cmd_prefix)" -fg $fg -bg $bg -font $ft -anchor e
 } else {
   ...
   if { $ent_flag } \
   {
    if !$tag {
     ...
     [string match "kin*" $pb_cmd_procname] } \
     {
      ...
      [string match $pb_cmd_procname \
     [string range $mom_sys_arr(post_startblk) 7 end]] } \
     {
      bind $ent_widget <Return> ""
      $ent_widget config -state disabled
     } else \
...
proc UI_PB_cmd_CheckCmdState { cmd_name state_type } {
  ...
  if ![info exists gPB(special_cmd_state_list)] { return 1 }
