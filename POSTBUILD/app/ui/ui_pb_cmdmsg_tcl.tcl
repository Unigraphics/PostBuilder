...
if [string match "PB_CMD_vnc__*" $cur_cmd_name] {
...
-state disabled
set file  $gPB(pb_cmd_icon)
UI_PB_cmd_ImportVNCCmds command_page
...
[string match "PB_CMD_vnc_*" $cmd_name] } {
set list_cmd 1
...
if { [PB_com_FindObjFrmName "PB_CMD_vnc____map_machine_tool_axes" command] < 0 } {
...
global post_object
global gPB
if [string match "command_page" $flag] {
UI_PB_DisplayProgress
...
if [info exists Page::($page_obj,rename_index)] {
UI_PB_debug_ForceMsg "=====> $ent  $Page::($page_obj,rename_index)"
}
...
$del config -state disabled
} elseif [string match "PB_CMD_vnc__*" $command::($cmd_obj,name)] {
...
tk_messageBox -parent [UI_PB_com_AskActiveWindow] -type ok \
-icon warning -title "$gPB(cust_cmd,import,error,title)" \
...
label $top_frm.prc -text " proc    VNC_CMD_" -fg $fg -bg $bg -font $ft
} else {
