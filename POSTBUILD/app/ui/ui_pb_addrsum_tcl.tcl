...
proc UI_PB_ads__CreateAdrAttributes { PAGE_OBJ grid } {
  ...
  for { set count 0 } { $count < $no_addr } { incr count } \
  {
   ...
   UI_PB_ads__CreateFirstColAttr $page_obj $grid [expr $count + 1] $add_name $add_desc
...
proc UI_PB_ads__CreateFirstColAttr { page_obj grid no_row add_name add_desc } {
  ...
  if { [PB_com_object_name_of_class_is_external $add_name address] } {
   UI_PB_com_DisableWindow $add_frm
   bind $but <Enter> ""
  }
 }
...
proc UI_PB_ads_EditOk_CB { page_obj new_add_page win add_button_id } {
  ...
  if { ![UI_PB_addr_ApplyCurrentAddrData new_add_page] } {
   return
  }
  PB_com_DeleteObject $new_add_page
  destroy $win
  ...
  __ads_UpdateAddressDataWidgets $add_name
 }
...
proc UI_PB_ads__DisableDataAttr { grid add_name data_type fmt_name } {
  if { [PB_com_object_name_of_class_is_external $fmt_name format] } {
   $grid.dat_$add_name.num  config -state disabled
   $grid.dat_$add_name.text config -state disabled
   ...
   $grid.dat_$add_name.num  config -state normal
   $grid.dat_$add_name.text config -state normal
  }
  switch $data_type \
  {
   "Numeral"  {
...
proc UI_PB_ads__CreateSecColAttr { page_obj grid no_row add_name } {
  ...
  if { [PB_com_object_name_of_class_is_external $fmt_name format] } {
   foreach frm [list $dat_frm $pls_frm $lez_frm $int_frm $dec_frm $fra_frm $trz_frm] {
    UI_PB_com_DisableWindow $frm
   }
  }
  ...
  if { [PB_com_object_name_of_class_is_external $add_name address] } {
   foreach frm [list $mod_frm $min_frm $max_frm $tra_frm] {
    UI_PB_com_DisableWindow $frm
   }
   bind $tra_frm <Enter> ""
  }
...
proc __ads_Sync_Filename { varName index op } {
  ...
  if { ![string match "*$filename.cdl" $mom_sys_arr(OWN_CDL_File_Folder)] } {
   ...
   if { [llength $old_str_list] == 1 } {
    set str "\$UGII_CAM_POST_CURRENT_DIR/$filename.cdl"
...
#=======================================================================
proc __ads_UpdateAddressDataWidgets { add_name } {
...
proc UI_PB_ads_TabAddrsumCreate { PAGE_OBJ } {
  ...
  foreach add_name $addr_name_list \
  {
   ...
   if { $index == -1 } \
   {
    ...
    UI_PB_ads__CreateFirstColAttr $page_obj $grid $prev_no_adds $add_name $add_desc
...
#=======================================================================
proc UI_PB_ads_ErrorWithAddressLeaders { args } {
  global gpb_addr_var
  set has_err 0
  foreach add { N X Y Z fourth_axis fifth_axis } {
   if { [info exists ::gpb_addr_var($add,leader_name)] } {
    if { ![UI_PB_addr_ValidateAddrLeader $add $::gpb_addr_var($add,leader_name)] } {
     set has_err 1
     break
    }
   }
...
proc __ads_cdl_ShowCdlFileDlg { wtext cdl_type {edit_flag 0} } {
  ...
  pack $btm_frm -side bottom -fill x -expand no -padx 3 -pady 3
  pack $top_frm -side top    -fill x -expand no -padx 5 -pady 5
  ...
  UI_PB_com_ResizeWindow $w
  raise $w; focus $w; grab $w
 }
