...
proc UI_PB_ads_SpecDefaultCallBack { } {
  ...
  if [PB_is_v 3.1] {
   ...
   lappend mom_var_list 13 "Inherit_UDE" 14 "PST_File_Name" 15 "PST_File_Folder"
   if {$::env(PB_UDE_ENABLED) == 1} {
    lappend mom_var_list 16 "Own_UDE" 17 "OWN_CDL_File_Folder"
   }
  }
...
proc UI_PB_ads_ValidateIncludeItems {PASS ERROR_INFO check_display} {
  ...
  if { $check_display == 1 || $check_display == 2 } {
   ...
   if { [info exists mom_sys_arr(Include_UDE)] && $mom_sys_arr(Include_UDE) == 1 } {
    if { $mom_sys_arr(UDE_File_Name) == "" || \
     ![string match "\$UGII_*" $mom_sys_arr(UDE_File_Name)] } {
     set pass 1
     set error_info 8
     } else {
...
proc UI_PB_ads_CreateSpecialChars { PAGE_OBJ page_frm } {
  ...
  if $include_ude_cdl {
   set label_list [list $gPB(ude,import,include_cdl,Label) \
   $gPB(ude,import,ihr_pst,Label) \
   $gPB(ude,import,ihr_folder,Label) \
   $gPB(ude,import,own_folder,Label)]
   label .dummy_label -text dummy
   set maxlen 0
   foreach one $label_list {
    .dummy_label config -text $one
   ...
   button $inc_frm.sel -text "$gPB(other,ude_select,Label)" -width 10
   pack $inc_frm.sel -anchor se -padx 10 -pady 5
   set cdl_frm [frame $ude_frm.cdl]
   pack $cdl_frm -fill x -padx 5 -expand yes
   frame $cdl_frm.frm -width $maxlen
   label $cdl_frm.frm.cdl_lbl -text $gPB(ude,import,include_cdl,Label) -anchor w
   entry $cdl_frm.name -width 45 -relief sunken -textvariable mom_sys_arr(UDE_File_Name)
   pack propagate $cdl_frm.frm 0
   pack $cdl_frm.frm -side left -padx 5 -fill y -anchor w
   pack $cdl_frm.frm.cdl_lbl -side left
   pack $cdl_frm.name  -fill x -expand yes -padx 5 -pady 5
   $inc_frm.sel config -command "__ads_BrowseUDE $cdl_frm.name"
   set sepr1 [UI_PB_com_AddSeparator $ude_frm.sepr1]
   pack $sepr1 -fill x -expand no -padx 5 -pady 5 ;#<10-19-07 gsl> -padx was 0.
   set ihr_frm [frame $ude_frm.ihr]
   pack $ihr_frm -fill x
   UI_PB_mthd_CreateCheckButton Inherit_UDE $ihr_frm ihr "$gPB(ude,import,ihr,Label)"
   button $ihr_frm.sel -text "$gPB(ude,import,sel,Label)" -width 10
   pack $ihr_frm.sel -anchor se -padx 10 -pady 5
   set pst_frm [frame $ude_frm.pst]
   set frm1 [frame $pst_frm.1]
   set frm2 [frame $pst_frm.2]
   pack $pst_frm -fill x
   pack $frm1 $frm2 -padx 5 -expand yes -fill x
   frame $frm1.frm -width $maxlen
   set cdl_lbl [label $frm1.frm.cdl -text $gPB(ude,import,ihr_pst,Label) -anchor w]
   entry $frm1.name_cdl -width 45 -relief sunken -textvariable mom_sys_arr(PST_File_Name) \
   -state disabled
   pack propagate $frm1.frm 0
   pack $frm1.frm -side left -padx 5 -fill y -anchor w
   pack $cdl_lbl  -side left
   pack $frm1.name_cdl -fill x -expand yes -padx 5 -pady 5
   frame $frm2.frm -width $maxlen
   set def_lbl [label $frm2.frm.def -text $gPB(ude,import,ihr_folder,Label) -anchor w]
   entry $frm2.name_def -width 45 -relief sunken -textvariable mom_sys_arr(PST_File_Folder) \
   -state disabled
   pack propagate $frm2.frm 0
   pack $frm2.frm -side left -padx 5 -fill y -anchor w
   pack $def_lbl  -side left
   pack $frm2.name_def -fill x -expand yes -padx 5 -pady 5
   if {$::tix_version == "4.1"} {
    $frm1.name_cdl config -bg $::paOption(entry_disabled_bg)
    $frm2.name_def config -bg $::paOption(entry_disabled_bg)
   }
   $ihr_frm.sel config -command "__ads_BrowsePost"
   set sepr2 [UI_PB_com_AddSeparator $ude_frm.sepr2]
   pack $sepr2 -fill x -expand no -padx 5 -pady 5 ;#<10-19-07 gsl> -padx was 0.
   set own_frm [frame $ude_frm.own]
   pack $own_frm -fill x
   UI_PB_mthd_CreateCheckButton Own_UDE $own_frm own "$gPB(ude,import,own,Label)"
   set own_str_frm [frame $ude_frm.ostr]
   frame $own_str_frm.frm -width $maxlen
   label $own_str_frm.frm.lbl -text $gPB(ude,import,own_folder,Label) -anchor w
   entry $own_str_frm.ent -width 20 -relief sunken -textvariable mom_sys_arr(OWN_CDL_File_Folder) \
   -state disabled
   pack propagate $own_str_frm.frm 0
   pack $own_str_frm.frm -side left -padx 5 -fill y -anchor w
   pack $own_str_frm.frm.lbl -side left
   pack $own_str_frm.ent -fill x -expand yes  -padx 5 -pady 3
   pack $own_str_frm -fill x -padx 5
   set dummy_frm [frame $ude_frm.dummy]
   pack $dummy_frm -fill x -pady 2
   if {$::env(PB_UDE_ENABLED) != 1} {
    $own_frm.own config -state disabled
   }
   set gPB(c_help,$inc_frm.inc)           "other,ude_include"
   set gPB(c_help,$cdl_frm.name)          "other,ude_file"
   set gPB(c_help,$inc_frm.sel)           "other,ude_select"
   set gPB(c_help,$ihr_frm.ihr)           "ude,import,ihr"
   set gPB(c_help,$frm1.name_cdl)         "ude,import,name_cdl"
   set gPB(c_help,$frm2.name_def)         "ude,import,name_def"
   set gPB(c_help,$ihr_frm.sel)           "ude,import,sel"
   set gPB(c_help,$own_frm.own)           "ude,import,own"
   set gPB(c_help,$own_str_frm.ent)       "ude,import,own_ent"
   set gPB(Include_UDE_Frame) [list $inc_frm.sel $cdl_frm.name $cdl_frm.frm.cdl_lbl]
   set gPB(Inherit_UDE_Frame) [list $ihr_frm.sel $frm1.name_cdl \
   $frm2.name_def $cdl_lbl $def_lbl]
   set gPB(Inherit_OWN_UDE_Frame) [list $own_str_frm.ent $own_str_frm.frm.lbl]
   $inc_frm.inc config -command "__ads_ToggleUDE"
   $ihr_frm.ihr config -command "__ads_ToggleIhrUDE"
...
proc __ads_ToggleUDE {} {
  global gPB paOption
  global mom_sys_arr tix_version
  set but [lindex $gPB(Include_UDE_Frame) 0]
  set ent [lindex $gPB(Include_UDE_Frame) 1]
  set lbl [lindex $gPB(Include_UDE_Frame) 2]
  if { $mom_sys_arr(Include_UDE) } {
   $but config -state normal
   $ent config -state normal
   bind $ent <KeyPress> "__ads_ValidateIncludeUdePath %W %K %A both"
...
proc __ads_ToggleIhrUDE {} {
  ...
  global tix_version
  global paOption
  set but     [lindex $gPB(Inherit_UDE_Frame) 0]
  set ent_cdl [lindex $gPB(Inherit_UDE_Frame) 1]
  set ent_def [lindex $gPB(Inherit_UDE_Frame) 2]
  set cdl_lbl [lindex $gPB(Inherit_UDE_Frame) 3]
  set def_lbl [lindex $gPB(Inherit_UDE_Frame) 4]
  if { $mom_sys_arr(Inherit_UDE) } {
   $but config -state normal
   $ent_cdl config -state normal
   $ent_def config -state normal
   bind $ent_cdl <KeyPress> "__ads_ValidateIncludeUdePath %W %K %A fname"
   bind $ent_cdl <KeyRelease> "__ads_ValidateIncludeUdePath_Release %W %K"
   bind $ent_def <KeyPress> "__ads_ValidateIncludeUdePath %W %K %A folder"
   bind $ent_def <KeyRelease> "__ads_ValidateIncludeUdePath_Release %W %K"
   if [string match $mom_sys_arr(PST_File_Folder) ""] {
    set mom_sys_arr(PST_File_Folder) "\$UGII_CAM_POST_DIR/"
   }
   if {$tix_version == "4.1"} {
    $ent_cdl config -bg $gPB(entry_color)
    $ent_def config -bg $gPB(entry_color)
   }
   if {$::tix_version == "8.4"} {
    $cdl_lbl config -state normal
    $def_lbl config -state normal
   }
   } else {
   $but config -state disabled
   $ent_cdl config -state disabled
   $ent_def config -state disabled
   bind $ent_cdl <KeyPress> {}
   bind $ent_cdl <KeyRelease> {}
   bind $ent_def <KeyPress> {}
   bind $ent_def <KeyRelease> {}
   if {$tix_version == "4.1"} {
    $ent_cdl config -bg $paOption(entry_disabled_bg)
    $ent_def config -bg $paOption(entry_disabled_bg)
   }
   if {$::tix_version == "8.4"} {
    $cdl_lbl config -state disabled
    $def_lbl config -state disabled
   }
  }
 }
