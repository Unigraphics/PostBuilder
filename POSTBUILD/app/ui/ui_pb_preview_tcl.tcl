...
$h add 0.$t -itemtype imagetext -text $seq_attr(0) \
...
$h add 0.6 -itemtype imagetext -text "$gPB(definition,comp_txt,Label)" \
-image $blk_img -style $style -state normal ;#<01-04-02 gsl>disabled
set t 0
foreach blk_obj $comp_blk_list \
{
...
if { $post_blk_list !=  "" } \
{
$h add 0.7 -itemtype imagetext -text "$gPB(definition,post_txt,Label)" \
-image $blk_img -style $style -state normal ;#<01-04-02 gsl>disabled
set t 0
...
if { $comment_blk_list !=  "" } \
{
$h add 0.8 -itemtype imagetext -text "$gPB(definition,oper_txt,Label)" \
-image $blk_img -style $style -state normal ;#<01-04-02 gsl>disabled
set t 0
...
6 { UI_PB_prv_CompBlockTemplate page_obj sec_indx }
7 { UI_PB_prv_PostBlkTemplate page_obj sec_indx }
8 { UI_PB_prv_CommentBlkTemplate page_obj sec_indx }
}
}
