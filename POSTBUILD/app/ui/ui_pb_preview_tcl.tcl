...
proc UI_PB_Preview { book_id pre_page_obj } {
  ...
  Book::CreatePage $prev_book dfn "$gPB(definition,tab,Label)" "" \
  ...
  if 0 {
   if { [PB_file_is_JE_POST_DEV] && ![PB_is_probe_post] } {
    $Book::($prev_book,book_id) raise dfn ;#<Aug-24-2017 gsl> Was "def".
    update idletasks
   }
  }
 }
...
proc UI_PB_prv_CreateSecPaneElems { PAGE_OBJ } {
  ...
  set pane [tixPanedWindow $sec_pane.pane -orient vertical -handlebg orange]
  ...
  if { ![string match "NEW" $::gPB(session)] } {
   ...
   set f1 [$pane add p1 -expand 1 -size 500]
   set f2 [$pane add p2 -expand 1 -size 0]
  }
  ...
  if { [info exists gPB(Edit_Post_Object)] && $gPB(Edit_Post_Object) } {
   if { ![info exists gPB(text_select_bg)] } {
    set gPB(text_select_bg) [lindex [$ltext cget -selectbackground] 0]
   }
   bind $ltext <1> "%W config -selectbackground $gPB(text_select_bg)"
   bind $rtext <1> "%W config -selectbackground $gPB(text_select_bg)"
   if { [string match "UI_PB_prv_EventHandler" [lindex [info level -1] 0]] } {
    if { [winfo exists $ltext.pop] } {
     set menu $ltext.pop
     } else {
     set menu [menu $ltext.pop -tearoff 0 -cursor ""]
    }
    bind $ltext <3> "UI_PB_cmd_AddPopUpMenu $page_obj $ltext $menu %X %Y"
    if { ![string match "NEW" $::gPB(session)] } {
     if { [winfo exists $rtext.pop] } {
      set menu $rtext.pop
      } else {
      set menu [menu $rtext.pop -tearoff 0 -cursor ""]
     }
     bind $rtext <3> "UI_PB_cmd_AddPopUpMenu $page_obj $rtext $menu %X %Y"
    }
    } else {
    bind $ltext <3> ""
    bind $rtext <3> ""
  ...
  bind $ltext <Enter> "focus %W"
  bind $rtext <Enter> "focus %W"
  focus $ltext
 }
...
proc UI_PB_prv_TclTreeSelection { page_obj args } {
  ...
  set first_indx [lindex [split $ent .] 1]
  set sec_indx   [lindex [split $ent .] 2]
  ...
  UI_PB_debug_ForceMsg "\n******* ent: $ent  first_indx: $first_indx  sec_indx: $sec_indx  display_code: $display_code *********\n"
  if { !$display_code } {
   $HLIST selection set $ent
   $HLIST anchor set $ent
   set first_indx 9 ;# non-existing index
   set sec_indx   0
   UI_PB_prv_DisplayTclCode first_indx sec_indx page_obj
   } else {
   set code_indx $first_indx
   global machType
   if { [string match "Wire EDM" $machType] } {
    if { $code_indx > 3 } { incr code_indx }
   }
   UI_PB_prv_DisplayTclCode code_indx sec_indx page_obj
  }
  set ent 0.$first_indx.$sec_indx
  if { [$HLIST info exists $ent] } {
   $HLIST selection set $ent
   $HLIST anchor set $ent
  }
  set Page::($page_obj,prev_selection) $ent
  focus -force $Page::($page_obj,ltext)
 }

#=======================================================================
proc __prv_ApplyCycleParameters { seq_obj event_obj } {
  if [llength $event::($event_obj,evt_elem_list)] \
  {
   ...
   foreach elem_obj $event::($event_obj,evt_elem_list) \
   {
    ...
    set block::($blk_obj,blk_owner) "Cycle Parameters"
    ...
    UI_PB_debug_ForceMsg "\n****************** Block $blk_obj $block::($blk_obj,block_name) owner >$block::($blk_obj,blk_owner)< *******************\n"
    unset blk_obj_attr
...
proc UI_PB_prv_DisplayTclCode { FIRST_INDX SEC_INDX PAGE_OBJ } {
  ...
  switch $first_indx \
  {
   ...
   8  {
    ...
    if { [string match "Cycles" $sequence::($seq_obj,seq_name)] && \
     ...
     if { [info exists ::machData(0)] && $::machData(0) == "Mill" } {
      ...
      set event_obj [lindex $sequence::($seq_obj,evt_obj_list) $sec_indx]
     }
     ...
     set event_obj [lindex $sequence::($seq_obj,evt_obj_list) $sec_indx]
     if { [string match "User Defined Axis Limit Action" $event::($event_obj,event_name)] } {
      if { [info exists Post::($post_object,axis_limit_evt_obj)] } {
       set event_obj $Post::($post_object,axis_limit_evt_obj)
       set sequence::($seq_obj,evt_obj_list) \
       [lreplace $sequence::($seq_obj,evt_obj_list) $sec_indx $sec_indx $event_obj]
       } else {
       set Post::($post_object,axis_limit_evt_obj) $event_obj
      }
     }
    }
    set evt_blks [list ""]
    if { ![info exists event_obj] || $event_obj == "" } {
     set evt_blks [list ""]
     } else {
  ...
  if { ![info exists event_obj] || $event_obj == "" } {
   set event_output [list ""]
   set event_name_list [list]
   UI_PB_prv_DisplayEventProc page_obj event_output event_name_list
   UI_PB_debug_ForceMsg "********* NULL Event *******************"
   return
  }
  set ::gPB_active_event_obj $event_obj
  if { [string match "Cycles" $sequence::($seq_obj,seq_name)] } {
   if 0 {
    if { ![info exists ::gPB_cycle_parameters_populated] } {
     foreach evt_obj $sequence::($seq_obj,evt_obj_list) {
      if { [string match "Cycle Parameters" $event::($evt_obj,event_name)] } {
       __prv_ApplyCycleParameters $seq_obj $evt_obj
       UI_PB_debug_ForceMsg "\n****************** Cycle Parameters ownership populated *******************\n"
       break
      }
     }
     set ::gPB_cycle_parameters_populated 1
    }
   }
  }
...
proc UI_PB_prv_CreateDefListElements { DEF_PAGE_OBJ INDEX } {
  ...
  $tree configure -browsecmd "UI_PB_prv_DefinitionTreeSelect $def_page_obj"
...
#=======================================================================
proc UI_PB_prv_DefinitionTreeSelect { page_obj args } {
  set tree $Page::($page_obj,tree)
  set HLIST [$tree subwidget hlist]
  set ent [$HLIST info selection]
  set index [string range $ent 2 [string length $ent]]
...
proc UI_PB_prv_WordSep { PAGE_OBJ } {
  ...
  set ltext $Page::($page_obj,ltext)
  set rtext $Page::($page_obj,rtext)
  PB_output_GetWordSeperator $mom_sys_arr(Word_Separator) new_word_sep
  UI_PB_prv_OutputWordSep ltext new_word_sep
  set mom_var_arr(0) "Word_Separator"
  PB_int_RetDefMOMVarValues mom_var_arr mom_var_value
  PB_output_GetWordSeperator $mom_var_value(Word_Separator) word_sep
  ...
  $text_widget insert end  "         \{\n"
   if { [string match "$::gPB(comment_blk)*" $block_name] || [string match "$::gPB(tcl_line_blk)*" $block_name] } {
    ...
    if { [string match "$::gPB(comment_blk)*" $block_name] } {
     $text_widget insert end "              [lindex $blk_value 0]\n"
     } else {
     if 0 {
  ...
  switch -exact -- $Book::($book_obj,current_tab) \
  {
   0 {
    ...
    if 0 {
     ...
     if { $first_indx == 0 && $sec_indx == 0 } \
     {
      UI_PB_prv_DisplayTclCode first_indx sec_indx page_obj
     }
    }
    if { [info exists Page::($page_obj,prev_selection)] } {
     UI_PB_prv_TclTreeSelection $page_obj $Page::($page_obj,prev_selection)
     } else {
   ...
   1 {
    ...
    UI_PB_prv_DefinitionTreeSelect $page_obj
...
#=======================================================================
proc UI_PB_prv_UdeTreeSelect { page_obj args } {
  set tree $Page::($page_obj,tree)
  set HLIST [$tree subwidget hlist]
  set ent [$HLIST info selection]
  set index [string range $ent 2 [string length $ent]]
  ...
  UI_PB_prv_DisplayUdeCode first_indx sec_indx third_indx page_obj
  focus -force $Page::($page_obj,ltext)
 }
