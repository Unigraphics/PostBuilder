...
proc UI_PB_file_OpenPost_unx {} {
  ...
  } else \
  {
   ...
   set fbox [$win subwidget fsbox]
   set ftypes [list \{*.pui\}\ \{$gPB(open,file_type)\}]
...
#=======================================================================
proc UI_PB_GetPostVersion { FILE_NAME VERSION STD_PUI_FILE } {
...
proc UI_PB_file_EditPost { dlg_id file_name args } {
  ...
  UI_PB_GetPostVersion file_name version std_pui_file
  set ver_check [string compare $gPB(Postbuilder_PUI_Version) $version]
  if { $ver_check > 0 } \
  {
   if [catch { PB_OpenOldVersion $file_name $std_pui_file } result] \
   {
    global post_object
    if [PB_com_HasObjectExisted $post_object] {
    }
    tk_messageBox -parent $top_win -type ok -icon error \
    -title $gPB(msg,dialog,title) \
    -message "$result"
...
proc UI_PB_file_SavePostAs_unx { args } {
  ...
  } else \
  {
   ...
   set fbox [$win subwidget fsbox]
   UI_PB_com_PositionWindow $win
   set ftypes [list \{*.pui\}\ \{$gPB(save_as,file_type)\}]
   $fbox config -filetypes $ftypes
   [$fbox subwidget cancel] config -width 10 -bg $paOption(app_butt_bg) \
...
proc UI_PB_SavePbFiles_unx { win select_file_name } {
  ...
  if { $file_string != "" } \
  {
   set ans [tk_messageBox -parent $win -type yesno -icon warning \
   -title $gPB(msg,dialog,title) \
...
proc UI_PB_ActivateOpenFileOpts { } {
  ...
  $mb entryconfigure $gPB(menu_index,file,exit)    -state normal   
  catch { $mb entryconfigure $gPB(menu_index,file,history) -state disabled } 
...
proc UI_PB_NewPost { } {
  ...
  entry $fbu.ent -width 24 -relief sunken -textvariable gPB(mach_user_controller)
...
proc UI_PB_file_SelectFile_unx { file_type FILE_VAR flag args } {
  ...
  switch $ext {
   "pui" {
    ...
    set open_file_type $gPB(open,file_type)
   }
   "tcl" {
...
proc __file_SelectFileName { win FILE_VAR flag file_name args } {
  ...
  if { $flag == "open" } {
   ...
   if {$tcl_platform(platform) == "unix"} {
    ...
    [$file subwidget listbox] delete 0
    if [file exists $file_name] {
     set ans [tk_messageBox -parent $awin -type yesno -icon warning \
     -title $gPB(msg,dialog,title) \
...
proc CreateMachineTypeOptMenu { widget mach_type } {
  ...
  set opt_labels(Mill,3MT)   "3-Axis Mill-Turn (XZC)"
...
proc CreateControllerOptMenu { widget mach_type } {
  ...
  if { [string index $dir 0] != "/"} {
   if { ![catch {set opts [exec ls $dir] } ] } {
    if [llength $opts] {
     set file_list_found 1
    }
   }
   } else {
   if { ![catch {set opts [eval tixListDir $dir 0 1 0 0] } ] } {
    if [llength $opts] {
     set file_list_found 1
    }
   }
...
proc __NewPost {w} {
  ...
  switch $mach_cntl_type {
   ...
   "System" {
    switch $mach_type {
     "Mill" { set mach "mill" }
     "Lathe" { set mach "lathe" }
     "Wedm" { set mach "wedm" }
     "Punch" { set mach "punch" }
    }
  ...
  if 0 {{
   ...
   set pb_std_file [list [file nativename $env(PB_HOME)/pblib/$stand_file_name.pui]]
  }}
  set env(PB_TESTING) 1
  if { [info exists env(PB_TESTING)]  &&  $env(PB_TESTING) } \
  {
   UI_PB_file_GetPostFragList pb_std_file $mach_type $controller $axisoption $output_unit
  }
