...
proc UI_PB_file_EditPost_mod { dlg_id file_name args } {
  ...
  if { $ver_check > 0 } \
  {
   if { $alter_unit_sub_post } {
    ...
    { ;# Not found the specified family templete in current PB.
     if { $legacy_controller_type != "" } \
     { ;# The legacy post file contained controller family inforamtion,
  ...
  if { ![info exists gPB(auto_qc)] || $gPB(auto_qc) != 1 } {
   if { [info exists ::disable_license] && $::disable_license == 0 } {
    if { [info exists ::LicInfo(user_right_limit)] } {
     if { $::LicInfo(user_right_limit) == "YES" } {
      ...
      if { [info exists ::LicInfo(is_encrypted_file)] && $::LicInfo(is_encrypted_file) == 1 } {
       ...
       if { $gPB(PB_LICENSE) == "UG_POST_AUTHOR" && $::LicInfo(SITE_ID_IS_OK_FOR_PT) == 0 } {
        if { [lsearch $gPB(lic_list) $::LicInfo(post_license)] >= 0 } {
         $gPB(file_menu) entryconfigure $gPB(menu_index,file,save_as) -state disabled
         tk_messageBox -message $gPB(msg,limit_to_change_license) -icon info \
...
proc __file_ShowBalloon { ent args } {
  ...
  {
   __file_GetCurrentSysControllerFullName $file_name full_name
...
#=======================================================================
proc __file_GetCurrentSysControllerFullName { file_name FULL_NAME args } {
  ...
  if [string match "* - *" $file_name] \
  {
   ...
   set full_name [file join $sys_dir $temp_name.pui]
  } else \
  {
...
proc __NewPost_mod {w} {
  ...
  switch $mach_cntl_type {
   ...
   "System" {
    if { $gPB(mach_sys_controller) == "" } {
     ...
     __file_GetCurrentSysControllerFullName $gPB(mach_sys_controller) controller
    }
   }
  }
  ...
  {
   __file_GetCurrentSysControllerFullName $gPB(mach_sys_controller) templete_file
   set templete_dir [file dirname $templete_file]
   if [__ParseMacroInfoFile $templete_dir] \
   {
  ...
  if { $controller != "Generic"  &&  $controller != "" } {
   ...
   if [string match "__NewPost_mod *" [info level -1]] \
   {
    ...
    if [string match "User" $mach_cntl_type] \
    {
     ...
     {
      __file_GetCurrentSysControllerFullName $gPB(mach_sys_controller) templete_file $mach_type
...
proc __CntlFamilySelection { file_name args } {
  ...
  __file_GetCurrentSysControllerFullName $gPB(mach_sys_controller) full_name
...
proc UI_PB_file_CheckMachResolution { { action "Continue" } } {
  ...
  if { $add_x == -1 || $add_y == -1 || $add_z == -1 } { return 1 }
  set msg ""
  set dismatch_flag 0
  set linear_prec [__GetDecimalCount $mom_kin_var(\$mom_kin_machine_resolution)]
  if { [__CheckAddressFormat $add_x $linear_prec] } {
   set dismatch_flag 1
   append msg "$gPB(address,tab,Label) X "
  }
  if { [__CheckAddressFormat $add_y $linear_prec] } {
   if $dismatch_flag {
    append msg "& Y "
    } else {
  ...
  if { [__CheckAddressFormat $add_z $linear_prec] } {
   if $dismatch_flag {
    append msg "& Z "
    } else {
  ...
  if [info exists mom_kin_var(\$mom_kin_4th_axis_min_incr)] {
   set axis_4th_prec [__GetDecimalCount $mom_kin_var(\$mom_kin_4th_axis_min_incr)]
   set add_4axis [PB_com_FindObjFrmName "fourth_axis" "address"]
   if { [__CheckAddressFormat $add_4axis $axis_4th_prec] } {
    set dismatch_flag 1
    append msg "$gPB(address,tab,Label) fourth_axis : $gPB(msg,check_resolution,rotary) \n\n"
   }
  }
  if [info exists mom_kin_var(\$mom_kin_5th_axis_min_incr)] {
   set axis_5th_prec [__GetDecimalCount $mom_kin_var(\$mom_kin_5th_axis_min_incr)]
   set add_5axis [PB_com_FindObjFrmName "fifth_axis" "address"]
   if { [__CheckAddressFormat $add_5axis $axis_5th_prec] } {
    set dismatch_flag 1
  ...
  if $dismatch_flag {
   ...
   if [string match "Pend" $action] {
    ...
    UI_PB_com_CreateTransientWindow $w "$gPB(msg,check_resolution,title)" "500x300+400+400" "" "" "" ""
    ...
    wm protocol $w WM_DELETE_WINDOW {set a 1}
