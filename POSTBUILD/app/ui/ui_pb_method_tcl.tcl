...
proc UI_PB_mthd_CreateLblEntry { data_type var inp_frm ext label args } {
  ...
  set has_chelp 0
  if { [llength $args] > 1 &&  [lindex $args 1] != "" && [lindex $args 1] != "\{\}"} {
   set has_chelp 1
   set csh [lindex $args 1]
  }
  if { [llength $args] > 2 &&  [lindex $args 2] == 1 && $data_type == "a" } {
   set gPB(tool_path_event,$var) 1
  }
  if { $has_chelp } {
  ...
  if { [info exists ::env(INCH_METRIC)] && $::env(INCH_METRIC) == 1 } {
   ...
   } elseif { [info exists ::env(SUB_POST_MODE)] && $::env(SUB_POST_MODE) == 1 } \
  {
   global mom_kin_var_main
   global mom_sys_arr_main
   set gb_arr_main "${gb_arr}_main"
   if { [info exists ::env(UNIT_SUB_POST_MODE)] && $::env(UNIT_SUB_POST_MODE) == 1 } {
    ...
    if { $unit_flag == 1 } {
     if { ![string compare $gPB(main_post_unit) "Millimeters"] } {
      set unit_flag "$gPB(sub_post,inch,Lable)"
      set main_unit_flag "$gPB(sub_post,metric,Lable)"
      } elseif { ![string compare $gPB(main_post_unit) "Inches"] } {
      set unit_flag "$gPB(sub_post,metric,Lable)"
      set main_unit_flag "$gPB(sub_post,inch,Lable)"
     }
     label $inp_frm.unit_lbl -text $unit_flag -width 2 \
     -font $::tixOption(fixed_font) \
     -fg $::paOption(unit_fg)
     label $inp_frm.main_unit_lbl -text $main_unit_flag -width 2 \
     -font $::tixOption(fixed_font) \
     -fg $::paOption(unit_fg)
     if { [string match "null" $var] || [string match "" $var] } {
      set ent_main [entry $inp_frm.1_main_$ext -width 10 -relief sunken -bd 2 -state disabled]
      } else {
      set ent_main [entry $inp_frm.1_main_$ext -width 10 -relief sunken -bd 2 -state disabled \
      -textvariable ${gb_arr_main}($var)]
     }
     if 0 {
      grid $inp_frm.$ext          -row 0 -column 0 -padx {0 35} -pady {6 0} -sticky w
      grid $inp_frm.1_$ext        -row 0 -column 1 -padx 0 -pady {6 0} -sticky ew
      grid $inp_frm.unit_lbl      -row 0 -column 2 -padx {15 5} -pady {6 0} -sticky e
      grid $inp_frm.1_main_$ext   -row 1 -column 1 -padx 0 -pady 6 -sticky ew
      grid $inp_frm.main_unit_lbl -row 1 -column 2 -padx {15 5} -pady 6 -sticky e
      } else {
      tixForm $inp_frm.$ext -top %12 -left %2 -right %50 -bottom %44
      tixForm $inp_frm.1_$ext -top %12 -left %50 -right %80 -bottom %44
      tixForm $inp_frm.unit_lbl -top %12 -left %80 -right %98 -bottom %44
      tixForm $inp_frm.1_main_$ext -top %56 -left %50 -right %80 -bottom %88
      tixForm $inp_frm.main_unit_lbl -top %56 -left %80 -right %98 -bottom %88
     }
     } else {
     if 0 {
      pack $inp_frm.$ext -side left -padx {0 40} -pady 6
      pack $inp_frm.1_$ext -side right -padx {0 35} -pady 6
      } else {
      tixForm $inp_frm.$ext   -top %25 -left %0  -right %50 -bottom %100
      tixForm $inp_frm.1_$ext -top %25 -left %50 -right %80 -bottom %100
     }
    }
   }
   } else {
  ...
  UI_PB_mthd_GetAddName $var Any add_name
  if { [string compare "" $add_name] } {
   bind $inp_frm.1_$ext <KeyRelease> "+UI_PB_com_FormatCode %W $add_name $gb_arr \\$var 1"
   UI_PB_com_FormatCode $inp_frm.1_$ext $add_name $gb_arr $var 1
  }
 }

#=======================================================================
proc UI_PB_mthd_GetAddNameOnTpthEvenPage { sys_var ADD_NAME } {
  upvar $ADD_NAME add_name
  switch -exact -- $sys_var \
  {
   ...
   "\$mom_sys_feed_rate_mode_code(DPM)" -
...
proc UI_PB_mthd_GetAddName { sys_var code_type ADD_NAME} {
  ...
  if { [string match "G" $code_type] } {
   ...
   set g_flag 0
   } elseif { [string match "Any" $code_type] } {
   set g_flag 2
   set add_name ""
  }
  if { $g_flag == 1 } {
   ...
   if { [info exists gPB(addr_expr,G_return)] && [lsearch -exact $gPB(addr_expr,G_return) $sys_var] >= 0 } {
    set add_name "G_return"
    } elseif { [info exists gPB(addr_expr,G_motion)] && [lsearch -exact $gPB(addr_expr,G_motion) $sys_var] >= 0 } {
    set add_name "G_motion"
    } elseif { [info exists gPB(addr_expr,G_plane)] && [lsearch -exact $gPB(addr_expr,G_plane) $sys_var] >= 0 } {
    set add_name "G_plane"
    } elseif { [info exists gPB(addr_expr,G_cutcom)] && [lsearch -exact $gPB(addr_expr,G_cutcom) $sys_var] >= 0 } {
    set add_name "G_cutcom"
    } elseif { [info exists gPB(addr_expr,G_adjust)] && [lsearch -exact $gPB(addr_expr,G_adjust) $sys_var] >= 0 } {
    set add_name "G_adjust"
    } elseif { [info exists gPB(addr_expr,G_feed)] && [lsearch -exact $gPB(addr_expr,G_feed) $sys_var] >= 0 } {
    set add_name "G_feed"
    } elseif { [info exists gPB(addr_expr,G_spin)] && [lsearch -exact $gPB(addr_expr,G_spin) $sys_var] >= 0 } {
    set add_name "G_spin"
    } elseif { [info exists gPB(addr_expr,G_mode)] && [lsearch -exact $gPB(addr_expr,G_mode) $sys_var] >= 0 } {
    set add_name "G_mode"
    } elseif { [info exists gPB(addr_expr,G)] && [lsearch -exact $gPB(addr_expr,G) $sys_var] >= 0 } {
    set add_name "G"
    } elseif { [info exists gPB(addr_expr,M_range)] && [lsearch -exact $gPB(addr_expr,M_range) $sys_var] >= 0 } {
    set add_name "M_range"
    } elseif { [info exists gPB(addr_expr,M_spindle)] && [lsearch -exact $gPB(addr_expr,M_spindle) $sys_var] >= 0 } {
    set add_name "M_spindle"
    } elseif { [info exists gPB(addr_expr,M_coolant)] && [lsearch -exact $gPB(addr_expr,M_coolant) $sys_var] >= 0 } {
    set add_name "M_coolant"
    } elseif { [info exists gPB(addr_expr,M)] && [lsearch -exact $gPB(addr_expr,M) $sys_var] >= 0 } {
    set add_name "M"
   }
  }
 }
...
proc UI_PB_mthd_CreateLblCodeEntry { data_type var inp_frm ext label args } {
  ...
  if [catch { set text [eval format "%s" $label] }] {
   set text ""
  }
...
proc UI_PB_mthd_CreatePane { this } {
  ...
  if { ![winfo exists $page_id.pane] } {
   set pane [tixPanedWindow $page_id.pane -orient horizontal -handlebg orange]
   } else {
   set pane $page_id.pane
  }
  pack $pane -expand yes -fill both
  set plist [$pane panes]
  if { [llength $plist] == 0 || [lsearch $plist p1] < 0 } {
   set f1 [$pane add p1 -expand 0 -size 350]
   } else {
   set f1 [$pane subwidget p1]
  }
  if { [lsearch $plist p2] < 0 } {
   set f2 [$pane add p2 -expand 1 -size 675]
   } else {
   set f2 [$pane subwidget p2]
  }
...
proc UI_PB_mthd_CreateMenu { this args } {
  ...
  menubutton $evt_frm.but -bitmap $cbx -width 15 -height 18 -bd 1 \
  -relief raised -menu $evt_frm.but.menu
  pack $evt_frm.but -side left -padx 2
...
proc UI_PB_mthd_CreateAddTrashinCanvas {this} {
  ...
  set add_comp_gray [image create compound -window $top_canvas \
  -bd 1 \
  -background #c0c0ff \
  -borderwidth 2 \
  -relief raised \
  -showbackground 1]
  $add_comp_gray add text -text $Page::($this,add_name) \
  -font $tixOption(bold_font) -justify center \
  -fg $paOption(disabled_fg)
  $add_comp_gray config -bg $paOption(app_butt_bg)  ;# -bg cannot be specified at creation!
  set dy [expr [expr $panel_hi / 2] + 2]
  if { [expr $::tcl_version > 8.0] } {
   $top_canvas create image 80 $dy -image $add_comp \
   -tag add_movable -disabledimage $add_comp_gray
   } else {
   $top_canvas create image 80 $dy -image $add_comp \
   -tag add_movable
  }
...
proc UI_PB_mthd_MouseWheel {wFired D X Y {dir y}} {
   ...
   if {[winfo exists $w]} {
    if { [string match [winfo class $w] "Scrollbar"] } { ;#<05-07-09 gsl> "string match" was "eq".
...
proc UI_PB_mthd_WheelForFrame {w} {
  ...
  foreach child $cl {
   set type [winfo class $child]
   if { ![string match $type "Entry"] && \
    ![string match $type "Button"] && \
...
proc UI_PB_mthd_CreateDDBlock {x y typeflag t_shift icon_name elem_text {ops_nows_force NULL}} {
   ...
   switch -exact -- $typeflag {
    ...
    eventblock_up {
     ...
     -text $elem_text -font $font
     set elem_obj $ops_nows_force
     if { $elem_obj > 0 } {
      if { $event_element::($elem_obj,exec_condition_obj) > 0 } {
       set cond_color $gPB(elem_condition_color)
       .ddBlock.c create rect 0 0 2 10 -fill $cond_color -outline $cond_color
      }
      if { $event_element::($elem_obj,suppress_flag) > 0 } {
       set supp_color $gPB(elem_suppress_color)
       .ddBlock.c create rect 0 10 2 21 -fill $supp_color -outline $supp_color
      }
     }
    }
    addressblock_up {
    ...
    eventblock_whole {
     set txt_width [font measure $font $elem_text]
     set w [expr $txt_width + 16]
     set cen_x [expr int($w/2) + 4]
     set h 25
     toplevel .ddBlock -bg $paOption(sunken_bg) -borderwidth 2 \
     -relief flat -highlightt 0
     wm overrideredirect .ddBlock 1
     wm geom .ddBlock ${w}x${h}+${x}+${y}
     canvas .ddBlock.c -bg $paOption(sunken_bg) -relief sunken \
     -bd 0 -highlightt 0
     pack .ddBlock.c -fill both -expand yes -side top
     .ddBlock.c create text $cen_x 10 \
     -text $elem_text -font $font
    }
   }
  }
...
proc UI_PB_mthd_MoveDDBlock {x y typeflag canvas_x canvas_y canvas_height canvas_width} {
  ...
  switch  -exact -- $typeflag {
   ...
   eventblock_whole {
    set ww [winfo width .ddBlock]
    set wh [winfo height .ddBlock]
    set rm [expr {$canvas_x + $canvas_width - $ww}]
    set lm $canvas_x
    set tm $canvas_y
    set bm [expr {$canvas_y + $canvas_height - $wh}]
    if { $x >= $lm && $x <= $rm && $y >= $tm && $y <= $bm } {
     wm geom .ddBlock +${x}+${y}
    }
    if { $x >= $lm && $x <= $rm && $y >= $tm && $y > $bm } {
     wm geom .ddBlock +${x}+${bm}
    }
    if { $x >= $lm && $x <= $rm && $y < $tm && $y <= $bm } {
     wm geom .ddBlock +${x}+${tm}
    }
    if { $x >= $lm && $x > $rm && $y >= $tm && $y <= $bm } {
     wm geom .ddBlock +${rm}+${y}
    }
    if { $x < $lm && $x <= $rm && $y >= $tm && $y <= $bm } {
     wm geom .ddBlock +${lm}+${y}
    }
   }
  }
 }
...
#=======================================================================
proc UI_PB_mthd_DisplayErrMsg { err_msg } {
  global gPB
  if 1 {
   set debug_state $gPB(SUPPRESS_ALL_DEBUG)
   set gPB(SUPPRESS_ALL_DEBUG) 0
   UI_PB_debug_Pause $err_msg 0
   set gPB(SUPPRESS_ALL_DEBUG) $debug_state
   return
  }
  set pname "ErrorMessage"
  set plname "Error Message"
  set new_msg_page [new Page $pname $plname]
  set main [UI_PB_com_AskActiveWindow]
  set win [toplevel $main.errmsg]
  set toplevel_index [llength $gPB(toplevel_list)]
  set win_index [expr $toplevel_index + 1]
  set gPB(toplevel_disable_$win_index) 1
  UI_PB_com_CreateTransientWindow $win  "$gPB(format,error,title)"  "500x300+200+200"  ""  ""  ""  ""
  bind $win <Destroy> "+ PB_com_DeleteObject $new_msg_page"
  __CreateErrMsgPage $new_msg_page $win $err_msg
  UI_PB_com_PositionWindow $win
 }

#=======================================================================
proc __CreateErrMsgPage { new_msg_page win err_msg } {
  global gPB
  set canvas_frame $win
  set top [frame $canvas_frame.top]
  set mid [frame $canvas_frame.mid]
  set bot [frame $canvas_frame.bottom]
  pack $bot -side bottom -fill both
  pack $top -side top -fill both
  pack $mid -side top -fill both -expand 1
  set wrapwidth [winfo screenwidth $win]
  set wrapwidth [expr {$wrapwidth-60-[winfo pixels $win 9m]}]
  label $top.msg -justify left -text "$gPB(format,check_1,error,msg) !" \
  -wraplength $wrapwidth -font $gPB(bold_font)
  canvas $top.bitmap -width 32 -height 32 -highlightthickness 0
  $top.bitmap create oval 0 0 31 31 -fill red -outline black
  $top.bitmap create line 9 9 23 23 -fill white -width 4
  $top.bitmap create line 9 23 23 9 -fill white -width 4
  grid $top.bitmap $top.msg \
  -in $top         \
  -row 0           \
  -padx 3m         \
  -pady 3m
  grid configure     $top.msg -sticky nsw -padx 3
  grid rowconfigure  $top 1 -weight 1
  grid columnconfigure $top 1 -weight 1
  set inf [frame $mid.info]
  text $inf.text             \
  -bd 2                 \
  -state normal         \
  -relief sunken        \
  ...
  -wrap char            \
  -yscrollcommand [list $inf.scroll set]
  scrollbar $inf.scroll -relief sunken -command [list $inf.text yview]
  pack $inf.scroll -side right -fill y
  pack $inf.text -side left -expand yes -fill both
  $inf.text insert 0.0 "$err_msg"
  $inf.text mark set insert 0.0
  bind $inf.text <ButtonPress-1> { focus %W }
  $inf.text configure -state disabled
  pack $inf -fill both -expand 1 -padx 8 -pady 8
  button $bot.ok \
  -text "$gPB(nav_button,ok,Label)" \
  -default normal \
  -command "destroy $win"
  grid $bot.ok    \
  -in $bot \
  ...
  button $bot.function    \
  -text "$gPB(save_as,title,Label)" \
  -default normal \
  -command "__SaveErrMsgPageToFile $inf.text"
  grid $bot.function \
  -in $bot \
  -column 1 \
  -row 0 \
...
#=======================================================================
proc __SaveErrMsgPageToFile { text_w } {
  if { ![string compare $::tcl_platform(platform) "windows"] } {
   set allFiles *.*
   } else {
   set allFiles *
  }
  set err_text [$text_w get 0.0 end-1c]
  set types [list                           \
  [list "Log Files" .log]      \
  [list "Text Files" .txt]     \
  ...
  -filetypes $types -defaultextension .log -parent [UI_PB_com_AskActiveWindow]]
  if {![string length $filename]} {
   return
  }
  set f [open $filename w]
  puts -nonewline $f $err_text
  close $f
 }
