...
#<Feb-26-2018 gsl> Overload mcload
if { $::tcl_version == 8.6 } {

   proc ::msgcat::pb_mcload { langdir } {
      set x 0
      foreach p [mcpreferences] {
         set langfile [file join $langdir $p.tcl]
   ...
   msgcat::pb_mcload $env(PB_HOME)/msgs
...
set gPB(event,misc,pattern_csys_start,name)    [::msgcat::mc MC(event,misc,pattern_csys_start,name)]
if { [string match "MC(event,misc,pattern_csys_start,name)" $gPB(event,misc,pattern_csys_start,name)] } {
   set gPB(event,misc,pattern_csys_start,name) "Pattern Set Coordinate System Start"
}
set gPB(event,misc,pattern_csys_end,name)      [::msgcat::mc MC(event,misc,pattern_csys_end,name)]
if { [string match "MC(event,misc,pattern_csys_end,name)" $gPB(event,misc,pattern_csys_end,name)] } {
   set gPB(event,misc,pattern_csys_end,name)   "Pattern Set Coordinate System End"
}

# Toggle to enable subprogram output
set gPB(listing,subprog_out,check,Label)       [::msgcat::mc MC(listing,subprog_out,check,Label)]
if { [string match "MC(listing,subprog_out,check,Label)" $gPB(listing,subprog_out,check,Label)] } {
   set gPB(listing,subprog_out,check,Label)    "Enable Subprogram Output"
}
set gPB(listing,subprog_out,check,Context)     [::msgcat::mc MC(listing,subprog_out,check,Context)]
if { [string match "MC(listing,subprog_out,check,Context)" $gPB(listing,subprog_out,check,Context)] } {
   set gPB(listing,subprog_out,check,Context)  "This toggle will notify NX/Post core processor that this post\
                                                is equipped with the ability to produce subprogram output."
}

# Address leader assignment error
set gPB(address,leader,err_msg)                [::msgcat::mc MC(address,leader,err_msg)]
if { [string match "MC(address,leader,err_msg)" $gPB(address,leader,err_msg)] } {
   set gPB(address,leader,err_msg)             "Leader of Address X, Y, Z, fourth_axis, fifth_axis and N\
                                                should not be configured with expression containing variables.\
                                                \n\nThis checking can be disabled by setting gPB(ALLOW_VAR_SYS_LEADER)\
                                                to \"1\" in your ui_pb_user_resource.tcl file."
}
...
set gPB(event,misc,before_contour_end,name)    [::msgcat::mc MC(event,misc,before_contour_end,name)]
if { [string match "MC(event,misc,before_contour_end,name)" $gPB(event,misc,before_contour_end,name)] } {
   set gPB(event,misc,before_contour_end,name) "Before Contour End"
}
...
# pb1847
#=============================================================================
# Toggle to post/output transition path
set gPB(listing,tran_path,check,Label)         [::msgcat::mc MC(listing,tran_path,check,Label)]
if { [string match "MC(listing,tran_path,check,Label)" $gPB(listing,tran_path,check,Label)] } {
   set gPB(listing,tran_path,check,Label)      "Output Transition Path"
}
set gPB(listing,tran_path,check,Context)       [::msgcat::mc MC(listing,tran_path,check,Context)]
if { [string match "MC(listing,tran_path,check,Context)" $gPB(listing,tran_path,check,Context)] } {
   set gPB(listing,tran_path,check,Context)    "This toggle will enable NX/Post to process and output\
                                                the events resulted from the transition path between operations."
}
...
# pb12.02
#=============================================================================
set gPB(block,def_popup,Label)         [::msgcat::mc MC(block,def_popup,Label)]
if { [string match "MC(block,def_popup,Label)" $gPB(block,def_popup,Label)] } {
   set gPB(block,def_popup,Label)      "Definition"
}

set gPB(msg,info,obj_undef)            [::msgcat::mc MC(msg,info,obj_undef)]
if { [string match "MC(msg,info,obj_undef)" $gPB(msg,info,obj_undef)] } {
   set gPB(msg,info,obj_undef)         "Information about this object not yet configured."
}

set gPB(msg,info,vnc_cmd_by_pb)        [::msgcat::mc MC(msg,info,vnc_cmd_by_pb)]
if { [string match "MC(msg,info,vnc_cmd_by_pb)" $gPB(msg,info,vnc_cmd_by_pb)] } {
   set gPB(msg,info,vnc_cmd_by_pb)     "VNC command generated by Post Builder\
                                        to convey the settings of a post to its VNC driver."
}

set gPB(msg,info,vnc_util)             [::msgcat::mc MC(msg,info,vnc_util)]
if { [string match "MC(msg,info,vnc_util)" $gPB(msg,info,vnc_util)] } {
   set gPB(msg,info,vnc_util)          "VNC utility commands defined in PB_CMD_vnc__define_misc_procs."
}

set gPB(msg,info,sim_cmd)              [::msgcat::mc MC(msg,info,sim_cmd)]
if { [string match "MC(msg,info,sim_cmd)" $gPB(msg,info,sim_cmd)] } {
   set gPB(msg,info,sim_cmd)           "SIM Commands to be used in a simulation driver."
}

set gPB(msg,info,pb_vnc_cmd)           [::msgcat::mc MC(msg,info,pb_vnc_cmd)]
if { [string match "MC(msg,info,pb_vnc_cmd)" $gPB(msg,info,pb_vnc_cmd)] } {
   set gPB(msg,info,pb_vnc_cmd)        "Commands generated by Post Builder to interface\
                                        key post events with a VNC driver."
}

set gPB(msg,info,vnc_broker_cmd)       [::msgcat::mc MC(msg,info,vnc_broker_cmd)]
if { [string match "MC(msg,info,vnc_broker_cmd)" $gPB(msg,info,vnc_broker_cmd)] } {
   set gPB(msg,info,vnc_broker_cmd)    "Broker command used in a VNC driver to execute other commands."
}

set gPB(msg,event,delete_confirm)      [::msgcat::mc MC(msg,event,delete_confirm)]
if { [string match "MC(msg,event,delete_confirm)" $gPB(msg,event,delete_confirm)] } {
   set gPB(msg,event,delete_confirm)   "Do you really want to delete event"
}
...
# pb12.01
#=============================================================================
set gPB(event,misc,user_limit_action,name)        [::msgcat::mc MC(event,misc,user_limit_action,name)]
if { [string match "MC(event,misc,user_limit_action,name)" $gPB(event,misc,user_limit_action,name)] } {
  # set gPB(event,misc,user_limit_action,name)     "User Defined Axis Limit Action"
   set gPB(event,misc,user_limit_action,name)     [::msgcat::mc MC(machine,axis,violation,user,evt_title)]
}

set gPB(machine,axis,violation,omit,Label)        [::msgcat::mc MC(machine,axis,violation,omit,Label)]
if { [string match "MC(machine,axis,violation,omit,Label)" $gPB(machine,axis,violation,omit,Label)] } {
  # set gPB(machine,axis,violation,omit,Label)     "Omit Output"
   set gPB(machine,axis,violation,omit,Label)     [::msgcat::mc MC(machine,resolution,abort,Label)] ;# Abort
}

set gPB(func,edit_note,Label)        [::msgcat::mc MC(func,edit_note,Label)]
if { [string match "MC(func,edit_note,Label)" $gPB(func,edit_note,Label)] } {
  # set gPB(func,edit_note,Label)     "Edit Notes"
   set gPB(func,edit_note,Label)     [::msgcat::mc MC(func,help,Label)] ;# "Information"
}
...
# pb12.00
#=============================================================================
set gPB(block,info_popup,Label)        [::msgcat::mc MC(block,info_popup,Label)]
if { [string match "MC(block,info_popup,Label)" $gPB(block,info_popup,Label)] } {
   set gPB(block,info_popup,Label)     "Info"
}
...
# pb11.02
#=============================================================================
set gPB(event,misc,region_start_marker,name)      [::msgcat::mc MC(event,misc,region_start_marker,name)]
set gPB(event,misc,region_end_marker,name)        [::msgcat::mc MC(event,misc,region_end_marker,name)]
if [string match "MC(event,misc,region_start_marker,name)" $gPB(event,misc,region_start_marker,name)] {
   set gPB(event,misc,region_start_marker,name)   "Start of Region Marker"
   set gPB(event,misc,region_end_marker,name)     "End of Region Marker"
}
...
#<Aug-25-2016 gsl> Where to add Condition labels???
# ::msgcat::mcset $gPB(LANG) "MC(seq,combo,custom,Label)"             "Custom Command" ;# Sequence page combo option
# ::msgcat::mcset $gPB(LANG) "MC(seq,cus_trans,title,Label)"          "Custom Command" ;# Window title of command editor off sequence
# ::msgcat::mcset $gPB(LANG) "MC(tool,cus_trans,title,Label)"         "Custom Command" ;# Window title of command editor off toolpath events
