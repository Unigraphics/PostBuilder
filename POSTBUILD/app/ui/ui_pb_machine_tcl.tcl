...
set title [label $top_frm.title -text "Post Information" -font $tixOption(bold_font) \
-bg $paOption(title_bg) -fg $paOption(title_fg) -bd 2 -relief raised -anchor c]
pack $title -side top -fill x
UI_PB_mthd_CreateScrollWindow $top_frm body info_win
...
set desc_lbl [label $info_win.desc_lbl -text "Description" \
...
grid $oth -sticky ew
label $oth.mach_lbl -text "Machine Type" -bg $lbl_bg \
-font $tixOption(bold_font) -justify left
...
label $oth.unit_lbl -text "Output Unit" -bg $lbl_bg \
-font $tixOption(bold_font) -justify left
...
label $oth.hist_lbl -text "History" -bg $lbl_bg \
...
grid $oth.mach_lbl $oth.mach_ent -padx 20 -pady 5 -sticky ew
grid $oth.kine_lbl $oth.kine_ent -padx 20 -pady 5 -sticky ew
grid $oth.unit_lbl $oth.unit_ent -padx 20 -pady 5 -sticky ew
grid $oth.cntl_lbl $oth.cntl_ent -padx 20 -pady 5 -sticky ew
grid [label $oth.yyy -text ""]
grid $oth.hist_lbl -
...
pack $fc.yes -side left  -padx 15 -expand yes
pack $fc.no  -side right -padx 15 -expand yes
...
grid $top_frm.mul -padx 5 -pady $dy -sticky news
...
Page::CreateLblFrame $top_frm tilt  "Wire Tilt Control"
set tilt [$top_frm.tilt subwidget frame]
radiobutton $tilt.ang -text "Angles" -anchor w \
-variable mom_kin_var(\$mom_kin_wire_tilt_output_type) -value ANGLES
radiobutton $tilt.coo -text "Coordinates" -anchor w \
...
set pf [frame $top_frm.pf -bg $paOption(header_bg) -relief solid -bd 1]
label $pf.postunit -text "$gPB(machine,gen,out_unit,Label) $output_unit" \
-font $tixOption(bold_font) -fg $paOption(special_fg) \
...
-text "This Rotary Axis Can Be Incremental" \
...
grid $ftol -sticky ew
set positive 1
Page::CreateLblEntry f \$mom_kin_linearization_tol $ftol par \
...
grid $fm - -pady 10
set positive 1
Page::CreateLblEntry f \$mom_kin_linearization_tol $fm par \
"$gPB(machine,axis,rotary_lintol,Label)" "$positive" \
"machine,axis,rotary_lintol"
...
-font $tixOption(bold_font) -fg $paOption(special_fg) -bg $paOption(special_bg)
...
pack $f.lbl -side left  -padx 2 -pady 3
pack $f.ent -side right -padx 2 -pady 4
grid $frame_id.type                 -sticky w  -padx 5
grid $frame_id.plane      -         -sticky ew -padx 5
...
proc x_UI_PB_mach_ConfigRotaryAxisNavButtons { win frame } {
global paOption
...
{
"3" {
set ImageWindowText [list 3-Axis $machType]
append imagefile $directory pb_m3x_v.gif
}
"4T" -
"4H" {
if { [ValidateMachObjAttr 4] == "TCL_ERROR" } { return }
set ImageWindowText [list 4-Axis $machType]
...
set ImageWindowText [list 5-Axis $machType]
...
{
"2" {
set ImageWindowText [list 2-Axis $machType]
