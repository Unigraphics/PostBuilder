...
proc UI_PB_mach_SetMachKinvars { MACH_PAGE_OBJ } {
  ...
  if { [string match "XZC" $machData(1)] } {
   ...
   if { ![string compare "4-Axis" $machData(1)] } {
    lappend mill_4th_param {"\$mom_kin_4th_axis_plane" \
     "\$mom_kin_4th_axis_leader" \
     "\$mom_kin_linearization_flag" \
     "\$mom_kin_linearization_tol" \
...
proc UI_PB_mach_DisplayPostInfo { PAGE_OBJ win } {
  ...
  if { $__ShowLicInfo } {
   ...
   set POST_LICE_BIT 0
   set POST_EXPR_BIT 0
   if { [info exists ::LicInfo(post_author_id)] && $::LicInfo(post_author_id) != "" } {
    set POST_AUTH_BIT 1
   }
   if { [info exists ::LicInfo(post_site_id)] && $::LicInfo(post_site_id) != "" } {
    set POST_SITE_BIT 1
   }
   if { [info exists ::LicInfo(post_license)] && $::LicInfo(post_license) != "" } {
    set POST_LICE_BIT 1
   }
   if { [info exists ::LicInfo(post_expiration)] && $::LicInfo(post_expiration) != "" } {
    set POST_EXPR_BIT 1
   }
   if { $POST_AUTH_BIT } {
    label $oth.auth_lbl -text "Author ID" -bg $lbl_bg \
    -font $tixOption(bold_font) -justify left -anchor w
    entry $oth.auth_ent -width 60 -relief $ent_relief
    $oth.auth_ent insert 0 $::LicInfo(post_author_id)
    $oth.auth_ent config -bg $ent_bg -state disabled
   }
   if { $POST_SITE_BIT } {
    label $oth.site_lbl -text "Site ID" -bg $lbl_bg \
    -font $tixOption(bold_font) -justify left -anchor w
    entry $oth.site_ent -width 60 -relief $ent_relief
    $oth.site_ent insert 0 $::LicInfo(post_site_id)
    $oth.site_ent config -bg $ent_bg -state disabled
   }
   if { $POST_LICE_BIT } {
    ...
    label $oth.lice_lbl -text "License Key" -bg $lbl_bg \
    -font $tixOption(bold_font) -justify left -anchor w
    entry $oth.lice_ent -width 60 -relief $ent_relief
    $oth.lice_ent insert 0 $license
    $oth.lice_ent config -bg $ent_bg -state disabled
   }
   if { $POST_EXPR_BIT } {
    label $oth.expr_lbl -text "Expiration Date" -bg $lbl_bg \
    -font $tixOption(bold_font) -justify left -anchor w
    entry $oth.expr_ent -width 60 -relief $ent_relief
    $oth.expr_ent insert 0 $::LicInfo(post_expiration)
    $oth.expr_ent config -bg $ent_bg -state disabled
   }
  }
  ...
  if { $__ShowLicInfo } {
   grid [UI_PB_com_AddSeparator $oth.sep] - -padx 20 -pady 4 -sticky ew
   if { $POST_AUTH_BIT } {
    grid $oth.auth_lbl $oth.auth_ent -padx 20 -pady 4 -sticky ew
   }
   if { $POST_SITE_BIT } {
    grid $oth.site_lbl $oth.site_ent -padx 20 -pady 4 -sticky ew
   }
   if { $POST_LICE_BIT } {
    grid $oth.lice_lbl $oth.lice_ent -padx 20 -pady 4 -sticky ew
   }
   if { $POST_EXPR_BIT } {
    grid $oth.expr_lbl $oth.expr_ent -padx 20 -pady 4 -sticky ew
   }
  }
  if { [info exists ::env(SUB_POST_MODE)] && $::env(SUB_POST_MODE) == 1 } {
   if { [info exists ::env(UNIT_SUB_POST_MODE)] && $::env(UNIT_SUB_POST_MODE) == 1 } {
...
proc UI_PB_mach_DefineAxisLimitAction { parent_w } {
  ...
  if { ![info exists Post::($post_object,axis_limit_evt_obj)] } \
  {
   ...
   set event::($limit_evt_obj,event_name)  "User Defined Axis Limit Action"
   set event::($limit_evt_obj,event_label) {$gPB(event,misc,user_limit_action,name)}
  ...
  } else \
  {
   ...
   UI_PB_debug_ForceMsg "\nEEEEE+++++ Event >$limit_evt_obj< >$event::($limit_evt_obj,event_name)< retireved. +++++EEEEE\n"
   infoObj_ForceMsg $limit_evt_obj
  }
  ...
  "$gPB(event,misc,user_limit_action,name)" \
  "800x500+200+200" "" "" \
...
proc UI_PB_mach_DefLimitActOk_CB { parent_w page_obj event_obj } {
  ...
  if { ![info exists event_rest_data(2)] } {
   set event::($event_obj,rest_value) $event::($event_obj,def_value)
   array set event_rest_data $event::($event_obj,rest_value)
  }
