...
proc UI_PB_Def_OthElemTab { book_id page_img book_obj} {
  ...
  set w [$book_id subwidget $Page::($page_obj,page_name)]
  set f $w.othparm.sf.scrolled
  ...
  switch -exact -- $Book::($book_obj,current_tab) \
  {
   0 {
    ...
    if { [info exists Page::($page_obj,active_blk_obj)] } \
    {
     ...
     if { $block::($block_obj,active_blk_elem_list) == "" } \
     {
      set raise_page 1
      set blk_page_flag 3
     } elseif [UI_PB_file_CheckNameForOldVersion "block" first_long_name 0] \
     {
      set raise_page 1
   ...
   1 {
    ...
    if { [info exists Page::($page_obj,act_addr_obj)] } \
    {
     ...
     if { [UI_PB_add_ValidateFormat $page_obj add_page_flag_msg] } {
      set add_page_flag 5
     }
