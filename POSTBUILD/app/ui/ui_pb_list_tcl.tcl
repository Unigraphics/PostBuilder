...
set bg_clr $paOption(header_bg) ;# steelBlue ;# navyBlue
...
-variable mom_sys_arr(Output_VNC) \
-relief solid -bd 2 -font $tixOption(bold_font) \
-anchor c -padx 12 -pady 7 -bg #ffff99 ;# white ;# lightGoldenRod3 ;# yellow
...
set res [tk_messageBox -parent $acwin -type yesno -icon error \
-title $gPB(msg,dialog,title) \
-message $gPB(setup,file_err,Msg)]
if [string match "yes" $res] {
...
{y-margin} {
$w format border $x1 $y1 $x2 $y2 \
-fill 1 -relief raised -bd 2 -bg $bg(main)
}
{s-margin} {
$w format border $x1 $y1 $x2 $y2 \
...
$grid size row 1       -pad0 0 -pad1 0 -size auto
...
set code_but [label $top_frm.but -text " $edit_com_name " -font bold \
-bg $paOption(active_color) -relief solid -bd 1] ;#<01-17-08 gsl> -relief was sunken
...
pack $code_but $top_but -side left -padx 20 -pady 15 -fill x
pack $combo_frm -side right -padx 30 -pady 15 -fill x
...
-width $paOption(trough_wd)
pack $content_frm.scr -fill both ;# was -fill x
...
pack $desc_lbl $desc.scr -side left -fill x -expand yes
...
bind $ent1 <Leave> "__isv_ValidateNoneempty %W"
bind $ent1 <FocusOut> "__isv_ValidateNoneempty %W"
...
pack $des_lbl -side top ;#<11-02-07 gsl> was -side left -padx 5.
pack $precom_text -side bottom -fill both -padx 3 -pady 3
...
bind $pre_code <KeyPress> "UI_PB_com_DisableSpecialChars %W %K"
...
lappend obj_attr(1) ""
lappend obj_attr(1) ""
lappend obj_attr(1) "  ## For any reason, you may uncomment next line to skip processing of this command."
lappend obj_attr(1) "  ##"
lappend obj_attr(1) "# return \$mom_sim_o_buffer"
lappend obj_attr(1) ""
lappend obj_attr(1) ""
lappend obj_attr(1) "  ## Make sure that token below matches the exact string to be processed."
lappend obj_attr(1) "  ##"
...
lappend obj_attr(1) "   global mom_sim_simulate_block mom_sim_nc_code"
lappend obj_attr(1) ""
lappend obj_attr(1) ""
lappend obj_attr(1) "   set mom_sim_simulate_block 1"
...
lappend obj_attr(1) "  #"
lappend obj_attr(1) "  # You will set the match level and/or provide handler callback with respect to"
...
lappend obj_attr(1) "  #   0 = Any match with token retained"
lappend obj_attr(1) "  #   1 = Exact match \(with no numerals trailing the token\)"
lappend obj_attr(1) "  #   2 = Extended match \(with trailing numerals - Value stored in mom_sim_nc_code\)"
lappend obj_attr(1) "  #   3 = Any match"
lappend obj_attr(1) "  #"
...
lappend obj_attr(1) "  #  -1 = Match found but callback not found"
lappend obj_attr(1) "  #"
lappend obj_attr(1) "   set result \[VNC_parse_nc_word o_buff \"\$NC_token\" \$match_level \$callback\]"
lappend obj_attr(1) ""
lappend obj_attr(1) "   if { \$result } {"
lappend obj_attr(1) ""
lappend obj_attr(1) "     ## Extract coordinates data by using VNC_parse_motion_word, if needed"
...
lappend obj_attr(1) "     ##"
lappend obj_attr(1) "     # set mom_sim_simulate_block \[PB_SIM_call VNC_parse_motion_word \$o_buff\]"
lappend obj_attr(1) ""
lappend obj_attr(1) ""
lappend obj_attr(1) "     ## Perform other processes with respect to this token."
lappend obj_attr(1) ""
lappend obj_attr(1) ""
lappend obj_attr(1) "     ## When necessary, clear block buffer to prevent further process of this block."
lappend obj_attr(1) "     # set o_buff \"\""
lappend obj_attr(1) ""
lappend obj_attr(1) ""
lappend obj_attr(1) "     ## When necessary, uncomment next statement to disable any subsequent simulation for this block."
lappend obj_attr(1) "     # set mom_sim_simulate_block 0"
lappend obj_attr(1) "   }"
lappend obj_attr(1) ""
...
bind $t <Button-3> "_cmd_AddPopUpMenu $t $menu %X %Y"
focus $text_widget
...
"PB_CMD_vnc__init_machine_tool_axes" \
"PB_CMD_vnc__process_nc_block"]
set old_sub_list [list "PB_CMD_vnc__old_set_nc_definitions" \
...
lappend mom_sim_arr(\$mom_sim_vnc_com_list) "PB_CMD_vnc____old_map_machine_tool_axes"
set mom_sim_arr(\$mom_sim_vnc_com_list) [ltidy $mom_sim_arr(\$mom_sim_vnc_com_list)]
