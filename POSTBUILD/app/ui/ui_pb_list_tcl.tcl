...
proc UI_PB_list_AddOtherElemPage { book_id page_obj } {
  ...
  tixLabelFrame $g2.link  -labelside none
  ...
  pack $g2.link -side bottom -fill both -pady 10 ;#<11-16-10 gsl> Added -pady ;#<12-15-10 wbh> Deleted -padx
  set f3  [$g2.out  subwidget frame]
  set f3a [$g2.tcl  subwidget frame]
  set f3b [$g2.link subwidget frame]
...
proc __isv_SetVNCSwitchSense { page_obj frame } {
  ...
  -variable mom_sys_arr(Output_VNC) \
  -relief solid -bd 2 -font $tixOption(bold_font) \
  -anchor c -padx 12 -pady 7 -fg black -bg lightYellow ;# white ;# gray90 ;# lemonChiffon1 ;# lightCyan ;# paleGoldenRod
...
proc UI_PB_isv_setup_ValidateToolname {output_mess} {
  ...
  if {[info exists mom_sim_arr(\$mom_sim_tool_list)]} {
   if {$mom_sim_arr(\$mom_sim_tool_list) != ""} {
    foreach tool_num $mom_sim_arr(\$mom_sim_tool_list) {
...
proc __isv_CreateSpecCustomPage { page_obj win cus_list} {
  ...
  pack $bot_canvas -side bottom -anchor s -fill x -padx 3 -pady 3 ;#<07-15-09 wbh> 3 was 10. Add -pady
...
proc __isv_AddNewSpecialCommand { win_list } {
  ...
  if { $cmd_obj <= 0 } {
   ...
   lappend obj_attr(1) ""
   lappend obj_attr(1) ""
   lappend obj_attr(1) "  ## Make sure the token below matches exactly with the string to be processed."
   lappend obj_attr(1) "  ##"
...
proc __isv_ReviewCodePage { page_obj args} {
  ...
  pack $review_frm -side top -fill both -padx 3 -pady 0
