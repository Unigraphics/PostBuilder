...
#-------------------------------------------------------------------------------
proc UI_PB_debug_ForcePause { msg } {
#-------------------------------------------------------------------------------
#<Mar-30-2016>
#
   set __debug_option $::gPB(SUPPRESS_ALL_DEBUG) 
   set ::gPB(SUPPRESS_ALL_DEBUG) 0

    UI_PB_debug_Pause $msg 0

   set ::gPB(SUPPRESS_ALL_DEBUG) $__debug_option
}
...
update
  ...
  global tixOption paOption
   ...
   if [catch { tixScrolledText $t.lsw -scrollbar auto } lsw] {
...
#-------------------------------------------------------------------------------
proc __debug_close_file { w } {
#-------------------------------------------------------------------------------
  #<04-08-2015 gsl> Destroy w

   if [winfo exists $w] {
      if { [info exists ::__debug_fid] } {
         if { $::__debug_fid != "0" && $::__debug_fid != "" } {
            flush $::__debug_fid
            close $::__debug_fid
         }
         unset ::__debug_fid
...
#-------------------------------------------------------------------------------
  ...
  global gPB
   ...
   if { ![info exists gPB(debug_index)] } {
      set gPB(debug_index) 0
   }

   if { [winfo exists $gPB(debug_win)] } {
      tixDestroy $gPB(debug_win)
   }
  ...
  #<04-08-2015 gsl> Pass w to callback
  # Callback to be executed when window is <Close or X'>ed or <destroy>ed.
  # wm protocol $w WM_DELETE_WINDOW "__debug_close_file $w"
   bind $w <Destroy> "+ __debug_close_file $w"

   set f [frame $w.f -relief sunken -bd 1]
   pack $f -padx 2 -pady 6 -fill both -expand yes
update

   if { ![winfo exists $f.lsw] } {
      set lsw [tixScrolledText $f.lsw -scrollbar auto]
      pack $lsw -side bottom -fill both -expand yes
      set ltext [$lsw subwidget text]
     #<Apr-19-2017 gsl> Use bigger font
     # $ltext config  -bg black -fg lightGreen -font $tixOption(fixed_font) -wrap word -bd 0
      $ltext config  -bg black -fg lightGreen -font {{consolas bold} 11} -wrap word -bd 0
  ...
  global gPB
   ...
   if { $gPB(DEBUG) } \
   {
      ...
      if { [string match "windows" $tcl_platform(platform)] } \
      {
        ...
        #<Oct-14-2016 gsl> Error when msg win is killed during PB session
         if { [catch { $gPB(MSG) tag configure redbg -background red } res] } {
            UI_PB_debug_CreateMsgWin
     ...
     #<02-11-2014 gsl> Only close the output file when window is closed.
      if ![info exists ::__debug_fid] {
         set ::__debug_fid "0"
      }
      if { $::__debug_fid == "0" } {
        ...
        # If output file cannot be open, try it in temp dir
         if [catch { set ::__debug_fid [open $gPB(Debug_Log) a] }] {

            set tmp_dir ""
            if { [info exists env(UGII_TMP_DIR)] } {
               set tmp_dir $env(UGII_TMP_DIR)
            } elseif { [info exists env(TEMP)] } {
               set tmp_dir $env(TEMP)
            }

            if { $tmp_dir != "" } {
               set gPB(Debug_Log) [file nativename "$tmp_dir/__$env(LOGNAME)_PB_debug_[clock clicks].log"]
               catch { set ::__debug_fid [open $gPB(Debug_Log) a] }
            }
         }
      }

      if { $::__debug_fid != "0" && $::__debug_fid != "" } {
         puts $::__debug_fid "$str$s<"
         flush $::__debug_fid
      } else {
        # Debug output file may not be open after all, disable debug!
         set debug_state 0
      }
   }
...
#-------------------------------------------------------------------------------
proc UI_PB_debug_ForceMsg_no_trace { s } {
#-------------------------------------------------------------------------------
  global gPB
   ...
   UI_PB_debug_DisplayMsg $s no_debug

   set gPB(DEBUG) $debug_state
}

#-------------------------------------------------------------------------------
proc UI_PB_debug_ForceMsg { s args } {
#-------------------------------------------------------------------------------
  global gPB
   ...
   UI_PB_debug_DisplayMsg $s [join $args]
