...
proc __mseq_SuppressElemOK { addr_obj } {
  ...
  for {set i 1} {$i < [llength $blk_list]} {incr i 2} \
  {
   ...
   if { $blk_obj && $block::($blk_obj,block_nof_elements) == 1 } \
   {
    set evt_list [list]
  ...
  if [info exists msg_arr] \
  {
   ...
   -type ok -icon error -message \
   "Address \"$add_name\" can not be suppressed.\
   \nIt has been used as single element in\
   \n$msg\n\
   \nSuppressing output of this Address will result in invalid\
   empty Blocks.  Instead, you need to remove the use of this Address\
   from the Events or Blocks that contain the reference."
