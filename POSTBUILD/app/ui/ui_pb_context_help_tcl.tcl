...
proc __turn_on_CSH { w } {
  ...
  for { set j 0 } { $j < [llength $wlist] } { incr j } \
  {
   ...
   UI_PB_debug_DisplayMsg "Widget Item : $witem" no_debug
...
#=======================================================================
proc UI_PB_chelp__display_msg { title text args } {
  global gPB
  if [info exists gPB(master_pid)] {
   comm::comm send $gPB(master_pid) [list UI_PB_chelp__display_msg_mod $title $text $args]
   } else {
   UI_PB_chelp__display_msg_mod $title $text $args
  }
 }

#=======================================================================
proc UI_PB_chelp__display_msg_mod { title text args } {
  global gPB
  global paOption
  ...
  ;#                      ^^
  ;#<May-24-2017 gsl> was dy
  if { $above_cur } {
   update idletasks ;# <= Required update to realize the size of window
   set ww [winfo reqwidth  $win]
   set wh [winfo reqheight $win]
   wm geometry $win "+[expr $x + 0]+[expr $y - $dy - $wh]"
   focus $win
   } else {
   UI_PB_com_PositionWindow $win "non_deco"
  }
  bind $win <Any-ButtonPress> "UI_PB_chelp__dismiss_msg $win %X %Y"
 }
