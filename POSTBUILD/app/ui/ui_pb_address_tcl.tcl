#26
UI_PB_AddPatchMsg "2007.0.0" "<04-11-12>  Fix error with Address' Max/Min value"
...
proc UI_PB_Def_Address {book_id addr_page_obj} {
  ...
  __addr_AddDisplayParams $addr_page_obj 0
...
proc UI_PB_addr_CreateTreePopupElements { page_obj X Y x y } {
  ...
  $active_index == $cursor_entry } \
  {
   ...
   if { $mseq_attr(4) == 1 } \
   {
    if { ![PB_com_object_is_external $active_addr] } {
...
proc UI_PB_addr_UpdateAddrNameEntry { page_obj index } {
  ...
  {
   set col [string range $index 2 end]
   set style [UI_PB_com_GetDisplayStyle $active_addr]
   $HLIST delete entry $index
...
proc UI_PB_addr_CheckAddressName { ADDRESS_NAME ADDR_OBJ } {
  ...
  if { $address_name == "" } {
   return 2
   } elseif { ![UI_PB_com_ValidateObjectName $address_name] } {
   return 4
  }
...
proc UI_PB_addr_UpdateAddrNameData { PAGE_OBJ } {
  ...
  if { [string compare $prev_add_name $cur_add_name] != 0 } \
  {
   ...
   __addr_UpdateAddrAttr addr_obj_attr $gPB_address_name
...
proc UI_PB_addr_DisplayNameList { PAGE_OBJ OBJ_INDEX } {
  ...
  for { set ix 0 } { $ix < $count } { incr ix } \
  {
   ...
   set style [UI_PB_com_GetDisplayStyle $addr_obj]
...
proc UI_PB_addr_CreateAddress { page_obj } {
  ...
  if { ![UI_PB_addr_ApplyCurrentAddrData page_obj] } {
   return
  }
  ...
  __addr_UpdateAddrAttr ADDRESSOBJATTR $add_name
  UI_PB_addr_DisplayNameList page_obj obj_index
  __addr_AddDisplayParams $page_obj $obj_index
  __addr_SetAddrParams $page_obj
...
proc UI_PB_addr_PasteAddress { page_obj } {
  ...
  if { $temp_index != $obj_index } \
  {
   ...
   __addr_UpdateAddrAttr buff_obj_attr $add_name
   UI_PB_addr_DisplayNameList page_obj obj_index
   __addr_AddDisplayParams $page_obj $obj_index
   __addr_SetAddrParams $page_obj
   set Page::($page_obj,selected_index) "0.$obj_index"
...
proc UI_PB_addr_CreateMiddleFrameParam { PAGE_OBJ } {
  ...
  if { $disable_widget } {
   ...
   label $max.unit -text "$unit_flag_a" -width 2 \
   -font $::tixOption(fixed_font) \
   -fg $::paOption(unit_fg)
   tixLabelEntry $max.val_main \
   -label " " \
   -options {
    label.width 18
    label.anchor w
    entry.width 12
    entry.anchor e
    entry.textVariable ADDRESSOBJATTR(40)
   }
   label $max.unit_main -text "$unit_flag_b" -width 2 \
   -font $::tixOption(fixed_font) \
   -fg $::paOption(unit_fg)
  }
  ...
  if { $disable_widget } {
   if 1 {
    tixForm $max.val -left %2 -right %85 -top %4 -bottom %27
    tixForm $max.unit -left %88 -right %96 -top %4 -bottom %27
    tixForm $max.val_main -left %2 -right %85 -top %34 -bottom %57
    tixForm $max.unit_main -left %88 -right %96 -top %34 -bottom %57
    tixForm $max.err -left %2 -right %98 -top %65 -bottom %98
    } else {
    grid $max.val -row 0 -column 0 -padx {10 5} -pady {4 2} -sticky ew
    grid $max.unit -row 0 -column 1 -sticky ew
    grid $max.val_main -row 1 -column 0 -padx {10 5} -pady {4 2} -sticky ew
    grid $max.unit_main -row 1 -column 1 -sticky ew
    grid $max.err -padx 10  -pady 2 -columnspan 2 -sticky ew
   }
   } else {
  ...
  if { $disable_widget } {
   label $min.unit -text "$unit_flag_a" -width 2 \
   -font $::tixOption(fixed_font) \
   -fg $::paOption(unit_fg)
   tixLabelEntry $min.val_main \
   -label " " \
   -options {
    label.width 18
    label.anchor w
    entry.width 12
    entry.anchor e
    entry.textVariable ADDRESSOBJATTR(60)
   }
   label $min.unit_main -text "$unit_flag_b" -width 2 \
   -font $::tixOption(fixed_font) \
   -fg $::paOption(unit_fg)
  }
  ...
  if { $disable_widget } {
   if 1 {
    tixForm $min.val -left %2 -right %85 -top %4 -bottom %27
    tixForm $min.unit -left %88 -right %96 -top %4 -bottom %27
    tixForm $min.val_main -left %2 -right %85 -top %34 -bottom %57
    tixForm $min.unit_main -left %88 -right %96 -top %34 -bottom %57
    tixForm $min.err -left %2 -right %98 -top %65 -bottom %98
    } else {
    grid $min.val -row 0 -column 0 -padx {10 5} -pady {4 2} -sticky ew
    grid $min.unit -row 0 -column 1 -sticky ew
    grid $min.val_main -row 1 -column 0 -padx {10 5} -pady {4 2} -sticky ew
    grid $min.unit_main -row 1 -column 1 -sticky ew
    grid $min.err -padx 10  -pady 2 -columnspan 2 -sticky ew
   }
   } else {
...
#=======================================================================
proc UI_PB_addr_ValidateAddrLeader { add_name leader } {
  if { [info exists ::gPB(ALLOW_VAR_SYS_LEADER)] && $::gPB(ALLOW_VAR_SYS_LEADER) == 1 } {
   return 1
  }
  set coord_add_list { N X Y Z fourth_axis fifth_axis }
  if { [lsearch $coord_add_list $add_name] >= 0 } {
   if { [string match "*\$*" $leader] } {
    return 0
   }
  }
  return 1
...
proc UI_PB_addr_ApplyCurrentAddrData { PAGE_OBJ } {
  ...
  {
   set act_addr_obj $Page::($page_obj,act_addr_obj)
   if { ![UI_PB_addr_ValidateAddrLeader $::ADDRESSOBJATTR(0) $::ADDRESSOBJATTR(8)] } {
   ...
   __addr_UpdateAddrAttr ADDRESSOBJATTR $addr_name
   PB_int_AddApplyObject act_addr_obj ADDRESSOBJATTR
   global mom_kin_var
   if { [string match "fourth_axis" $addr_name] } {
    if { [info exists mom_kin_var(\$mom_kin_4th_axis_leader)] } {
     set mom_kin_var(\$mom_kin_4th_axis_leader) \
     $address::($act_addr_obj,add_leader)
    }
    } elseif { [string match "fifth_axis" $addr_name] } {
...
proc UI_PB_addr_AddressApply { PAGE_OBJ } {
  ...
  if { [info exists Page::($page_obj,act_addr_obj)] } \
  {
   ...
   UI_PB_debug_ForceMsg "\n What's wrong with fmt_name : >$fmt_name< \n"
...
proc UI_PB_addr_AddDefaultCallBack { page_obj args } {
  global ADDRESSOBJATTR
  set act_addr_obj $Page::($page_obj,act_addr_obj)
  if { ![PB_com_object_is_external $act_addr_obj] } {
   ...
   __addr_UpdateAddrAttr def_addr_obj_attr $cur_addr_name
   __addr_SetAddressAttr $page_obj
   __addr_SetAddrParams $page_obj
  } ;# Not external
  if { [llength $args] > 0 } \
  {
...
proc UI_PB_addr_AddRestoreCallBack { page_obj args } {
  ...
  if { ![PB_com_object_is_external $act_addr_obj] } {
   ...
   __addr_UpdateAddrAttr rest_addressobjattr $cur_addr_name
   __addr_SetAddressAttr $page_obj
   __addr_SetAddrParams $page_obj
  } ;# Not external
  if { [llength $args] > 0 } \
...
proc UI_PB_addr_SelectItem { page_obj args } {
  ...
  if { [info exists Page::($page_obj,act_addr_obj)] } {
   set ent_s [lindex $args 0]
   if { $ent_s == "" }  {
    set ent_s [$HLIST info selection]
    } elseif { [string match "0" $ent_s] }  {
    set ent_s 0.0
   }
   PB_int_RetAddressObjList add_obj_list
   set act_add_obj $Page::($page_obj,act_addr_obj)
   set act_add_index [lsearch $add_obj_list $act_add_obj]
   if { [string compare $ent_s "0.$act_add_index"] } {
    if { [UI_PB_add_ValidateFormat $page_obj err_msg] } {
     $HLIST selection clear
     $HLIST anchor clear
     $HLIST selection set 0.$act_add_index
     $HLIST anchor set 0.$act_add_index
  ...
  if { [info exists Page::($page_obj,selected_index)] } \
  {
   ...
   if { [string length $gPB_address_name] > 32 && \
  ...
  __addr_AddDisplayParams $page_obj [string range $ent 2 end]
  UI_PB_debug_ForceMsg "\n ADDRESSOBJATTR to be displayed: >[array get ADDRESSOBJATTR]<"
  __addr_SetAddrParams $page_obj
  $HLIST selection clear
  $HLIST anchor clear
  ...
  set f $Page::($page_obj,middle_frame)
  if { [PB_com_object_is_external $addr_obj] } {
   return
  }
  ...
  if { $disable_widget } {
   $f.lea_val configure  -state disabled
   $fmt.new   configure  -state disabled
   $fmt.sel   configure  -state disabled
   $fmt.but   configure  -state disabled
   $f.tra_val configure  -state disabled
   [$f.mod.radio subwidget frame].yes configure  -state disabled
   [$f.mod.radio subwidget frame].no  configure  -state disabled
   [$f.max subwidget frame].val_main  configure  -state disabled
   [$f.min subwidget frame].val_main  configure  -state disabled
   [$f.max subwidget frame].err       configure  -state disabled
   [$f.min subwidget frame].err       configure  -state disabled
  }
 }
...
proc __addr_SetActionButtonsSens { page_obj add_obj } {
  ...
  {
   if { [info exists ::env(UNIT_SUB_POST_MODE)] && $::env(UNIT_SUB_POST_MODE) == 1 } {
    $left_pane_id.f.new configure -state disabled
    $left_pane_id.f.del configure -state disabled
    $left_pane_id.f.pas configure -state disabled
    return
   }
  }
...
#=======================================================================
proc __addr_AddDisplayParams { page_obj index } {
  global ADDRESSOBJATTR
  global gPB_address_name
  PB_int_RetAddressObjList addr_obj_list
  if { $index >= 0 } \
...
#=======================================================================
proc __addr_SetAddrParams { page_obj } {
  global ADDRESSOBJATTR
  global add_dis_attr
  global mom_sys_arr
  set middle_frame $Page::($page_obj,middle_frame)
  ...
  set addr_obj $Page::($page_obj,act_addr_obj)
  set addr_name $address::($addr_obj,add_name)
  set addr_glob_var $address::($addr_obj,rep_mom_var)
  PB_com_MapMOMVariable mom_sys_arr addr_obj addr_glob_var blk_elem_text
  ...
  if { [PB_com_object_is_external $addr_obj] } {
   ...
   set ::gPB(toplevel_disable_$win_index) 1
   UI_PB_addr_ActivateAddPageWidgets $page_obj $win_index
   bind $f.lea_val <3> "tk_popup $f.pop  %X %Y"
   bind $f.tra_val <3> "tk_popup $f.pop1 %X %Y"
  }
  UI_PB_addr_SetEditButtonStatus $page_obj $ADDRESSOBJATTR(1)
 }
...
proc UI_PB_addr_SetEditButtonStatus { page_obj fmt_name } {
  if { [PB_com_is_true Page::($page_obj,act_addr_obj)] } {
   set addr_obj $Page::($page_obj,act_addr_obj)
   if { [PB_com_object_is_external $addr_obj] } {
    return
   }
  ...
  {
   if { [info exists ::env(UNIT_SUB_POST_MODE)] && $::env(UNIT_SUB_POST_MODE) == 1 } {
    $fmt_frm.but configure -state disabled
    return
   }
  }
 }
...
#=======================================================================
proc __addr_SetAddressAttr { page_obj } {
  global gpb_addr_var
  global ADDRESSOBJATTR
  global Format_Name
  ...
  if { [info exists ::env(SUB_POST_MODE)] && $::env(SUB_POST_MODE) == 1 } \
  {
   if { [info exists ::env(UNIT_SUB_POST_MODE)] && $::env(UNIT_SUB_POST_MODE) == 1 } {
    ...
    set add_name_main "${add_name}_$lbl"
    set ADDRESSOBJATTR(40) $gpb_addr_var($add_name_main,add_max)
    set ADDRESSOBJATTR(60) $gpb_addr_var($add_name_main,add_min)
    set fmt_name $gpb_addr_var($add_name_main,fmt_name)
    set idx [expr [string length $fmt_name] - 1]
    if { ![string compare "Inches" $gPB(main_post_unit)] } {
     if { [string match "Feed_IP\[MR\]" $fmt_name] } {
      set fmt_name "Feed_MMP[string range $fmt_name $idx $idx]"
     }
     } else {
     if { [string match "Feed_MMP\[MR\]" $fmt_name] } {
      set fmt_name "Feed_IP[string range $fmt_name $idx $idx]"
     }
    }
    set ADDRESSOBJATTR(1) $fmt_name
    if { [lsearch {"N" "X" "Y" "Z" "fourth_axis" "fifth_axis"} $add_name] == -1 } {
     set gpb_addr_var($add_name,leader_name)   $gpb_addr_var($add_name_main,leader_name)
     set gpb_addr_var($add_name,trailer)       $gpb_addr_var($add_name_main,trailer)
     set gpb_addr_var($add_name,trail_status)  $gpb_addr_var($add_name_main,trail_status)
    }
   }
   } else {
  ...
  set ADDRESSOBJATTR(13) $gpb_addr_var($add_name,zero_format_name)
  set ADDRESSOBJATTR(14) $gpb_addr_var($add_name,is_dummy)
  set ADDRESSOBJATTR(15) $gpb_addr_var($add_name,is_external)
  set Format_Name $ADDRESSOBJATTR(1)
 }

#=======================================================================
proc __addr_UpdateAddrAttr { TEMP_ADDOBJATTR add_name } {
  UI_PB_debug_ForceMsg "\n >[info level 0] SATRT<"
  upvar $TEMP_ADDOBJATTR temp_addobjattr
  UI_PB_debug_ForceMsg_no_trace "\n >[array get temp_addobjattr]<"
  global gpb_addr_var
  ...
  set gpb_addr_var($add_name,zero_format_name) $temp_addobjattr(13)
  set gpb_addr_var($add_name,is_dummy)         $temp_addobjattr(14)
  set gpb_addr_var($add_name,is_external)      $temp_addobjattr(15)
  UI_PB_debug_ForceMsg "\n >[info level 0] END<"
 }
...
proc UI_PB_addr_UnsetAddrAttr { add_name } {
  global gpb_addr_var
  PB_com_unset_var  gpb_addr_var($add_name,name)
  PB_com_unset_var  gpb_addr_var($add_name,fmt_name)
  PB_com_unset_var  gpb_addr_var($add_name,modal)
  PB_com_unset_var  gpb_addr_var($add_name,modl_status)
  PB_com_unset_var  gpb_addr_var($add_name,add_max)
  PB_com_unset_var  gpb_addr_var($add_name,max_status)
  PB_com_unset_var  gpb_addr_var($add_name,add_min)
  PB_com_unset_var  gpb_addr_var($add_name,min_status)
  PB_com_unset_var  gpb_addr_var($add_name,leader_name)
  PB_com_unset_var  gpb_addr_var($add_name,trailer)
  PB_com_unset_var  gpb_addr_var($add_name,trail_status)
  PB_com_unset_var  gpb_addr_var($add_name,incremental)
  PB_com_unset_var  gpb_addr_var($add_name,zero_format)
  PB_com_unset_var  gpb_addr_var($add_name,zero_format_name)
  PB_com_unset_var  gpb_addr_var($add_name,is_dummy)
  PB_com_unset_var  gpb_addr_var($add_name,is_external)
 }
...
proc UI_PB_addr_CreateAddressPage { win add_obj NEW_ADD_PAGE addr_mode } {
  ...
  global paOption
  global add_dis_attr
  UI_PB_debug_ForceMsg "\n [info level 0] => [info level -1] \n"
  if { ![string match "*UI_PB_blk_BringUpBlkElemPage*" [info level -1]] } {
...
proc __CreateAddressPageParams { ADD_PAGE_OBJ entry_flag} {
  ...
  __addr_SetAddrParams $add_page_obj
 }
...
proc UI_PB_add_EditOkCallBack { blk_page_obj add_page_obj win blk_elem_obj } {
  ...
  if { [UI_PB_add_ValidateFormat $add_page_obj err_msg] } {
   UI_PB_mthd_DisplayErrMsg "$err_msg"
   return
  }
  ...
  unset rest_blk_elem_attr
  if { ![UI_PB_addr_ApplyCurrentAddrData add_page_obj] } {
   return
  }
  set add_obj $block_element::($blk_elem_obj,elem_add_obj)
...
#=======================================================================
proc UI_PB_add_ValidateFormat { page_obj ERR_MSG } {
  ...
  if { [info exists Page::($page_obj,act_addr_name)] &&\
   [info exists gPB(addr_expr,$Page::($page_obj,act_addr_name))] } {
   set act_addr_name $Page::($page_obj,act_addr_name)
   PB_int_RetFormatObjList fmt_obj_list
   ...
   if { ![string compare "Numeral" $for_obj_attr(1)] } {
    foreach addr_var $gPB(addr_expr,$act_addr_name) {
     set state 0
     ...
     if { $state } {
      UI_PB_fmt_AppendFormatErrorMsg err_message_list $addr_var $act_addr_name for_obj_attr
     }
    }
   }
  }
...
proc UI_PB_add_ValidateExp { exp for_name } {
  ...
  if { [expr $::tcl_version > 8.0] } {
   if { ![UI_PB_cmd_ValidateExpOK "Word Expression" [encoding convertto utf-8 $exp]] } {
    return 1
   }
   } else {
...
proc UI_PB_add_EditCancelCallBack { add_page_obj win args} {
  ...
  UI_PB_addr_AddRestoreCallBack $add_page_obj $args
  PB_com_DeleteObject $add_page_obj
  destroy $win
 }
...
proc UI_PB_add_NewOkCallBack { blk_page_obj add_page_obj win blk_elem_obj \
  ...
  set tmp_elem_text_var [PB_output_EscapeSpecialControlChar $elem_text_var]
  global Format_Name
  if { [UI_PB_add_ValidateExp $tmp_elem_text_var $Format_Name] } {
   return
  }
  set ADDRESSOBJATTR(0) $gPB_address_name
  ...
  if { [string match $format::($fmt_obj,for_dtype) "Text String"] } {
   set elem_text_var $tmp_elem_text_var
  }
