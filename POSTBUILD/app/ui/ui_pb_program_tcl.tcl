#14
set gPB(WEDM_SHOW_CYCLE_MISC) 1
...
proc UI_PB_prog_EnableWindow { CHAP } {
  ...
  if { $Book::($sect,current_tab) == 0 } \
  {
   ...
   if { [PB_is_probe_post] } {
    UI_PB_debug_ForceMsg "********** disable 0.0 & 0.4 **********"
    $h entryconfig 0.0 -state disabled
    $h entryconfig 0.4 -state disabled
   }
  }
 }
  ...
  foreach event_obj $sequence::($seq_obj,evt_obj_list) \
  {
   ...
   foreach row_evt_elem $event::($event_obj,evt_elem_list) \
   {
    foreach evt_elem $row_evt_elem \
    {
     ...
     UI_PB_evt_UpdateEvtElemForceOutputAttr $evt_elem
    }
   }
  }
  UI_PB_evt_CreateSeqAttributes page_obj
...
proc UI_PB_evt_CreateTree { PAGE_OBJ } {
  ...
  if { $::gPB(ENABLE_POST_CONFIGURATION) } {
   $h add 0     -itemtype imagetext -text "Post Configuration" -image [tix getimage pb_setup] -style $style
   } else {
  ...
  if { $::gPB(WEDM_SHOW_CYCLE_MISC) || ![string match "Wire EDM" $machType] } {
   $h add 0.2.3 -itemtype imagetext \
   -text "$gPB(prog,tree,misc,Label)" \
   -image $subseq -style $style1
  }
  ...
  -image $seq -style $style
  if { 0 && [PB_is_probe_post] } {
   UI_PB_debug_ForceMsg "********** create prog tab **********"
   $h entryconfig 0.0 -state disabled
   $h entryconfig 0.4 -state disabled
...
proc UI_PB_evt_CreateSeqAttributes { PAGE_OBJ } {
  ...
  switch $Page::($page_obj,prev_seq) \
  {
   ...
   4  {
    set seq_obj [lindex $Page::($page_obj,seq_obj_list) 7]
    UI_PB_evt_SeqComponents page_obj seq_obj
    set Page::($page_obj,active_seq) $seq_obj
   }
   Virtual_NC \
   {
    set seq_obj [lindex $Page::($page_obj,seq_obj_list) 9]
    UI_PB_evt_SeqComponents page_obj seq_obj
    set Page::($page_obj,active_seq) $seq_obj
   }
  }
...
#=======================================================================
proc UI_PB_evt_Misc { PAGE_OBJ } {
  upvar $PAGE_OBJ page_obj
  UI_PB_evt_DeleteObjectsPrevSeq page_obj
  set seq_obj [lindex $Page::($page_obj,seq_obj_list) 5]
  UI_PB_evt_SetSequenceObjAttr seq_obj
  set Page::($page_obj,prev_seq) 2.3
  UI_PB_evt_UnpackDispNcFrame page_obj
  if {$Page::($page_obj,topcanvas_flag) == 0} \
  {
   pack forget $Page::($page_obj,top_canvas)
...
#=======================================================================
proc __ValidateSeqEvtOutput { page_obj seq_index } {
  set seq_obj [lindex $Page::($page_obj,seq_obj_list) $seq_index]
  foreach event_obj $sequence::($seq_obj,evt_obj_list) {
...
#=======================================================================
proc UnmanageWindow { win } {
  set wMan [winfo manager $win]
  if { $wMan != "" } {
   eval $wMan forget $win
  }
 }
...
proc UI_PB_evt_progItemSelection { page_obj args } {
  ...
  if { $::gPB(ENABLE_POST_CONFIGURATION) } {
   if { [string match "0" $ent] } {
    UI_PB_debug_ForceMsg "\n %%% Post Config 1 %%% \n"
    if { [info exists Page::($page_obj,post_config_visited)] && \
     $Page::($page_obj,post_config_visited) } {
     UI_PB_debug_ForceMsg "\n %%% Post Config 3 %%% \n"
     return 1
    }
    set Page::($page_obj,post_config_visited) 1
    UI_PB_evt_DeleteApplyEvtElemsOfSeq page_obj
    set canvas_frame $Page::($page_obj,canvas_frame)
    UnmanageWindow $canvas_frame.frame
    UnmanageWindow $canvas_frame.nc
    if { ![info exists Page::($page_obj,post_config)] } {
     UI_PB_mthd_CreateScrollWindow $canvas_frame config param_win y
     set Page::($page_obj,post_config) $canvas_frame.config
     set Page::($page_obj,param_win)   $param_win
    }
    pack $Page::($page_obj,canvas_frame).box -side bottom -fill x -padx 3 -pady 3
    pack $Page::($page_obj,post_config) -fill both -expand yes -padx 3 -pady 3
    UI_PB_debug_ForceMsg "\n %%% Post Config 2 %%% \n"
    return 1
   }
   } else {
   set Page::($page_obj,post_config_visited) 0
  }
  ...
  if { [llength $args] && [lindex $args 0] == 99 } {
   UI_PB_debug_ForceMsg "\n %%% Validate output of Sequence args: $args %%% \n"
   set index 99
  }
  if { ![info exists Page::($page_obj,post_config_visited)] } {
   set Page::($page_obj,post_config_visited) 0
  }
  UI_PB_debug_ForceMsg "\n %%% prev_seq : $Page::($page_obj,prev_seq)  index : $index %%% \n"
  if { $Page::($page_obj,prev_seq) != $index || \
   $Page::($page_obj,post_config_visited) } {
   set prev_seq $Page::($page_obj,prev_seq)
   if { $prev_seq < 2 || $prev_seq >= 3 } {
    UI_PB_debug_ForceMsg "\n %%% Validate output of Sequence $prev_seq %%% \n"
    if { ![__ValidateSeqEvtOutput $page_obj $prev_seq] } {
     $HLIST selection clear
     $HLIST anchor clear
     $HLIST selection set 0.$prev_seq
     $HLIST anchor set 0.$prev_seq
     return 0
    }
   }
   if { $index == 99 } {
   ...
   if { $::gPB(ENABLE_POST_CONFIGURATION) } {
    ...
    $Page::($page_obj,post_config_visited) } \
    {
     UnmanageWindow $Page::($page_obj,post_config)
     UI_PB_debug_ForceMsg "\n %%% seq_index : $seq_index %%% \n"
     if { $seq_index < 2 || $seq_index > 5 } \
     {
      pack $Page::($page_obj,canvas_frame).nc -side top -fill x
      } else {
      UnmanageWindow $Page::($page_obj,canvas_frame).nc
      UnmanageWindow $Page::($page_obj,canvas_frame).box
     }
...
#=======================================================================
proc UI_PB_evt_UpdateEvtElemForceOutputAttr { evt_elem { type normal } } {
  set block_obj $event_element::($evt_elem,block_obj)
  if { ! [string match "normal" $block::($block_obj,blk_type)] } {
   return
  }
  set active_force_addr_list ""
  set force_addr_list $event_element::($evt_elem,force_addr_list)
  if { [string match "active" $type] } {
  ...
  set event_element::($evt_elem,force_addr_list) $active_force_addr_list
 }
