...
proc UI_PB_lnk_SyncBlockNameWithEvent { event_obj } {
  ...
  foreach evt_elem_obj $evt_attr(2) {
   ...
   if { [string match "comment" $block::($blk_obj,blk_type)] } {
    continue
   }
   if { [string match "command" $block::($blk_obj,blk_type)] && \
    ([string match "$::gPB(condition_cmd_prefix)*" $block::($blk_obj,block_name)] || \
    [string match "ELSE"                          $block::($blk_obj,block_name)] || \
    [string match "END"                           $block::($blk_obj,block_name)]) } {
    continue
   }
   switch $blk_type {
    "start" {
...
proc UI_PB_progtpth_ValidatePrevPageData { BOOK_OBJ args } {
  ...
  switch -exact -- $current_tab \
  {
   0 {
    ...
    if { ![UI_PB_evt_progItemSelection $page_obj 99] } {
     set raise_page 1
    }
   }
   3 { ;#<11-27-01 gsl> Addresses Summary page
    ...
    set max_err [UI_PB_ads_ValidateMaxN]
    if { $max_err } {
     set raise_page 1
    }
   }
   5 { ;# Custom Command page
    if { [info exists Page::($page_obj,active_cmd_obj)] } \
    {
     ...
     if { [string match "$::gPB(condition_cmd_prefix)*" $command::($cmd_obj,name)] } then {
      set cur_cmd_name $::gPB(condition_cmd_prefix)
      append cur_cmd_name $pb_cmd_procname
      } elseif { [string match "$::gPB(check_block_cmd_prefix)*" $command::($cmd_obj,name)] } then {
 ...
 switch -exact -- $current_tab \
 {
  3 {
   if { [info exists addrsum_err] && $addrsum_err } {
    ...
    tk_messageBox -parent [UI_PB_com_AskActiveWindow] \
    -type ok -icon error \
    -message "$gPB(msg,add_max1) N $gPB(msg,add_max2) $max_err."
    } elseif { $leader_err } {
