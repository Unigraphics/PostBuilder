...
if { ![string compare $::tcl_platform(platform) "windows"] } {
   ...
   switch $gPB(LANG) {
      ...
      default {
         ...
         set gPB(font)                {ansi 9 bold}
         set gPB(font_sm)             {ansi 8}
         set gPB(bold_font)           {ansi 10 bold}
         set gPB(bold_font_lg)        {ansi 11 bold}
         set gPB(italic_font)         {ansi 10 italic bold}
         set gPB(italic_font_normal)  {ansi 10 italic}
        # set gPB(fixed_font)          {{courier new} 10}
        # set gPB(fixed_font_sm)       {courier 7}
      }
   }

   set gPB(fixed_font)          {{courier new} 10 bold}

   if { [lsearch [font families] {Lucida Console}] >= 0 } {
      set gPB(fixed_font_sm)       {{lucida console} 8}
   } else {
      set gPB(fixed_font_sm)       {{courier new} 8 bold}
   }
