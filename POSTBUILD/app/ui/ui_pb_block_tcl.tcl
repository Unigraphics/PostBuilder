...
UI_PB_AddPatchMsg "2007.0.0" "<09 AUG 2011>\t Fix error on Program & Operation sequence page"
UI_PB_AddPatchMsg "2007.0.0" "<09 NOV 2011>\t Display main page of NX manual for Post Builder" ;# Ref. ui_pb_resource.tcl
...
proc UI_PB_blk_EditBlockName { page_obj index } {
  ...
  if { !$err && \
  [info exists Post::($post_object,axis_limit_evt_obj)] } \
  {
   set evt_obj $Post::($post_object,axis_limit_evt_obj)
   set evt_name $event::($evt_obj,event_name)
   set evt_name [string tolower $evt_name]
   if { [string match "$evt_name*" $gPB_block_name] } \
   {
    set err 1
   }
  }
  ...
  $new_frm.ent config -width $gPB(MOM_block_name_len)
  focus $new_frm.ent
  ...
  bind $new_frm.ent <KeyPress>      "+UI_PB_com_RestrictStringLength %W %K block"
  bind $new_frm.ent <Control-Key-v> "UI_PB_com_Validate_Control_V %W %K %A block"
...
proc UI_PB_blk_UpdateBlkEntry { page_obj index } {
  ...
  set tree $Page::($page_obj,tree)
  set HLIST [$tree subwidget hlist]
  set style [UI_PB_com_GetDisplayStyle $active_blk]
...
proc UI_PB_blk_DisplayNameList { PAGE_OBJ OBJ_INDEX } {
  ...
  {
   set style [UI_PB_com_GetDisplayStyle $blk_obj]
   set blk_name $block::($blk_obj,block_name)
   if { $blk_name != "$mom_sys_arr(seqnum_block)" && \
    ![string match "*_turbo" $blk_name] && \
   ![string match "just_mcall_sinumerik" $blk_name] } \
   {
...
proc UI_PB_blk_SelectItem { page_obj args } {
  ...
  if { [info exists Page::($page_obj,selected_index)] } \
  {
   ...
   if { [info exists gPB_block_name] && \
    [string length $gPB_block_name] > 64 && \
   $Page::($page_obj,selected_index) != -1 } \
   {
    $HLIST selection clear
    $HLIST anchor clear
    ...
    -icon error -message "$gPB(address,maximum_name_msg)"
    return UI_PB_ERROR
   }
  }
  if 0 {
  ...
  if { [info exists Page::($page_obj,active_blk_obj)] } \
  {
   ...
   } else \
   {
    if { $Page::($page_obj,double_click_flag) } \
    {
     ...
     $HLIST add 0.$act_index -itemtype imagetext -text $gPB_block_name -image $file \
     -style $style -at $act_index
     set Page::($page_obj,double_click_flag) 0
     UI_PB_blk_UpdateBlkNameData page_obj
     unset Page::($page_obj,rename_index)
    }
...
proc UI_PB_blk_DisplayBlockAttr { PAGE_OBJ BLOCK_OBJ } {
  ...
  set bindings [bind $Page::($page_obj,bot_canvas) <3>]
  if { [PB_com_object_is_external $block_obj] } {
   UI_PB_blk_DisableBlkPageWidgets $page_obj
   if { [string match "*UI_PB_blk_CanvasPopupMenu*" $bindings] } {
    UI_PB_debug_ForceMsg "\nbbbbb bot_canvas <3> bindings >$bindings< bbbbb\n"
    bind $Page::($page_obj,bot_canvas) <3> ""
   }
   } else {
   if { ![string match "*UI_PB_blk_CanvasPopupMenu*" $bindings] } {
    UI_PB_debug_ForceMsg "\nbbbbb bot_canvas <3> bindings >$bindings< bbbbb\n"
...
proc UI_PB_blk_DeleteCellsIcons {PAGE_OBJ BLOCK_OBJ} {
  ...
  UI_PB_tpth_DeleteExecAttrSymbols page_obj block_obj
 }
...
proc UI_PB_blk_CreateBlockImages { PAGE_OBJ BLOCK_OBJ } {
  ...
  foreach blk_elem_obj $block::($block_obj,active_blk_elem_list) \
  {
   ...
   if { [string match "$::gPB(condition_cmd_prefix)*" $block::($block_obj,block_name)] } {
    set blk_elem_text "IF($block::($block_obj,block_name))"
   }
   if { [string match "Macro" $blk_elem_addr] } \
   {
    UI_PB_blk_AdjustMacroText blk_elem_text $blk_elem_obj
   }
...
proc UI_PB_blk_AddBindProcs {PAGE_OBJ} {
  ...
  $top_canvas itemconfig add_movable -state normal
 }
...
proc UI_PB_blk_AddUnBindProcs { PAGE_OBJ } {
  ...
  $top_canvas itemconfig add_movable -state disabled
 }
...
proc UI_PB_blk_RestoreCallBack { page_obj } {
  ...
  if { [PB_com_object_is_external $block_obj] } {
   set evt_elem_obj 0
   if { [info exists Page::($page_obj,in_focus_evt_elem)] } \
   {
    set evt_elem_obj $Page::($page_obj,in_focus_evt_elem)
    } elseif { [info exists Page::($page_obj,seq_page_active_evt_elem)] } \
   {
    set evt_elem_obj $Page::($page_obj,seq_page_active_evt_elem)
   }
   if { $evt_elem_obj == 0 } {
    UI_PB_debug_ForceMsg "\n External standalone block >$block::($block_obj,block_name)< - No Restore. \n"
    return
   }
  }
  ...
  UI_PB_blk_UpdateForceOutputAttr page_obj block_obj restore
  UI_PB_blk_CreateBlockImages page_obj block_obj
  set block_owner $block::($block_obj,blk_owner)
  UI_PB_blk_CreateBlockCells page_obj block_obj active_blk_elem_obj \
  block_owner
...
proc UI_PB_blk_DefaultCallBack { page_obj } {
  ...
  if { [PB_com_object_is_external $block_obj] } {
   ...
   if { $evt_elem_obj == 0 } {
    UI_PB_debug_ForceMsg "\n External standalone block >$block::($block_obj,block_name)< - No Default. \n"
    return
   }
  }
  UI_PB_blk_DeleteCellsIcons page_obj block_obj
  ...
  foreach blk_elem_obj $block::($block_obj,elem_addr_list) \
  {
   ...
   if 0 {
    ...
    if { [info exists address::($add_obj,add_name)] } {
     set add_name $address::($add_obj,add_name)
     } else {
     set add_name $add_obj
    }
    UI_PB_debug_ForceMsg "\n+++ elem_addr_list of $block_element::($blk_elem_obj,parent_name) : $add_obj => $add_name \n"
   }
  ...
  UI_PB_blk_UpdateForceOutputAttr page_obj block_obj default
  UI_PB_blk_CreateBlockImages page_obj block_obj
  set block_owner $block::($block_obj,blk_owner)
  UI_PB_blk_CreateBlockCells page_obj block_obj active_blk_elem_obj \
  block_owner
...
proc __blk_DeleteAllElems { page_obj } {
  ...
  update
  if { [info exists Page::($page_obj,event_obj)] } {
   UI_PB_debug_ForceMsg "\n ##### event_obj >$Page::($page_obj,event_obj)< #####\n"
   set evt_obj $Page::($page_obj,event_obj)
   set evt_name $event::($evt_obj,event_name)
   set evt_elem_obj_list $event::($evt_obj,evt_elem_list)
      ...
      if 0 {
       if { ![string match "post" $block_element::($blk_elem_obj,owner)] } {
        if { [string match "Cycle Parameters" $evt_name] } {
         ...
         } elseif { ![string match "Cycle Parameters" $block_element::($blk_elem_obj,owner)] && \
         ![string match "Cycle Parameters" $block_element::($blk_elem_obj,owner)] } {
         UI_PB_blk_DeleteBlockElement $page_obj $blk_elem_obj 0 0
        }
       }
       } else {
       if { [string match "$evt_name" $block_element::($blk_elem_obj,owner)] } {
        UI_PB_blk_DeleteBlockElement $page_obj $blk_elem_obj 0 0
       }
      }
     }
   ...
   UI_PB_debug_ForceMsg "\n ##### blk_obj >$blk_obj< blk_elem_list >$blk_elem_list< #####\n"
   foreach blk_elem_obj $block::($blk_obj,active_blk_elem_list) {
    UI_PB_debug_ForceMsg "\n    >>>>> blk_elem_owner >$block_element::($blk_elem_obj,owner)< <<<<<\n"
    if { ![string match "post"                         $block_element::($blk_elem_obj,owner)] &&\
    [string match "$block::($blk_obj,blk_owner)" $block_element::($blk_elem_obj,owner)] } \
    {
     UI_PB_blk_DeleteBlockElement $page_obj $blk_elem_obj 0 0
    }
   }
...
#=======================================================================
proc UI_PB_blk_CombSelection { page_obj base_addr index args } {
  global tixOption
  global mom_sys_arr
  global gPB
  ...
  {
   if { [llength $args] > 0 && [string match "TCL" [lindex $args]] } {
    set ::gPB(comment_is_tcl_selected) 1  ;# To talk to UI_PB_cmd_CreateCommentDlgParam & etc.
    set word_desc "$gPB(event,combo,tcl_line,Label)"
    } else {
   ...
   } elseif { [string compare $base_addr "Command"] == 0 } \
  {
   if { [llength $args] > 0 } {
    switch [lindex $args 0] {
     IF {
      set app_text "IF - New Condition"
      set new_word_mom_var "IF"
      set word_desc "New Condition"
     }
     ELSE {
      set app_text "ELSE"
      set new_word_mom_var $app_text
      set word_desc "Condition"
     }
     END {
      set app_text "END"
      set new_word_mom_var $app_text
    ...
    set desc "Condition ($app_text)"
    set comb_var $desc
    set Page::($page_obj,comb_var) $desc
    set Page::($page_obj,new_elem_mom_var) $new_word_mom_var
    set Page::($page_obj,sel_base_addr) Condition  ;#<Jul-19-2016 gsl> Was $base_addr
    if 1 {
     set ::gPB__new_elem_mom_var $Page::($page_obj,new_elem_mom_var)
     set ::gPB__comb_var         $Page::($page_obj,comb_var)
     set ::gPB__base_addr        $Page::($page_obj,sel_base_addr)
     UI_PB_debug_ForceMsg "\n +++ (1) ::gPB__new_elem_mom_var $::gPB__new_elem_mom_var $::gPB__comb_var $::gPB__base_addr +++ \n"
    }
    return
    } else {
    PB_int_RetMOMVarAsscAddress base_addr word_mom_var_list
    ...
    {
     set app_text "New"
     set word_desc "Custom Command"
    }
   }
   } elseif { [string compare $base_addr "Condition"] == 0 } \
  {
   switch $index {
    0 {
     set new_word_mom_var "IF Condition"
     set app_text "New"
     set word_desc "New Condition"
     set add_leader ""
    }
    1 {
     set new_word_mom_var "ELSE"
     set app_text "ELSE"
     set word_desc "Condition"
     set add_leader ""
    }
    2 {
     set new_word_mom_var "END"
     set app_text "END"
     set word_desc "Condition"
     set add_leader ""
    }
    default {
     set idx [expr $index - 3]
     set tmp_base_addr Command
     PB_int_RetMOMVarAsscAddress tmp_base_addr word_mom_var_list
     set app_text [lindex $word_mom_var_list $idx]
     set add_leader ""
     set new_word_mom_var $app_text
     set word_desc [lindex $WordDescArray($tmp_base_addr) $idx]
    }
   }
   } elseif { [string compare $base_addr "Macro"] == 0 } \
  {
   if { $index > 0 } \
   {
    PB_int_RetFunctionBlks func_blk_list
    set func_obj [lindex $func_blk_list [expr $index - 1]]
    set app_text $function::($func_obj,id)
    set add_leader ""
    set new_word_mom_var $app_text
    set word_desc $function::($func_obj,description)
   } else \
   {
    set new_word_mom_var "New Macro"
    set app_text "New"
    set word_desc "Custom Macro"
    set add_leader ""
   }
  } else \
  {
  ...
  if { [string compare $base_addr "Command"] == 0 } {}
  if 1 {
   set ::gPB__new_elem_mom_var $Page::($page_obj,new_elem_mom_var)
   set ::gPB__comb_var         $Page::($page_obj,comb_var)
   set ::gPB__base_addr        $Page::($page_obj,sel_base_addr)
   UI_PB_debug_ForceMsg "\n +++ (2) ::gPB__new_elem_mom_var $::gPB__new_elem_mom_var $::gPB__comb_var $::gPB__base_addr +++ \n"
  }
 }
...
proc UI_PB_blk_CreateMenuElement { PAGE_OBJ comb_widget element_index \
  ...
  } else \
  {
   ...
   for { set count 0 } { $count < $list_len } { incr count } \
   {
    ...
    if { [string match "G_motion" $word] } {
     if { [string match "*Cycle Code Representative*" $desc] } {
      UI_PB_debug_ForceMsg "\n +++ G_motion - skip-option $desc +++ \n"
      continue
     }
    }
    PB_com_MapMOMVariable mom_sys_arr add_obj mom_var app_text
...
proc UI_PB_blk_CreateMenuOptions { PAGE_OBJ page_type } {
  ...
  if { $page_type == "event" } \
  {
   ...
   if { $gPB(ENABLE_TCL_STATEMENT) } {
    $comb_widget add command -label "$gPB(event,combo,tcl_line,Label)" \
    -command "UI_PB_blk_CombSelection $page_obj Comment 0 TCL"
   }
   ...
   for { set count 0 } { $count < [llength $cmd_proc_list] } { incr count } \
   {
    ...
    if { [string match "PB_CMD_before_motion"          $cmd_name] || \
     ...
     [string match "PB_CMD_kin*"                   $cmd_name] || \
     [string match "ELSE"                          $cmd_name] || \
     [string match "END"                           $cmd_name] || \
     [string match "$gPB(condition_cmd_prefix)*"   $cmd_name] || \
     [string match "$gPB(check_block_cmd_prefix)*" $cmd_name] } { continue }
    if { [lsearch $special_cmd_list $cmd_name] >= 0 } { continue }
    if { [string compare "Custom Command" $cmd_name] == 0 } {
     set cmd_name "$gPB(block,cmd,word_desc,Label)"
     $comb_widget.cmd add command -label $cmd_name \
     -command "UI_PB_blk_CombSelection $page_obj $word $count"
     $comb_widget.cmd add separator
     } else {
     $comb_widget.cmd add command -label $cmd_name \
     -command "UI_PB_blk_CombSelection $page_obj $word $count"
    }
   ...
   if 1 \
   {
    $comb_widget add cascade -label $gPB(block,func_combo,Label) \
    -menu $comb_widget.func
    PB_int_RetFunctionNames func_name_list
    catch {destroy $comb_widget.func}
    menu $comb_widget.func
    set word "Macro"
    $comb_widget.func add command -label $gPB(block,custom_func,Label) \
    -command "UI_PB_blk_CombSelection $page_obj $word 0"
    set temp_index [lsearch $func_name_list "just_mcall_sinumerik"]
    set mac_len [llength $func_name_list]
    if { $mac_len > 0 } {
     $comb_widget.func add separator
    }
    for { set count 0 } { $count < $mac_len } { incr count } \
    {
     if { $count == $temp_index } continue
     set func_name [lindex $func_name_list $count]
     set mac_index [expr $count + 1]
     $comb_widget.func add command -label $func_name \
     -command "UI_PB_blk_CombSelection $page_obj $word $mac_index"
    }
   }
   if { [info exists gPB(ENABLE_IF_CONDITION)] && $gPB(ENABLE_IF_CONDITION) } {
    $comb_widget add cascade -label "Condition" \
    -menu $comb_widget.cond
    if { [winfo exists $comb_widget.cond] } {
     destroy $comb_widget.cond
    }
    menu $comb_widget.cond
    set condition_cmd_added 0
    set word "Command" ;# Faked "Condition"
    $comb_widget.cond add command -label "IF(New Condition)" \
    -command "UI_PB_blk_CombSelection $page_obj $word 0 IF"
    $comb_widget.cond add command -label ELSE \
    -command "UI_PB_blk_CombSelection $page_obj $word 0 ELSE"
    $comb_widget.cond add command -label END \
    -command "UI_PB_blk_CombSelection $page_obj $word 0 END"
    for { set count 0 } { $count < [llength $cmd_proc_list] } { incr count } \
    {
     set cmd_name [lindex $cmd_proc_list $count]
     if { [string match "$gPB(condition_cmd_prefix)*" $cmd_name] } {
      if { [lsearch $special_cmd_list $cmd_name] >= 0 } {
       ...
       if { $condition_cmd_added == 0 } {
        $comb_widget.cond add separator
        set condition_cmd_added 1
       }
       $comb_widget.cond add command -label "IF($cmd_name)" \
       -command "UI_PB_blk_CombSelection $page_obj $word $count CONDITION"
      }
     }
    }
   } ;# gPB(ENABLE_IF_CONDITION)
   $comb_widget add separator
  }
  ...
  if { [info exists ::gPB__new_elem_mom_var] } {
   UI_PB_debug_ForceMsg "\n +++ Restore ::gPB__new_elem_mom_var $::gPB__new_elem_mom_var $::gPB__comb_var $::gPB__base_addr +++ \n"
...
proc UI_PB_blk_CreateBlockCells { PAGE_OBJ BLOCK_OBJ ACTIVE_BLK_ELEM_OBJ \
  ...
  global paOption
  set use_font $::gPB(nc_code_font)  ;# ::tixOption(fixed_font_sm)
  ...
  if { $n_comp == 1 } \
  {
   set blk_elem [lindex $block::($block_obj,active_blk_elem_list) 0]
   set blk_elem_addr $block_element::($blk_elem,elem_add_obj)
   if { [string match "Command"   $blk_elem_addr] || \
    [string match "Comment"   $blk_elem_addr] || \
    [string match "Condition" $blk_elem_addr] || \
   [string match "Macro"     $blk_elem_addr] } \
   {
    set blk_elem_mom_var $block_element::($blk_elem,elem_mom_variable)
    UI_PB_com_RetImageAppdText blk_elem_addr blk_elem_mom_var img_name blk_elem_text
    if { [string match "comment" $block::($block_obj,blk_type)] } {
     UI_PB_debug_ForceMsg "\n>>>>> Comment block_name: >$block::($block_obj,block_name)< <<<<<\n"
     if { [string match "$::gPB(tcl_line_blk)*" $block::($block_obj,block_name)] } {
      set blk_elem_text "# $blk_elem_mom_var"
     }
    }
    ...
    if { [string match "Macro" $blk_elem_addr] } \
    {
     UI_PB_blk_AdjustMacroText blk_elem_text $blk_elem
    }
    set txt_width [font measure $use_font $blk_elem_text]
    set w_cell [expr $txt_width + 28]
    set block::($block_obj,wider_cell) $w_cell
  ...
  for { set i 0 } { $i < $n_comp } { incr i 1 } {
   ...
   UI_PB_debug_ForceMsg "\n +++ active_blk_elem_list of $block_element::($blk_elem_obj,parent_name) : $add_obj => $add_name \n"
   set j  [expr 2 * $i]
   ...
   if 0 {
    if { $i == 0 } {
     $c lower $block_element::($blk_elem_obj,div_id) $block::($block_obj,rect)
    }
   }
   set k [expr $j + 1]
   set x0 $x1
   ...
   if { [info exists Page::($page_obj,seq_page_active_evt_elem)] } \
   {
    set evt_elem $Page::($page_obj,seq_page_active_evt_elem)
    if { [lsearch $event_element::($evt_elem,force_addr_list) \
    $block_element::($blk_elem_obj,elem_add_obj)] >= 0 } \
    {
     append opt_img _f
    }
    } elseif { [llength $args] > 0 } \
   {
    set evt_elem [lindex $args 0]
    if { $evt_elem != "" && $evt_elem != 0 && \
     [lsearch $event_element::($evt_elem,force_addr_list) \
    $block_element::($blk_elem_obj,elem_add_obj)] >= 0 } \
    {
     append opt_img _f
    }
   }
   if { 0 && $block_element::($blk_elem_obj,force) } \
   {
    append opt_img _f
   }
   ...
   $blk_elem_add_obj == "Macro" } \
   {
    set address_name $blk_elem_add_obj
    if { [string match "start_of_HEAD__*" $block_owner] || \
     [string match "end_of_HEAD__*"   $block_owner] } {
     if { [string compare $blk_elem_owner $block_owner] != 0 } {
      set blk_elem_owner $block_owner
      set block_element::($blk_elem_obj,owner) $blk_elem_owner
      set block::($block_obj,blk_owner) $block_owner
     }
    }
   } else \
   {
   ...
   UI_PB_debug_ForceMsg "\n%%%%%  elmt owner : $blk_elem_owner  block owner : $block_owner %%%%%\n"
   if { [string compare $blk_elem_owner $block_owner] == 0 } \
   {
  ...
  {
   set im [$c itemconfigure $block_element::($active_blk_elem_obj,icon_id) -image]
   [lindex $im end] configure -relief sunken -bg $paOption(sunken_bg)
   set Page::($page_obj,active_blk_elem) $active_blk_elem_obj
  }
  if { [info exists evt_elem] && $evt_elem != "" && $evt_elem != 0 } {
   UI_PB_tpth_CreateExecAttrSymbols page_obj evt_elem
  }
 }
...
proc UI_PB_blk_IconBindProcs { PAGE_OBJ } {
  ...
  if { ![string match "*UI_PB_blk_popup_close_cb*" [bind $bot_canvas <ButtonRelease-1>]] } {
   bind $bot_canvas <ButtonRelease-1>         "+UI_PB_blk_popup_close_cb $page_obj"
  }
  if { ![string match "*UI_PB_blk_popup_close_cb*" [bind $bot_canvas <3>]] } {
...
proc UI_PB_blk_NonMovableElemPopup { PAGE_OBJ EVENT_OBJ ACT_BLK_OBJ \
  ...
  $block_element::($active_blk_elem,elem_add_obj) == "Macro" } \
  {
...
proc UI_PB_blk_BlockPopupMenu { PAGE_OBJ EVENT_OBJ ACT_BLK_OBJ ACTIVE_BLK_ELEM \
  ...
  {
   UI_PB_debug_ForceMsg "\n +++ block_element::($active_blk_elem,elem_mom_variable) = $block_element::($active_blk_elem,elem_mom_variable) +++ \n"
  ...
  if { $base_addr == "Command" || $base_addr == "Comment" || $base_addr == "Macro" } \
  {
   $blockpopup add checkbutton -label "$gPB(block,opt_popup,Label)" \
   -state disabled
  } else \
...
proc UI_PB_blk_BlockPopupMenu_v3 { PAGE_OBJ EVENT_OBJ ACT_BLK_OBJ ACTIVE_BLK_ELEM x y args } {
  ...
  if { [string match "just_mcall_sinumerik" $block::($act_blk_obj,block_name)] } {
   set edit_allowed 0
  }
  set block_name $block::($act_blk_obj,block_name)
  if { [string match "MOM_*" $block_element::($active_blk_elem,elem_mom_variable)] ||\
   [string match "ELSE"  $block_element::($active_blk_elem,elem_mom_variable)] ||\
   [string match "END"   $block_element::($active_blk_elem,elem_mom_variable)] } {}
  if { [string match "MOM_*" $block_element::($active_blk_elem,elem_mom_variable)] ||\
   [string match "ELSE"  $block_name] ||\
  [string match "END"   $block_name] } \
  {
   UI_PB_debug_ForceMsg "\n>>>>> elem_mom_variable: >$block_element::($active_blk_elem,elem_mom_variable)<<<<<\n"
   } elseif { $edit_allowed } {
   UI_PB_debug_ForceMsg "\n +++ >$block_name< edit_allowed = $edit_allowed +++ \n"
   $blockpopup add command -label "$gPB(block,edit_popup,Label)" \
   -state normal \
   -command "UI_PB_blk_EditAddress $page_obj $act_blk_obj $active_blk_elem"
   incr n_opt
   ...
   } elseif { $edit_allowed } \
  {
   ...
   for { set count 0 } { $count < $mom_var_length } { incr count } \
   {
    ...
    if { [string match "G_motion" $base_addr] } {
     if { [string match "*Cycle Code Representative*" $subdesc] } {
      UI_PB_debug_ForceMsg "\n +++ G_motion - skip-V3 $subdesc +++ \n"
      continue
     }
    }
    set mom_var [lindex $word_mom_var_list $count]
   ...
   } elseif { $option_allowed } \
  {
   ...
   -command "UI_PB_blk_AddNowsOpt $page_obj popupvar \
   $base_addr $active_blk_elem"
   if { [info exists Page::($page_obj,event_name)] || \
    [info exists Page::($page_obj,seq_page_active_evt_elem)] } {
    set add_flag 1
    if { [info exists Page::($page_obj,seq_page_active_evt_elem)] } {
     set active_evt_elem $Page::($page_obj,seq_page_active_evt_elem)
     } elseif { [string match "$event::($event_obj,event_name)" "$Page::($page_obj,event_name)"] } {
     UI_PB_tpth_GetEvtElemByPos page_obj event_obj active_evt_elem $x $y
     if { $active_evt_elem == 0 } {
      set add_flag 0
     }
     } else {
     set add_flag 0
    ...
    if { $add_flag } {
     set addr_obj $block_element::($active_blk_elem,elem_add_obj)
     if { [lsearch $event_element::($active_evt_elem,force_addr_list) $addr_obj] != -1 } {
      set popupvar(2) 1
      } else {
      set popupvar(2) 0
     }
     $blockpopup add checkbutton -label "$gPB(block,force_popup,Label)" \
     -variable popupvar(2)  -state normal \
     -command "UI_PB_blk_AddForceOpt_v8 $page_obj popupvar \
     $base_addr $active_blk_elem $active_evt_elem"
    }
   }
   incr n_opt
  }
...
#=======================================================================
proc UI_PB_blk_EditFuncPrefix { page_obj evt_elem_obj func_obj blk_elem type args } {
  global gPB popupvar
  set evt_flag 0
  if { [llength $args] > 0 } \
  {
   set evt_flag 1
   set seq_obj [lindex $args 0]
  }
  if { [string compare $type "Add"] == 0 && \
  $popupvar(prefix) == 0 } \
  {
   set block_element::($blk_elem,func_prefix) ""
   if { $evt_flag } \
   {
   ...
   } else \
   {
    ...
    UI_PB_blk_ReplaceIcon page_obj $base_addr $blk_elem
    UI_PB_tpth_ReplaceExecAttrSymbols page_obj evt_elem_obj "cond"
    UI_PB_tpth_ReplaceExecAttrSymbols page_obj evt_elem_obj "suppress"
   }
   return
  }
  set canvas_frame $Page::($page_obj,canvas_frame)
  set win [toplevel $canvas_frame.func_prefix]
  set function::($func_obj,temp_prefix) $block_element::($blk_elem,func_prefix)
  set toplevel_index [llength $gPB(toplevel_list)]
  set win_index [expr $toplevel_index + 1]
  set gPB(toplevel_disable_$win_index) 1
  if { $evt_flag } \
  {
   ...
   "$gPB(block,prefix,Label)" "" \
   "" "UI_PB_DisableProgPageWidgets $page_obj $seq_obj" "" \
   "destroy $win; UI_PB_evt_ActivateProgPageWidgets $page_obj $seq_obj $win_index"
   set cb_arr(gPB(nav_button,ok,Label)) \
   [list UI_PB_blk_OkFuncPrefix $win $page_obj $evt_elem_obj $func_obj $blk_elem $seq_obj]
  } else \
  {
   UI_PB_com_CreateTransientWindow $win \
   "$gPB(block,prefix,Label)" "" \
   "" "UI_PB_blk_DisableBlkPageWidgets $page_obj" "" \
   "destroy $win; UI_PB_blk_ActivateBlkPageWidgets $page_obj $win_index"
   set cb_arr(gPB(nav_button,ok,Label)) \
   [list UI_PB_blk_OkFuncPrefix $win $page_obj $evt_elem_obj $func_obj $blk_elem]
  }
  set text_frm [frame $win.frame]
  pack $text_frm -side top -fill both -expand yes
  label $text_frm.lab -text "$gPB(block,prefix,Label)" -anchor w
  entry $text_frm.ent -textvariable function::($func_obj,temp_prefix) \
  ...
  "unset function::($func_obj,temp_prefix); \
  destroy $win"
...
#=======================================================================
proc UI_PB_blk_OkFuncPrefix { win page_obj evt_elem_obj func_obj blk_elem args } {
  set prefix_text $function::($func_obj,temp_prefix)
  set block_element::($blk_elem,func_prefix) $prefix_text
  unset function::($func_obj,temp_prefix)
  set evt_flag 0
  if { [llength $args] > 0 } \
  {
...
proc UI_PB_blk_ActivateBlkPageWidgets { page_obj win_index } {
  ...
  if { $gPB(toplevel_disable_$win_index) } \
  {
   ...
   set top_canvas $Page::($page_obj,top_canvas)
   if { [winfo exists $top_canvas.f.ent] } {
...
proc UI_PB_blk_DisableBlkPageWidgets { page_obj } {
  if { [info exists Page::($page_obj,page_type)] && \
   [string match "seq" $Page::($page_obj,page_type)] } {
   UI_PB_debug_ForceMsg "\n +++ page_type : $Page::($page_obj,page_type) +++ \n"
   return
  }
  ...
  if { [info exists Page::($page_obj,name_frame)] } {
   ...
   [$name_frm.name subwidget entry] config -state disabled
  }
  if 0 {
   if { [info exists Page::($page_obj,tree)] } \
   {
  ...
  UI_PB_blk_popup_close_cb $page_obj
 }
...
proc UI_PB_blk_EditAddress { blk_page_obj act_blk_obj active_blk_elem args } {
   ...
   } elseif { $block_element::($active_blk_elem,elem_add_obj) == "Comment" } \
  {
   ...
   act_blk_obj Edit
   } elseif { $block_element::($active_blk_elem,elem_add_obj) == "Macro" } \
  {
   set event_obj 0
   set event_elem_obj 0
   UI_PB_tpth_BringFuncPage blk_page_obj event_obj event_elem_obj \
   act_blk_obj Edit
  } else \
  {
...
proc __blk_ConstructEditAddress { blk_page_obj new_add_page \
  ...
  if { [string match "User*" $elem_desc] || \
   [string match "New*"  $elem_desc] || \
  [string match "$gPB(block,user,word_desc,Label)" $elem_desc] }\
  {
...
#=======================================================================
proc UI_PB_blk_AddForceOpt_v8 { page_obj POPUPVAR base_addr active_blk_elem \
  active_evt_elem } {
  upvar $POPUPVAR popupvar
  global mom_sys_arr
  ...
  set addr_obj $block_element::($active_blk_elem,elem_add_obj)
  set force_addr_list $event_element::($active_evt_elem,force_addr_list)
  set indx [lsearch $force_addr_list $addr_obj]
  if { $popupvar(2) } {
   if { $indx == -1 } {
    lappend event_element::($active_evt_elem,force_addr_list) $addr_obj
   }
   } else {
   if { $indx != -1 } {
    set event_element::($active_evt_elem,force_addr_list) \
    [lreplace $force_addr_list $indx $indx]
   }
  }
  if { ![info exists Page::($page_obj,in_focus_evt_elem)] } {
   set temp_flag 1
   set Page::($page_obj,in_focus_evt_elem) $active_evt_elem
   } else {
   set temp_flag 0
  }
  UI_PB_blk_ReplaceIcon page_obj $base_addr $active_blk_elem
  if { $temp_flag } {
   unset Page::($page_obj,in_focus_evt_elem)
  }
  global gPB
  if { [info exists gPB(DisableEnterCB)] } { unset gPB(DisableEnterCB) }
 }
...
proc UI_PB_blk_ReplaceIcon { PAGE_OBJ base_addr blk_elem_obj } {
  ...
  $block_element::($blk_elem_obj,elem_add_obj) == "Macro" } \
  {
   set blk_elem_addr $block_element::($blk_elem_obj,elem_add_obj)
  } else \
  {
  ...
  UI_PB_com_RetImageAppdText blk_elem_addr blk_elem_mom_var \
  img_name blk_elem_text
  if { [string match "Comment" $block_element::($blk_elem_obj,elem_add_obj)] || \
   [string match "Command" $block_element::($blk_elem_obj,elem_add_obj)] || \
  [string match "Macro" $block_element::($blk_elem_obj,elem_add_obj)] } \
  {
   if { [string match "Comment" $block_element::($blk_elem_obj,elem_add_obj)] } {
    if { $::gPB(comment_is_tcl_selected) || $::gPB(comment_is_tcl) } {}
    if { $::gPB(comment_is_tcl) } {
     set blk_elem_text "# $blk_elem_mom_var"
    }
    } elseif { [string match "Command" $block_element::($blk_elem_obj,elem_add_obj)] } {
    if { [string match "$::gPB(condition_cmd_prefix)*" $command::($blk_elem_mom_var,name)] } {
     set blk_elem_text "IF($command::($blk_elem_mom_var,name))"
    }
   }
   if { [string match "Macro" $block_element::($blk_elem_obj,elem_add_obj)] } \
   {
    UI_PB_blk_AdjustMacroText blk_elem_text $blk_elem_obj
   }
   UI_PB_blk_AdjustLayoutRectForIcon page_obj blk_elem_text $blk_elem_obj
  }
  ...
  if { $evt_elem_obj > 0  &&  [info exists event_element::($evt_elem_obj,force_addr_list)] } {
   if { [lsearch $event_element::($evt_elem_obj,force_addr_list) \
   $block_element::($blk_elem_obj,elem_add_obj)] >= 0 } \
   {
    append opt_img _f
   }
...
proc UI_PB_blk_AddrFocusOn { page_obj x y } {
  ...
  if { $Page::($page_obj,in_focus_elem) != $Page::($page_obj,out_focus_elem) } \
  {
...
proc UI_PB_blk_CreateBalloon { PAGE_OBJ } {
  ...
  {
   if 0 {     set cmd_obj $block_element::($in_focus_elem,elem_mom_variable)
   ...
   } elseif { $blk_elmt_type == "Comment" } \
  {
   ...
   UI_PB_blk_DeleteBalloon page_obj
   return
   } elseif { $blk_elmt_type == "Macro" } \
  {
   set func_obj $block_element::($in_focus_elem,elem_mom_variable)
   function::GetDisplayText $func_obj blk_elem_text
   if { [info exists block_element::($in_focus_elem,func_prefix)] && \
   $block_element::($in_focus_elem,func_prefix) != "" } \
   {
    set temp_str $block_element::($in_focus_elem,func_prefix)
    append temp_str " " $blk_elem_text
    set blk_elem_text $temp_str
    unset temp_str
   }
  } else \
  {
   ...
   [string match "rapid*" $add_name] } \
   {
    if { [string match "rapid1" $add_name] } \
    {
     set add_name "X"
     } elseif { [string match "rapid2" $add_name] } \
    {
     set add_name "Y"
     } elseif { [string match "rapid3" $add_name] } \
    {
     set add_name "Z"
    }
   }
  ...
  if { $blk_elmt_type == "Command"  || \
   ...
   if { [string match "$::gPB(tcl_line_blk)*" $block::($blk_obj,block_name)] } {
    set elem_desc "$::gPB(tcl_label)"
   }
   if { $blk_elmt_type == "Macro" } {
    set elem_desc $function::($func_obj,description)
   }
   if { $blk_elem_text == "" } {
    append bal_desc "( $elem_desc )"
    } else {
...
proc UI_PB_blk_StartDragAddr { page_obj x y } {
  ...
  if { [string compare "$cur_img_tag" "movable"] == 0 } \
  {
   ...
   if { [info exists Page::($page_obj,seq_page_active_evt_elem)] } {
    set evt_elem_obj $Page::($page_obj,seq_page_active_evt_elem)
    if { [lsearch $event_element::($evt_elem_obj,force_addr_list) \
     $block_element::($focus_blk,elem_add_obj)] >= 0 } {
     append opt_nows_force _f
    }
   }
   if { 0 && [expr {$block_element::($focus_blk,force) == 1}] } {
    append opt_nows_force  _f
   }
...
proc UI_PB_blk_EndDragAddr {page_obj} {
  ...
  $c bind movable <Enter> "UI_PB_blk_AddrFocusOn $page_obj %x %y"
...
proc UI_PB_blk_DeleteBlockElement { page_obj active_blk_elem x y } {
  ...
  if { [info exists Page::($page_obj,event_obj)] } \
  {
   ...
   if { [info exists Page::($page_obj,in_focus_evt_elem)] } {
    ...
    set Page::($page_obj,source_evt_elem_obj) $Page::($page_obj,in_focus_evt_elem)
    set evt_elem $Page::($page_obj,source_evt_elem_obj)
    ...
    if { $block::($blk_obj,blk_type) == "normal" } {
     ...
     UI_PB_tpth_DeleteARow   page_obj event_obj evt_elem
    }
   }
...
proc UI_PB_blk_UpdateBlkElements { PAGE_OBJ BLOCK_OBJ SOURCE_CELL_NUM } {
  ...
  UI_PB_blk_UpdateForceOutputAttr page_obj block_obj normal
  UI_PB_blk_CreateBlockImages page_obj block_obj
  set block_owner $block::($block_obj,blk_owner)
...
proc UI_PB_blk_TrashConnectLine { PAGE_OBJ } {
  upvar $PAGE_OBJ page_obj
  if { [llength [$Page::($page_obj,top_canvas) gettags new_comp]] == 0 } { return }
  global paOption
  set vtx {}
  set panel_hi $Page::($page_obj,panel_hi)
  set x0 [lindex $Page::($page_obj,trash) 1]
  ...
  lappend vtx $x1 $y1
  lappend vtx [expr $x1 - 2] [expr $y1 - 2]
  ...
  } else \
  {
    ...
    eval { $top_canvas create poly } $vtx { -fill $paOption(focus) \
     -width 2 -outline $paOption(focus) \
    -tag connect_line}
   }
  }
...
proc UI_PB_blk_ItemFocusOff1 {page_obj} {
  global paOption
  set top_canvas $Page::($page_obj,top_canvas)
  if { [info exists Page::($page_obj,add_button_focus_on)] && \
  $Page::($page_obj,add_button_focus_on) } \
  {
   set im [$top_canvas itemconfigure current -image]
...
proc UI_PB_blk_ItemStartDrag1 { page_obj x y X Y } {
  ...
  if { $::gPB(comment_is_tcl_selected) } {
  ...
  if { [info exists ::gPB__new_elem_mom_var] } {
   ...
   unset ::gPB__comb_var
   unset ::gPB__base_addr
  }
  UI_PB_debug_ForceMsg "\n+++ UI_PB_blk_ItemStartDrag1 Page::($page_obj,new_elem_mom_var) >$Page::($page_obj,new_elem_mom_var)<\
  >$Page::($page_obj,comb_var)< >$Page::($page_obj,sel_base_addr)< +++\n"
  if 0 { ;# Retain selection for all components not just condition commands.
   set new_elem_mom_var $Page::($page_obj,new_elem_mom_var)
   if { [string match "IF" $new_elem_mom_var] ||\
    [string match "*$::gPB(condition_cmd_prefix)*" $new_elem_mom_var] } {}
  }
  if 1 {
   set ::gPB__new_elem_mom_var $Page::($page_obj,new_elem_mom_var)
   set ::gPB__comb_var         $Page::($page_obj,comb_var)
...
proc UI_PB_blk_DrawIconInAllCanvas { page_obj x y X Y } {
  ...
  {
   set image_name "pb_blank_addr"
   set app_text "$::gPB(block,oper_combo,Label)"
   if { $::gPB(comment_is_tcl) } {
    set app_text "# $::gPB(event,combo,tcl_line,Label)"
   }
   } elseif { [string compare $new_elem_addr "Command"]   == 0 } \
  {
   set image_name "pb_blank_addr"
   set app_text $new_elem_mom_var
   } elseif { [string compare $new_elem_addr "Condition"] == 0 } \
  {
   set image_name "pb_blank_addr"
   switch $new_elem_mom_var {
    IF {
     set app_text "IF(New Condition)"
    }
    ELSE -
    ...
    default {
     set app_text "IF($new_elem_mom_var)"
    }
   }
   set Page::($page_obj,sel_base_addr) Command
   } elseif { [string compare $new_elem_addr "Text"] == 0 } \
  {
   ...
   set no_chars 4  ;# Limited to "Text"
   UI_PB_com_TrimOrFillAppText app_text no_chars
   } elseif { [string compare $new_elem_addr "Macro"] == 0 } \
  {
   set image_name "pb_blank_addr"
   set app_text $new_elem_mom_var
   if { ![string compare $new_elem_mom_var ""] } { set app_text "Macro" }
  } else \
  {
   PB_int_RetAddrObjFromName new_elem_addr add_obj
   UI_PB_com_RetImageAppdText add_obj new_elem_mom_var image_name app_text
  }
  set dx 0
  set dy [expr $panel_hi + 2]
...
proc UI_PB_blk_HighlightRect { page_obj x y } {
  ...
  if { [info exists Page::($page_obj,rect_col)] == 1 } \
  {
   ...
   -outline $Page::($page_obj,rect_col)
  }
  ...
  $Page::($page_obj,rect_region)] } \
  {
   ...
   $bot_canvas itemconfigure $block::($block_obj,rect) -fill $high_color -outline $high_color
   set Page::($page_obj,active_blk_obj) $block_obj
...
proc UI_PB_blk_BlockConnectLine { PAGE_OBJ RECT_COORD } {
  ...
  set x1 [expr $x1 + 1]
  set y1 [expr [$bot_canvas canvasy $y1] + 1]
  lappend vtx $x1 $y1
  set x1 [expr $x1 - 2]
  set y1 [expr $y1 - 2]
  lappend vtx $x1 $y1
...
#=======================================================================
proc UI_PB_blk_CreateBlockPage { BLOCK_OBJ WIN NEW_BLK_PAGE blk_mode args } {
  upvar $BLOCK_OBJ block_obj
  upvar $WIN win
  ...
  if { [llength $args] > 0 } {
   set evt_elem_obj [lindex $args 0]
   if { $evt_elem_obj != "" && $evt_elem_obj != 0 } {
    set Page::($new_blk_page,seq_page_active_evt_elem) $evt_elem_obj
    set Page::($new_blk_page,rest_exec_cond_obj) $event_element::($evt_elem_obj,exec_condition_obj)
    set Page::($new_blk_page,rest_suppress_flag) $event_element::($evt_elem_obj,suppress_flag)
    set Page::($new_blk_page,rest_force_addr_list) $event_element::($evt_elem_obj,force_addr_list)
   }
  }
...
proc UI_PB_blk_CheckBlockName { BLOCK_NAME BLK_OBJ } {
  ...
  if { $block_name == "" } {
   return 2
   } elseif { ![UI_PB_com_ValidateObjectName $block_name] } {
   return 4
  }
  ...
  if 0 {
   if { [string match {[A-Z]*} $block_name] } {
    return 4
   }
  }
  ...
  if { [string match "$::gPB(comment_blk)*" $block_name] || [string match "$::gPB(tcl_line_blk)*" $block_name] } {
   return 3
  }
  PB_int_RetBlkObjList blk_obj_list
...
#=======================================================================
proc UI_PB_blk_EditOk_CB { page_obj seq_page_obj seq_obj elem_obj {block_obj 0} } {
  global tixOption
  global gPB
  global gPB_block_name
  if { $block_obj == 0 } {
  ...
  if { [info exists Page::($seq_page_obj,bot_canvas)] } {
...
#=======================================================================
proc UI_PB_blk_EditCancel_CB { page_obj seq_page_obj seq_obj elem_obj {block_obj 0} } {
  global paOption
  if { $block_obj == 0 } {
   set block_obj $event_element::($elem_obj,block_obj)
  }
...
proc UI_PB_blk_NewBlkOk_CB { page_obj seq_page_obj seq_obj evt_obj elem_obj } {
  ...
  $img config -relief raised -bg $paOption(special_fg)
  UI_PB_evt_CreateMenuOptions seq_page_obj seq_obj
  UI_PB_evt_CreateExecAttrSymbols seq_page_obj elem_obj
 }
...
#=======================================================================
proc UI_PB_blk_ModalityDialog { SEQ_EVT_PAGE_OBJ SEQ_EVT_OBJ ELEM_OBJ WIN } {
  upvar $SEQ_EVT_PAGE_OBJ seq_evt_page_obj
  upvar $SEQ_EVT_OBJ seq_evt_obj
  ...
  set block_obj $event_element::($elem_obj,block_obj)
  block::readvalue $block_obj blk_obj_attr
  set force_addr_list $event_element::($elem_obj,force_addr_list)
  set bot_frm [frame $win.bot]
  pack $bot_frm -side bottom -fill x
  set label_list {"gPB(nav_button,cancel,Label)" \
  "gPB(nav_button,ok,Label)"}
  set cb_arr(gPB(nav_button,cancel,Label)) \
  "UI_PB_blk_ModalityCancel_CB $win $seq_evt_page_obj \
  $seq_evt_obj $elem_obj"
  set cb_arr(gPB(nav_button,ok,Label)) \
  "UI_PB_blk_ModalityOk_CB $win $seq_evt_page_obj $seq_evt_obj \
  ...
  -width $paOption(trough_wd)
  pack $top_frm.scr -side top -fill both
  set addr_frm [$top_frm.scr subwidget window]
  if { [string match "event" $Page::($seq_evt_page_obj,page_type)] } \
  {
   set blk_elem_list $block::($block_obj,active_blk_elem_list)
   set event_flag 1
  } else \
  {
   set blk_elem_list $blk_obj_attr(2)
   set event_flag 0
  }
  set count 0
  foreach blk_elem_obj $blk_elem_list \
  {
   ...
   if { [lsearch $force_addr_list $add_obj] >= 0 } {
    set blk_modality($count) 1
    } else {
    set blk_modality($count) 0
   }
   set state "normal"
   if { $event_flag && \
    ! [UI_PB_blk_AllowForceOpt $seq_evt_page_obj $seq_evt_obj $block_obj \
   $blk_elem_obj $add_obj] } \
   {
    set state "disabled"
   }
   PB_com_MapMOMVariable mom_sys_arr add_obj blk_elem_obj_attr(1) app_text
   PB_int_ApplyFormatAppText add_obj app_text
   set inp_frm [frame $addr_frm.$count -relief solid -bd 2]
   ...
   -state $state -font $tixOption(bold_font) -relief flat -bd 2 -anchor w
...
#=======================================================================
proc UI_PB_blk_ModalityCancel_CB { win seq_evt_page_obj seq_evt_obj elem_obj} {
  global paOption
  destroy $win
  if { [string match "seq" $Page::($seq_evt_page_obj,page_type)] } {
   set block_obj $event_element::($elem_obj,block_obj)
   set bot_canvas $Page::($seq_evt_page_obj,bot_canvas)
   ...
   "UI_PB_evt_BlockFocusOff $seq_evt_page_obj $seq_evt_obj"
  }
 }

#=======================================================================
proc UI_PB_blk_ModalityOk_CB { win seq_evt_page_obj seq_evt_obj elem_obj \
  BLK_MODALITY } {
  global paOption
  ...
  foreach blk_elem_obj $blk_elem_list \
  {
   if { $blk_modality($count) } {
    lappend force_addr_list $block_element::($blk_elem_obj,elem_add_obj)
   }
   incr count
  }
  set old_force_addr_list $event_element::($elem_obj,force_addr_list)
  set event_element::($elem_obj,force_addr_list) $force_addr_list
  if { $event_flag } \
  {
   foreach blk_elem_obj $blk_elem_list \
   {
    ...
    set base_addr $address::($addr_obj,add_name)
    UI_PB_blk_ReplaceIcon seq_evt_page_obj $base_addr $blk_elem_obj
   }
  }
  destroy $win
  if { ! $event_flag } {
...
#=======================================================================
proc UI_PB_blk_AdjustMacroText { APP_TEXT blk_elem_obj } {
  upvar $APP_TEXT app_text
  if { [info exists block_element::($blk_elem_obj,func_prefix)] && \
  $block_element::($blk_elem_obj,func_prefix) != "" } \
  {
   set temp_str $block_element::($blk_elem_obj,func_prefix)
   append temp_str " " $app_text
   set app_text $temp_str
   set use_font $::gPB(nc_code_font)  ;# tixOption(fixed_font_sm)
   UI_PB_com_TrimOrFillAppTextForSpecial app_text use_font
  }
 }

#=======================================================================
proc UI_PB_blk_AdjustLayoutRectForIcon { PAGE_OBJ APP_TEXT blk_elem_obj } {
  upvar $PAGE_OBJ page_obj
  upvar $APP_TEXT app_text
  set font $::gPB(nc_code_font)  ;# tixOption(fixed_font_sm)
  set x0 $block_element::($blk_elem_obj,rect_corn_x0)
  set x1 $block_element::($blk_elem_obj,rect_corn_x1)
  set old_width [expr $x1 - $x0]
  set txt_width [font measure $font $app_text]
  set rect_width [expr $txt_width + 28]
  if { $old_width == $rect_width } { return }
  if { ![info exists Page::($page_obj,active_blk_obj)] } { return }
  set block_obj $Page::($page_obj,active_blk_obj)
  set h_cell $Page::($page_obj,h_cell)       ;# cell height
  set w_cell $rect_width                     ;# cell width
  set w_divi $Page::($page_obj,w_divi)       ;# divider width
  set cell_color paleturquoise
  set divi_color turquoise
  set main_cell_color $::paOption(can_bg)
  ...
  if 0 {
   set x0 $x_orig
   set y0 $y_orig
   set x1 [expr $x0 + $w_cell + [expr 4 * $w_divi]]
   set y1 [expr $y0 + $h_cell + [expr 4 * $w_divi]]
  }
  set x0 [expr $x_orig - 3]
  set y0 $y_orig
  set x1 [expr $x0 + $w_cell + [expr 4 * $w_divi] + 6]
  set y1 [expr $y0 + $h_cell + [expr 4 * $w_divi]]
  set rect_color $divi_color
  set block::($block_obj,rect) [UI_PB_com_CreateRectangle $page_obj $x0 $y0 $x1 $y1 \
  $rect_color $main_cell_color "" "whole_block"]
  ...
  set x1 [expr $x0 + $w_cell]
  set block_element::($blk_elem_obj,rect_corn_x1) $x1
  set block_element::($blk_elem_obj,xc) [expr [expr $x0 + $x1] / 2]
  $c delete $block_element::($blk_elem_obj,rect)
  set block_element::($blk_elem_obj,rect) [UI_PB_com_CreateRectangle $page_obj \
  $x0 $y0 $x1 $y1 $cell_color $divi_color "" "stationary"]
  set x0 $x1
...
#=======================================================================
proc UI_PB_blk_AllowForceOpt { page_obj evt_obj blk_obj blk_elem_obj add_obj } {
  set force_flag 1
  set bot_canvas $Page::($page_obj,bot_canvas)
  set icon_id $block_element::($blk_elem_obj,icon_id)
  set im [$bot_canvas itemconfigure $icon_id -image]
  set icon_tag [lindex [$bot_canvas itemconfigure $icon_id -tags] end]
  if { [string match "nonmovable" $icon_tag] } {
   set blk_owner $block::($blk_obj,blk_owner)
   PB_int_RetCycleComAndSharedEvts cycle_com_evt cycle_shared_evts
   if { ! [string match "$cycle_com_evt" "$blk_owner"] || \
    ! [string match "$cycle_com_evt" $event::($evt_obj,event_name)] } {
    set force_flag 0
   }
   if { [string match "post" "$blk_owner"] && \
    [string match "$cycle_com_evt" $event::($event_obj,event_name)] } {
    set force_flag 1
   }
  }
  if { [string match "rapid*" $address::($add_obj,add_name)] } {
   set force_flag 1
  }
  return $force_flag
 }
...
#=======================================================================
proc UI_PB_blk_UpdateForceOutputAttr { PAGE_OBJ BLOCK_OBJ action } {
  upvar $PAGE_OBJ page_obj
  upvar $BLOCK_OBJ block_obj
  if { ![info exists Page::($page_obj,seq_page_active_evt_elem)] } {
   return
  ...
  set evt_elem_obj $Page::($page_obj,seq_page_active_evt_elem)
  if { $event_element::($evt_elem_obj,block_obj) != $block_obj } {
   return
  }
  UI_PB_tpth_DeleteExecAttrSymbols page_obj block_obj
  set active_blk_elem_list $block::($block_obj,active_blk_elem_list)
  if { $active_blk_elem_list == "" } {
   set event_element::($evt_elem_obj,force_addr_list) ""
   set event_element::($evt_elem_obj,exec_condition_obj) 0
   set event_element::($evt_elem_obj,suppress_flag) 0
   return
  }
  if { [string match "default" $action] } {
   array set evt_elem_obj_attr $event_element::($evt_elem_obj,def_value)
   set event_element::($evt_elem_obj,exec_condition_obj) $evt_elem_obj_attr(2)
   set event_element::($evt_elem_obj,suppress_flag) $evt_elem_obj_attr(3)
   set force_addr_list $evt_elem_obj_attr(4)
   } elseif { [string match "restore" $action] } {
   set event_element::($evt_elem_obj,exec_condition_obj) $Page::($page_obj,rest_exec_cond_obj)
   set event_element::($evt_elem_obj,suppress_flag) $Page::($page_obj,rest_suppress_flag)
   set force_addr_list $Page::($page_obj,rest_force_addr_list)
   } else {
   set force_addr_list $event_element::($evt_elem_obj,force_addr_list)
  }
  set active_force_addr_list ""
  foreach blk_elem $active_blk_elem_list {
   set addr_obj $block_element::($blk_elem,elem_add_obj)
   if { [lsearch $force_addr_list $addr_obj] != -1 } {
    lappend active_force_addr_list $addr_obj
   }
  }
  set event_element::($evt_elem_obj,force_addr_list) $active_force_addr_list
 }
