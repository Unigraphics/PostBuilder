...
proc UI_PB_blk_CutABlock { page_obj } {
  ...
  {
   delete $Page::($page_obj,buff_obj_attr)
...
proc UI_PB_blk_SelectElement { page_obj focus_blk_elem } {
  ...
  UI_PB_blk_ConfigureLeader page_obj focus_blk_elem
 }
...
#=======================================================================
proc __UI_PB_mseq_PopupSetOptions { page_obj X Y x y } {
  global gPB
  ...
  if {$popup_flag == 0} \
  {
   ...
   {
    $menu add command -label "$gPB(wseq,popup_delete,Label)" -state disabled
   }
   set Page::($page_obj,popup_flag) 0
  }
  tk_popup $menu $X $Y
...
proc UI_PB_blk_EditAddress { blk_page_obj act_blk_obj active_blk_elem } {
  ...
  } else \
  {
   ...
   if { $add_name == "Text" } \
   {
    ...
    UI_PB_com_CreateTextEntry blk_page_obj active_blk_elem Text
...
#=======================================================================
proc UI_PB_blk_BringUpBlkElemPage { blk_page_obj active_blk_elem } {
...
proc UI_PB_blk_CreateBalloon { PAGE_OBJ } {
  ...
  if { $blk_elmt_type == "Command"  || \
   ...
   append desc $blk_elem_text " - " $elem_desc
   } else {
   append desc $add_name " - " $elem_desc
  }
  global gPB_help_tips
...
proc UI_PB_blk_DeleteBlockElement { page_obj active_blk_elem x y } {
  ...
  if { [info exists gPB(animation)]  &&  $gPB(animation) } {
   __blk_AnimateBlockElem $page_obj $active_blk_elem
   if [info exists gPB(DisableEnterCB)] { unset gPB(DisableEnterCB) }
   return
  }
  if { [info exists Page::($page_obj,event_obj)] } \
...
#=======================================================================
proc __blk_AnimateBlockElem { page_obj blk_elem_obj } {
  global gPB
  set c $Page::($page_obj,bot_canvas)
  $c xview moveto 0
  $c yview moveto 0
  ...
  {
   UI_PB_tpth_BlkFocusOn   $page_obj $evt_obj $x $y
