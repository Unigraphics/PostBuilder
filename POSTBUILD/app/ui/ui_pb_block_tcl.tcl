...
UI_PB_AddPatchMsg "2007.0.0" "<09 NOV 2011>\t Display main page of NX manual for Post Builder" ;# ui_pb_resource.tcl
...
proc UI_PB_blk_CombSelection { page_obj base_addr index} {
   ...
   } elseif { [string compare $base_addr "Macro"] == 0 } \
  {
   ...
   } else \
   {
    ...
    set app_text ""
    set word_desc "New Custom Macro"
    set add_leader ""
   }
  } else \
  {
  ...
  if { $page_type == "event" } \
  {
   ...
   for {set count 0} {$count < [llength $cmd_proc_list]} {incr count} \
   {
    ...
    if { [string match "PB_CMD_before_motion" $cmd_name] || \
     ...
     [string match "$gPB(condition_cmd_prefix)*" $cmd_name] } { continue }
...
proc UI_PB_blk_CreateBlockCells { PAGE_OBJ BLOCK_OBJ ACTIVE_BLK_ELEM_OBJ \
  ...
  if { $n_comp == 1 } \
  {
   ...
   [string match "Macro" $blk_elem_addr] } \
   {
    ...
    set use_font $::tixOption(fixed_font_sm)
...
proc UI_PB_blk_ReplaceIcon { PAGE_OBJ base_addr blk_elem_obj } {
  ...
  if { [info exists Page::($page_obj,in_focus_evt_elem)] } \
  {
   ...
   if { [lsearch $force_addr_list $block_element::($blk_elem_obj,elem_add_obj)] >= 0 } \
   {
    append opt_img _f
   }
...
proc UI_PB_blk_AdjustLayoutRectForIcon { PAGE_OBJ APP_TEXT blk_elem_obj } {
  ...
  set txt_width [font measure $::tixOption(fixed_font_sm) $app_text]
