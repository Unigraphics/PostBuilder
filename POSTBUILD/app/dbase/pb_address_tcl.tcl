#25
UI_PB_AddPatchMsg "2005.0.0" "<07-09-08> Fixed problem with creating new Word"

#=======================================================================
proc PB_adr_AdrInitParse__Jan-10-2018 { this POST_OBJ OBJ_LIST FOBJ_LIST } {
  ...
  {
   PB_int_RetAddrNameList exist_addr_name_list
   if ![string match "" $exist_addr_name_list] \
   {
    set no_addr_flag 1 ;# Need to skip some addresses
   }
  }
  if { [info exists gPB(unit_sub_post_last_def)] && $gPB(unit_sub_post_last_def) } {
   if { ![string compare "Inches" $gPB(main_post_unit)] } {
    set postfix "_IN"
    } else {
    set postfix "_MM"
   }
   array set tmp_add_arr $Post::($post_obj,Indep_add_arr)
   foreach item [array names tmp_add_arr] {
    if { [string match "*,data" $item] } {
     set tmp_add_name "[lindex [lindex $tmp_add_arr($item) 0] 1]$postfix"
     set tmp_add_data [lreplace $tmp_add_arr($item) 0 0 "  ADDRESS $tmp_add_name "]
     set address_arr($item) $tmp_add_data
     } else {
     set address_arr($item) $tmp_add_arr($item)
    }
   }
   set tmp_f_name [lindex $Post::($post_obj,Indep_add_arr) 0]
   set tmp_f_name [string range $tmp_f_name 0 [expr [string first "," $tmp_f_name] - 1]]
   set arr_size($tmp_f_name,address) [llength $Post::($post_obj,Indep_add_list)]
   lappend file_name_list $tmp_f_name
   unset tmp_add_arr
  }
  if 0 {
  ...
  UI_PB_debug_ForceMsg "\n ++++++ Parsing Address from files org list: $file_name_list\n"
  UI_PB_debug_ForceMsg "\n ++++++ Parsing Address from files: $file_list\n"
   ...
   for { set count 0 } { $count < $no_of_address } { incr count } \
   {
    ...
    foreach line $address_arr($f_name,$count,data) \
    {
     if { [string match "*ADDRESS*" $line] }\
     {
      ...
      if { $no_addr_flag && \
      [lsearch $exist_addr_name_list $obj_attr(0)] == -1 } \
      {
       break
      }
     ...
     if { $add_start } \
     {
      ...
      if { $open_flag } \
      {
        ...
        *ZERO_FORMAT* \
        {
         ...
         PB_com_RetObjFrmName zero_fmt fobj_list ret_code
         if { $ret_code } \
         {
        ...
        *FORMAT* {
         set fmt_name [lindex $line 1]
         PB_com_RetObjFrmName fmt_name fobj_list ret_code
         if [PB_file_is_JE_POST_DEV] {
          if { $ret_code == 0 } {
           set fmt_attr(0) "$fmt_name"
           set fmt_attr(1) "Text String"
           for { set ii 2 } { $ii < 8 } { incr ii } {
            set fmt_attr($ii) 0
           }
           set fmt_attr(8) 1  ;# is_dummy
           PB_fmt_CreateFmtObj fmt_attr fobj_list
           set ret_code [lindex $fobj_list end]
          }
         }
      ...
      if { $close_flag } \
      {
       ...
       UI_PB_debug_ForceMsg " +++++ New Address $obj_attr(0) : [array get obj_attr] +++++\n"
  ...
  foreach f_name $file_list \
  {
   ...
   for { set count 0 } { $count < $no_of_address } { incr count } \
   {
    ...
    foreach line $address_arr($f_name,$count,data) \
    {
     ...
     if { [string match "*ADDRESS *" $temp_line] }\
     {
      set add_start 1
      set add_leader_var ""
      PB_adr_InitAdrObj obj_attr
     ...
     if { $add_start } \
     {
      if { $open_flag == 0 } \
      {
       set open_flag [string match "\{*" $temp_line]
       }
       if { $close_flag == 0 } \
       {
       set close_flag [string match "*\}" $temp_line]
      }
      if { $open_flag } \
      {
       switch -glob $temp_line \
       {
        "ZERO_FORMAT *" \
        {
         set zero_fmt [lindex $temp_line 1]
         PB_com_RetObjFrmName zero_fmt fobj_list ret_code
         if { $ret_code } \
         {
          set obj_attr(12) $ret_code
        ...
        "FORMAT *" \
        {
         set fmt_name [lindex $temp_line 1]
         PB_com_RetObjFrmName fmt_name fobj_list ret_code
         if [PB_file_is_JE_POST_DEV] {
          if { $ret_code == 0 } {
           set fmt_attr(0) "$fmt_name"
        ...
        "FORCE *" \
        {
         set obj_attr(2) [lindex $temp_line 1]
         set obj_attr(3) 1
        }
        "MAX *" {
         set obj_attr(4) [lindex $temp_line 1]
         if { [lindex $temp_line 2] == "" } \
         {
          set obj_attr(5) "Truncate"
         } else \
         {
          set obj_attr(5) [lindex $temp_line 2]
         }
        }
        "MIN *" {
         set obj_attr(6) [lindex $temp_line 1]
         if { [lindex $temp_line 2] == "" } \
         {
          set obj_attr(7) "Truncate"
         } else \
         {
          set obj_attr(7) [lindex $temp_line 2]
         }
        }
        "LEADER *" \
        {
         PB_com_RemoveBlanks temp_line ;#<Jan-10-2018 gsl> This is needed for next command to work.
         PB_adr_ExtractLeader $post_obj temp_line leader add_leader_var
         set obj_attr(8) $leader
        }
        "TRAILER *" \
        {
         PB_com_RemoveBlanks temp_line
         PB_adr_ExtractLeader $post_obj temp_line trailer add_trailer_var
         set obj_attr(9) $trailer
         set obj_attr(10) 1
        }
        "INCREMENTAL *" \
        {
         set obj_attr(11) [lindex $temp_line 1]
        }
       }
      }
      if { $close_flag } \
...
proc PB_adr_CreateAdrObj { OBJ_ATTR OBJ_LIST } {
  ...
  upvar $OBJ_LIST obj_list
  global post_object
  if [catch { expr $obj_attr(1) + 1 }] {
   set fmt_obj_list $Post::($post_object,fmt_obj_list)
   PB_com_RetObjFrmName obj_attr(1) fmt_obj_list ret_code
   set obj_attr(1) $ret_code
  }
  if { [string trim $obj_attr(1)] == "" } {
   UI_PB_debug_ForceMsg " ++++++ No Format specified for Address: $obj_attr(0)  fmt_obj: $obj_attr(1)"
   return
  }
  ...
  UI_PB_debug_ForceMsg " ++++++ Parsing Address: $obj_attr(0): $object  fmt_obj: $obj_attr(1)"
  for { set i 0 } { $i < 13 } { incr i } {
   if ![info exists obj_attr($i)] {
    set obj_attr($i) ""
   }
  }
  PB_mthd_SetObjExtAttr obj_attr "address"
  address::setvalue $object obj_attr
  address::readvalue $object obj_attr
  address::DefaultValue $object obj_attr
  PB_mthd_CatalogPostExtObj obj_attr "address"
  set fmt_obj $obj_attr(1)
  if { [string length $fmt_obj] > 0 } {
