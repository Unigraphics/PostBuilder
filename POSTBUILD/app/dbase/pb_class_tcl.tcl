...
proc ~Post {this} {
  ...
  if [info exists Post::($this,def_parse_obj)] \
  {
   eval delete $Post::($this,def_parse_obj)
  }
...
proc ~block {this} {
  ...
  {
   if { [info exists block_element::($element,parent_name)] } \
   {
    delete $element
   }
  }
  ...
  foreach def_elem $obj_attr(2) \
  {
   ...
   {
    if { [info exists block_element::($def_elem,parent_name)] } \
    {
     delete $def_elem
    }
   }
  ...
  if { $block::($this,active_blk_elem_list) != "" } \
  {
   foreach act_elem $block::($this,active_blk_elem_list) \
   {
    ...
    {
     if { [info exists block_element::($act_elem,parent_name)] } \
     {
      delete $act_elem
     }
    }
