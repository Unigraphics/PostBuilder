...
proc PB_evt_CreateSeqEvents { POST_OBJ EVT_OBJ_LIST EVT_LIST_NAME EVT_LIST_LABEL \
  ...
  if [string match $sequence_name "tpth_ctrl"] {
   if [info exists Post::($post_obj,$evt_ude_list_name)] {
  ...
  for { set sz 0 } { $sz < $arr_sz } { incr sz } \
  {
   ...
   if { [string match "User Defined Axis Limit Action" $evt_obj_attr(0)] } {
    set Post::($post_obj,axis_limit_evt_obj) [lindex $evt_obj_list end]
   }
...
proc PB_evt_RetCycleEventElements { EVT_BLK_LIST POST_OBJ EVENT_NAME \
  ...
  upvar $EVT_BLK_LIST_NAME evt_blk_list_name
  global mom_sys_arr ;#<06-08-09 wbh>
  UI_PB_debug_ForceMsg " +++++ [info level 0] => 0 Event name: $event_name"
  ...
  if { $com_blank_flag || [UI_PB_tpth_IsCycleCustom $event_name] } {
   ...
   foreach blk_name $evt_blk_list \
   {
    ...
    UI_PB_debug_ForceMsg " +++++ [info level 0] => 1 Event name: $event_name  elem_index = $elem_index"
  ...
  set elem_index 0
  UI_PB_debug_ForceMsg "\n +++++ com_evt_blk_list = $com_evt_blk_list \n"
  foreach com_blk $com_evt_blk_list \
  {
   ...
   if { $com_ret == 0 } \
   {
    set evt_obj_attr(0) $com_blk  ;# $event_name  ;#<Jan-23-2018 gsl> Didn't seem to matter - $com_blk
    set evt_obj_attr(1) $com_blk_obj
    UI_PB_debug_ForceMsg " +++++ 2 Event name: $event_name  elem_index = $elem_index"
  ...
  if { $added_own_blks_flag == 0 } \
  {
   ...
   foreach blk_name $evt_blk_list \
   {
    ...
    set evt_obj_attr(0) $blk_name
    set evt_obj_attr(1) $blk_obj
    UI_PB_debug_ForceMsg " +++++ 4 Event name: $event_name  elem_index = $elem_index"
    PB_evt_SetEvtElemExecutionAttr event_name evt_obj_attr $elem_index
    if { $func_suppress_flag } \
    {
     set evt_obj_attr(3) 1
   ...
   } elseif { [string compare $event_name "Rapid Move"] == 0 } \
  {
   ...
   foreach blk_row $evt_blk_list\
   {
    foreach row_elem $blk_row\
    {
     ...
     set evt_elem_obj_attr(0) $block::($blk_obj,block_name)
     set evt_elem_obj_attr(1) $blk_obj
...
proc PB_evt_CreateRapidToBlock { EVENT_OBJ EVT_ELEM_LIST MOM_SYS_VAR } {
  ...
  if { $mom_sys_var(\$cycle_start_blk) } \
  {
   UI_PB_debug_ForceMsg "\n+++++ evt_elem_list before : >$evt_elem_list< +++++\n"
   ...
   foreach evt_elem_obj $evt_elem_list \
   {
    ...
    if { $ret } \
    {
     ...
     UI_PB_debug_ForceMsg "\n^^^^^ Cycle Rapid To block constructed added above cycle startblk. ^^^^^\n"
    }
    lappend act_evt_elem_list $evt_elem_obj
   }
   set evt_elem_list $act_evt_elem_list
   UI_PB_debug_ForceMsg "\n+++++ evt_elem_list after  : >$evt_elem_list< +++++\n"
  } else \
  {
   if 0 {
    foreach evt_elem_obj $evt_elem_list \
    ...
    {
     set block_obj $event_element::($evt_elem_obj,block_obj)
     set ret [PB_evt_CheckCycleRefWord block_obj 1]
     if { $ret } \
     {
      foreach new_elem $new_elem_list \
      {
       set evt_elem_list [linsert $evt_elem_list $idx $new_elem]
       incr idx
      }
      break
     }
...
#=======================================================================
proc __ListEvtElemNames { evt_elem_list } {
  set blk_names_list [list]
  foreach evt_elem $evt_elem_list \
  {
   ...
   if [info exists event_element::($evt_elem,block_obj)] {
    ...
    if [info exist block::($blk_obj,block_name)] {
     set blk_name "Block $block::($blk_obj,block_name)"
     } else {
     set blk_name "Evt Elem $event_element::($evt_elem,evt_elem_name)"
    }
   }
   if { $blk_name == " " } {
    set blk_name "Evt Elem $event_element::($evt_elem,evt_elem_name)"
   }
   lappend blk_names_list $blk_name
  }
  return "$blk_names_list"
 }
...
proc PB_evt_CreateCycleStartBlock { EVENT_OBJ EVT_ELEM_LIST MOM_SYS_VAR \
  ...
  UI_PB_debug_ForceMsg "\n @@@@@ <1> <$strt_blk_indx> - Elements in event $event::($event_obj,event_name) \n [__ListEvtElemNames $evt_elem_list]\n"
  set __check_mcall 0
  if { [PB_com_IsSiemensController    $::gPB(post_controller_type) 1] ||\
   [PB_com_IsHeidenhainController $::gPB(post_controller_type)] } {
   set __check_mcall 1
  ...
  switch $mom_sys_var(\$cycle_start_blk) \
  {
   ...
   1   {
    ...
    if 1 {
     foreach cyc_elem $evt_elem_list \
     {
      set blk_obj $event_element::($cyc_elem,block_obj)
      UI_PB_debug_ForceMsg "\n +++++ Block in event $event::($event_obj,event_name) $block::($blk_obj,block_name) +++\n"
      if { [string match "just_mcall_sinumerik" $block::($blk_obj,block_name)] } {
       set post_blk_name "PB_CMD_config_cycle_start"
       break
      }
     }
    }
    ...
    if [PB_com_IsSiemensController $gPB(post_controller_type) 1] \
    {
     if { [string match "startblk" $post_blk_name] } {
      ...
      set elem_address ""
      set elem_mom_var ""
      lappend elem_mom_var "PB_CMD_config_cycle_start"
      set elem_owner {"post"}
     }
    } else \
    {
    ...
    $Post::($::post_object,rst_post_startblk) != "" } \
    {
     ...
     UI_PB_debug_ForceMsg "\n +++++ rst_post_startblk = rst_blk_name  temp_flag = $temp_flag +++\n"
     if $temp_flag \
     {
  ...
  foreach cyc_set_elem $evt_elem_list \
  {
   ...
   if { $__check_mcall } {
    set com_ret [PB_evt_CheckCycleRefWord block_obj 1 MCALL]
    } else {
  ...
  if { [info exists com_ret] && $com_ret } \
  {
   ...
   UI_PB_debug_ForceMsg "\n @@@@@ <2.a> <$strt_blk_indx> - Elements in event $event::($event_obj,event_name) \n [__ListEvtElemNames $evt_elem_list]\n"
  } else \
  {
   if { !$mom_sys_var(\$cycle_start_blk) } \
   {
   ...
   UI_PB_debug_ForceMsg "\n @@@@@ <2.b> <$strt_blk_indx> - Elements in event $event::($event_obj,event_name) \n [__ListEvtElemNames $evt_elem_list]\n"
  ...
  if { $mom_sys_var(\$cycle_start_blk) } \
  {
   if 1 { ;#<Apr-05-2018 gsl> Should we undo everything???
    set evt_elem_list [linsert $evt_elem_list $set_indx $new_elem_list]
    UI_PB_debug_ForceMsg "\n @@@@@ <3.a> <$strt_blk_indx> - Elements in event $event::($event_obj,event_name) \n [__ListEvtElemNames $evt_elem_list]\n"
    if { [info exists com_ret] && $com_ret } \
    {
     set rapid_indx 0
     foreach evt_elem $evt_elem_list \
     {
      ...
      if { [info exists block::($block_obj,block_name)] && \
       [string match "post_rapidto" $block::($block_obj,block_name)] } {
       set set_indx $rapid_indx
       break
      }
      incr rapid_indx
     }
     if 0 {
      if { $strt_blk_indx < $set_indx } { set strt_blk_indx $set_indx }
      set evt_elem_list [linsert $evt_elem_list $strt_blk_indx $cyc_set_elem]
     }
     set evt_elem_list [linsert $evt_elem_list $set_indx $cyc_set_elem]
     UI_PB_debug_ForceMsg "\n @@@@@ <3.b> <$strt_blk_indx> - Elements in event $event::($event_obj,event_name) \n [__ListEvtElemNames $evt_elem_list]\n"
     UI_PB_debug_ForceMsg "\n +++++ 1. cycle set blk inserted to strt_blk_indx $strt_blk_indx +++\n"
    }
    } else { ;# if 0
    if { [info exists com_ret] && $com_ret } \
    {
     set evt_elem_list [linsert $evt_elem_list $set_indx $cyc_set_elem]
     UI_PB_debug_ForceMsg "\n +++++ 1. cycle set blk inserted to set_indx $set_indx +++\n"
    }
    if 0 {
     if { [PB_com_IsSiemensController $::gPB(post_controller_type) 1] || \
     [PB_com_IsHeidenhainController $::gPB(post_controller_type)] } \
     ...
     {
      lappend evt_elem_list $new_elem_list
     }
     } else {
     set evt_elem_list [linsert $evt_elem_list $set_indx $new_elem_list]
    }
  ...
  } else \
  {
   if 1 {
    ...
    set evt_elem_list [linsert $evt_elem_list $strt_blk_indx $cyc_set_elem]
    UI_PB_debug_ForceMsg "\n @@@@@ <3.c> <$strt_blk_indx> - Elements in event $event::($event_obj,event_name) \n [__ListEvtElemNames $evt_elem_list]\n"
    UI_PB_debug_ForceMsg "\n +++++ 2. cycle set blk inserted back to $strt_blk_indx (where start blk was) +++\n"
    } else {
    set evt_elem_list [linsert $evt_elem_list $set_indx $cyc_set_elem]
    UI_PB_debug_ForceMsg "\n +++++ 2. cycle set blk inserted back to $set_indx +++\n"
   }
  }
  foreach evt_elem $evt_elem_list \
  {
...
proc PB_evt_SetEvtElemExecutionAttr { EVENT_NAME ELEM_ATTR elem_index } {
  ...
  UI_PB_debug_ForceMsg "\n +++++ Execution events: $event_name  blk_obj = $blk_obj\n"
  if { $blk_obj <= 0 } {
   return
  }
  ...
  UI_PB_debug_ForceMsg "\n +++++ Execution events: $event_name $index => elem_attr_list = $elem_attr_list\n"
  foreach elem_attr $elem_attr_list {
   set id [lindex $elem_attr 0]
   UI_PB_debug_ForceMsg "\n +++++ Execution events: $event_name  id = $id  elem_index = $elem_index\n"
   if { $id == $elem_index } {
    set cond_name [lindex $elem_attr 1]
    UI_PB_debug_ForceMsg "\n +++++ Execution events: $event_name  id = $id  elem_index = $elem_index  cond_name = $cond_name\n"
