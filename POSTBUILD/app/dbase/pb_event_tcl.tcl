...
proc PB_evt_SetEvtElemExecutionAttr { EVENT_NAME ELEM_ATTR elem_index } {
  ...
  foreach elem_attr $elem_attr_list {
   ...
   if { $id == $elem_index } {
    ...
    if { [string match "$gPB(condition_cmd_prefix)*" $cond_name] } {
    ...
    set elem_obj_attr(3) [lindex $elem_attr 2]
    if { [llength $elem_attr] >= 4 } {
     ...
     UI_PB_debug_ForceMsg "   +++   event_name: $event_name  forced words: $addr_name_list { $addr_obj_list }"
     set elem_obj_attr(4) $addr_obj_list
    }
    return
   }
