...
#=======================================================================
proc PB_output_GetBlkObjAttr__Jul-06-2017 { BLK_OBJ_LIST ADD_OBJ_LIST BLK_NAME_ARR \
  BLK_VALUE_ARR} {
  ...
  foreach block_obj $blk_obj_list \
  {
   ...
   if { 1 && [string match "rapid_*" $blk_obj_attr(0)] } {
    UI_PB_debug_ForceMsg "\n\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
    foreach blk_elm $blk_obj_attr(2) {
     set add_obj $block_element::($blk_elm,elem_add_obj)
     UI_PB_debug_ForceMsg "\n >>>>>>  block_element : >$address::($add_obj,add_name)< of $block_element::($blk_elm,parent_name) \n"
    }
    UI_PB_debug_ForceMsg ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
   }
   PB_blk_RetBlkFrmBlkAttr blk_obj_attr blk_value_list
   set blk_value_arr($indx) $blk_value_list
   UI_PB_debug_ForceMsg "\n ++++++ [info level 0] $blk_obj_attr(0): $blk_value_arr($indx)\n"
...
proc PB_output_GetMomSysVars { MOM_SYS_NAME_ARR MOM_SYS_VAL_ARR } {
  ...
  if { [lsearch $word_mom_var_list($add_name) "\$mom_sys_advanced_turbo_output"] < 0 } {
   lappend word_mom_var_list($add_name) "\$mom_sys_advanced_turbo_output"
   if { ![info exists mom_sys_var_arr(\$mom_sys_advanced_turbo_output)] } {
    set mom_sys_var_arr(\$mom_sys_advanced_turbo_output) 0
   }
  }
...
#=======================================================================
proc PB_output_EscapeSpecialControlChar_no_dollar_no_quote { s1 } {
  set s2 ""
  for { set i 0 } { $i < [string length $s1] } { incr i } {
   set c [string index $s1 $i]
   if { ![string compare $c "\\"]  || \
...
proc PB_output_GetListFileVars { LIST_NAME_ARR LIST_VAL_ARR } {
  ...
  incr idx
  set list_name_arr($idx) "mom_sys_output_transition_path"
  set list_val_arr($idx)  "0"
  if { [info exists ListingFile::($listfile_obj,tran_path)] } \
  {
   if { $ListingFile::($listfile_obj,tran_path) } \
   {
    set list_val_arr($idx)  "1"
   }
  }
  incr idx
  set list_name_arr($idx) "mom_sys_post_output_subprogram_enabled"
  set list_val_arr($idx)  "0"
  if { [info exists ListingFile::($listfile_obj,subprog_out)] } \
  {
   if { $ListingFile::($listfile_obj,subprog_out) } \
   {
    set list_val_arr($idx)  "1"
   }
  }
...
proc PB_output_GetMomKinVars { KIN_NAME_ARR KIN_VAL_ARR } {
  ...
  foreach mom_kin_var $mom_kin_name_list\
  {
   ...
   if { [string match "mom_kin_5th_axis_limit_action" $kin_name_arr($ind)] } {
    set mom_kin_var_arr($mom_kin_var) $mom_kin_var_arr(\$mom_kin_4th_axis_limit_action)
   }
     ...
     if { $::env(PB_UDE_ENABLED) == 1 } {
      if { ![string compare $sequence::($seq_obj,seq_name) "Machine Control"] } {
       if [info exists event::($evt_obj,ude_event_obj)] {
        ...
        set post_name ""
        if { [info exists ude_event::($ueo,post_event)] } {
         set post_name $ude_event::($ueo,post_event)
        }
        if { ![string compare $post_name ""] } {
         if { [info exists ude_event::($ueo,name)] } {
...
proc PB_output_GetBlkName { BLOCK_OBJ OUTPUT_NAME } {
  ...
  } else \
  {
   ...
   {
    if { [string match "$::gPB(condition_cmd_prefix)*" $command::($cmd_elem,name)] } {
     set output_name "IF($command::($cmd_elem,name))"
     } else {
...
proc PB_PB2DEF_write_block_templates { FILE_ID BLK_NAME_ARR BLK_VAL_ARR } {
  ...
  for { set count 0 } { $count < $no_blks } { incr count } \
  {
   ...
   puts $file_id "  \{"
    if { [string match "$::gPB(comment_blk)*"  $blk_name_arr($count)] || \
     [string match "$::gPB(tcl_line_blk)*" $blk_name_arr($count)] } {
     if 0 {
      global post_object
      array set mom_sys_var_arr $Post::($post_object,mom_sys_var_list)
...
proc PB_PB2DEF_main { PARSER_OBJ OUTPUT_DEF_FILE } {
  ...
  puts $deff_id "\{"
   PB_output_GetWordSeperator $mom_sys_var(Word_Separator) word_sep
...
proc PB_PB2TCL_write_sys_var_arr { TCLF_ID } {
  ...
  puts $tclf_id "  set [format %-40s mom_sys_word_separator] \
  \"[PB_output_EscapeSpecialControlChar $mom_sys_arr(Word_Separator)]\""
  puts $tclf_id "  set [format %-40s mom_sys_end_of_block] \
  \"[PB_output_EscapeSpecialControlChar $mom_sys_arr(End_of_Block)]\""
...
proc MY_PB_output_GetMomFlyVars { fly_name_arr fly_val_arr } {
  ...
  if { [string compare $rapid_1 ""] } \
  {
   if [info exists block::($rapid_1,block_name)] \
   {
    ...
    set val(13) "rapid_traverse"
    Post::GetObjList $post_object block blk_obj_list
    PB_com_RetObjFrmName val(13) blk_obj_list blk_obj
    if { $blk_obj > 0 } {
     UI_PB_debug_ForceMsg "\n+++++> rapid_1 updated >$blk_obj< <+++++"
     set mom_sys_var_arr(\$pb_rapid_1) $blk_obj
     set rapid_1 $blk_obj
     set __update_sys_vars 1
    }
   }
  }
  ...
  if { [string compare $rapid_2 ""] } \
  {
   if [info exist block::($rapid_2,block_name)] \
   {
    ...
    if { [string match "TRUE" $val(12)] } {
     set val(14) "rapid_spindle"
     if { !$__update_sys_vars } {
      Post::GetObjList $post_object block blk_obj_list
     }
     PB_com_RetObjFrmName val(14) blk_obj_list blk_obj
     if { $blk_obj > 0 } {
      UI_PB_debug_ForceMsg "\n+++++> rapid_2 updated >$blk_obj< <+++++"
      set mom_sys_var_arr(\$pb_rapid_2) $blk_obj
      set rapid_2 $blk_obj
      set __update_sys_vars 1
     }
    }
  ...
  UI_PB_debug_ForceMsg "\n+++++> rapid_1 >$rapid_1< rapid_2 >$rapid_2< <+++++"
  if { $__update_sys_vars } {
   set Post::($post_object,mom_sys_var_list) [array get mom_sys_var_arr]
   array set ::mom_sys_arr [array get mom_sys_var_arr]
  }
 }
...
puts $tclf_id "proc MOM_start_of_program { } {"
  ...
  puts $tclf_id ""
  puts $tclf_id ""
  puts $tclf_id "  set ::mom_sys_start_program_clock_seconds \[clock seconds\]"
  puts $tclf_id ""
  if { [info exist ::gPB(PB_POST_DEBUG)] &&\
   [string length [string trim $::gPB(PB_POST_DEBUG)]] > 0 } {
 ...
 if { [info exists mom_sys_arr(\$mom_sys_mill_turn_type)] && \
  ...
  foreach link $linked_posts {
   ...
   if [string match "this_post" $post] {
    ...
    puts $tclf_id ""
    set post "\[file tail \$mom_sys_master_post\]"
   }
    ...
    if [info exists blk_val_arr($blk_idx)] {
     ...
     if $rapid_work_plane_change {
      ...
      if [string match $blk_name $blk] {
       ...
       if [llength $elem_list] {
        set blk_elem_arr(${blk_name}_xz) $elem_list
       }
      }
...
puts $tclf_id "proc MOM_before_motion \{ \} \{"
  ...
  if [__TurboPostOK] {
   puts $tclf_id ""
   puts $tclf_id "   if \[CMD_EXIST $::gPB(ATP,config_before_motion)\] {"
    puts $tclf_id "      $::gPB(ATP,config_before_motion)"
   puts $tclf_id "   }"
  }
...
puts $tclf_id "proc MOM_machine_mode { } {"
  ...
  puts $tclf_id "   }"
  puts $tclf_id ""
  puts $tclf_id "  # Reload post for simple mill-turn"
...
puts $tclf_id "proc PB_machine_mode { } {"
  ...
  puts $tclf_id "   if { \!\[string compare \$mom_machine_mode \"MILL\"\] } {"
   ...
   puts $tclf_id ""
   puts $tclf_id "      set tcl_file   \"\${cam_post_dir}\${mom_sys_lathe_postname}.tcl\""
   puts $tclf_id "      set def_file   \"\${cam_post_dir}\${mom_sys_lathe_postname}.def\""
   puts $tclf_id ""
...
#=======================================================================
proc __output_MOM_head_body__06-27-2019_backup { EVENT_OUTPUT } {
  upvar $EVENT_OUTPUT event_output
  lappend event_output "   global mom_warning_info"
  lappend event_output ""
  lappend event_output "   global mom_sys_in_operation"
...
proc PB_PB2TCL_write_tcl_procs { TCLF_ID } {
  ...
  PB_output_RetBlksModality blk_mod_arr
  set evt_name_list [lsort $evt_name_list]
  set useless_handler_list [list MOM_peck_drill MOM_drill_csink MOM_break_chip PB_feedrates]
...
lappend event_output "proc PB_user_def_axis_limit_action \{ args \} \{"
  ...
  if [info exists Post::($post_object,axis_limit_evt_obj)] \
  {
   ...
   if { $no_of_lists } \
   {
    __output_BlockOfEvent evt_name evt_blocks blk_mod_arr no_of_lists event_output evt_obj
   }
  }
...
#=======================================================================
lappend event_output "proc USER_DEF_AXIS_LIMIT_ACTION \{ args \} \{"
  lappend event_output "#============================================================="
  PB_output_CallDebugMsg S event_output
  global post_object
  if [info exists Post::($post_object,axis_limit_evt_obj)] \
...
#=======================================================================
proc __output_Turbo_MOM_do_template { event_name n_blank block_name BLK_MOD_ARR \
  ...
  if 0 {
   if { [string match "MOM_linear_move" $event_name] } {
    set output_buffer ::gPB(turbo_linear_handler)
    } elseif { [string match "MOM_circular_move" $event_name] } {
    set output_buffer ::gPB(turbo_circular_handler)
    } else {
    set output_buffer ::gPB(turbo_rapid_handler)
   }
  }
  set blank_string ""
  for { set i 0 } { $n_blank > 0 && $i < $n_blank } { incr i } {
   append blank_string " "
  }
  set template_string ""
  if [llength $args] {
  ...
  if { [string match "$::gPB(comment_blk)*"  $block_name] } {
   Post::GetObjList $::post_object comment cmt_blk_obj_list
   PB_com_RetObjFrmName block_name cmt_blk_obj_list cmt_blk_obj
   if { $cmt_blk_obj > 0 } {
    __output_Turbo_OperatorMsg $event_name $cmt_blk_obj $blank_string
   }
   } elseif { [string match "$::gPB(tcl_line_blk)*" $block_name] } {
   return
   } else {
   set line_text "$block_name"
   __output_Turbo_ExecAttr $event_name line_text blank_string cond_cmd suppress_flag force_name_list
  }
 }

#=======================================================================
proc __output_Turbo_OperatorMsg { evt_name cmt_blk_obj indent } {
  set leader $indent
  switch $evt_name {
   MOM_linear_move {
    set evt turbo_linear
    set sys_var ::mom_sys_linear_turbo_blocks
   }
   MOM_circular_move {
    set evt turbo_circular
    set sys_var ::mom_sys_circular_turbo_blocks
   }
   MOM_rapid_move {
    set evt turbo_rapid
    set sys_var ::mom_sys_rapid_turbo_blocks
   }
   default {
    return
   }
  }
  if { ![info exists ::gPB(${evt}_command_index)] } {
   set ::gPB(${evt}_command_index) 0
  }
  set idx $::gPB(${evt}_command_index)
  set cmd PB_${evt}_command_${idx}
  incr ::gPB(${evt}_command_index)
  __outputBlankLine ::gPB(${evt}_handler)
  lappend ::gPB(${evt}_handler) "${leader}if \{ \$::mom_sys_advanced_turbo_option(OUTPUT_OPER_MSG) \} \{"
   lappend ::gPB(${evt}_handler) "${leader}   lappend $sys_var ${cmd}"
   lappend ::gPB(${evt}_handler) "${leader}\}\n"
  block::readvalue $cmt_blk_obj cmt_blk_obj_attr
  set blk_elem [lindex $cmt_blk_obj_attr(2) 0]
  set elem_var $block_element::($blk_elem,elem_mom_variable)
  regsub -all {\\} $elem_var {\\\\} elem_var
  set     ::gPB(${evt}_command,$idx) ""
  lappend ::gPB(${evt}_command,$idx) "\n"
  lappend ::gPB(${evt}_command,$idx) "#============================================================="

#=======================================================================
lappend ::gPB(${evt}_command,$idx) "proc $cmd \{ \} \{"
  lappend ::gPB(${evt}_command,$idx) "#============================================================="
  if { [info exists block::($cmt_blk_obj,var_list)] && [llength $block::($cmt_blk_obj,var_list)] } {
   lappend ::gPB(${evt}_command,$idx) "   global [join $block::($cmt_blk_obj,var_list)]"
  }
  array set mom_sys_var_arr $Post::($::post_object,mom_sys_var_list)
  ...
  lappend ::gPB(${evt}_command,$idx) "   MOM_output_literal \"$comment_start$elem_var$comment_end\""
  lappend ::gPB(${evt}_command,$idx) "\}"
}
...
proc __output_MOM_do_template { OUTPUT_BUFFER n_blank block_name BLK_MOD_ARR \
  ...
  if { [string match "$::gPB(comment_blk)*"  $block_name] ||\
   [string match "$::gPB(tcl_line_blk)*" $block_name] } {
   global post_object
   Post::GetObjList $post_object comment cmt_blk_obj_list
   PB_com_RetObjFrmName block_name cmt_blk_obj_list cmt_blk_obj
   if { $cmt_blk_obj > 0 } {
    ...
    set elem_var_org "# $elem_var" ;# $elem_var
    regsub -all {\\} $elem_var {\\\\} elem_var
    UI_PB_cmd_ValidateExpOK "Operator Message" $elem_var
    set block::($cmt_blk_obj,var_list) $gPB(CMD_VAR_LIST)
    set gPB(check_cc_unknown_command) $cc_state
    __outputBlankLine output_buffer
    if { [string match "$::gPB(tcl_line_blk)*" $block_name] } {
     if { [string length $blank_string] > 1 } {
      set blank_cmt [string range $blank_string 1 end]
      } else {
      set blank_cmt ""
     }
     lappend output_buffer "${blank_cmt}$elem_var_org"
     } else {
...
lappend event_output "proc $event_move \{ \} \{"
  ...
  lappend event_output "   ABORT_EVENT_CHECK"
  lappend event_output ""
 }
...
#=======================================================================
proc __TurboPostOK { {event_name "MOM_linear_move"} } {
  if { ![info exists ::mom_sys_arr(\$mom_sys_advanced_turbo_output)] ||\
   !$::mom_sys_arr(\$mom_sys_advanced_turbo_output) } {
   return 0
  }
  if { ![string match "MOM_linear_move"   $event_name] &&\
   ![string match "MOM_circular_move" $event_name] &&\
   ![string match "MOM_rapid_move"    $event_name] } {
   return 0
  }
  if { [string match "MOM_rapid_move" $event_name] &&\
   [string match "TRUE" [PB2TCL_read_fly_var mom_fly_work_plane_change] ] } {
   return 0
  }
  if [__IsMillPost] {
   return 1
...
#=======================================================================
proc __output_TurboConfig { EVENT_OUTPUT } {
  upvar $EVENT_OUTPUT event_output
  if [__TurboPostOK] {
   lappend event_output ""
   lappend event_output "  # Configure turbo output settings"
   lappend event_output "   if \{ \$::mom_sys_advanced_turbo_output \} \{"
    lappend event_output "      if \{ \[CMD_EXIST PB_CONFIG_ADVANCED_TURBO_OUTPUT\] \} \{"
     lappend event_output "         PB_CONFIG_ADVANCED_TURBO_OUTPUT"
    lappend event_output "      \}"
    lappend event_output "   \} else \{"
    lappend event_output "      if \{ \[CMD_EXIST CONFIG_TURBO_OUTPUT\] \} \{"
     lappend event_output "         CONFIG_TURBO_OUTPUT"
    lappend event_output "      \}"
   lappend event_output "   \}"
   } else {
   if [__IsMillPost] {
...
#=======================================================================
proc __output_DefaultDataOfEvent { EVT_NAME EVENT_OUTPUT } {
  upvar $EVENT_OUTPUT event_output
  upvar $EVT_NAME evt_name
  global cycle_init_flag
...
lappend event_output "proc ${evt_name}_move \{ \} \{"
  ...
  {
   UI_PB_debug_ForceMsg "\n+++++> event::($evt_obj,event_name) : $event::($evt_obj,event_name) <+++++"
...
lappend event_output "proc $event_move \{ \} \{"
 ...
 MOM_circular_move \
 {
  ...
  if [__TurboPostOK $evt_name] {
   if [info exist ::gPB(turbo_circular_command_index)] {
    for { set idx 0 } { $idx < $::gPB(turbo_circular_command_index) } { incr idx } {
     unset ::gPB(turbo_circular_command,$idx)
    }
    unset ::gPB(turbo_circular_command_index)
   }
   set     ::gPB(turbo_circular_handler) ""
   lappend ::gPB(turbo_circular_handler) "\n"
   lappend ::gPB(turbo_circular_handler) "#============================================================="

#=======================================================================
lappend ::gPB(turbo_circular_handler) "proc PB_config_turbo_circular_move \{ \} \{"
  lappend ::gPB(turbo_circular_handler) "#============================================================="
  lappend ::gPB(turbo_circular_handler) "   set ::mom_sys_circular_turbo_blocks \"\""
  lappend ::gPB(turbo_circular_handler) ""
  lappend ::gPB(turbo_circular_handler) "   if \{ \$::mom_sys_advanced_turbo_option(ABORT_EVENT_CHECK) \} \{"
   lappend ::gPB(turbo_circular_handler) "      lappend ::mom_sys_circular_turbo_blocks ABORT_EVENT_CHECK"
  lappend ::gPB(turbo_circular_handler) "   \}"
  lappend ::gPB(turbo_circular_handler) ""
  lappend ::gPB(turbo_circular_handler) "   if \{ \$::mom_sys_advanced_turbo_option(CALL_CIRCLE_SET) \} \{"
   lappend ::gPB(turbo_circular_handler) "      lappend ::mom_sys_circular_turbo_blocks CIRCLE_SET"
  lappend ::gPB(turbo_circular_handler) "   \}"
 }
}
MOM_from_move \
...
MOM_linear_move \
{
 ...
 lappend event_output ""
 if { ![string match "*_wedm" $cur_machine] } {
  lappend event_output "   HANDLE_FIRST_LINEAR_MOVE"
  lappend event_output ""
 }
 if [__TurboPostOK $evt_name] {
  if [info exist ::gPB(turbo_linear_command_index)] {
   for { set idx 0 } { $idx < $::gPB(turbo_linear_command_index) } { incr idx } {
    unset ::gPB(turbo_linear_command,$idx)
   }
   unset ::gPB(turbo_linear_command_index)
  }
  set     ::gPB(turbo_linear_handler) ""
  lappend ::gPB(turbo_linear_handler) "\n"
  lappend ::gPB(turbo_linear_handler) "#============================================================="

#=======================================================================
lappend ::gPB(turbo_linear_handler) "proc PB_config_turbo_linear_move \{ \} \{"
  lappend ::gPB(turbo_linear_handler) "#============================================================="
  lappend ::gPB(turbo_linear_handler) "   set ::mom_sys_linear_turbo_blocks \"\""
  lappend ::gPB(turbo_linear_handler) ""
  lappend ::gPB(turbo_linear_handler) "   if \{ \$::mom_sys_advanced_turbo_option(ABORT_EVENT_CHECK) \} \{"
   lappend ::gPB(turbo_linear_handler) "      lappend ::mom_sys_linear_turbo_blocks ABORT_EVENT_CHECK"
  lappend ::gPB(turbo_linear_handler) "   \}"
  lappend ::gPB(turbo_linear_handler) ""
  lappend ::gPB(turbo_linear_handler) "   if \{ \$::mom_sys_advanced_turbo_option(HANDLE_1ST_LINEAR) \} \{"
   lappend ::gPB(turbo_linear_handler) "      lappend ::mom_sys_linear_turbo_blocks HANDLE_FIRST_LINEAR_MOVE"
  lappend ::gPB(turbo_linear_handler) "   \}"
  lappend ::gPB(turbo_linear_handler) ""
 }
...
MOM_cutcom_on  \
{
 ...
 lappend event_output "   if { \[info exists mom_cutcom_adjust_register\] } {"
  ...
  lappend event_output "      if { \$mom_cutcom_adjust_register < \$cutcom_register_min ||\\"
   ...
   lappend event_output ""
   lappend event_output "         unset mom_cutcom_adjust_register"
  lappend event_output "      }"
 lappend event_output "   }"
}
MOM_spindle_rpm   -
...
MOM_rapid_move \
{
 ...
 lappend event_output ""
 if [__TurboPostOK $evt_name] {
  if [info exist ::gPB(turbo_rapid_command_index)] {
   for { set idx 0 } { $idx < $::gPB(turbo_rapid_command_index) } { incr idx } {
    unset ::gPB(turbo_rapid_command,$idx)
   }
   unset ::gPB(turbo_rapid_command_index)
  }
  set     ::gPB(turbo_rapid_handler) ""
  lappend ::gPB(turbo_rapid_handler) "\n"
  lappend ::gPB(turbo_rapid_handler) "#============================================================="

#=======================================================================
lappend ::gPB(turbo_rapid_handler) "proc PB_config_turbo_rapid_move \{ \} \{"
  lappend ::gPB(turbo_rapid_handler) "#============================================================="
  lappend ::gPB(turbo_rapid_handler) "   set ::mom_sys_rapid_turbo_blocks \"\""
  lappend ::gPB(turbo_rapid_handler) ""
  lappend ::gPB(turbo_rapid_handler) "   if \{ \$::mom_sys_advanced_turbo_option(ABORT_EVENT_CHECK) \} \{"
   lappend ::gPB(turbo_rapid_handler) "      lappend ::mom_sys_rapid_turbo_blocks ABORT_EVENT_CHECK"
  lappend ::gPB(turbo_rapid_handler) "   \}"
  lappend ::gPB(turbo_rapid_handler) ""
  } elseif [__TurboPostOK] {
  set     ::gPB(turbo_rapid_handler) ""
  lappend ::gPB(turbo_rapid_handler) "\n"
  lappend ::gPB(turbo_rapid_handler) "#============================================================="
...
lappend event_output ""
if { ![string compare [PB2TCL_read_fly_var mom_fly_work_plane_change] "TRUE"] } {
...
#=======================================================================
proc __output_RapidEventData { EVENT_OUTPUT BLK_MOD_ARR OUTPUT_ATTR_ARR n_blank } {
  upvar $EVENT_OUTPUT event_output
  upvar $BLK_MOD_ARR blk_mod_arr
  upvar $OUTPUT_ATTR_ARR output_attr_arr
  set blank ""
  if { $n_blank > 0 } { append blank [format %${n_blank}c 32] }
  set rapid_traverse [PB2TCL_read_fly_var mom_fly_rapid_traverse_blk]
  set rapid_spindle  [PB2TCL_read_fly_var mom_fly_rapid_spindle_blk]
  lappend event_output ""
  lappend event_output "$blank   global mom_sys_control_out mom_sys_control_in"
  lappend event_output "$blank   set co \"\$mom_sys_control_out\""
  lappend event_output "$blank   set ci \"\$mom_sys_control_in\""
  lappend event_output ""
  lappend event_output ""
  lappend event_output "$blank   if { !\[info exists mom_cycle_spindle_axis\] } {"
   lappend event_output "$blank      set mom_cycle_spindle_axis 2"
  lappend event_output "$blank   }"
  lappend event_output "$blank   if { !\[info exists spindle_first\] } {"
   lappend event_output "$blank      set spindle_first NONE"
  lappend event_output "$blank   }"
  lappend event_output "$blank   if { !\[info exists rapid_spindle_inhibit\] } {"
   lappend event_output "$blank      set rapid_spindle_inhibit FALSE"
  lappend event_output "$blank   }"
  lappend event_output "$blank   if { !\[info exists rapid_traverse_inhibit\] } {"
   lappend event_output "$blank      set rapid_traverse_inhibit FALSE"
  lappend event_output "$blank   }"
  lappend event_output ""
  lappend event_output "$blank   switch \$mom_cycle_spindle_axis \{"
   lappend event_output "$blank      0 \{"
    lappend event_output "$blank         if \[llength \$rapid_spindle_x_blk\] \{"
     lappend event_output "$blank            set spindle_block  ${rapid_spindle}_x"
     lappend event_output "$blank            PB_SET_RAPID_MOD \$rapid_spindle_mod \$rapid_spindle_x_blk aa mod_spindle"
     lappend event_output "$blank         \} else \{"
     lappend event_output "$blank            set spindle_block  \"\""
    lappend event_output "$blank         \}"
    lappend event_output "$blank         if \[llength \$rapid_traverse_yz_blk\] \{"
     lappend event_output "$blank            set traverse_block ${rapid_traverse}_yz"
     lappend event_output "$blank            PB_SET_RAPID_MOD \$rapid_traverse_mod \$rapid_traverse_yz_blk aa mod_traverse"
     lappend event_output "$blank         \} else \{"
     lappend event_output "$blank            set traverse_block  \"\""
    lappend event_output "$blank         \}"
   lappend event_output "$blank      \}"
   lappend event_output ""
   lappend event_output "$blank      1 \{"
    lappend event_output "$blank         if \[llength \$rapid_spindle_y_blk\] \{"
     lappend event_output "$blank            set spindle_block  ${rapid_spindle}_y"
     lappend event_output "$blank            PB_SET_RAPID_MOD \$rapid_spindle_mod \$rapid_spindle_y_blk aa mod_spindle"
     lappend event_output "$blank         \} else \{"
     lappend event_output "$blank            set spindle_block  \"\""
    lappend event_output "$blank         \}"
    lappend event_output "$blank         if \[llength \$rapid_traverse_xz_blk\] \{"
     lappend event_output "$blank            set traverse_block ${rapid_traverse}_xz"
     lappend event_output "$blank            PB_SET_RAPID_MOD \$rapid_traverse_mod \$rapid_traverse_xz_blk aa mod_traverse"
     lappend event_output "$blank         \} else \{"
     lappend event_output "$blank            set traverse_block  \"\""
    lappend event_output "$blank         \}"
   lappend event_output "$blank      \}"
   lappend event_output ""
   lappend event_output "$blank      2 \{"
    lappend event_output "$blank         if \[llength \$rapid_spindle_z_blk\] \{"
     lappend event_output "$blank            set spindle_block  ${rapid_spindle}_z"
     lappend event_output "$blank            PB_SET_RAPID_MOD \$rapid_spindle_mod \$rapid_spindle_z_blk aa mod_spindle"
     lappend event_output "$blank         \} else \{"
     lappend event_output "$blank            set spindle_block  \"\""
    lappend event_output "$blank         \}"
    lappend event_output "$blank         if \[llength \$rapid_traverse_xy_blk\] \{"
     lappend event_output "$blank            set traverse_block ${rapid_traverse}_xy"
     lappend event_output "$blank            PB_SET_RAPID_MOD \$rapid_traverse_mod \$rapid_traverse_xy_blk aa mod_traverse"
     lappend event_output "$blank         \} else \{"
     lappend event_output "$blank            set traverse_block  \"\""
    lappend event_output "$blank         \}"
   lappend event_output "$blank      \}"
   lappend event_output ""
   lappend event_output "$blank      default \{"
    lappend event_output "$blank         set spindle_block  $rapid_spindle"
    lappend event_output "$blank         set traverse_block $rapid_traverse"
    lappend event_output "$blank         PB_SET_RAPID_MOD \$rapid_spindle_mod \$rapid_spindle_blk aa mod_spindle"
    lappend event_output "$blank         PB_SET_RAPID_MOD \$rapid_traverse_mod \$rapid_traverse_blk aa mod_traverse"
   lappend event_output "$blank      \}"
  lappend event_output "$blank   \}"
  lappend event_output ""
  lappend event_output "$blank   if \{ \!\[string compare \$spindle_first \"TRUE\"\] \} \{"
   lappend event_output "$blank      if \{ \!\[string compare \$rapid_spindle_inhibit \"FALSE\"\] \} \{"
    lappend event_output "$blank         if \{ \[string compare \$spindle_block \"\"\] \} \{"
     __output_RapidBlkWithAttr event_output output_attr_arr [expr $n_blank + 12] spindle
     lappend event_output "$blank         \} else \{"
     lappend event_output "$blank            MOM_output_literal \"\$co Rapid Spindle Block is empty! \$ci\""
    lappend event_output "$blank         \}"
   lappend event_output "$blank      \}"
   lappend event_output "$blank      if \{ \!\[string compare \$rapid_traverse_inhibit \"FALSE\"\] \} \{"
    lappend event_output "$blank         if \{ \[string compare \$traverse_block \"\"\] \} \{"
     __output_RapidBlkWithAttr event_output output_attr_arr [expr $n_blank + 12] traverse
     lappend event_output "$blank         \} else \{"
     lappend event_output "$blank            MOM_output_literal \"\$co Rapid Traverse Block is empty! \$ci\""
    lappend event_output "$blank         \}"
   lappend event_output "$blank      \}"
   lappend event_output "$blank   \} elseif \{ \!\[string compare \$spindle_first \"FALSE\"\] \} \{"
   lappend event_output "$blank      if \{ \!\[string compare \$rapid_traverse_inhibit \"FALSE\"\] \} \{"
    lappend event_output "$blank         if \{ \[string compare \$traverse_block \"\"\] \} \{"
     __output_RapidBlkWithAttr event_output output_attr_arr [expr $n_blank + 12] traverse
     lappend event_output "$blank         \} else \{"
   ...
   lappend event_output "$blank      \}"
   lappend event_output "$blank   \} else \{"
   __output_RapidBlkWithAttr event_output output_attr_arr [expr $n_blank + 6] traverse $rapid_traverse
  lappend event_output "$blank   \}"
 }
...
#=======================================================================
proc __output_RapidBlkElems { FILE_ID blank } {
  upvar $FILE_ID file_id
  ...
  for { set count 0 } { $count < $no_blks } { incr count } \
  {
   if [string match "rapid_traverse" $blk_name_arr($count)] {
    ...
    lappend file_id "$blank   set rapid_traverse_blk \{[join $gPB(rapid_traverse_blk)]\}"
    if { $rapid_work_plane_change } {
     ...
     lappend file_id "$blank   set rapid_traverse_xy_blk \{[join $gPB(rapid_traverse_xy_blk)]\}"
     ...
     lappend file_id "$blank   set rapid_traverse_yz_blk \{[join $gPB(rapid_traverse_yz_blk)]\}"
     ...
     lappend file_id "$blank   set rapid_traverse_xz_blk \{[join $gPB(rapid_traverse_xz_blk)]\}"
    }
   } ;# rapid_traverse
   if { $rapid_work_plane_change } {
    if [string match "rapid_spindle" $blk_name_arr($count)] {
     ...
     lappend file_id "$blank   set rapid_spindle_blk \{[join $gPB(rapid_spindle_blk)]\}"
     ...
     lappend file_id "$blank   set rapid_spindle_x_blk \{[join $gPB(rapid_spindle_x_blk)]\}"
     ...
     lappend file_id "$blank   set rapid_spindle_y_blk \{[join $gPB(rapid_spindle_y_blk)]\}"
     ...
     lappend file_id "$blank   set rapid_spindle_z_blk \{[join $gPB(rapid_spindle_z_blk)]\}"
    } ;# rapid_spindle
   } ;# rapid_work_plane_change
  }
 }
...
#=======================================================================
proc __output_BlockOfEvent { EVT_NAME EVT_BLOCKS BLK_MOD_ARR NO_OF_LISTS \
  ...
  for { set jj 0 } { $jj <= $last_list } { incr jj } \
  {
    ...
    if { $elem_index < $evt_elem_len } \
    {
     ...
     if { $sub_evt_elem_list != "" } \
     {
      ...
      if { [info exists event_element::($evt_elem_obj,indent)] && \
       ...
       set n_indent $event_element::($evt_elem_obj,indent)
       UI_PB_debug_ForceMsg "+++++> n_indent : $n_indent  block_name : $block_name  evt_elem_obj : $evt_elem_obj <+++++"
       if { $n_indent } { append indent [format %${n_indent}c 32] }
      }
      if { $if_indent } { append indent [format %${if_indent}c 32] }
      set temp_addr_list ""
   ...
   UI_PB_debug_ForceMsg "+++++> Block Name : $block_name  isupper : $isupper <+++++"
   set blk_obj 0
   if { [llength $block_name] == 1 } {
   ...
   if { $blk_obj == 0 } \
   {
    UI_PB_debug_ForceMsg "+++++> non Block Name : $block_name  $blk_obj  isupper : $isupper <+++++"
    if { [llength $block_name] > 1 } \
    {
   ...
   set t_indent [string length $indent]
   if { $isupper && ![info exists block::($blk_obj,block_name)] } \
   {
    ...
    if { $kk == 0 && ![string compare $cycle_init_flag "TRUE"] && $has_cycle_start } \
    {
     ...
     switch $block_name {
      ...
      default {
       if { [string match "IF(*)" $block_name] } {
        incr if_indent $idt
        incr if_found
       }
      }
     }
     if { $block_name == "ELSE" || $block_name == "END" } {
      if { [string length $indent] > $idt } {
       set indent [string range $indent $idt end]
       } else {
       set indent ""
      }
     }
     ...
     switch $block_name {
      ...
      END  {
       set end_if ""
       if { [info exists ::gPB(END_IF_MARKER)] && [string trim $::gPB(END_IF_MARKER)] != "" } {
        set end_if [string trim $::gPB(END_IF_MARKER)]
       }
       if { $end_if != "" } {
       set block_name "\} \;# $end_if"
       } else {
      set block_name "\}"
     }
     set __turbo_line 0
    ...
    default {
     if { [string match "IF(*)" $block_name] } {
      set blk [string trimright $block_name ")"]
      set blk [string range $blk 3 end]
      if { $if_indent == 3 } {
       lappend event_output ""
      }
      set block_name "if \{ \[$blk\] \} \{"
       set __turbo_line 0
      }
     }
    }
    set t_indent [string length $indent]
    UI_PB_debug_ForceMsg "+++++> Substituted non Block Name : $block_name <+++++"
    if { $isMacro } \
    {
     PB_output_FunctionProc $blk_obj event_output cond_cmd suppress_flag $need_right_brace $indent
     if [__TurboPostOK $evt_name] {
      __output_Turbo_FunctionProc $evt_name $blk_obj event_output cond_cmd suppress_flag $need_right_brace $indent
     }
    } else \
    {
     set temp_blank_prefix ""
     if { $idt > 0 } {
      append temp_blank_prefix [format %${idt}c 32]
      if $need_right_brace { ;# This seems to only occur with cycles
       append temp_blank_prefix [format %${idt}c 32]
      }
     }
     append temp_blank_prefix $indent
     if { [info exists mom_sys_arr(post_startblk)] && \
     [string match $mom_sys_arr(post_startblk) $block_name] } \
     {
     ...
     } else \
     {
      ...
      if [__TurboPostOK $evt_name] {
       switch $evt_name {
        MOM_linear_move {
         set __output_stream ::gPB(turbo_linear_handler)
        }
        MOM_circular_move {
         set __output_stream ::gPB(turbo_circular_handler)
        }
        default {
         set __output_stream ::gPB(turbo_rapid_handler)
        }
       }
       set event [lindex [split $evt_name _] 1]
       if { $__turbo_line } {
        if { [string match "PB_CMD_*" $block_name] } {
         if { $::gPB(TURBO_EXEC_COMMAND) } {
          set line_text "   lappend ::mom_sys_${event}_turbo_blocks $block_name"
          } else {
          set line_text "lappend ::mom_sys_${event}_turbo_blocks $block_name"
         }
         } elseif { [string match "MOM_*" $block_name] } {
         __output_Turbo_MOM_command $evt_name $block_name $temp_blank_prefix
         set line_text ""
        }
       }
       if { [string trim $line_text] != "" } {
        if { $__turbo_line } {
         if { $::gPB(TURBO_EXEC_COMMAND) && [string match "PB_CMD_*" $block_name] } {
          __outputBlankLine $__output_stream
          __outputTextLine "if \{ \$::mom_sys_advanced_turbo_option(EXECUTE_PB_CMD) \} \{" \
           $temp_blank_prefix $__output_stream
          }
          } else {
          if { [string match "if *" [string trim $line_text]] } {
           __outputBlankLine $__output_stream
          }
         }
         __outputExecAttr line_text temp_blank_prefix cond_cmd suppress_flag force_name_list $__output_stream
         if { $__turbo_line } {
          if { $::gPB(TURBO_EXEC_COMMAND) && [string match "PB_CMD_*" $block_name] } {
          __outputTextLine "\}" $temp_blank_prefix $__output_stream
         }
        }
       }
      }
   ...
   } else \
   {
    ...
    {
     set rapid_traverse [PB2TCL_read_fly_var mom_fly_rapid_traverse_blk]
     UI_PB_debug_ForceMsg "+++++> $evt_name >$block_name< >$rapid_traverse< \n"
     if { ![string compare $rapid_traverse "$block_name"] } \
     {
      ...
      foreach temp_evt_elem $event::($evt_obj,evt_elem_list) {
       set blk_obj $event_element::($temp_evt_elem,block_obj)
       if { [string match "rapid_traverse" $block::($blk_obj,block_name)] } \
       {
        if 0 {
      ...
      if { ![string compare $cur_machine "mill"] && \
       ...
       __output_RapidBlkElems  event_output $indent
       __output_RapidAddrForce event_output [expr $t_indent + 3] "rapid_traverse" force_name_list
      }
     }
     if { ![string compare $cur_machine "mill"] && \
     ![string compare [PB2TCL_read_fly_var mom_fly_work_plane_change] "TRUE"] } \
     {
      set rapid_spindle [PB2TCL_read_fly_var mom_fly_rapid_spindle_blk]
      UI_PB_debug_ForceMsg "+++++> $evt_name >$block_name< >$rapid_spindle< \n"
      if { ![string compare $rapid_spindle "$block_name"] } \
      {
       ...
       foreach temp_evt_elem $event::($evt_obj,evt_elem_list) {
        set blk_obj $event_element::($temp_evt_elem,block_obj)
        if { [string match "rapid_spindle" $block::($blk_obj,block_name)] } \
        {
         if 0 {
          foreach temp_addr $event_element::($temp_evt_elem,force_addr_list) \
          {
       ...
       __output_RapidAddrForce event_output [expr $t_indent + 3] "rapid_spindle" force_name_list
       __output_RapidEventData event_output blk_mod_arr rapid_output_attr_arr $t_indent
       continue
    ...
    set n_blank [expr { 3 + $t_indent }]
    switch $evt_name \
    {
     MOM_start_of_program \
     {
      __output_MOM_do_template event_output $n_blank $block_name blk_mod_arr cond_cmd suppress_flag force_name_list
      set is_done2 1
      UI_PB_debug_ForceMsg "+++++> $evt_name is_done2 = $is_done2 <+++++"
     }
     MOM_start_of_path    -
      ...
      if { $jj <= $pre_list && [llength [lindex $evt_blocks $jj]] > 0 } \
      {
       if { $kk == 0 && ![string compare $cycle_init_flag "TRUE"] && $has_cycle_start } \
       {
        ...
        if { $need_right_brace } { ;# This seems to only occur with cycles
         ...
         __output_MOM_do_template event_output $n_blank $block_name blk_mod_arr\
         cond_cmd suppress_flag force_name_list
         if [__TurboPostOK $evt_name] {
          __output_Turbo_MOM_do_template $evt_name $n_blank $block_name blk_mod_arr\
          cond_cmd suppress_flag force_name_list
         }
         UI_PB_debug_ForceMsg "+++++> $block_name n_blank : $n_blank <+++++"
        }
        set is_done2 1
        UI_PB_debug_ForceMsg "+++++> $evt_name is_done2 = $is_done2 <+++++"
 ...
 switch $evt_name \
 {
  ...
  {
   if { [string match "*_wedm" $cur_machine] } {
    lappend event_output "   catch \{ MOM_rapid_move \}"
    } else {
    lappend event_output ""
    lappend event_output "  global mom_programmed_feed_rate"
   ...
   if 0 { ;#<Sep-14-2016 gsl> Output @ beginning of the handler
    if [__TurboPostOK] {
     lappend event_output ""
     lappend event_output "  # Configure turbo output settings"
     lappend event_output "   if \{ \$::mom_sys_advanced_turbo_output \} \{"
  ...
  {
   lappend event_output ""
   lappend event_output "  # Write tool list with time in commentary data"
   lappend event_output "   LIST_FILE_TRAILER"
   lappend event_output ""
   lappend event_output "  # Close warning and listing files"
 ...
 if { $if_found != 0 || $else_found != 0 } {
  set msg "Conditional logic is incomplete in event handler of $$evt_name!"
  UI_PB_debug_ForceMsg "\n %%% $msg %%% \n"
...
proc PB_output_GetEventProcData { EVT_OBJ EVT_NAME EVT_BLOCKS BLK_MOD_ARR \
  ...
  UI_PB_debug_ForceMsg_no_trace "\n EEEEE Get event >$evt_name< => changed_evt >$changed_evt_name< EEEEE \n"
  lappend event_output "\n"
  lappend event_output "#============================================================="
...
lappend event_output "proc $changed_evt_name \{ \} \{"
  lappend event_output "#============================================================="
  PB_output_CallDebugMsg S event_output
  if { [string match "MOM_end_of_program" $changed_evt_name] } {
   lappend event_output "  global mom_program_aborted mom_event_error"
  }
  if { $::env(PB_UDE_ENABLED) == 1 } {
  ...
  if { $::env(PB_UDE_ENABLED) == 1 } {
   ...
   foreach event $event_obj_list {
    set post_name ""
    if { [info exists ude_event::($event,post_event)] } {
  ...
  if { [lsearch $udc_name_list $evt_name] >= 0 && $::env(PB_UDE_ENABLED) == 1 } {
   ...
   __output_DefaultDataOfEvent changed_evt_name event_output
  }
  switch $evt_name \
  {
   "PB_start_of_program" \
   {
    lappend event_output "   if \[CMD_EXIST PB_CMD_kin_start_of_program\] \{"
   ...
   "MOM_start_of_path" \
   {
    ...
    lappend event_output ""
    lappend event_output "   if \[CMD_EXIST PB_CMD_kin_start_of_path\] \{"
   ...
   "MOM_end_of_path" \
   {
    ...
    lappend event_output ""
    lappend event_output "   if { !\[info exists mom_sys_add_cutting_time\] } {"
     lappend event_output "      set mom_sys_add_cutting_time 0.0"
    lappend event_output "   }"
    lappend event_output "   if { !\[info exists mom_sys_add_non_cutting_time\] } {"
     lappend event_output "      set mom_sys_add_non_cutting_time 0.0"
    lappend event_output "   }"
    lappend event_output ""
    ...
    lappend event_output ""
    lappend event_output "   if \[CMD_EXIST PB_CMD_kin_end_of_path\] \{"
   ...
   "MOM_end_of_program" \
   {
    if { [info exists mom_sys_arr(\$is_linked_post)] && $mom_sys_arr(\$is_linked_post) == 1 } {
     if { [info exists mom_sys_arr(\$mom_sys_mill_turn_type)] && \
      [string match "SIMPLE_MILL_TURN" $mom_sys_arr(\$mom_sys_mill_turn_type)] } {
      set master_post ::mom_sys_mill_postname
      } else {
      set master_post ::mom_sys_master_post
     }
     lappend event_output ""
     lappend event_output "  # Restore definition to the master post's"
     lappend event_output "   if { !\[string match \"\$::mom_sys_this_post_name\" \[file tail \${$master_post}\]\] } {"
      lappend event_output "      MOM_load_definition_file   \"\${$master_post}.def\""
     lappend event_output "   }"
     lappend event_output ""
    }
   }
  ...
  if { $no_of_lists > 0 } \
  {
   __output_BlockOfEvent evt_name evt_blocks blk_mod_arr \
  ...
  switch $evt_name \
  {
   ...
   "MOM_first_move" \
   {
    __output_TurboConfig event_output
   }
   "PB_start_of_program" \
   {
    if [__TurboPostOK] {
     ...
     lappend event_output "   if { \$::mom_sys_advanced_turbo_output } {"
      lappend event_output ""
      lappend event_output "      MOM_set_turbo_mode FALSE"
      lappend event_output "      MOM_set_turbo_mode TRUE ADVANCED"
      lappend event_output ""
      lappend event_output "      set ::mom_kin_is_turbo_output FALSE"
      lappend event_output ""
      lappend event_output "      set ::mom_sys_advanced_turbo_option(ABORT_EVENT_CHECK)  0"
      lappend event_output "      set ::mom_sys_advanced_turbo_option(HANDLE_1ST_LINEAR)  0"
      lappend event_output "      set ::mom_sys_advanced_turbo_option(OUTPUT_OPER_MSG)    0"
      lappend event_output "      set ::mom_sys_advanced_turbo_option(CALL_CIRCLE_SET)    0"
      lappend event_output "      set ::mom_sys_advanced_turbo_option(DO_FORCE_SUPPRESS)  0"
      lappend event_output "      set ::mom_sys_advanced_turbo_option(SET_SEQNUM_ON_OFF)  0"
      if { $::gPB(TURBO_EXEC_COMMAND) } {
       lappend event_output "      set ::mom_sys_advanced_turbo_option(EXECUTE_PB_CMD)     0"
      }
      if { ![__TurboPostOK "MOM_rapid_move"] } {
       lappend event_output ""
       lappend event_output "      MOM_set_turbo_rapid FALSE"
      }
     lappend event_output "   }"
     lappend event_output ""
     lappend event_output "   if \[CMD_EXIST $::gPB(ATP,customize_post)\] {"
      lappend event_output "      $::gPB(ATP,customize_post)"
     lappend event_output "   }"
    }
    lappend event_output ""
    lappend event_output "   if \[CMD_EXIST PB_CMD_kin_start_of_program_2\] {"
   ...
   "MOM_end_of_path" \
   {
    ...
    if [__TurboPostOK] {
     lappend event_output ""
     lappend event_output "   if \{ \[CMD_EXIST $::gPB(ATP,config_end_of_path)\] \} \{"
      lappend event_output "      $::gPB(ATP,config_end_of_path)"
     lappend event_output "   \}"
    }
    if { [info exists mom_sys_arr(Output_VNC)] && $mom_sys_arr(Output_VNC) == 1 } {
     lappend event_output ""
   ...
   {
    lappend event_output ""
    lappend event_output "   if \[CMD_EXIST PB_CMD_kin_end_of_program\] {"
 ...
 if [__TurboPostOK $evt_name] {
  if { [info exists ::gPB(turbo_linear_handler)] && [string match "MOM_linear_move" $evt_name] } {
   lappend ::gPB(turbo_linear_handler) ""
   lappend ::gPB(turbo_linear_handler) "   MOM_set_turbo_blocks LINEAR \$::mom_sys_linear_turbo_blocks"
  lappend ::gPB(turbo_linear_handler) "\}"
  foreach s $::gPB(turbo_linear_handler) {
   lappend event_output $s
  }
  unset ::gPB(turbo_linear_handler)
 }
 if { [info exists ::gPB(turbo_circular_handler)] && [string match "MOM_circular_move" $evt_name] } {
  lappend ::gPB(turbo_circular_handler) ""
  lappend ::gPB(turbo_circular_handler) "   MOM_set_turbo_blocks CIRCULAR \$::mom_sys_circular_turbo_blocks"
 lappend ::gPB(turbo_circular_handler) "\}"
 foreach s $::gPB(turbo_circular_handler) {
  lappend event_output $s
 }
 unset ::gPB(turbo_circular_handler)
}
if { [info exists ::gPB(turbo_rapid_handler)] && [string match "MOM_rapid_move" $evt_name] } {
 lappend ::gPB(turbo_rapid_handler) ""
 lappend ::gPB(turbo_rapid_handler) "   MOM_set_turbo_blocks RAPID \$::mom_sys_rapid_turbo_blocks"
lappend ::gPB(turbo_rapid_handler) "\}"
foreach s $::gPB(turbo_rapid_handler) {
 lappend event_output $s
}
unset ::gPB(turbo_rapid_handler)
}
if [info exist ::gPB(turbo_linear_command_index)] {
for { set idx 0 } { $idx < $::gPB(turbo_linear_command_index) } { incr idx } {
 foreach line $::gPB(turbo_linear_command,$idx) {
  lappend event_output $line
 }
 unset ::gPB(turbo_linear_command,$idx)
}
unset ::gPB(turbo_linear_command_index)
...
if [info exist ::gPB(turbo_circular_command_index)] {
for { set idx 0 } { $idx < $::gPB(turbo_circular_command_index) } { incr idx } {
 foreach line $::gPB(turbo_circular_command,$idx) {
  lappend event_output $line
 }
 unset ::gPB(turbo_circular_command,$idx)
}
...
if [info exist ::gPB(turbo_rapid_command_index)] {
for { set idx 0 } { $idx < $::gPB(turbo_rapid_command_index) } { incr idx } {
 foreach line $::gPB(turbo_rapid_command,$idx) {
  lappend event_output $line
 }
 unset ::gPB(turbo_rapid_command,$idx)
}
...
} elseif { [__TurboPostOK] && [string match "MOM_rapid_move" $evt_name] } {
if [info exists ::gPB(turbo_rapid_handler)] {
foreach s $::gPB(turbo_rapid_handler) {
 lappend event_output $s
}
unset ::gPB(turbo_rapid_handler)
   ...
   switch $file_type {
    "POST" {
     if { ![string match "*PB_CMD_vnc_*" $cmd_blk] && \
      ![string match "ELSE" $cmd_blk] && ![string match "END" $cmd_blk] } {
      if { ![info exists ::LicInfo(to_encrypt)] || !$::LicInfo(to_encrypt) } {
...
puts $tclf_id "proc $cmd_blk \{ \} \{"
  ...
  foreach line $cmd_proc_arr($cmd_blk) \
  {
   ...
   if { [string first "\#" [string trim $line]] < 0 && \
    [string first "MOM_" [string trim $line]] < 0 } { ;# Not a comment line or part of MOM command
    if { $__add_debug_call == -1 } {
     set i1 [string first "$::gPB(PB_POST_DEBUG_command) RETURN" $line]
     if { $i1 >= 0 } {
      if [expr $::tcl_version > 8.0] {
       set line [string map [list "$::gPB(PB_POST_DEBUG_command) RETURN [incr ret_idx]\; return" "return"] $line]
       } else {
       set i2 [string first "return" $line]
       set s1 [string range $line 0 [expr $i1 - 1]]
       set s2 [string range $line $i2 end]
       set line $s1$s2
      }
     ...
     if { $irs >= 0 } {
      if [expr $::tcl_version > 8.0] {
       ...
       set s1 [string range $line 0 [expr $irs - 1]]
       set s2 "$::gPB(PB_POST_DEBUG_command) RETURN [incr ret_idx]\; return"
       set s3 [string range $line $ire end]
       set line $s1$s2$s3
      }
      set __last_return 1
     }
...
proc PB_PB2TCL_write_other_procs { TCLF_ID } {
  ...
  if [info exists Post::($post_object,other_proc_list)] \
  {
   ...
   {
    if { ![string compare "EQ_is_zero"  $proc_name] || \
     ![string compare "EQ_is_equal" $proc_name] } {
     continue
    }
     ...
     switch $proc_name \
     {
      ...
      default \
      {
       if { ![string match "VNC_*" $proc_name] } {
        ...
        if [llength $proc_data($proc_name,args)] {
...
proc PB_PB2TCL_write_list_file_var { TCLF_ID } {
  ...
  puts $tclf_id "\n"
  puts $tclf_id "     if { \[string match \"OFF\" \$mom_sys_warning_output\] } {"
   puts $tclf_id "        catch { rename MOM__util_print ugpost_MOM__util_print }"

#=======================================================================
puts $tclf_id "        proc MOM__util_print { args } {}"
puts $tclf_id "     }"
puts $tclf_id "\n"
...
puts $tclf_id "\n"
puts $tclf_id "     if { \[string match \"OFF\" \$mom_sys_warning_output\] } {"
 puts $tclf_id "        catch { rename MOM__util_print \"\" }"
 puts $tclf_id "        catch { rename ugpost_MOM__util_print MOM__util_print }"
puts $tclf_id "     }"
puts $tclf_id "\n"
puts $tclf_id "   #============================================================="
...
puts $tclf_id "   proc MOM_before_output { } {"
  ...
  puts $tclf_id "      }"
  puts $tclf_id ""
  puts $tclf_id "     # Write output buffer to the listing file with warnings"
...
proc PB_PB2TCL_write_post_header { tclf_id } {
  ...
  puts $tclf_id "  set cam_post_dir \[MOM_ask_env_var UGII_CAM_POST_DIR\]"
  puts $tclf_id ""
  puts $tclf_id "  set mom_sys_this_post_dir  \"\[file dirname \[info script\]\]\""
  puts $tclf_id "  set mom_sys_this_post_name \"\[file rootname \[file tail \[info script\]\]\]\""
  puts $tclf_id ""
  puts $tclf_id ""
...
proc PB_output_adjust_var_value { case {loc PST} } {
  ...
  switch $case {
   1 {
    if { ![info exists LicInfo(post_site_id)] || [string length $LicInfo(post_site_id)] == 0 } {
...
proc PB_PB2TCL_main { OUTPUT_TCL_FILE } {
   ...
   if [string match "UG_POST_AUTHOR" $gPB(PB_LICENSE)] {
    if { ![string compare $gPB(session) "NEW"] } {
     if { $gPB(license_authorize) == 1 && [string compare $gPB(license_to_use) ""] } {
      if { ![info exists LicInfo(site_id)] || [string length $LicInfo(site_id)] == 0 } {
     ...
     if { [info exists LicInfo(is_encrypted_file)] && $LicInfo(is_encrypted_file) == 1 } {
      set author_id_checked 0
      if { [info exists LicInfo(post_author_id)] } {
       if { ![string compare $gPB(PB_SITE_ID) $LicInfo(post_author_id)] } {
        set author_id_checked 1
       }
       } else {
      ...
      if { $author_id_checked } {
       if { [lsearch $gPB(lic_list) $LicInfo(post_license)] >= 0 } {
        ...
        } else { ;# What would be the case here that the author does not own the license?
       ...
       } else { ;# User is not the author of the edited post.
       set LicInfo(site_id) $LicInfo(post_site_id)
       set LicInfo(license) $LicInfo(post_license)
       set case 6
       set to_encrypt 1
      }
      } else { ;# Post is not encrypted,
      if { $gPB(license_authorize) == 1 && [string compare $gPB(license_to_use) ""] } {
       if { ![info exists LicInfo(site_id)] || [string length $LicInfo(site_id)] == 0 } {
        set LicInfo(site_id) $gPB(PB_SITE_ID)
       }
  ...
  if { ![info exists LicInfo(post_license)] } { set LicInfo(post_license) "" }
  __Pause "case = $case, to_encrypt = $to_encrypt, LicInfo(post_license) = $LicInfo(post_license)"
  if { $to_encrypt == 1 } {
   if { [string compare "UG_POST_EXE 495" $LicInfo(license)] == 0 } {
    if { ![info exists LicInfo(author_id)] && [info exists LicInfo(post_author_id)] } {
     set LicInfo(author_id) $LicInfo(post_author_id)
    }
    if { ![info exists LicInfo(expiration)] && [info exists LicInfo(post_expiration)] } {
     set LicInfo(expiration) $LicInfo(post_expiration)
    }
   }
...
#=======================================================================
puts $tclf_id "proc PB_CONFIG_ADVANCED_TURBO_OUTPUT \{ \} \{"
  puts $tclf_id "#============================================================="
  puts $tclf_id ""
  puts $tclf_id "   PB_config_turbo_linear_move"
  puts $tclf_id "   PB_config_turbo_circular_move"
  puts $tclf_id "   PB_config_turbo_rapid_move"
  puts $tclf_id ""
  puts $tclf_id "   if \{ \[CMD_EXIST $::gPB(ATP,config_output)\] \} \{"
   puts $tclf_id "      $::gPB(ATP,config_output)"
  puts $tclf_id "   \}"
 puts $tclf_id "\}"
}
...
proc PB_PB2VNC_write_misc { FILE_ID } {
  ...
  puts $file_id ""
  set val $mom_sys_var(Word_Separator)
...
#=======================================================================
proc __output_Turbo_FunctionProc { evt_name blk_obj EVENT_OUTPUT COND_CMD SUPPRESS_FLAG need_right_brace indent } {
  upvar $EVENT_OUTPUT  event_output
  upvar $COND_CMD      cond_cmd
  upvar $SUPPRESS_FLAG suppress_flag
  if $need_right_brace {
...
lappend ::gPB(${evt}_command,$idx) "proc $cmd \{ \} \{"
  lappend ::gPB(${evt}_command,$idx) "#============================================================="
  set need_right_brace 0 ; set indent ""
  PB_output_FunctionProc $blk_obj ::gPB(${evt}_command,$idx) cond_cmd suppress_flag $need_right_brace $indent
  lappend ::gPB(${evt}_command,$idx) "\}"
}

#=======================================================================
proc PB_output_FunctionProc { func_obj EVENT_OUTPUT COND_CMD SUPPRESS_FLAG \
  need_right_brace args } {
  ...
  set force_name_list "" ;# No forced address for macro function call
...
puts $tclf_id "proc PB_call_macro \{ macro_name \{ prefix \"\" \} \{ suppress_seqno 0 \} args \} \{"
  ...
  puts $tclf_id "   if \[CMD_EXIST OUTPUT_MACRO\] \{"
   puts $tclf_id "      OUTPUT_MACRO \$text_string \$suppress_seqno"
   puts $tclf_id "   \} else \{"
   puts $tclf_id "      if \{ \$suppress_seqno \} \{"
    puts $tclf_id "         MOM_suppress once N"
    puts $tclf_id "         MOM_output_literal \$text_string"
...
#=======================================================================
proc __output_Turbo_ExecAttr { evt_name TEXT PREFIX COND_CMD SUPPRESS_FLAG FORCE_NAME_LIST } {
  ...
  PB_com_unset_var cmd
  if { $suppress_flag || $force_name_list != "" } {
   if { ![info exists ::gPB(${evt}_command_index)] } {
    set ::gPB(${evt}_command_index) 0
   }
   set idx $::gPB(${evt}_command_index)
...
lappend ::gPB(${evt}_command,$idx) "proc $cmd \{ \} \{"
 ...
 __outputBlankLine ::gPB(${evt}_handler)
 if { $cond_cmd != "" } {
  lappend ::gPB(${evt}_handler) "${prefix}if \{ \[$cond_cmd\] \} \{"
   if { [info exists cmd] } {
    lappend ::gPB(${evt}_handler) "${prefix}   if \{ \$::mom_sys_advanced_turbo_option(DO_FORCE_SUPPRESS) \} \{"
     lappend ::gPB(${evt}_handler) "${prefix}      lappend $sys_var ${cmd}"
     lappend ::gPB(${evt}_handler) "${prefix}   \}\n"
   }
   lappend ::gPB(${evt}_handler) "${prefix}   lappend $sys_var $line_text"
   lappend ::gPB(${evt}_handler) "${prefix}\}"
  } else {
  if { [info exists cmd] } {
   lappend ::gPB(${evt}_handler) "${prefix}if \{ \$::mom_sys_advanced_turbo_option(DO_FORCE_SUPPRESS) \} \{"
 ...
 if { [info exists cmd] } {
  if { $suppress_flag } {
   lappend ::gPB(${evt}_command,$idx) "   MOM_suppress once N"
  }
  if { $force_name_list != "" } {
   lappend ::gPB(${evt}_command,$idx) "   MOM_force Once $force_name_list"
  }
  lappend ::gPB(${evt}_command,$idx) "\}"
}
}
...
#=======================================================================
proc __output_Turbo_MOM_command-X { evt_name cmd_string prefix } {
  switch $evt_name {
   MOM_linear_move {
    set evt turbo_linear
    set sys_var ::mom_sys_linear_turbo_blocks
...
lappend ::gPB(${evt}_command,$idx) "proc $cmd \{ \} \{"
  ...
  __outputBlankLine ::gPB(${evt}_handler)
  if { [info exists cmd] } {
   lappend ::gPB(${evt}_handler) "${prefix}if \{ \$::mom_sys_advanced_turbo_option(SET_SEQNUM_ON_OFF) \} \{"
    lappend ::gPB(${evt}_handler) "${prefix}   lappend $sys_var ${cmd}"
    lappend ::gPB(${evt}_handler) "${prefix}\}"
  }
  if { [info exists cmd] } {
   lappend ::gPB(${evt}_command,$idx) "   $cmd_string"
   lappend ::gPB(${evt}_command,$idx) "\}"
 }
}

#=======================================================================
proc __output_Turbo_MOM_command { evt_name cmd_string prefix } {
  switch $evt_name {
   MOM_linear_move {
    set evt turbo_linear
    set sys_var ::mom_sys_linear_turbo_blocks
  ...
  __outputBlankLine ::gPB(${evt}_handler)
  lappend ::gPB(${evt}_handler) "${prefix}if \{ \$::mom_sys_advanced_turbo_option(SET_SEQNUM_ON_OFF) \} \{"
   lappend ::gPB(${evt}_handler) "${prefix}   lappend $sys_var ${cmd_string}"
   lappend ::gPB(${evt}_handler) "${prefix}\}"
 }

#=======================================================================
proc __outputTextLine { line_text line_prefix EVENT_OUTPUT } {
  upvar $EVENT_OUTPUT event_output
  if { [string trim $line_text] == "" } {
   lappend event_output ""
   } else {
...
#=======================================================================
proc __outputBlankLine { EVENT_OUTPUT } {
  upvar $EVENT_OUTPUT event_output
  set s [string trim [lindex $event_output end]]
  if { $s != "" && ![string match "if *" $s] && ![string match "*\{" $s] && ![string match "#*" $s] } {
    lappend event_output ""
   }
  }
...
proc __outputExecAttr { TEXT PREFIX COND_CMD SUPPRESS_FLAG FORCE_NAME_LIST \
  ...
  upvar $COND_CMD cond_cmd
  upvar $SUPPRESS_FLAG sup_flag
  upvar $FORCE_NAME_LIST force_name_list
  upvar $EVENT_OUTPUT event_output
  if { $cond_cmd != "" } {
   __outputBlankLine event_output
...
puts $tclf_id "proc PB_load_address_redefinition \{ \} \{"
  ...
  puts $tclf_id ""
  set var $mom_kin_var(\$mom_kin_machine_type)
  if [string match "*wedm*" $var] \
  {
   set machine wedm
  } elseif [string match "*_axis_*" $var] \
  ...
  puts $tclf_id "  puts \$tmp_file \"MACHINE $machine\""
  puts $tclf_id "  puts \$tmp_file \"FORMATTING\""
  puts $tclf_id "  puts \$tmp_file \"\{\""
   for { set count 0 } { $count < $no_adds } { incr count } \
   {
