...
proc PB_output_GetMomSysVars {MOM_SYS_NAME_ARR MOM_SYS_VAL_ARR} {
  ...
  set mom_sys_val_arr($ind) $mom_sys_var_arr(Comment_Start)
  incr ind
  set mom_sys_name_arr($ind) "mom_sys_control_in"
  set mom_sys_val_arr($ind) $mom_sys_var_arr(Comment_End)
...
proc PB_output_RetBlksModality { BLK_MOD_ARR } {
  ...
  if [info exists Post::($post_object,post_blk_list)] {
   ...
   foreach post_blk $post_blk_list \
   {
    ...
    set blk_mod_arr($block_name) $blk_mod_adds
    UI_PB_debug_DisplayMsg "Post Block : $block_name"
   }
  }
  set comp_blk_list ""
  ...
  foreach comp_blk $comp_blk_list \
  {
   ...
   set blk_mod_arr($block_name) $blk_mod_adds
   UI_PB_debug_DisplayMsg "Couposit Block : $block_name"
  }
 }
...
puts $tclf_id "proc MOM_machine_mode \{\} \{"
  ...
  puts $tclf_id "    catch \{PB_CMD_machine_mode\}"
 puts $tclf_id "\}"
}
...
#=======================================================================
proc PB_output_OutputBlkModality { BLOCK_NAME BLK_MOD_ARR EVENT_OUTPUT } {
  upvar $BLOCK_NAME block_name
  upvar $BLK_MOD_ARR blk_mod_arr
  upvar $EVENT_OUTPUT event_output
  if {[info exists blk_mod_arr($block_name)]} \
  {
   if { $blk_mod_arr($block_name) != "" } \
   {
    ...
    } else \
    {
     switch $evt_name \
     {
      MOM_rapid_move \
      {
       ...
       {
        set rapid_traverse \
        [PB2TCL_read_fly_var mom_fly_rapid_traverse_blk]
        set rapid_spindle \
        [PB2TCL_read_fly_var mom_fly_rapid_spindle_blk]
     ...
     switch $evt_name \
     {
      ...
      {
       PB_output_OutputBlkModality block_name blk_mod_arr event_output
      ...
      default \
      {
       if {$jj <= $pre_list && [llength [lindex $evt_blocks $jj]] > 0} \
       {
        if { $kk == 0 && $cycle_init_flag == "TRUE" && $has_cycle_start } \
        {
         ...
         PB_output_OutputBlkModality block_name blk_mod_arr \
         event_output
         if $need_right_brace {
          lappend event_output "      MOM_do_template $block_name"
          } else {
       ...
       [llength [lindex $evt_blocks $jj]] > 0} \
       {
...
#=======================================================================
proc __output_ForceRapidAddr { EVENT_OUTPUT block_name BLK_MOD_ARR } {
  upvar $EVENT_OUTPUT event_output
  upvar $BLK_MOD_ARR blk_mod_arr
  set new_mod_list ""
  ...
  foreach add_name $blk_mod_addr \
  {
   switch $add_name \
   {
    ...
    default  { lappend new_mod_list $add_name }
   }
  }
  if { $new_mod_list != "" } \
  {
   set new_mod_list [join $new_mod_list " "]
   lappend event_output "      MOM_force Once $new_mod_list"
  }
 }
...
proc PB_PB2TCL_write_other_procs { TCLF_ID } {
  ...
  if [info exists Post::($post_object,other_proc_list)] \
  {
   ...
   foreach proc_name $Post::($post_object,other_proc_list) \
   {
    ...
    {
     puts $tclf_id "$proc_data($proc_name,comment)"
    }
