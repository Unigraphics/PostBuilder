...
proc PB_output_GetListFileVars { LIST_NAME_ARR LIST_VAL_ARR } {
  ...
  {
   set list_name_arr(10) "mom_sys_pb_link_var_mode"
   set list_val_arr(10)  "ON"
  } else \
  {
   set list_name_arr(10) "mom_sys_pb_link_var_mode"
   set list_val_arr(10)  "OFF"
  }
...
if { ![string match "3_axis_mill_turn" $cur_machine] && \
 ...
 if { ![string compare $fourth_dirn "SIGN_DETERMINES_DIRECTION"] || ![string compare $fifth_dirn "SIGN_DETERMINES_DIRECTION"] } {
...
puts $tclf_id "proc FEEDRATE_SET { } {"
 ...
 puts $tclf_id "}"
 puts $tclf_id "\n"
 if 0 { ;# This is defined in ugpost_bass.tcl.
  puts $tclf_id "#============================================================="
...
puts $tclf_id "proc MOM_head { } {"
  ...
  if 0 {
   ...
   puts $tclf_id "   if { \[info exists mom_sys_in_operation\] && \$mom_sys_in_operation == 1 } {"
    ...
    puts $tclf_id "      CATCH_WARNING \"HEAD event should not be assigned to an operation (\$mom_operation_name).\""
    puts $tclf_id "return"
   puts $tclf_id "   }"
   puts $tclf_id ""
   if 0 {
    ...
    puts $tclf_id "   if { !\[info exists mom_sys_in_operation\] || !\$mom_sys_in_operation } {"
     puts $tclf_id "      set mom_warning_info \"HEAD event should not be assigned to an operation.\""
     puts $tclf_id "      MOM_catch_warning"
     puts $tclf_id "return"
    puts $tclf_id "   }"
    puts $tclf_id ""
   ...
   puts $tclf_id "   if { \[info exists mom_head_name\] } {"
    ...
    puts $tclf_id "   } else {"
    puts $tclf_id "      CATCH_WARNING \"No HEAD event has been assigned.\""
    puts $tclf_id "return"
   puts $tclf_id "   }"
   puts $tclf_id ""
   ...
   puts $tclf_id "   if { !\[info exists mom_sys_postname(\$NEXT_HEAD)\] } {"
    puts $tclf_id ""
    puts $tclf_id "      CATCH_WARNING \"Post is not specified with Head (\$NEXT_HEAD).\""
    puts $tclf_id ""
     ...
     if { ![string compare "MOM_rapid_move" $evt_name] } \
     {
      ...
      ![string compare [PB2TCL_read_fly_var mom_fly_work_plane_change] "TRUE"] } \
      {
       ...
       if { ![string compare $rapid_traverse "$block_name"] } \
       {
        ...
        continue
        } elseif { ![string compare $rapid_spindle "$block_name"] } \
       {
        continue
       }
       set is_done2 1
   ...
   puts $tclf_id "catch {"
    puts $tclf_id "   if { \$ugii_version >= 5 } {"
     puts $tclf_id "      if { \[file exists \"\[file dirname \[info script\]\]/$user_tcl\"\] } {"
      ...
      puts $tclf_id "         source \"\${cam_post_dir}$user_tcl\""
      puts $tclf_id "      } elseif { \[file exists \"$user_tcl\"\] } {"
      puts $tclf_id "        # From the specified directory"
      puts $tclf_id "         source \"$user_tcl\""
      puts $tclf_id "      } else {"
      puts $tclf_id "         CATCH_WARNING \"User's Tcl: $user_tcl not found!\""
     puts $tclf_id "      }"
     puts $tclf_id "   } else {"
     puts $tclf_id "      if { \[file exists \"\${cam_post_dir}$user_tcl\"\] } {"
      puts $tclf_id "         source \"\${cam_post_dir}$user_tcl\""
      puts $tclf_id "      } else {"
      puts $tclf_id "         CATCH_WARNING \"User's Tcl: \${cam_post_dir}$user_tcl not found!\""
     puts $tclf_id "      }"
    puts $tclf_id "   }"
   puts $tclf_id "}"
   puts $tclf_id "\n"
...
proc PB_PB2VNC_write_vnc_base { FILE_ID } {
  ...
  global gPB
  global env
  set vnc_base     vnc_base_v[join [split $gPB(vnc_base_version) .] ""]
  if 0 {
...
puts $file_id "proc VNC_ask_shared_library_suffix { } {"
 ...
 puts $file_id "if { !\[info exists mom_sim_vnc_base_version\] || \\"
  puts $file_id "      \[string compare \"$gPB(vnc_base_version)\" \$mom_sim_vnc_base_version\] > 0 } {"
  puts $file_id ""
...
proc PB_PB2TCL_write_sub_post_tcl { TCLF_ID } {
  ...
  foreach var [lsort -dictionary [array names sys_var_arr]] {
   ...
   switch $value \
   {
    ...
    "MMPR" \
    {
     ...
     puts $tclf_id "  set [format "%-40s  %-5s" [string trim $var \$] \"$value\"]"
    }
    default \
    {
     if { [string match "\$mom_sys_feed_param(*,format)" $var] } {
  ...
  foreach var [lsort -dictionary [array names kin_var_arr]] {
   puts $tclf_id "  set [format "%-40s  %-5s" [string trim $var \$] \"$mom_kin_var($var)\"]"
  }
  puts $tclf_id "  set [format "%-40s  %-5s" "mom_kin_output_unit" \"$mom_kin_var(\$mom_kin_output_unit)\"]"
  puts $tclf_id ""
  puts $tclf_id ""
  puts $tclf_id "#============================================================="
...
puts $tclf_id "proc VNC_load_alternate_unit_settings \{ \} \{"
  ...
  foreach tmp_sim_var $sim_kin_var_list {
   ...
   if { [info exists mom_kin_var(\$$tmp_sim_var_t)] } {
    puts $tclf_id "   set [format "%-40s  %-5s" $tmp_sim_var \"$mom_kin_var(\$$tmp_sim_var_t)\"]"
   }
  }
  puts $tclf_id ""
