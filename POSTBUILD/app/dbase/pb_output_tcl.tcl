...
#=======================================================================
proc PB_output_CallDebugMsg { flag OUTPUT } {
  if { [info exist ::gPB(PB_POST_DEBUG)] &&\
   [string length [string trim $::gPB(PB_POST_DEBUG)]] > 0 } {
   upvar $OUTPUT output
   if [info exists output] {
    switch $flag {
     S { lappend output " $::gPB(PB_POST_DEBUG) START" }
     E { lappend output " $::gPB(PB_POST_DEBUG) END" }
     default {
      lappend output " $::gPB(PB_POST_DEBUG)"
     }
    }
    } else {
    switch $flag {
     S { puts $OUTPUT   " $::gPB(PB_POST_DEBUG) START" }
     E { puts $OUTPUT   " $::gPB(PB_POST_DEBUG) END" }
     default {
      puts $OUTPUT   " $::gPB(PB_POST_DEBUG)"
     }
    }
   }
  }
...
proc PB_PB2TCL_write_other_procs { TCLF_ID } {
  ...
  if [info exists Post::($post_object,other_proc_list)] \
  {
   ...
   if { ![info exists ::gPB(PB_POST_DEBUG_excluded)] } {
    set ::gPB(PB_POST_DEBUG_excluded) [list]
   }
...
puts $tclf_id "proc $proc_name \{ [join $proc_data($proc_name,args)] \} \{"
  ...
  if { [string first "PB_POST__debug_msg" $proc_data($proc_name,proc)] > 0 } {
  ...
  if { $__add_debug_call_start != 0  ||  $__add_debug_call != 0 } {
   if { [lsearch $::gPB(PB_POST_DEBUG_excluded) $proc_name] >= 0 } {
    set __add_debug_call_start 0
    set __add_debug_call 0
   }
  }
  ...
  {
   if { $__add_debug_call == -1 } {
    if { [string first "PB_POST__debug_msg" $line] > 0 } {
     continue
    }
   }
   puts $tclf_id "$line"
...
#=======================================================================
puts $tclf_id "     proc PB_POST__debug_msg { args } {"
  puts $tclf_id "     #----------------------------------"
  puts $tclf_id "        OPERATOR_MSG \">>>>> Dummy PB_POST__debug_msg is in use.\
  Import real command from pb_cmd_debug_utils.tcl. <<<<<\""
 puts $tclf_id "     }"
 puts $tclf_id ""
}
puts $tclf_id ""
