...
proc PB_pps_CleanUpTclFileObjs { args } {
  ...
  if { [llength $args] > 0 } {
   ...
   set event_name_list [array get event_proc_data "*,name"]
...
proc PB_pps_CreateTclFileObjs { POST_OBJECT tcl_file_name sync args } {
  ...
  UI_PB_debug_ForceMsg ">>> event_name_list : $event_name_list"
  set llen [llength $event_name_list]
  for { set i 0 } { $i < $llen } { incr i 2 } \
  {
   set event_item [lindex $event_name_list $i]
...
if { $add_proc } \
 ...
 ![string match "ELSE*" $event_item] && \
 ![string match "END*"  $event_item] && \
![string match "VNC_*" $event_item] } \
{
 set event_name [lindex [split $event_item ,] 0]
 ...
 if { [info exists gPB(pui_ui_overwrite)] && $gPB(pui_ui_overwrite) } {
  UI_PB_debug_ForceMsg ">>> >$tcl_file_name< overwrite = $gPB(pui_ui_overwrite)\n    $event_name \n"
  set oth_proc_list [ladd $oth_proc_list end $event_name "no_dup"]
  set oth_proc_data($event_name,comment) $event_proc_data($event_name,comment)
  set oth_proc_data($event_name,args)    $event_proc_data($event_name,args)
  set oth_proc_data($event_name,proc)    $event_proc_data($event_name,proc)
...
proc PB_pps_UpdatePostAttr {} {
  ...
  uplevel 1 {
   ...
   set mom_var_list [info vars "mom_sys_warning_output_option"]
   if [llength $mom_var_list] {
    set obj_attr(warn_opt) $mom_sys_warning_output_option
   }
   ...
   set mom_var_list [info vars "mom_sys_output_transition_path"]
   if [llength $mom_var_list] {
    set obj_attr(tran_path) $mom_sys_output_transition_path
   }
   set mom_var_list [info vars "mom_sys_post_output_subprogram_enabled"]
   if [llength $mom_var_list] {
    set obj_attr(subprog_out) $mom_sys_post_output_subprogram_enabled
   }
   ...
   foreach v $mom_sys_list {
    if { [array exists $v] } {
     if { [string match "mom_sys_leader" $v] } {
      foreach { add lead } [array get $v] {
       if { ![UI_PB_addr_ValidateAddrLeader $add $lead] } {
        set ${v}($add) $add
        PB_int_RetAddressObjList add_obj_list
        PB_com_RetObjFrmName add add_obj_list add_obj
        UI_PB_mthd_DisplayErrMsg "Address \"$add\" should not be configured with expression \"$lead\"\
        as its leader. Reset to address name.\
        \n\n$::gPB(address,leader,err_msg)"
        address::readvalue $add_obj add_obj_attr
        set add_obj_attr(8) $add
        address::setvalue $add_obj add_obj_attr
        address::DefaultValue $add_obj add_obj_attr
        if { [string match "fourth_axis" $add] } {
         if { [info exists mom_kin_arr(\$mom_kin_4th_axis_leader)] } {
          set mom_kin_arr(\$mom_kin_4th_axis_leader) \
          $address::($add_obj,add_leader)
         }
         } elseif { [string match "fifth_axis" $add] } {
         if { [info exists mom_kin_arr(\$mom_kin_5th_axis_leader)] } {
          set mom_kin_arr(\$mom_kin_5th_axis_leader) \
          $address::($add_obj,add_leader)
         }
        }
        set Post::($post_object,mom_kin_var_list)     [array get mom_kin_arr]
...
global errorInfo
set parse_tcl 1
PB_com_unset_var __bail_out_msg
...
if { [info exists __bail_out_msg] } {
 ...
 return [error "" $__bail_out_msg]
}
if $fake_source {
 rename source ""
 rename $source_cmd source
}
if { ![info exists gPB(dummy_procs)] } {
 set gPB(dummy_procs) ""
}
...
if { [string match "proc *" $temp_line] == 1 } \
 {
  ...
  if { $proc_is_dummy } {
   if { [llength [info commands "$event_name"]] == 0 } {
    if ![string match "*::*" $event_name] {

#=======================================================================
proc $event_name { args } { return 1 } ;#<02-23-05 gsl> 1 was 0.
}
if { [lsearch $gPB(dummy_procs) $event_name] < 0 } {
 lappend gPB(dummy_procs) $event_name
}
}
} else {
...
UI_PB_debug_ForceMsg "\n>>> PB_pps_ParseTclFile >$tcl_file_name< \n    +++ $event_name { $real_args_list }\n"
set proc_data ""
set proc_body ""
...
if { [string match "*_vnc.tcl" $tcl_file_name] &&\
 ...
 if { ($proc_start && [string match "VNC_load_post_definitions*" $event_name]) ||\
  ...
  set __check_syntax_flag $::gPB(check_cc_syntax_error)
  set ::gPB(check_cc_syntax_error) 0
  if { [catch { PB_proc_ValidateCustCmd $event_name $proc_body res }] } {
   UI_PB_debug_ForceMsg "$event_name \n\n[join $proc_body \n] \n\n$errorInfo"
   PB_com_unset_var comment_lines
  }
  set ::gPB(check_cc_syntax_error) $__check_syntax_flag
 }
}
if { $proc_start } {
...
UI_PB_debug_ForceMsg "\>> keep_cmd = $keep_cmd <<<\n"
if $keep_cmd {
...
set key "unset"
if [string match "*$key *" $ee] {
if [__find_keyword "$key" $ee] {
 set idx [string first "$key " $ee]
 set head [string range $ee 0 [expr $idx - 1]]
 set idx [string wordend $ee $idx]
 set tail [string range $ee $idx end]
 set ee "${head}__UNSET${tail}"
}
}
if [string match "global *;#*" $ee] {
set idx [string first ";#" $ee]
set ee [string range $ee 0 [expr $idx -1]]
} elseif [string match "global *#*" $ee] {
set idx [string first "#" $ee]
set ee [string range $ee 0 [expr $idx -1]]
}
if 0 {
...
while { $__PB_proc_ok == 0 } {
 ...
 if [catch { eval [join $__PB_tmp_proc_data \n] } __PB_res] {
  ...
  UI_PB_debug_ForceMsg "Command Syntax Error $__PB_cmd_name : $__PB_res\n"
  ...
  if [string match "*variable*exists*" $__PB_res] {
   ...
   if { $__PB_idx >= 0 } {
    ...
    UI_PB_debug_ForceMsg "Undefined Variable : >$__PB_this_var<"
    set __NO_REF 1
    if [string match "*\$$__PB_var*" $__PB_tmp_proc_data] {

#=======================================================================
UI_PB_debug_ForceMsg "Undefined Variable : >$__PB_var< match in proc body"
 set __add_glob_sym 0
 if { $tcl_version >= "8.0" } {
  if { [string match "::*" $__PB_var] } {
   set __PB_var [string range $__PB_var 2 end]
   set __add_glob_sym 1
  }
 }
 set idx_s [string first $__PB_var $__PB_tmp_proc_data]
 if { $idx_s > 0 } {
  set idx_e [string wordend $__PB_tmp_proc_data $idx_s]
  if { [expr $idx_s + [string length $__PB_var]] == $idx_e ||\
   [string index $__PB_tmp_proc_data $idx_e] == "(" } {

#=======================================================================
UI_PB_debug_ForceMsg "Undefined Variable : >$__PB_var< referenced in proc idx_s = $idx_s idx_e = $idx_e"
 set __NO_REF 0
}
}
if { $__add_glob_sym } {
set __PB_var ::$__PB_var
set __add_glob_sym 0
}
}
if { $__NO_REF } {
...
#=======================================================================
UI_PB_debug_ForceMsg "Undefined Variable : >$__PB_var< not found in proc body."
 if [catch { global $__PB_this_var } res] {
  UI_PB_debug_ForceMsg "1. Caught global $__PB_this_var : $res"
 }
 if { [string match "*($__PB_var)*" $__PB_tmp_proc_data] } {
  set val $$__PB_var
  } else {
 ...
 if { [catch { set $__PB_var $val } res] } {
  UI_PB_debug_ForceMsg "1. Caught setting $__PB_var $val : $res"
  unset $__PB_this_var
  set $__PB_var $val
 }
 UI_PB_debug_ForceMsg "Resolve unknown global var $__PB_this_var ==> set $__PB_var $val"
 ...
 foreach __PB_e $__PB_tmp_proc_data {
  if { [string match "*global* $__PB_this_var *" $__PB_e] ||\
   [string match "*global* $__PB_this_var"   $__PB_e] } {
   UI_PB_debug_ForceMsg "Unknown var >$__PB_this_var< global declaration found"
 ...
 foreach __PB_e $__PB_tmp_proc_data {
  if { [string match "*gets * $__PB_this_var*" $__PB_e] ||\
   ...
   UI_PB_debug_ForceMsg "Unknown var >$__PB_this_var< come thru via gets or uplevel"
   set __PB_var_is_global 1
   if [catch { global $__PB_this_var } res] {
    UI_PB_debug_ForceMsg "2. Caught global $__PB_this_var : $res"
   }
   break
  }
 }
 ...
 if { [llength $args] && [lindex $args 0] == "exp" } {
  UI_PB_debug_ForceMsg "Resolve unknown var $__PB_this_var in expression ==> global $__PB_this_var"
  set __PB_var_is_global 1
  if [catch { global $__PB_this_var } res] {
   UI_PB_debug_ForceMsg "3. Caught global $__PB_this_var : $res"
  }
 }
 if { $tcl_version >= "8.0" } {
  if { [string match "::*" $__PB_this_var] } {
   set __PB_this_var [string range $__PB_this_var 2 end]
   if [catch { global $__PB_this_var } res] {
    UI_PB_debug_ForceMsg "3.1 Caught global $__PB_this_var : $res"
   }
   set __PB_var_is_global 1
   set __PB_this_var ::$__PB_this_var
   UI_PB_debug_ForceMsg "Unknown var >$__PB_this_var< come in as global"
  }
 }
 if $__PB_var_is_global {
  ...
  if { [catch { set $__PB_var $val } res] } {
   UI_PB_debug_ForceMsg "2. Caught setting $__PB_var $val : $res"
   unset $__PB_this_var
   set $__PB_var $val
  }
  UI_PB_debug_ForceMsg "Resolve unknown global var $__PB_this_var ==> set $__PB_var $val"
  ...
  if { $gPB(check_cc_syntax_error) } {
   set __PB_proc_ok -1
   UI_PB_debug_ForceMsg "Command Syntax Full Error : $__PB_res_more"
   set __PB_idx [string last "body line" $__PB_res_more]
   if { $__PB_idx >= 0 } {
    if 0 { ;#<Oct-14-2016 gsl> Not used, see changes below
    ...
    set err_line ""
    set __save_err_line 0
    foreach err [split $__PB_res_more \n] {
     if { $__save_err_line } {
      set err_line $err
      break
     }
     if { [string match "*while executing*" [string trim $err]] } {
      set __save_err_line 1
     }
    }
    if { $err_line != "" } { ;# strip off quotes
     set err_line [string trim $err_line \"]
    }
    UI_PB_debug_ForceMsg "err_line : >$err_line<"
    set __PB_line_num [llength $__PB_tmp_proc_data]
    if { $__PB_line_num > 0 } {
     set __PB_ii 1
     foreach __PB_e $__PB_tmp_proc_data {
      if { $err_line != "" && [string match "*$err_line*" $__PB_e] } {
       set __PB_ii $__PB_line_num
      }
      if { $__PB_ii > $__PB_line_num } { break }
      set __PB_e [string trim $__PB_e]
      if { ![string match ""                  $__PB_e] && \
       ![string match "\#*"               $__PB_e] && \
       ![string match "*global *"         $__PB_e] && \
       [string match "* $__PB_this_var*" $__PB_e] } {
       foreach t [split $__PB_e] {
        if { $t != " " } {
         if { ![info exists s] } {
          concat s "$t"
          } else {
       ...
       UI_PB_debug_ForceMsg "__PB_e $__PB_ii: >$__PB_e< \n\
       __PB_this_var : $__PB_this_var  __PB_var : $__PB_var  __PB_var_is_global : $__PB_var_is_global"
       set __PROCEED 0
       if [string match "*$__PB_this_var*" $__PB_e] {
        set idx_s [string first $__PB_this_var $__PB_e]
        if { $idx_s > 0 } {
         set idx_e [string wordend $__PB_e $idx_s] ;# Not including (); good!
         if { [expr $idx_s + [string length $__PB_this_var]] == $idx_e } {
          UI_PB_debug_ForceMsg "Found undefined var : >$__PB_this_var<"
          set __PROCEED 1
         }
        }
       }
       if { $__PROCEED } {
        UI_PB_debug_ForceMsg "Resolve unknown var : >$__PB_var< ==> Found @ line $__PB_ii : >$__PB_e< ==> Setting global $__PB_this_var"
        if [catch { global $__PB_this_var } res] {
         UI_PB_debug_ForceMsg "4. Caught global $__PB_this_var : $res"
        }
        if { [string match "*($__PB_var)*" $__PB_tmp_proc_data] } {
         set val $$__PB_var
         } else {
        ...
        if { ![string match "*array set *" $__PB_e] } {
         if { [catch { set $__PB_var $val } res] } {
          UI_PB_debug_ForceMsg "3. Caught setting $__PB_var $val : $res"
          unset $__PB_this_var
          set $__PB_var $val
          UI_PB_debug_ForceMsg "Caught setting unknown global var >$__PB_this_var< ==> set >$__PB_var< >$val<"
          } else {
          UI_PB_debug_ForceMsg "Resolve unknown global var >$__PB_this_var< ==> set >$__PB_var< >$val<"
         }
         } else {
         UI_PB_debug_ForceMsg "Unknown var $__PB_this_var is an array ==> >[array get $__PB_this_var]<"
   ...
   UI_PB_debug_ForceMsg "Iteration: $__PB_loop_idx for unknown var >$__PB_this_var<"
   incr __PB_loop_idx
   if { $__PB_loop_idx > $__PB_n_iter } {
    break
    } else {
    continue
   }
   } else { ;# if { !$gPB(check_cc_syntax_error) }
   if { [string match "*($__PB_var)*" $__PB_tmp_proc_data] } {
    set val $$__PB_var
    } else {
    set val $__PB_val
   ...
   if { [catch { set $__PB_var $val } res] } {
    UI_PB_debug_ForceMsg "4. Caught setting $__PB_var $val : $res"
    unset $__PB_this_var
    set $__PB_var $val
   }
   UI_PB_debug_ForceMsg "Resolve unknown global var $__PB_this_var ==> set $__PB_var $val"
...
UI_PB_debug_ForceMsg "Iteration: $__PB_loop_idx for custom command >$__PB_cmd_name<"
incr __PB_loop_idx
...
if { [string match "PB_CMD_init_new_iks" $__PB_cmd_name] ||\
 ...
 uplevel 1 {
     ...
     if [string match "mom_kin_rotary_axis_method" $var] {
      ...
      foreach i { 0 1 2 } {
       if [info exists [set var]($i)] {
        ...
        UI_PB_debug_ForceMsg "\n>>> Fetched : mom_kin_var(\$[set var]($i\)) >$mom_kin_var(\$[set var]($i\))< \n"
       }
      }
     }
    }
  ...
  UI_PB_debug_ForceMsg "\n>>> Machine type : $mom_kin_var(\$mom_kin_machine_type) \n"
  if $fetch_iks {
   ...
   if { ![info exists mom_kin_var(\$mom_kin_4th_axis_vector(0\))] || \
    ...
    UI_PB_debug_ForceMsg "\n>>> Init IKS mom_kin_4th_axis_vector \n"
   }
   ...
   if { ![info exists mom_kin_var(\$mom_kin_5th_axis_vector(0\))] || \
    ...
    UI_PB_debug_ForceMsg "\n>>> Init IKS mom_kin_5th_axis_vector \n"
   }
...
if { [PB_is_v 6.0.2] == 0 } {
if { ![info exists gPB(non_override_mom_sim)] } {
 set gPB(non_override_mom_sim) 0
}
if { $gPB(non_override_mom_sim) == 0 } {
...
if { [string match "proc MOM*" $temp_line] } {
  ...
  if { [lsearch $to_be_delete $proc_name] >= 0 } {
   ...
   if { [info exists the_one] && [string length $the_one] > 0 } {
    set exists_elem_list $event::($the_one,evt_elem_list)
    set is_to_added 1
    foreach elms $exists_elem_list {
     foreach one $elms {
      if { [info exists event_element::($one,evt_elem_name)] && \
       [string match $event_element::($one,evt_elem_name) $cmd_obj_attr(0)] } {
       set is_to_added 0
       break
       } else {
       UI_PB_force_log_msg "1. Event $event::($the_one,event_name) ($exists_elem_list) element >$one< has no name. \n"
      }
     }
    }
    if { $is_to_added } {
...
foreach one $elms {
 if { [info exists event_element::($one,evt_elem_name)] &&\
  [string match $event_element::($one,evt_elem_name) $command_name] } {
  set is_to_added 0
  break
  } else {
  UI_PB_force_log_msg "2. Event $event::($event_obj,event_name) ($exists_elem_list) element >$one< has no name. \n"
 }
}
}
if $is_to_added {
