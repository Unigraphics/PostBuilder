    ...
    if { ![string match "*cycle_breakchip*" $var] } {
     if { ![info exists g_codes_arr($var)] } {
      ...
      if ![string match {$*} [lindex $line 1]] {
       set text [__prv_TransEventName [lindex $line 1] 1]
       } else {
     ...
     if { $merge_marker && \
      ...
      if { [llength $line] == 3  &&  [string length [lindex $line 2]] } {
       if ![string match {$gPB*} [lindex $line 2]] {
        set evt_label_arr($index) [__prv_TransEventName [lindex $line 2] 1]
        } else {
     ...
     if { $tpth_ctrl_seq || $tpth_mot_seq || $tpth_cycle_seq } \
     {
      ...
      if {$::env(PB_UDE_ENABLED) == 0} {
       ...
       if $tpth_ctrl_seq {
        ...
        {Spindle CSS} {Thread Wire} {Cut Wire} {Set Mode}]
...
proc PB_pui_WriteSIMVariables { PUIFID MOM_SIM_ARR } {
  ...
  foreach sim_var $arr_names \
  {
   ...
   if { $len > 10 } {
    ...
    for { set i 1 } { $i < [expr $len - 2]} { incr i } {
