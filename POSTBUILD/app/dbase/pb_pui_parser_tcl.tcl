...
#=======================================================================
proc PB_pui_CompareVersion { this_ver target_ver } {
...
#=======================================================================
proc PB_pui_ReadPuiCollectMomSysVars { OBJECT POST_OBJ pui_file } {
  upvar $OBJECT object
  upvar $POST_OBJ post_obj
  global gPB
  global output_unit mach_axis
  set mom_sys_var_start 0
  if { [catch { open "$pui_file" r } file_id] } {
   return 0
  }
  while { [gets $file_id line] >= 0 } \
  {
   ...
   switch -glob -- "$line"\
   {
    ...
    "## MOM SYS VARIABLES END" {
     if { [info exists add_name_list] } \
     {
      ...
      foreach add {G_feed PB_Tcl_Var PB_Rapid PB_ZeroFmt} \
      {
       if { [info exists add_mom_var_arr($add,idx)] } \
       {
        set i $add_mom_var_arr($add,idx)
        if { [info exists add_name_list($i)] } \
        {
         foreach e $add_mom_var_list($i) {
          set var [lindex $e 0]
          ...
          if { $var != "" && $val != "" } {
           if { ![ catch {eval format %s "$val"} res ] } {
            set mom_sys_var_arr($var) "$val"
            UI_PB_debug_ForceMsg "\n   +++++++   $add : $var $val eval format OK => mom_sys_var_arr($var) defined. +++++\n"
            } else {
            UI_PB_debug_ForceMsg "\n   +++++++   $add : $var $val eval format failed => mom_sys_var_arr($var) not defined. +++++\n"
            set mom_sys_var_arr($var) ""
           }
          }
         }
        }
            ...
            if { [UI_PB_tpth_SetIJKExpr $machine $mode expr_i expr_j expr_k] } {
             ...
             switch "$add_name_list($i)" {
              ...
              "J" {
               ...
               set desc "$desc Y-Axis"
              }
              "K" {
               set expr "$expr_k"
               set desc "$desc Z-Axis"
              }
             }
     ...
     UI_PB_debug_ForceMsg " ++++++ MOM SYS vars collected +++++ "
   ...
   if { $mom_sys_var_start } \
   {
    ...
    if { [string match "PB_Dummy" $mvar] } {
     UI_PB_debug_ForceMsg "\n PB_Dummy : $line_val \n"
     if { [string match "*User_Def_Axis_Limit_Act*" $line_val] } {
      for { set ii 0 } { $ii < [llength $line_val] } { incr ii } {
       set lvar [lindex $line_val $ii]
       if { [string match "*User_Def_Axis_Limit_Act*" [lindex $lvar 0]] } {
        set tmp_list "\"[lindex $lvar 0]\""
        for { set jj 1 } { $jj < [llength $lvar] } { incr jj } {
         set elm [lindex $lvar $jj]
         if { [string match "*IF*" $elm] } {
          regsub {IF\(} $elm "" elm
          regsub {\)}   $elm "" elm
         }
         regsub "{" $elm "\\{" elm
           regsub "}" $elm "\\}" elm
         lappend tmp_list \"$elm\"
        }
        set line_val [lreplace $line_val $ii $ii [join $tmp_list]]
        UI_PB_debug_ForceMsg "\n User_Def_Axis_Limit_Act : $line_val \n"
        break
       }
      }
     }
    ...
    if { [string match "PB_Dummy" $mvar] } {
     if { [string match "*PB_POST_DEBUG*" $line_val] } {
      for { set ii 0 } { $ii < [llength $line_val] } { incr ii } {
       set lvar [lindex $line_val $ii]
       if { [string match "*PB_POST_DEBUG*" [lindex $lvar 0]] } {
        set ::gPB(PB_POST_DEBUG) [lindex $lvar 1]
        UI_PB_debug_ForceMsg "\n ::gPB(PB_POST_DEBUG) = $::gPB(PB_POST_DEBUG) \n"
        break
       }
      }
     }
    } ;# "PB_Dummy"
    if { [string match "Operator Message" $mvar] } {
     set mvar "Operator_Message"
    }
  ...
  while { [gets $File::($object,FilePointer) line] >= 0 } \
  {
   ...
   switch -glob -- "$line"\
   {
    ...
    "## LISTING FILE END"            {
     ...
     {
      if { ![info exists obj_attr(warn_opt)] } {
       set obj_attr(warn_opt) FILE
      }
    ...
    "## GCODES END"                  {
     ...
     if { [info exists g_codes] } \
     {
      ...
      PB_com_unset_var g_codes
      PB_com_unset_var g_codes_desc
      PB_com_unset_var g_codes_arr
     }
     continue
    }
    "## MCODES START"                {
    ...
    "## COMPOSITE BLOCKS END"        {
     ...
     if { $::gPB(DISABLE_COMP_BLK) } {
      if { [info exists Post::($post_obj,comp_blk_list)] && \
       [llength $Post::($post_obj,comp_blk_list)] > 0 } {
       UI_PB_debug_ForceMsg " +++++ COMPOSITE BLOCKS >$Post::($post_obj,comp_blk_list)< +++++\n"
       set ::gPB(DISABLE_COMP_BLK) 0
      }
     }
     continue
    }
    ...
    "## EVENTS ELEMENT EXECUTION START" {
     set mom_sys_var_collected [PB_pui_ReadPuiCollectMomSysVars object post_obj $File::($object,FileName)]
     array set mom_sys_var_arr $Post::($post_obj,mom_sys_var_list)
     UI_PB_debug_ForceMsg " +++++ Collect MOM SYS VARS from >$File::($object,FileName)< => >$mom_sys_var_collected<  +++++\n"
     ...
     set ver_check 1
     if { 0 && [PB_pui_CompareVersion $gPB(this_PUI_version) "2008.0.3.1"] >= 0 } {
      set ver_check [PB_pui_CompareVersion $gPB(this_PUI_version) "$gPB(Postbuilder_PUI_Version)"]
      } else {
      set ver_check [PB_pui_CompareVersion $gPB(this_PUI_version) "2008.0.4.1"]
      UI_PB_debug_ForceMsg " +++++ ver_check == $ver_check  gPB(this_PUI_version) = $gPB(this_PUI_version)"
     }
     if { $ver_check < 0 } {
      set cyc_com_evt [lindex $Post::($post_obj,cyl_com_evt) 0]
      set shared_cyc_evt_list [lindex $Post::($post_obj,cyl_evt_sh_com_evt) 0]
      UI_PB_debug_ForceMsg " +++++ cyc_com_evt: $cyc_com_evt"
      UI_PB_debug_ForceMsg " +++++ shared_cyc_evt_list: $shared_cyc_evt_list"
      set evt_name_arr_list $Post::($post_obj,tpth_cycle_evt_list)
      array set evt_blk_arr $Post::($post_obj,tpth_cycle_evt_blk_list)
      set com_evt_indx [lsearch $evt_name_arr_list $cyc_com_evt]
      set com_evt_indx [lindex $evt_name_arr_list [expr $com_evt_indx - 1]]
      set com_cyc_evt_blk_list $evt_blk_arr($com_evt_indx)
      UI_PB_debug_ForceMsg " +++++ com_cyc_evt_blk_list: $com_cyc_evt_blk_list"
      if { [info exists ::mom_sys_arr(post_startblk)]  &&  $::mom_sys_arr(post_startblk) != "" } {
       set post_startblk $::mom_sys_arr(post_startblk)
       } elseif { [info exists Post::($post_obj,rst_post_startblk)]  && \
       $Post::($post_obj,rst_post_startblk) != "" } {
       set post_startblk $Post::($post_obj,rst_post_startblk)
       } else {
       set post_startblk "PB_CMD_config_cycle_start"
      }
      set n_com_cyc_blk 0
      set idx [lsearch $com_cyc_evt_blk_list $post_startblk]
      UI_PB_debug_ForceMsg " +++++ post_startblk : $post_startblk  idx : $idx"
      set n_com_cyc_blk [llength $com_cyc_evt_blk_list]
      if 0 {
       if { $idx > 0  &&  [string first "just_mcall_sinumerik" $com_cyc_evt_blk_list] > 0 } {
        set n_com_cyc_blk [expr $idx - 1]
       }
       } else {
       set idx -1
       foreach com_blk $com_cyc_evt_blk_list {
        incr idx
        if { [string match "*just_mcall_sinumerik*" $com_blk] } {
         set n_com_cyc_blk $idx
         break
        }
       }
      }
      UI_PB_debug_ForceMsg " +++++ n_com_cyc_blk: $n_com_cyc_blk"
     }
     continue
    }
    "## EVENTS ELEMENT EXECUTION END"  {
     set evt_elem_exec_start 0
     if { $ver_check < 0  &&  $n_com_cyc_blk > 0 } {
      if { [info exists com_exec_attr] && [llength $com_exec_attr] > 0 } {
       if { ![info exists exec_name_list] } {
        set index 0
        } else {
        set index [llength $exec_name_list]
       }
       foreach cyc $shared_cyc_evt_list {
        if { [lsearch $exec_name_list "$cyc"] < 0  && \
         ![UI_PB_tpth_IsCycleCustom $cyc] } {
         lappend exec_name_list "$cyc"
         set evt_elem_exec_arr($index) $com_exec_attr
         UI_PB_debug_ForceMsg "   +++++++   Reconstruct exec attr for $cyc : $evt_elem_exec_arr($index)"
         incr index
        }
       }
      }
    ...
    "## MOM SYS VARIABLES START" {
     if { $mom_sys_var_collected } { continue }
     set mom_sys_var_start 1
     set add_mom_indx 0
     set index -1
     set mom_var_found 0
    ...
    "## MOM SYS VARIABLES END" {
     ...
     if [info exists add_name_list] \
     {
         ...
         switch "$add_name_list($i)" {
          ...
          "PB_ZeroFmt" {
           foreach e $add_mom_var_list($i) {
            ...
            if { [string match "PB_Rapid" $add_name_list($i)] } {
             UI_PB_debug_ForceMsg "\n   +++++++   PB_Rapid : $var $val => $mom_sys_var_arr($var) +++++\n"
            }
           }
          }
         }
      ...
      foreach add {G_feed PB_Tcl_Var PB_Rapid PB_ZeroFmt} {
       if { ![info exist add_mom_var_arr($add,idx)] } {
        continue
       }
       set i $add_mom_var_arr($add,idx)
       if [info exists add_name_list($i)] {
   ...
   if { $post_info_start } {
    switch -glob -- "$line" \
    {
     ...
     "#  CONTROLLER TYPE END"     {
      set type_family_start 0
      set gPB(post_controller_type) [join $post_info end]
      unset post_info
      continue
     }
     "#  MACHINE AXIS START" {
     ...
     "#  HISTORY END"        {
      ...
      UI_PB_debug_ForceMsg "   %%%%%%%%%%%%%%%%%  gPB(post_history) $gPB(post_history)"
      ...
      if { [string match "Saved with *"   $last_save] ||\
       ...
       set w_list [split $last_save]
       set vi [lsearch $w_list "Version"]
       set gPB(post_last_saved_version) [lindex $w_list [expr $vi+1]]
       UI_PB_debug_ForceMsg "   %%%%%%%%%%%%%%%%%  gPB(post_last_saved_version)  $gPB(post_last_saved_version)"
      }
      unset post_info
      continue
     }
   ...
   if { $list_file_start } \
   {
    ...
    switch $Name \
    {
     ...
     Listfileflag_warn_opt      {set obj_attr(warn_opt)   $Value}
   ...
   if { $kinematic_var_start } \
   {
    ...
    if [string match "*mom_kin_machine_type*" $var] {
     set __pui_kin_machine_type $mom_kin_var($var)
    }
   }
   ...
   if { $gcode_start } \
   {
    ...
    if { ![info exists mach_axis] } { ;# Not a new post (see above for new post)
     if { [info exists __pui_kin_machine_type] && [string match "3_axis_mill" $__pui_kin_machine_type] } {
      if [string match "*mom_sys_feed_rate_mode_code(DPM)*" $var] {
       UI_PB_debug_ForceMsg "   %%%%%%%%%%%%%%%%%  DPM feedrate mode excluded from 3-axis posts"
       continue
      }
     }
    }
    if { [string match "*mom_sys_feed_rate_mode_code(IPM)*" $var] &&\
     ![string match "*\$gPB(g_code,feedmode_in,pm)*" [lindex $line 1]] } {
     set line [lreplace $line 1 1 "\$gPB(g_code,feedmode_in,pm)"]
    }
    if { [string match "*mom_sys_feed_rate_mode_code(IPR)*" $var] &&\
     ![string match "*\$gPB(g_code,feedmode_in,pr)*" [lindex $line 1]] } {
     set line [lreplace $line 1 1 "\$gPB(g_code,feedmode_in,pr)"]
    }
    if { [string match "*mom_sys_feed_rate_mode_code(MMPM)*" $var] &&\
     ![string match "*\$gPB(g_code,feedmode_mm,pm)*" [lindex $line 1]] } {
     set line [lreplace $line 1 1 "\$gPB(g_code,feedmode_mm,pm)"]
    }
    if { [string match "*mom_sys_feed_rate_mode_code(MMPR)*" $var] &&\
     ![string match "*\$gPB(g_code,feedmode_mm,pr)*" [lindex $line 1]] } {
     set line [lreplace $line 1 1 "\$gPB(g_code,feedmode_mm,pr)"]
    }
   ...
   if { $comp_blk_start } \
   {
    if { [llength [eval string trim $line]] } {
     UI_PB_debug_ForceMsg " +++++ COMPOSITE BLOCKS line >$line< +++++\n"
     if [info exists Post::($post_obj,comp_blk_list)] \
     {
   ...
   if { $cycle_evt_start } \
   {
    switch -glob -- "$line" \
    {
     ...
     "#Cycle Block Share Common Block Start"  {
      if [info exists Post::($post_obj,cyl_evt_sh_com_evt)] {
       ...
       UI_PB_debug_ForceMsg "\n   %%%%%%%%%%%%%%%%%  Existing cycle_shared_evts : $cycle_shared_evts"
       } else {
       set Post::($post_obj,cyl_evt_sh_com_evt) [list]
      }
    ...
    if { $cycle_share_evt_start } \
    {
     if 0 {
      ...
      if { [llength $Post::($post_obj,cyl_evt_sh_com_evt)] > 0 } {
       set cycle_shared_evts [lindex $cycle_shared_evts 0]
       UI_PB_debug_ForceMsg "\n   %%%%%%%%%%%%%%%%%    line : $line"
       foreach cyc [lindex $line 0] {
        if { [lsearch [lindex $Post::($post_obj,cyl_evt_sh_com_evt) 0] $cyc] < 0 } {
         UI_PB_debug_ForceMsg "\n   %%%%%%%%%%%%%%%%%    adding $cyc"
         lappend cycle_shared_evts $cyc
        }
       }
       if { [llength $cycle_shared_evts] > 0 } {
        set cycle_shared_evts "\{ $cycle_shared_evts \}"
       }
       } else {
       set cycle_shared_evts $line
      }
     }
     UI_PB_debug_ForceMsg "\n   %%%%%%%%%%%%%%%%%  cycle_shared_evts : $cycle_shared_evts"
    }
   }
   if { $glob_sequence_start } \
   {
    switch -glob -- "$line" \
    {
     ...
     "#Operation Start Sequence End"     {
      ...
      if [info exists evt_name_arr] \
      {
       ...
       [array get evt_blk_arr]
       if 0 { ;# New handlers added during the event parsing below where oper_start_seq is on -
        if { [info exists __pui_kin_machine_type] && [string match "*_wedm" $__pui_kin_machine_type] } {
         UI_PB_debug_ForceMsg "\n +++++ $File::($object,FileName) +++++\n"
         UI_PB_debug_ForceMsg "\n +++++ __pui_kin_machine_type $__pui_kin_machine_type +++++\n"
         if { [lsearch $Post::($post_obj,oper_start_evt_list) "Initial Move"] < 0 } {
          set iappr [lsearch $Post::($post_obj,oper_start_evt_list) "Approach Move"]
          if { $iappr >= 0 } {
           set Post::($post_obj,oper_start_evt_list) [linsert $Post::($post_obj,oper_start_evt_list) $iappr "Initial Move"]
           set Post::($post_obj,oper_start_evt_list) [linsert $Post::($post_obj,oper_start_evt_list) $iappr "First Move"]
           UI_PB_debug_ForceMsg "\n +++++ oper_start_evt_list   : >$Post::($post_obj,oper_start_evt_list)< +++++\n"
           UI_PB_debug_ForceMsg "\n +++++ oper_start_label_list : >$Post::($post_obj,oper_start_label_list)< +++++\n"
          }
         }
        }
       }
    ...
    if { $prog_start_seq || $oper_start_seq || $oper_end_seq || $prog_end_seq } \
    {
     ...
     if { $merge_marker && \
      ...
      for { set ii 0 } { $ii < [llength $evt_blk_arr($index)] } { incr ii } {
       set elm [lindex $evt_blk_arr($index) $ii]
       if { [string match "IF(*)" $elm] } {
        set elm [string trimright $elm ")"]
        set elm [string range $elm 3 end]
        set evt_blk_arr($index) [lreplace $evt_blk_arr($index) $ii $ii $elm]
       }
      }
      incr index
     }
     if { [info exists __pui_kin_machine_type] && [string match "*_wedm" $__pui_kin_machine_type] } {
      UI_PB_debug_ForceMsg "\n +++++ $File::($object,FileName) +++++\n"
      UI_PB_debug_ForceMsg "\n +++++ __pui_kin_machine_type $__pui_kin_machine_type +++++\n"
      if { [string match "Approach Move" $evt_name] } {
       if { [lsearch [array get evt_name_arr] "Initial Move"] < 0 } {
        incr index -1
        set __blk_list $evt_blk_arr($index)
        UI_PB_debug_ForceMsg "\n +++++ Insert Initial Move & First Move +++++\n"
        set evt_name_arr($index)  "Initial Move"
        set evt_label_arr($index) {$gPB(event,opr_start,init_move,name)}
        set evt_blk_arr($index)   [list]
        incr index
        set evt_name_arr($index) "First Move"
        set evt_label_arr($index) {$gPB(event,opr_start,fst_move,name)}
        set evt_blk_arr($index)   [list]
        incr index
        set evt_name_arr($index) "$evt_name"
        set evt_label_arr($index) {$gPB(event,opr_start,appro_move,name)}
        set evt_blk_arr($index)   $__blk_list
        incr index
       }
      }
     }
     } elseif { $tool_path_seq } \
    {
     ...
     if { $tpth_ctrl_seq || $tpth_mot_seq || $tpth_cycle_seq || $tpth_misc_seq } \
     {
      ...
      if { $tpth_cycle_seq } {
       if { [llength $evt_blk_arr($index)] > 1 } {  ;# Having more than 1 block
        set blk_list $evt_blk_arr($index)
        UI_PB_debug_ForceMsg "\n +++++ Cycle $evt_name_arr($index) blocks : $blk_list"
        set cyc_blk_name cycle_[join [split [string tolower $evt_name_arr($index)]] "_"]  ;# default block name
        set icount 0
        set blk_idx -1
        set blk_num 10
        foreach blk $blk_list {
         incr blk_idx
         if [string match "${cyc_blk_name}_10" $blk] {
          incr icount
          if { $icount == 1 } {
           continue
          }
          if { 0 } {
           set match_list [lsearch -all $blk_list $blk]
           set len [llength $match_list]
           if { $len > 1 } {  ;# More than 1 block found with same name
            UI_PB_debug_ForceMsg " +++++ Duplicate cycle blocks : $blk_list"
            for { set idx 1 } { $idx < $len } { incr idx } {
             set blk_idx [lindex $match_list $idx]
             set blk_name "${cyc_blk_name}_[incr blk_num]"
             set blk_list [lreplace $blk_list $blk_idx $blk_idx $blk_name]
            }
            UI_PB_debug_ForceMsg " +++++ Fixed duplicate cycle blocks : $blk_list"
           }
           } else {
           while { 0 } {
            set blk_idx [lsearch $blk_list $blk]
            if { $blk_idx >= 0 } {
             set blk_name "${cyc_blk_name}_[incr blk_num]"
             set blk_list [lreplace $blk_list $blk_idx $blk_idx $blk_name]
             } else {
        ...
        set evt_blk_arr($index) $blk_list
       }
       if { ![string compare "Cycle Parameters" $evt_name] && [info exists ::gPB(post_controller_type)] } {
        if { [PB_com_IsSiemensController    $gPB(post_controller_type) 1] ||\
         [PB_com_IsHeidenhainController $gPB(post_controller_type)] } {
         if { ![string match "*just_mcall_sinumerik*" $evt_blk_arr($index)] } {
          lappend evt_blk_arr($index) {just_mcall_sinumerik {} 0}
          __Pause "$File::($object,FileName) => $evt_name block list : $evt_blk_arr($index)"
         }
        }
       }
      }
      ...
      set ::gPB(EXIST_CYCLE_EVENT) [list \
      {Cycle Parameters} {Cycle Off} {Cycle Plane Change} {Drill} \
      ...
      if { $::env(PB_UDE_ENABLED) == 0 } {
       if { $tpth_cycle_seq } {
        ...
        if { [lsearch $::gPB(EXIST_CYCLE_EVENT) $evt_name] < 0 } {
   ...
   if { $evt_elem_exec_start } \
   {
    ...
    UI_PB_debug_ForceMsg " +++++ EXECUTION element: $line"
    if { $ver_check < 0  &&  $n_com_cyc_blk > 0 } {
     if { ![info exists com_exec_attr] } { set com_exec_attr [list] }
     if { [string match "$evar" $cyc_com_evt] } {
      set com_exec_attr [lrange $line 1 end]
     }
     set cyc_idx [lsearch $shared_cyc_evt_list "$evar"]
     if { $cyc_idx >= 0  &&  ![UI_PB_tpth_IsCycleCustom $evar] } {
      UI_PB_debug_ForceMsg "\n +++++ EXECUTION element: $cyc_idx $evar => n_com_cyc_blk : $n_com_cyc_blk"
      UI_PB_debug_ForceMsg " +++++ EXECUTION element: com_exec_attr : $com_exec_attr"
      set blk_idx 1
      set n_blk $n_com_cyc_blk
      foreach attr [lrange $line 1 end] {
       if { [lsearch $com_exec_attr $attr] == -1 } {
        set attr_idx [lindex $attr 0]
        UI_PB_debug_ForceMsg " +++++ EXECUTION element: attr_idx : $attr_idx  blk_idx : $blk_idx"
        set attr [lreplace $attr 0 0 [expr $attr_idx + $n_com_cyc_blk]]
        set line [lreplace $line $blk_idx $blk_idx $attr]
        UI_PB_debug_ForceMsg " +++++ EXECUTION element: line : $line"
       }
       incr blk_idx
       incr n_blk
      }
      UI_PB_debug_ForceMsg " +++++ EXECUTION element fudged: $line\n"
     }
    }
    ...
    } else \
    {
     ...
     UI_PB_debug_ForceMsg " +++++ EXECUTION element stored for $evar $index : $evt_elem_exec_arr($index)"
    }
   }
   if { $evt_ui_start } \
   {
   ...
   if { $mom_sys_var_start } \
   {
    ...
    if { ![info exists add_mom_var_arr($mvar,var)] ||\
    ![info exists add_mom_var_arr($mvar,idx)] } \
    {
     set add_name_list($add_mom_indx) $mvar
     set add_mom_var_list($add_mom_indx) $line_val
     set add_mom_var_arr($mvar,var) $add_mom_var_list($add_mom_indx)
...
proc PB_pui_WriteListingFileData { PUIFID LISTFILE_OBJ } {
  ...
  foreach var_name $arr_names \
  {
   switch "$var_name" \
   {
    ...
    "verbose"        \
    {
     if { $obj_attr($var_name) } {
      set obj_attr($var_name) "ON"
      } else {
      set obj_attr($var_name) "OFF"
     }
    }
    default {}
   }
   switch "$var_name" \
   {
    "listfile"       { puts $puifid "[format "%-30s %s" Listfileflag               $obj_attr($var_name)]" }
    "fname"          { puts $puifid "[format "%-30s %s" Listfilename               $obj_attr($var_name)]" }
    "head"           { puts $puifid "[format "%-30s %s" Listfileflag_head          $obj_attr($var_name)]" }
    "lines"          { puts $puifid "[format "%-30s %s" Listfilelines              $obj_attr($var_name)]" }
    "column"         { puts $puifid "[format "%-30s %s" Listfilecolumn             $obj_attr($var_name)]" }
    "oper"           { puts $puifid "[format "%-30s %s" Listfileflag_oper          $obj_attr($var_name)]" }
    "tool"           { puts $puifid "[format "%-30s %s" Listfileflag_tool          $obj_attr($var_name)]" }
    "start_path"     { puts $puifid "[format "%-30s %s" Listfileflag_start_path    $obj_attr($var_name)]" }
    "tool_chng"      { puts $puifid "[format "%-30s %s" Listfileflag_tool_chng     $obj_attr($var_name)]" }
    "end_path"       { puts $puifid "[format "%-30s %s" Listfileflag_end_path      $obj_attr($var_name)]" }
    "oper_time"      { puts $puifid "[format "%-30s %s" Listfileflag_oper_time     $obj_attr($var_name)]" }
    "setup_time"     { puts $puifid "[format "%-30s %s" Listfileflag_setup_time    $obj_attr($var_name)]" }
    "warn"           { puts $puifid "[format "%-30s %s" Listfileflag_warn          $obj_attr($var_name)]" }
    "warn_opt"       { puts $puifid "[format "%-30s %s" Listfileflag_warn_opt      $obj_attr($var_name)]" }
    "lpt_ext"        { puts $puifid "[format "%-30s %s" Listfileflag_lpt_ext       $obj_attr($var_name)]" }
    "review"         { puts $puifid "[format "%-30s %s" Listfileflag_review        $obj_attr($var_name)]" }
    "group"          { puts $puifid "[format "%-30s %s" Listfileflag_group         $obj_attr($var_name)]" }
    "ncfile_ext"     { puts $puifid "[format "%-30s %s" Listfileflag_ncfile_ext    $obj_attr($var_name)]" }
    "usertcl_check"  { puts $puifid "[format "%-30s %s" Listfileflag_usertcl_check $obj_attr($var_name)]" }
    "usertcl_name"   { puts $puifid "[format "%-30s %s" Listfileflag_usertcl_name  $obj_attr($var_name)]" }
    "verbose"        { puts $puifid "[format "%-30s %s" Listfileflag_verbose       $obj_attr($var_name)]" }
    "alt_unit_post_name" \
    { puts $puifid "[format "%-30s %s" Listfileflag_alt_unit_post_name        $obj_attr($var_name)]" }
    "use_default_unit_fragment" \
    { puts $puifid "[format "%-30s %s" Listfileflag_use_default_unit_fragment $obj_attr($var_name)]" }
    default {}
   }
  }
...
proc PB_pui_WriteSIMVariables { PUIFID MOM_SIM_ARR } {
  ...
  if { [llength $cmd_name_list] } {
   set idx 0
   foreach cmd $cmd_name_list {
    PB_com_RetObjFrmName cmd cmd_obj_list cmd_obj
    if { $cmd_obj > 0} {
     set cmd_objs_arr($cmd) $cmd_obj
     incr idx
     } else {
     set cmd_name_list [lreplace $cmd_name_list $idx $idx]
    }
   }
   if [string match "Standalone" $mom_sys_arr(VNC_Mode)] {
    set mom_sim_arr(\$mom_sim_vnc_com_list) $cmd_name_list
    } else {
    set mom_sim_arr(\$mom_sim_sub_vnc_list) $cmd_name_list
   }
  }
  ...
  foreach sim_var $arr_names \
  {
   ...
   if { $len > 10 } {
    ...
    if { [string match "\{*\}" $first_item] } {
     ...
     puts $puifid "    [format "%-40s  %s"  \"$sim_var\" \\]"
     puts $puifid "    \"\{$first_item\}\ \\"
    }
  ...
  if 0 {  ;#<05/13/2016 gsl> List has been prepared earlier.
   Post::GetObjList $post_object command cmd_obj_list
   set cmd_name_list [list]
   if [string match "Standalone" $mom_sys_arr(VNC_Mode)] {
    if { [info exists mom_sim_arr(\$mom_sim_vnc_com_list)] } {
  ...
  set last_idx [expr $length - 1]
  global vnc_desc_arr
  for { set count 0 } { $count < $length } { incr count } {
   set name [lindex $cmd_name_list $count]
   set cmd_obj $cmd_objs_arr($name)
   command::readvalue $cmd_obj cmd_attr
   if 0 {  ;#<05/13/2016 gsl> List has been cleaned up & verified already.
...
proc PB_pui_WriteEvtElemExecutionData { PUIFID SEQ_OBJ_LIST } {
  ...
  global post_object
  set cyl_sh_com_evt [lindex $Post::($post_object,cyl_evt_sh_com_evt) 0]
  UI_PB_debug_ForceMsg "\n ++++++ Cycle shared common events: $cyl_sh_com_evt \n"
  UI_PB_debug_ForceMsg "\n ++++++ Cycle common event: [lindex $Post::($post_object,cyl_com_evt) 0] \n"
   ...
   foreach evt_obj $seq_events_list \
   {
    ...
    foreach evt_elem_obj $event::($evt_obj,evt_elem_list) \
    {
     ...
     foreach sub_elem_obj $sub_elem_list \
     {
      ...
      if { [info exists event_element::($sub_elem_obj,indent)] && \
      $event_element::($sub_elem_obj,indent) > 0 } \
      {
       if { $elem_str == "" } {
        set elem_str "$index NULL 0"
       }
       if { $addr_name_list != "" } {
        append elem_str " $event_element::($sub_elem_obj,indent)"
        } else {
        append elem_str " \{\} $event_element::($sub_elem_obj,indent)"
       }
      }
    ...
    if { $exec_list != "" } \
    {
     ...
     {
      UI_PB_debug_ForceMsg "\n ++++++ Cycle EXECUTION attr for $event::($evt_obj,event_name) $index/$len : \{$elem_str\}"
      if { $index == $len } \
      {
      ...
      foreach grp_obj $item_obj_attr(3) \
      {
       ...
       puts $puifid "        \{\{\"$grp_obj_attr(0)\" $grp_obj_attr(1) \
        ...
        foreach mem_obj $grp_obj_attr(3) \
        {
         ...
         {
          append tmp_3mem_out "[string trim $mem_obj_attr(5) \"]"  ;#<09-03-2015 gsl> "$mem_obj_attr(5)"
         }
...
proc PB_pui_WriteMOMVariables { PUIFID ADD_NAME_LIST ADD_OBJ_LIST \
  ...
  foreach add_name $add_obj_name_list \
  {
   ...
   if [info exists word_mom_var($add_name)] \
   {
    ...
    if [string match "PB_Dummy" $add_name] {
     ...
     if { ![info exists mom_sys_arr(PB_POST_DEBUG)] && [info exists ::gPB(PB_POST_DEBUG)] } {
      if { $::gPB(PB_POST_DEBUG) != "" } {
       set mom_var_arr(PB_POST_DEBUG) $::gPB(PB_POST_DEBUG)
       lappend word_mom_var($add_name) "PB_POST_DEBUG"
      }
     }
     if { [info exists mom_sys_arr(PB_POST_DEBUG)] && [info exists ::gPB(PB_POST_DEBUG)] } {
      set mom_var_arr(PB_POST_DEBUG) $::gPB(PB_POST_DEBUG)
     ...
     if 0 {
      set idx [lsearch -exact $word_mom_var($add_name) "User_Def_Axis_Limit_Act"]
      if { $idx >= 0 } {
       foreach elm $mom_var_arr(User_Def_Axis_Limit_Act) {
       }
      }
     }
     set mom_var_arr(UDE_File_Name) $Post::($post_object,UDE_File_Name)
     UI_PB_debug_ForceMsg "######### MOM_VAR_ARR ######### : \n[array get mom_var_arr]"
    }
    if [string match "PB_Tcl_Var" $add_name] {
     if { [info exists mom_sys_arr(\$mom_sys_advanced_turbo_output)] && \
      [lsearch -exact $word_mom_var($add_name) "\$mom_sys_advanced_turbo_output"] == -1 } {
      lappend word_mom_var($add_name) "\$mom_sys_advanced_turbo_output"
      set mom_var_arr(\$mom_sys_advanced_turbo_output) $mom_sys_arr(\$mom_sys_advanced_turbo_output)
      UI_PB_debug_ForceMsg "\n #########  Add mom_sys_advanced_turbo_output : >$mom_sys_arr(\$mom_sys_advanced_turbo_output)<\n"
     }
    }
    if [string match "Command" $add_name] {
     foreach var $word_mom_var($add_name) {
      if { [string match "PB_CMD_vnc*" $var] ||\
       [string match "ELSE" $var] || [string match "END" $var] } {
    ...
    for { set count 0 } { $count < $no_mom_var } { incr count } \
    {
     ...
     if { $add_name == "Command" || $add_name == "Operator_Message" } \
     {
      if { $add_name == "Command" } {
       set var_value ""
       } else {
       ...
       set var_value [PB_output_EscapeSpecialControlChar_no_dollar $var_value]
      } ;# Operator_Message
      if { [info exists cmt_blk_obj]  &&  $cmt_blk_obj > 0  &&\
       [string match "$::gPB(tcl_line_blk)*" $block::($cmt_blk_obj,block_name)] } {
       set var_desc $::gPB(tcl_label)
       } else {
  ...
  if { $rap_1_obj != "" } \
  {
   if [info exists block::($rap_1_obj,block_name)] {
...
proc PB_pui_UpdateUsedElementExecAttr { post_obj CURRENT_EVT_LIST } {
  ...
  UI_PB_debug_ForceMsg "   +++++++ Updated exec_name_list:    $exec_name_list"
  UI_PB_debug_ForceMsg "   +++++++ Updated evt_elem_exec_arr: [array get evt_elem_exec_arr]"
 }
