  ...
  while { [gets $File::($object,FilePointer) line] >= 0 } \
  {
   ...
   switch -glob -- "$line"\
   {
    ...
    "## EVENTS ELEMENT EXECUTION START" {
     ...
     if { 0 && [PB_pui_CompareVersion $gPB(this_PUI_version) "2008.0.3.1"] >= 0 } {
      ...
      UI_PB_debug_ForceMsg " +++++ ver_check == $ver_check"
     }
     if { $ver_check < 0 } {
   ...
   if $post_info_start {
    switch -glob -- $line \
    {
     ...
     "#  HISTORY END"        {
      ...
      if { [string match "Saved with *"   $last_save] ||\
       ...
       set vi [string first "Version" $last_save]
       set vi [expr [string wordend $last_save $vi] + 1]
       set vj [expr [string first " on" $last_save $vi] - 1]
       set gPB(post_last_saved_version) [string range $last_save $vi $vj]
    ...
    if { $description_start || $controller_start || $history_start || $type_family_start || $machine_axis_start } {
     if { [regexp "^#" [string trim $line]] } { set line "" }
   ...
   if { $gcode_start } \
   {
    ...
    if { ![info exists mach_axis] } { ;# Not a new post (see above for new post)
     if { [info exists __pui_kin_machine_type] && [string match "3_axis*" $__pui_kin_machine_type] } {
   ...
   if { $evt_elem_exec_start } \
   {
    ...
    if { $ver_check < 0  &&  $n_com_cyc_blk > 0 } {
     ...
     if { $cyc_idx >= 0  &&  ![UI_PB_tpth_IsCycleCustom $evar] } {
      UI_PB_debug_ForceMsg " +++++ EXECUTION element: $cyc_idx $evar"
      set blk_idx 1
