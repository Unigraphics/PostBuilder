...
proc PB_ude_UdeInitParse { this POST_OBJ } {
  ...
  if { [info exists Post::($post_obj,ude_obj)] } {
   set ude_obj_list      $Post::($post_obj,ude_obj)
  }
  if { $ude_obj_list != "" } {
...
proc PB_ude_CreateParamObjAttr { ATTR_LIST PARAM_OBJ_LIST } {
  ...
  foreach attr $attr_list\
  {
   switch -glob $attr \
   {
    ...
    OPTIONS*  {
     set temp_string [string trim [string trimleft $attr "OPTIONS"]]
     set options ""
     foreach opt [split $temp_string ","] {
      append options "\"[string trim $opt]\","
     }
     set param_obj_attr(3) [string trimright $options ","]  ;# Remove last ","
    }
    TOGGLE*   {
     if { ![string compare $param_obj_attr(1) s] } {
      set param_obj_attr(2) [lindex $attr 1]
   ...
   o - b     {
    ...
    if { $dval_srch == -1 } \
    {
     ...
     if 0 {
      ...
      {
       i {
        lappend attr_list {DEFVAL "0"}
       }
       d {
        lappend attr_list {DEFVAL "0.0"}
       }
       b {
        lappend attr_list {DEFVAL "FALSE"}
       }
       default {
        lappend attr_list {DEFVAL ""}
       }
...
proc PB_ude_SetGlobalVars { } {
  ...
  if { ![info exists gPB(EXIST_CYCLE_EVENT)] } {
