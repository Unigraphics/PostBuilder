...
proc PB_int_CheckForCmdBlk { CMD_BLK_OBJ CMD_BLK_NAME } {
  ...
  if { $ret_code == 0 } { ;# No obj created
   ...
   {
    global gPB
    if { ![string match "$gPB(check_block_cmd_prefix)*" $cmd_blk_name] } \
    {
     UI_PB_debug_ForceMsg "\n@@@@@ PB_int_CheckForCmdBlk : Bad cmd_blk_name ==>$cmd_blk_name<\n"
     return 4
    }
   }
   return 0
...
proc PB_int_DefineUIGlobVar { } {
  ...
  set mom_sys_arr(Word_Separator) $Post::($post_object,word_sep)
  ...
  UI_PB_debug_ForceMsg "\n@@@@@ MOM_SYS_ARR : [array get mom_sys_arr]\n"
...
proc PB_int_RetCommentBlkName { COMMENT_BLK_NAME } {
  ...
  if { $::gPB(comment_is_tcl) } {
   set act_blk_obj_attr(0) "$::gPB(tcl_line_blk)"
   } else {
   set act_blk_obj_attr(0) "$::gPB(comment_blk)"
  }
  set comment_blk_list $Post::($post_object,comment_blk_list)
...
proc PB_int_RetDefMOMVarValues { MOM_VAR_ARR MOM_VAR_VALUE } {
  ...
  for { set count 0 } { $count < $no_mom_var } { incr count } \
  {
   ...
   {
    if { [info exists def_mom_var($mom_var)] } {
     set mom_var_value($mom_var) $def_mom_var($mom_var)
     } else {
     UI_PB_debug_ForceMsg "\n +++ def_mom_var($mom_var) not found, set to blank <<<\n"
     set mom_var_value($mom_var) ""
    }
   }
  }
 }
...
proc PB_OpenOldVersion { pui_file_name { flag 0 } } {
  ...
  UI_PB_debug_ForceMsg "\n>>> From $pui_file_name => machine_type = $machine_type  axis_option = $axis_option <<<\n"
  set gPB(MACHINE_TYPE) $machine_type
  set gPB(AXIS_OPTION)  $axis_option
  ...
  if { $flag == 1 } {
   ...
   {
    set cntl_full_name ""
    UI_PB_file_GetCurrentSysControllerFullName $gPB(mach_sys_controller) cntl_full_name $machine_type
...
proc PB_file_Open { pui_file_name args } {
  ...
  global gPB
  global errorInfo
  if { [info exists ::gPB(DISABLE_COMP_BLK_default)] } {
   set ::gPB(DISABLE_COMP_BLK) $::gPB(DISABLE_COMP_BLK_default)
  }
  ...
  {
   UI_PB_file_GetCurrentSysControllerFullName $gPB(mach_sys_controller) templete_file $machine_type
...
proc PB_file_Create { pui_file_list args } {
  ...
  if { [llength [info commands AUTO_CLAMP_1]] > 0 } {
   rename AUTO_CLAMP_1 ""
  }
  if { [llength [info commands AUTO_CLAMP_2]] > 0 } {
   rename AUTO_CLAMP_2 ""
  }
  ...
  foreach pui_file $pui_file_list \
  {
   ...
   if [ catch { PB_com_ReadPuiFile pui_file pui_file_id post_object } result ] {
    ...
    UI_PB_debug_ForceMsg "Error from PB_com_ReadPuiFile: $errorInfo"
    return [ error "$errorInfo" ]
   }
   if { [PB_is_v $gPB(VNC_release)] == 0 } {
    array set mom_sys_arr $Post::($post_object,mom_sys_var_list)
...
proc PB_file_ParseMainPost { main_post } {
  ...
  UI_PB_debug_ForceMsg ">>>>> Indep_add_arr: [array get address_arr] <<<<<\n"
 }
...
proc PB_file_CreateAlterUnitSubPost { pui_file_list args } {
  ...
  set ret_code [PB_file_CreatePostObjects $post_object]
  if 0 {
   PB_file_GetPostAttr $pui_file ::machType ::axisoption output_unit
   set ::mom_kin_var(\$mom_kin_output_unit) $output_unit
  }
  UI_PB_debug_ForceMsg "\n=====> Sub-post unit: $::mom_kin_var(\$mom_kin_output_unit) $::gPB(main_post_unit) <=====\n"
  UI_PB_debug_ForceMsg "\n=====> Indep_add_list: $Post::($post_object,Indep_add_list) <=====\n"
  UI_PB_debug_ForceMsg "\n=====> Indep_add_arr : $Post::($post_object,Indep_add_arr) <=====\n"
  if { [llength $Post::($::post_object,ext_add_list)] > 0 } {
   if { ![string compare "Inches" $gPB(main_post_unit)] } {
    set postfix "_IN"
    } else {
    set postfix "_MM"
   ...
   foreach add $Post::($::post_object,ext_add_list) {
    PB_com_RetObjFrmName add add_obj_list add_obj
    if { $add_obj <= 0 } {
     UI_PB_debug_ForceMsg "\n=====> Address add : >$add< Not found <=====\n"
     continue
    }
    set add_sub $add
    if { [string match "*$postfix" $add] } {
     set idx [expr [string length $add] - 4]
     set add_sub [string range $add 0 $idx]
    }
    PB_com_RetObjFrmName add_sub add_obj_list add_sub_obj
    if { $add_sub_obj <= 0 } {
     UI_PB_debug_ForceMsg "\n=====> Address add : >$add< => add_sub : >$add_sub< Not found <=====\n"
     continue
    }
    address::readvalue $add_sub_obj add_sub_obj_attr
    UI_PB_debug_ForceMsg "\n=====> Origl Address $add_sub : [array get add_sub_obj_attr] <=====\n"
    set add_sub_obj_attr(1)  "$address::($add_obj,add_format)"
    set add_sub_obj_attr(8)  "$address::($add_obj,add_leader)"
    set add_sub_obj_attr(9)  "$address::($add_obj,add_trailer)"
    set add_sub_obj_attr(10) "$address::($add_obj,add_trailer_status)"
    set add_sub_obj_attr(11) "$address::($add_obj,add_incremental)"
    set address::($add_sub_obj,leader_var) "$address::($add_obj,leader_var)"
    address::setvalue     $add_sub_obj add_sub_obj_attr
    address::DefaultValue $add_sub_obj add_sub_obj_attr
    UI_PB_debug_ForceMsg "\n=====> Fixed Address $add_sub : $add_sub_obj >[array get add_sub_obj_attr]<=====\n"
   }
  }
   ...
   if [string match "Standalone" $mom_sys_arr(VNC_Mode)] {
    if [string match "Generic" $controller] {
     ...
     if { [string compare $old_version "2007.5.0.0"] >= 0  && \
      [string compare $old_version "2010.0.2.1"] < 0 } {
      set old_version "2007.0.2.0"  ;#<Jun-15-2017 gsl> One release before "2007.5.0.0"
...
proc PB_file_DeleteEmptyBlock { } {
  ...
  foreach block_obj $blk_obj_list \
  {
   ...
   if { $block::($block_obj,block_name) == "rapid_spindle" } {
    array set mom_sys_var_arr $Post::($post_object,mom_sys_var_list)
    if { ![info exists mom_sys_var_arr(\$rap_wrk_pln_chg)] || !$mom_sys_var_arr(\$rap_wrk_pln_chg) } \
    {
     if [info exists blk_elem_list] \
     {
      unset blk_elem_list
...
proc PB_file_CreatePostObjects { post_object } {
  ...
  if [catch { PB_int_DefineUIGlobVar } res] {
   UI_PB_debug_ForceMsg "\nPB_int_DefineUIGlobVar error : $res\n"
  }
  Post::GetObjList $post_object address add_obj_list
  ...
  __file_GetAddVarOnTpthEventPage
  return 0
 }
...
proc PB_file_CreateSpecialEvents { POST_OBJ } {
  ...
  set kin_machine_type $::mom_kin_var(\$mom_kin_machine_type)
  PB_com_GetMachAxisType $kin_machine_type machType axisOption
  UI_PB_debug_ForceMsg "\nKKKKK+++++ >$kin_machine_type $machType $axisOption< +++++KKKKK\n"
  set event_name "User Defined Axis Limit Action" ;#<Sep-12-2017 gsl> Was "User_Def_Axis_Limit_Act".
  set ud_limit_action_name  $event_name
  set ud_limit_action_label {$gPB(event,misc,user_limit_action,name)}
  if { $machType == "Mill"  &&  $axisOption != "3" } {
   ...
   global gPB
   if { ![info exists Post::($post_obj,axis_limit_evt_obj)] } {
    set evt_obj_attr(0) $event_name
    set evt_obj_attr(1) [llength $evt_blk_list]
    set evt_obj_attr(2) $evt_blk_list
    set evt_obj_attr(3) ""
    set evt_obj_attr(4) {$gPB(event,misc,user_limit_action,name)}
    ...
    UI_PB_debug_ForceMsg "\nEEEEE+++++ Event >$evt_obj< >$event::($evt_obj,event_name)< added. +++++EEEEE\n"
    } else {
    set evt_obj $Post::($post_obj,axis_limit_evt_obj)
   }
   UI_PB_debug_ForceMsg "\nMMMMM+++++ MISC event list b4 >$Post::($post_obj,tpth_misc_evt_list)< +++++MMMMM\n"
   UI_PB_debug_ForceMsg "\nMMMMM+++++ MISC label list b4 >$Post::($post_obj,tpth_misc_label_list)< +++++MMMMM\n"
   UI_PB_debug_ForceMsg "\nMMMMM+++++ MISC block list b4 >$Post::($post_obj,tpth_misc_evt_blk_list)< +++++MMMMM\n"
   if { [lsearch $Post::($post_obj,tpth_misc_label_list) $ud_limit_action_name]  < 0  && \
   [lsearch $Post::($post_obj,tpth_misc_label_list) $ud_limit_action_label] < 0 } \
   {
    set idx [expr [llength $Post::($post_obj,tpth_misc_label_list)]/2]
    set Post::($post_obj,tpth_misc_evt_list)     [linsert $Post::($post_obj,tpth_misc_evt_list)     end $idx $ud_limit_action_name]
    set Post::($post_obj,tpth_misc_label_list)   [linsert $Post::($post_obj,tpth_misc_label_list)   end $idx $ud_limit_action_label]
    set Post::($post_obj,tpth_misc_evt_blk_list) [linsert $Post::($post_obj,tpth_misc_evt_blk_list) end $idx $evt_blk_list]
    UI_PB_debug_ForceMsg "\nMMMMM+++++ MISC event list aft >$Post::($post_obj,tpth_misc_evt_list)< +++++MMMMM\n"
    UI_PB_debug_ForceMsg "\nMMMMM+++++ MISC label list aft >$Post::($post_obj,tpth_misc_label_list)< +++++MMMMM\n"
    UI_PB_debug_ForceMsg "\nMMMMM+++++ MISC block list aft >$Post::($post_obj,tpth_misc_evt_blk_list)< +++++MMMMM\n"
   }
   foreach seq $Post::($post_obj,seq_obj_list) {
    if { [string match "Miscellaneous" $sequence::($seq,seq_name)] } {
     sequence::readvalue    $seq seq_obj_att
     if { [lsearch $seq_obj_att(1) $evt_obj] <= 0 } {
      lappend seq_obj_att(1) $evt_obj
      UI_PB_debug_ForceMsg "\nEEEEE+++++ Event >$evt_obj< >$event::($evt_obj,event_name)< set to seq $sequence::($seq,seq_name). +++++EEEEE\n"
     }
     sequence::setvalue     $seq seq_obj_att
     sequence::DefaultValue $seq seq_obj_att
     sequence::RestoreValue $seq
     break
    }
   }
   } else {
   ...
   {
    set idx [expr [llength $Post::($post_obj,tpth_misc_label_list)] - 2]
    set Post::($post_obj,tpth_misc_evt_list)     [lreplace $Post::($post_obj,tpth_misc_evt_list)     $idx end]
    set Post::($post_obj,tpth_misc_label_list)   [lreplace $Post::($post_obj,tpth_misc_label_list)   $idx end]
    set Post::($post_obj,tpth_misc_evt_blk_list) [lreplace $Post::($post_obj,tpth_misc_evt_blk_list) $idx end]
    foreach seq $Post::($post_obj,seq_obj_list) {
     if { [string match "Miscellaneous" $sequence::($seq,seq_name)] } {
      infoObj_ForceMsg $seq
      sequence::readvalue    $seq seq_obj_att
      for { set idx 0 } { $idx < [llength $seq_obj_att(1)] } { incr idx } {
       set evt_obj [lindex $seq_obj_att(1) $idx]
       if { [string match "$event_name" $event::($evt_obj,event_name)] } {
        infoObj_ForceMsg $evt_obj
        UI_PB_debug_ForceMsg "\nEEEEE+++++ Event >$evt_obj< >$event::($evt_obj,event_name)< removed. +++++EEEEE\n"
        PB_com_DeleteObject $evt_obj
        set seq_obj_att(1) [lreplace $seq_obj_att(1) $idx $idx]
        break
       }
      }
      sequence::setvalue     $seq seq_obj_att
   ...
   PB_com_unset_var Post::($post_obj,axis_limit_evt_obj)
  }
 }
...
proc __file_TidyPostObjects { args } {
  ...
  UI_PB_debug_ForceMsg "+++++ mom_kin_var +++++ $Post::($post_object,mom_kin_var_list)"
  ...
  if { ![string match "*_wedm" $mom_kin_var(\$mom_kin_machine_type)] &&\
   ![string match "*probe" $mom_kin_var(\$mom_kin_machine_type)] } {
   ...
   if { [PB_pui_CompareVersion $gPB(this_PUI_version) "2003.0.0"] < 0 } {
...
#=======================================================================
proc __file_GetAddVarOnTpthEventPage { } {
  global gPB
...
proc __file_ValidateBlocksInEvent { } {
  global post_object
  set seq_list {prog_start oper_start tpth_ctrl tpth_mot tpth_cycle tpth_misc oper_end prog_end linked_posts}
  Post::GetObjList $post_object block   blk_obj_list
  ...
  foreach seq $seq_list \
  {
   switch $seq \
   {
    ...
    "linked_posts" \
    {
     set evt_list     "linked_posts_evt_list"
     set evt_blk_list "linked_posts_evt_blk_list"
     set seq_name     "Linked Posts"
    }
   }
   if { ![info exists Post::($post_object,$evt_list)] } {
    set Post::($post_object,$evt_list) [list]
    continue
   }
   if { ![info exists Post::($post_object,$evt_blk_list)] } {
    set Post::($post_object,$evt_blk_list) [list]
    continue
   }
   ...
   for { set i 0 } { $i < $arr_size } { incr i } {
    foreach blk $evt_blk_list_arr($i) {
     ...
     if { $blk_obj <= 0 } {
      set blk_not_found 1
      if { $blk_not_found } {
       set blk_name [lindex $blk 0]
       if { [string match "ELSE" $blk_name] ||\
      ...
      if { $blk_not_found } {
       if [PB_file_is_JE_POST_DEV] {
        ...
        if { [llength $blk] > 1 } {
         ...
         set cmd_obj_attr(0)     $blk_name
         set cmd_obj_attr(1)     [list] ;# body
         if { [string match "$::gPB(comment_blk)*"  $blk_name] || \
          [string match "$::gPB(tcl_line_blk)*" $blk_name] } {  ;# Ref changes in ui_pb_block
  ...
  if { ![PB_file_is_JE_POST_DEV] } {
   ...
   foreach event_name $evt_name_list \
   {
    ...
    if { ![info exists event_proc_arr($changed_event_name)] } {
     ...
     if { [string compare $event_proc_arr($changed_event_name) $event_output] } {
...
proc PB_file_ParsePostUtilityFile { post_object } {
  ...
  __file_ParsePostConversionFile $post_object
  if { $change_overwrite_flag } {
   set gPB(pui_ui_overwrite) 0
  }
 }
...
#=======================================================================
proc __file_ParsePostConversionFile { post_object } {
  global gPB env
  set base_file ""
  switch "$::gPB(new_machine_type)" {
   "Mill" \
   {
    set base_file "$env(PB_HOME)/pblib/pb_mill_base.tcl"
   }
   "Lathe" \
   {
    set base_file "$env(PB_HOME)/pblib/pb_lathe_base.tcl"
   }
   "Wire EDM" \
   {
    set base_file "$env(PB_HOME)/pblib/pb_wedm_base.tcl"
   }
   "Mill/Turn" \
   {
    set base_file "$env(PB_HOME)/pblib/pb_millturn_base.tcl"
   }
  }
  if { ![file exists "$base_file"] } {
   return
  }
  UI_PB_debug_ForceMsg "\n@@@@@ __file_ParsePostConversionFile parse >$base_file< gPB(pui_ui_overwrite) : $::gPB(pui_ui_overwrite)\n"
  if { [catch { PB_pps_CreateTclFileObjs post_object $base_file 1 }] } {
   if 0 {
    if { $change_overwrite_flag } {
     set gPB(pui_ui_overwrite) 0
