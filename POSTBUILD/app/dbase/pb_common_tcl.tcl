...
#=======================================================================
proc PB_com_GetTimeString { } {
  if { $::tcl_version >= 8.6 } {
   set time_string [clock format [clock seconds] -timezone :localtime -format "%A, %B %d %Y, %T %Z"]
   } else {
...
#=======================================================================
proc PB_com_FindSeqObjOfEvent { event_obj } {
  foreach seq_obj $Post::($::post_object,seq_obj_list) {
   if { [lsearch $sequence::($seq_obj,evt_obj_list) $event_obj] >= 0 } {
    return $seq_obj
   }
  }
  return 0
...
#=======================================================================
proc PB_com_object_is_external { obj } {
  set class [string trim [classof $obj] :]
  if { [info exists ${class}::($obj,is_external)] && ( [set ${class}::($obj,is_external)] == 1 ) } {
   UI_PB_debug_ForceMsg "\n $class object $obj is external.\n"
   return 1
   } else {
   return 0
  }
...
#=======================================================================
proc PB_com_object_name_of_class_is_external { obj_name class } {
  set class [string tolower $class]
  set name_list ""
  switch $class \
  {
   format      { set name_list "ext_fmt_list" }
   address     { set name_list "ext_add_list" }
   block       { set name_list "ext_blk_list" }
   command     { set name_list "ext_cmd_list" }
   function    { set name_list "ext_fun_list" }
   ude_event   { set name_list "ext_ude_list" }
   cycle_event { set name_list "ext_cyc_list" }
  }
  if { $name_list != "" } {
   if { [info exists Post::($::post_object,$name_list)] } {
    if { [lsearch $Post::($::post_object,$name_list) $obj_name] >= 0 } {
     UI_PB_debug_ForceMsg "\n $class object $obj_name is external.\n"
     return 1
    }
   }
  }
...
#=======================================================================
proc PB_string_repeat { token count } {
  set string ""
  if { [string length $token] > 0 && $count > 0 } {
...
#=======================================================================
proc PB_com_is_true { VAR } {
  upvar $VAR var
  if [array exists var] {
   UI_PB_debug_ForcePause "Only scalar or single array element can be passed to PB_com_is_true!"
   return 0
  }
  if { [info exists var] } {
   set val [set var]
...
#=======================================================================
proc PB_com_DeleteObject { objs args } {
  foreach obj_id $objs {
   if [PB_com_HasObjectExisted $obj_id] {
    set class [string trim [classof $obj_id] ::]
    if { [string match "event" $class] } {
     set evt_name $event::($obj_id,event_name)
    }
    if [catch { eval delete $obj_id } res] {
     ...
     if { [string match "event" $class] } {
      if 0 {
       set seq [PB_com_FindSeqObjOfEvent $obj_id]
       if { $seq } {
        set __resave_seq 0
        ...
        if { $__resave_seq } {
         UI_PB_debug_ForceMsg "\nSSSSS+++++ Sequence >$seq< $sequence::($seq,seq_name) has Event >$obj_id< $evt_name< removed. +++++SSSSS\n"
      ...
      if { [lindex [Post::GetObjList $::post_object $class obj_list] 0] } { ;# 1st elem is not "0".
       if { [regexp -indices "$obj_id" $obj_list match] } {
        for { set ict 0 } { $ict < [llength $match] } { incr ict } {
         set idx [lsearch $obj_list $obj_id]
         if { $idx > -1 } {
...
#=======================================================================
proc PB_com_GetObjName { obj } {
  set obj_name ""
  if { $obj >= 0 } {
   set ClassName [string trim [classof $obj] ::]
   switch $ClassName {
    format    { set obj_name    $format::($obj,for_name)   }
    address   { set obj_name   $address::($obj,add_name)   }
    block     { set obj_name     $block::($obj,block_name) }
    event     { set obj_name     $event::($obj,event_name) }
    sequence  { set obj_name  $sequence::($obj,seq_name)   }
    command   { set obj_name   $command::($obj,name)       }
    function  { set obj_name  $function::($obj,id)         }
   }
  }
  return [string trim "$obj_name"]
 }
...
proc PB_com_GetNextObjName { obj_name class } {
  ...
  if [llength $name_list] {
   ...
   foreach name $name_list {
    ...
    if { $tail == "" } {
     ...
     } elseif { ![catch { expr $tail + 1 }] } {
     lappend new_name_list $name
    }
   }
  }
  if [llength $new_name_list] {
   set name_list [lsort -dictionary $new_name_list]
  ...
  if 0 {
   if { [string compare $name_found $obj_name] == 0 } {
    set idx 1
    set tl [split $obj_name "_"]
    if { [llength $tl] > 1 } {
     set idx [lindex $tl end]
     if ![catch { incr idx }] { ;# if the last token is a number
      set tl [lreplace $tl end end]
      set obj_name [join $tl "_"]
     }
    }
    set name_found ${obj_name}_$idx
   }
  }
  if { [string compare $name_found $obj_name] == 0 } {
   set idx 1
...
proc ladd { list index elmt_list args } {
  if [info exists list] \
  {
   ...
   if { [llength $args] && [lindex $args 0] == "no_dup" } \
   {
    ...
    set index [lsearch $new_list $elem]
  ...
  {
   foreach e $elmt_list {
    set new_list [linsert $new_list $index $e]
    incr index
   }
  }
  return $new_list
...
proc PB_com_FindObjFrmName { obj_name class } {
  ...
  if { ![string match "format"      $class] && \
   ...
   ![string match "sequence"    $class] && \
   ![string match "command"     $class] && \
   ![string match "ude_event"   $class] && \
   ![string match "cycle_event" $class] && \
   ![string match "function"    $class] } {
   UI_PB_log_msg "PB_com_FindObjFrmName cannot query object of class - \"$class\"."
   return -1
   } else {
   Post::GetObjList $post_object $class obj_list
  }
  if [info exists obj_list] \
  {
   foreach object $obj_list \
   {
    switch $class \
    {
     ...
     sequence    { set CheckName    $sequence::($object,seq_name)   }
     command     { set CheckName     $command::($object,name)       }
     ude_event   { set CheckName   $ude_event::($object,name)       }
     cycle_event { set CheckName $cycle_event::($object,name)       }
     function    { set CheckName    $function::($object,id)         }
    }
...
proc PB_com_ReadUdeFile { CDL_FILE_NAME CDL_FILE_ID POST_OBJ } {
  ...
  upvar $POST_OBJ post_obj
  UI_PB_debug_ForceMsg " ++++++ Parsing CDL file: $cdl_file_name\n"
...
proc PB_com_ReadDfltDefFile { DefFileName FILEID POST_OBJ } {
  ...
  upvar $FILEID FileId
  upvar $POST_OBJ post_obj
  UI_PB_debug_ForceMsg " ++++++ Parsing Def file: $FileName\n"
   ...
   for {set ii 0} {$ii < [expr $no_elements - 1]} {incr ii} \
   {
    for {set jj [expr $ii + 1]} {$jj < $no_elements} {incr jj} \
    {
     ...
     switch $ClassName \
     {
      ...
      function  { set ii_name $function::($ii_obj,id) }
     }
     ...
     switch $ClassName \
     {
      ...
      function  { set jj_name $function::($jj_obj,id) }
     }
...
proc PB_com_MapMOMVariable { MOM_SYS_VAR_ARR ADD_OBJ BLK_ELEM_MOM_VAR \
  ...
  if { [info exists ::need_update] && $::need_update == 1 } {
   set exp $blk_elem_mom_var
  }
...
proc PB_com_GetPrefixOfEvent { EVENT_NAME PREFIX } {
  ...
  if [string match "*_wedm" $cur_machine] {
   ...
   switch -glob "$event_name" \
   {
    ...
    "Return Move"        -
    "Feedrates"          -
    "User Defined Axis Limit Action" \
    {
     set prefix "PB"
    }
    default \
...
#=======================================================================
proc PB_com_IsSiemensController { controller_type args } {
  set ret_code 0
  set family_type [string tolower $controller_type]
  if { [llength $args] > 0 } \
  {
   set param_1 [lindex $args 0]
   if { $param_1 == 1 } \
   {
    set ret_code [string match "*sinumerik*" $family_type]
   }
   } else {
   set ret_code [string match "*siemens*" $family_type]
  }
  UI_PB_debug_ForceMsg "\n +++++ controller_type is Siemens = $ret_code \n"
  return $ret_code
 }

#=======================================================================
proc PB_com_IsHeidenhainController { controller_type args } {
  set ret_code 0
  set family_type [string tolower $controller_type]
  set ret_code [string match "*heidenhain_conversational*" $family_type]
  return $ret_code
 }
