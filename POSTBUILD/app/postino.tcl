...
#==============================================================================
proc UI_PB_decrypt_file { filename } {
 ;
}
...
## htm html chm hlp txt
 ...
 if {[ string compare $ext ".htm"]==0 || [ string compare $ext ".html"]==0} {
  ;#set ph $env(ProgramFiles)
  set ph "C:\\Program Files"
  catch { exec -- "$ph\\Internet Explorer\\iexplore.exe" "$fn1"  } ;
  return 0 ;
 }
...
#===========================================================
 ...
 #global tix_version
 #global ::disable_license  disable_license

 #global gPB
 #global LicInfo
 set  tcl_rcFileName   "~/wishrc.tcl" 
 global env
 set env(PB_DIR) $env(PB_HOME)/app
 set env(PWD) [pwd]
 puts "\n** Starting NX/Post Builder in $env(PWD) **\n"
...
wm withdraw .
if [catch {UI_PB_main .widget} err] {tk_messageBox -message $err}
...
/ui/ui_pb_ude_tcl.tcl
 ...
 if { [file exists $env(HOME)/POSTBUILD/user/PB_custom.tcl ] } {
  source [file nativename $env(HOME)/POSTBUILD/user/PB_custom.tcl]
 }  

 if { [file exists $env(PB_HOME)/user/PB_custom.tcl ] } {
  source [file nativename $env(PB_HOME)/user/PB_custom.tcl]
 } 
