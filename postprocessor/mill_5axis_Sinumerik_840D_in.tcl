########################## TCL Event Handlers ##########################
#
#  mill_5axis_Sinumerik_840D_in.tcl -
#
#  Created by lili  @  Monday, January 11, 2010 4:38:46 PM China Standard Time
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_4th_axis_max_limit                "36000"
  set mom_kin_4th_axis_min_incr                 "0.001"
  set mom_kin_4th_axis_min_limit                "-36000"
  ...
  set mom_kin_5th_axis_max_limit                "110"
  set mom_kin_5th_axis_min_incr                 "0.001"
  set mom_kin_5th_axis_min_limit                "-110"
...
#=============================================================
   ...
   if { ![info exists mom_pb_macro_arr($macro_name)] } {
      CATCH_WARNING "$macro_name: Not define this Macro."
      return
   }
   ...
   set pat1 {^[^\$]*\${([^\}]*)}.*}
   set pat2 {^[^\$]*\$([A-Za-z0-9_]*)[][() +-/*,]*.*}
   set cmd1 [concat regexp -indices \{$pat1\} \$temp_str match sub1]
   set cmd2 [concat regexp -indices \{$pat2\} \$temp_str match sub1]
   foreach param_attr $param_list {
      ...
      if { [llength $param_attr] > 0 } {
         ...
         while { $do_flag } {
            if { [eval $cmd1] == 1 } {
               set start_idx [lindex $sub1 0]
               set end_idx [lindex $sub1 1]
               set temp_var [string range $temp_str $start_idx $end_idx]
               set end_idx [string first "(" $temp_var]
               if { $end_idx > 0 } {
                  set temp_var [string range $temp_var 0 [expr $end_idx - 1]]
               }
               set end_idx [lindex $sub1 1]
               global $temp_var
               unset temp_var

               set start_idx [expr $start_idx - 2]
               set replace_str ""
               if { $start_idx > 0 } {
                  set replace_str [string range $temp_str 0 $start_idx]
               }
               append replace_str [string range $temp_str [expr $end_idx + 2] end]
               set temp_str $replace_str
            } elseif { [eval $cmd2] == 1 } {
               set start_idx [lindex $sub1 0]
               set end_idx [lindex $sub1 1]
               set temp_var [string range $temp_str $start_idx $end_idx]
               ...
               incr end_idx
               if { [string length $temp_str] == $end_idx } {
                  break
               }
               set temp_str [string range $temp_str $end_idx end]
            } else {
               break
            }
         }
         ...
         if [catch {eval $temp_cmd} res_val] {
            if [string match "*no such variable*" $res_val] {
               ...
               CATCH_WARNING " Error in calling $macro_name: $res_val"
               return
            }
         } elseif { $dtype } {
            ...
            if { [expr $data_val >= $max_val] } {
               set data_string [expr $max_val - 1]
               CATCH_WARNING "MAX/MIN WARNING in calling $macro_name: MAX: $data_string"
            } elseif { [expr $data_val <= $min_val] } {
               set data_string [expr $min_val + 1]
               CATCH_WARNING "MAX/MIN WARNING in calling $macro_name: MIN: $data_string"
            } else {
               if { $is_double } {
