########################## TCL Event Handlers ##########################
#
#  m5actt.tcl - 5_axis_dual_table
#
#    
#
#  Created by Postino @ Thursday, August 11, 2016 3:49:07 PM Pacific Daylight Time
...
#
# 02-22-10 gsl - Renamed PB_CMD_subroutine_xxx to PB_CMD_subprogram_xxx
#              - Renamed PB_CMD_subprogram_initial_move_indicator to
...
#
  ...
  set mom_sys_alt_unit_post_name                "m5actt__MM.pui"
...
#=============================================================
  ...
  global mom_motion_event
   ...
   set rapid_traverse_blk {G_feed G_motion G_mode X Y Z fourth_axis fifth_axis S M_spindle}
   set rapid_traverse_xy_blk {G_feed G_motion G_mode X Y fourth_axis fifth_axis S M_spindle}
   set rapid_traverse_yz_blk {G_feed G_motion G_mode Y Z fourth_axis fifth_axis S M_spindle}
   set rapid_traverse_xz_blk {G_feed G_motion G_mode X Z fourth_axis fifth_axis S M_spindle}
...
return
   ...
   set mom_sys_leader(fifth_axis) "$mom_orig_fifth_axis"
   set mom_fifth_axis_trailer ""
...
#=============================================================
# This is custom command is not portable and should
# not be used in other posts.
   ...
   set mom_fifth_axis_variable   "#500"

   set sub_name_header_text      "O"
   set sub_name_start_value      "100"
   set sub_name_footer_text      "M99"

# Subroutine call name
   set sub_call_text "M98 P"
...
# Switch to combine or split main and sub routines
   ...
   if { $mom_combine_main_sub_programs_switch == "ON" } {
      ...
      if { [file exists $sub_ptp_file_name] } {
         MOM_remove_file $sub_ptp_file_name
      }
   }
       ...
       if { $mom_blade_level_number == "0" } {
         ...
         if { $mom_initial_move_executed == "false" } {
            ...
            if { $mom_combine_main_sub_programs_switch == "OFF" } {
               set sub_ptp_file_name "${mom_output_file_directory}$sub_name_header_text$sub_program_name$mom_subprogram_number.${mom_sys_output_file_suffix}"
            }

            MOM_set_seq_off
            set mom_seq_no_backup $mom_sequence_number

            set mom_orig_fifth_axis $mom_sys_leader(fifth_axis)
            set mom_sys_leader(fifth_axis) "$mom_sys_leader(fifth_axis)\["
            set mom_fifth_axis_trailer "+$mom_fifth_axis_variable\]"
            MOM_open_output_file $sub_ptp_file_name
            MOM_output_text "$sub_name_header_text$sub_program_name$mom_subprogram_number"
...
#=============================================================
   ...
   if { [info exist mom_blade_number_of_blades_geometry] } {
      ...
      while {$blade_count < $mom_blade_number_of_blades_geometry} {
         ...
         set blade_angle [expr (360.0/$mom_blade_number_of_blades_geometry)*$blade_count]

         set b_a "[format "%5.3f" $blade_angle]"

         MOM_output_literal "#500=$b_a"
