########################## TCL Event Handlers ##########################
#
#  mill_5axis_actt_Sinumerik_840D_in.tcl -
#
#  Created by ling  @  Monday, February 22, 2010 6:40:47 PM Pacific Standard Time
...
#              - Moved call to PB_CMD_subprogram_initial_move_set
#                to the end of MOM_initial_move
#              - Added condition in PB_CMD_output_Sinumerik_setting
#                not to output CYCLE832 in subprograms (w.r.t. mom_initial_move_executed)
#              - Set mom_initial_move_executed "false" in PB_CMD_subprogram_initial_move
#                after MOM_initial_move is called.
 ...
 # Output High Speed Machining codes
  if { [info exists sinumerik_version] && [string match "V5" $sinumerik_version] } {
     ...
     if { ![string match "Point to Point" $mom_operation_type] && ![string match "Hole Making" $mom_operation_type] } {
       ...
       #<02-22-10 gsl> Not to output this call to a subprogram
        global mom_initial_move_executed
        if { ![info exists mom_initial_move_executed] || [string match "false" $mom_initial_move_executed] } {
     ...
     if {![string match "Point to Point" $mom_operation_type] && ![string match "Hole Making" $mom_operation_type] } {
       ...
       #<02-22-10 gsl> Not to output this call to a subprogram
       #               _camtolerance can not be DEF and set in a subroutine. (CSE bug?)
        global mom_initial_move_executed
        if { ![info exists mom_initial_move_executed] || [string match "false" $mom_initial_move_executed] } {
...
#<02-22-10 gsl> Save this tol to be output again in a subprogram
    if 0 {
     global mom_sys_camtolerance
     set mom_sys_camtolerance $tol
    }
