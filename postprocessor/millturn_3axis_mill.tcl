########################## TCL Event Handlers ##########################
#
#  millturn_3axis_mill.tcl - 3_axis_mill_turn
#
#    
#
#  Created by Siemens/PLM @ Thursday, June 27 2019, 13:33:57 -0700
...
#
  ...
  set mom_sys_alt_unit_post_name                "millturn_3axis_mill__MM.pui"
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_mill_turn_type                    "SIMPLE_MILL_TURN"
...
#=============================================================
  ...
  # Restore definition to the master post's
   if { ![string match "$::mom_sys_this_post_name" [file tail ${::mom_sys_mill_postname}]] } {
...
#
   ...
   global mom_sys_mill_turn_mode  mom_sys_mill_turn_type

   if { ![info exists mom_sys_mill_turn_mode] } {
     # This variable is set in PB_CMD_kin_init_mill_turn
     # to indicate the mill-turn mode has been initialized.
return
   }

   if { [info exists mom_sys_mill_turn_type] && \
       ![string match "SIMPLE_MILL_TURN" $mom_sys_mill_turn_type] } {
return
   }
   ...
   if { ![string compare $mom_machine_mode "MILL"] } {
      ...
      if { ![string compare $mom_sys_current_head "TURN"] } {
   ...
   } elseif { ![string compare $mom_machine_mode "TURN"] } {

      if { ![string compare $mom_sys_current_head "MILL"] } {
      ...
      set tcl_file   "${cam_post_dir}${mom_sys_lathe_postname}.tcl"
      set def_file   "${cam_post_dir}${mom_sys_lathe_postname}.def"
      ...
      if [file exists "$tcl_file"] {
         ...
         if [file exists "$tcl_file"] {
            ...
            CATCH_WARNING "Lathe post $mom_sys_lathe_postname not found"
         }
      }
...
#=============================================================
  ...
  global mom_motion_event
   ...
   set rapid_spindle_blk {G_adjust G_feed G_motion G_mode X Y Z H M_coolant}
   set rapid_spindle_x_blk {G_adjust G_feed G_motion G_mode X H M_coolant}
   set rapid_spindle_y_blk {G_adjust G_feed G_motion G_mode Y H M_coolant}
   set rapid_spindle_z_blk {G_adjust G_feed G_motion G_mode Z H M_coolant}
   set rapid_traverse_blk {G_feed G_motion G_mode X Y Z fourth_axis S M_spindle}
   set rapid_traverse_xy_blk {G_feed G_motion G_mode X Y fourth_axis S M_spindle}
   set rapid_traverse_yz_blk {G_feed G_motion G_mode Y Z fourth_axis S M_spindle}
   set rapid_traverse_xz_blk {G_feed G_motion G_mode X Z fourth_axis S M_spindle}
...
#<Aug-01-2017 gsl> pb12.0x
   ...
   set v1(3) [expr $v1(3) - $mom_kin_4th_axis_ang_offset]

   if { ![string compare "reverse" $mom_kin_4th_axis_rotation] } { set v1(3) [expr -1*$v1(3)] }

   set ang [LIMIT_ANGLE $v1(3)]
...
# Oct-25-2018 gsl   - Revert previous revision that suppresses the output of motion's end point
# Mar-21-2019 gsl   - Update end pos when interpolation has stopped.
#
...
return
   ...
   set status [CONVERT_POINT mom_prev_mcs_goto mom_prev_tool_axis save_org_pos mom_sys_radius_output_mode mom_prev_pos]
...
# OPERATOR_MSG ">> [info level 0] mom_prev_pos = [ARR_sort_array_vals mom_prev_pos]"
# OPERATOR_MSG ">> [info level 0] mom_pos      = [ARR_sort_array_vals mom_pos]"
  ...
  #
   ...
   while { $loop == 0 } {
     ...
     # Allow up to 100 interpolated points of each motion -
      if { $all_count > 100 } {
          # OPERATOR_MSG "Linearization of a motion has reached the limit of 100 segments."
         break
      }
      ...
      if { $count > 20 } {
        # Each half-segment will not iterate more than 20 times
         ...
         LINEARIZE_OUTPUT
          ...
          # OPERATOR_MSG "Search for interpolated point has reached 20 iterations, output this point."
         break
        ...
        #<10/25/2018 gsl> Revert next revision -
        # <Aug-01-2016 shuai> PR7768537. Fix duplicated end point problem.
        # VEC3_sub mom_pos save_pos dev_o

         set __output_point 1

        #<10/25/2018 gsl> Add checking of delta_angle against output resolution of rotary axis
         if { [EQ_is_lt [VEC3_mag dev] $::mom_kin_machine_resolution] &&\
              [EQ_is_lt $delta_angle   $::mom_kin_4th_axis_min_incr ] } {

           #<10/25/2018 gsl> Recompute angle for subsequent output
           ...
           #<21-Mar-2019 gsl> Break here will cause end position of the motion not being updated properly. Only suppress output.
           # break

             # OPERATOR_MSG "Suppress output of nearly identical point"
            set __output_point 0
         }
        ...
        # => What if conversion failed???
         ...
         if { [string match "FAIL" $status] } {
           # Bail out???
             # OPERATOR_MSG "Failed to find mid-point"
         }
