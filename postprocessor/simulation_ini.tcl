   ...
   for {set i 0} {$i<$mom_isv_tool_count} {incr i} {
      ...
      if {$mom_isv_tool_number($i) == 0 || [lsearch $tool_number_list $mom_isv_tool_number($i)] >=0} {
         if {$mom_isv_tool_number($i) == 0} {
            MOM_output_to_listing_device "Tool $mom_isv_tool_name($i) has tool number zero! Its info is not outputted into to_ini.ini file."
         } else {
            MOM_output_to_listing_device "Tool $mom_isv_tool_name($i) has tool number $mom_isv_tool_number($i) duplicated with other tool! Its info is not outputted into to_in.ini file."
         }
      } else {
         ...
         for {set j 0} {$j<$mom_isv_tracking_point_count($i)} {incr j} {
            if {$mom_isv_tool_adjust_register($i,$j) == 0 || [lsearch $tool_tp_number_list $mom_isv_tool_adjust_register($i,$j)] >=0} {
               if {$mom_isv_tool_adjust_register($i,$j) == 0} {  
                  MOM_output_to_listing_device "Tool $mom_isv_tool_name($i) has tracking poing's adjust register number as zero!"
                  MOM_output_to_listing_device "The tracking point's data is not outputed into to_ini.ini file!" 
               } else {   
