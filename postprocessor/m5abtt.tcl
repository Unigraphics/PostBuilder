########################## TCL Event Handlers ##########################
#
#  m5abtt.tcl - 5_axis_dual_table
#
#    This is a 5-Axis Milling Machine With
#     Dual Rotary Tables.
#
#  Created by Siemens/PLM @ Thursday, May 02 2019, 13:42:24 -0700
#  with Post Builder version 1899.
#
########################################################################
...
# 11-20-13 gsl - Do not zero everything in MOM_end_of_path;
#                it causes table to unwind unnecessarily.
# 04-09-14 gsl - Use new version of LINEARIZE_LOCK_MOTION & LOCK_AXIS_MOTION (of PB903)
# 04-10-14 gsl - Incorporate changes of <09-06-13 Allen> in LOCK_AXIS for PR6932644
#                to implement lock axis for 4 axis machine.
# 03-29-2016 gsl - Change mill_5_cmds.tcl to test with LOCK_AXIS_MOTION of pb11.0 with fix for 7655606
#                  (without forcing shortest-distance rotations) - Seem to work
# 05-02-2019 gsl - Enahnced with sample handlers & codes for subprogram output
#
#
}
  ...
  set mom_sys_alt_unit_post_name                "m5abtt__MM.pui"
...
#=============================================================
  global mom_program_aborted mom_event_error
   ...
   if { [PB_CMD__check_block_running_post_oper_path] } {
      MOM_abort_event
   }

   if { [PB_CMD__check_block_subprogram_output_on] } {
...
#=============================================================
   ...
   MOM_force Once G_mode fourth_axis fifth_axis
...
#<11-28-2018 gsl> pb1872 - Must reload mom_prev_rot_ang_4th & mom_prev_rot_ang_5thto fix 8553652
#
...
#=============================================================
# This command should be called in MOM_end_of_program
...
#=============================================================
# This command should be called in MOM_initial_move & MOM_first_move 
# under the condition when subprogram output is enabled.
...
#
# 06-17-2013 gsl - Do not abort event, by default, when signal is "1".
# 07-16-2015 gsl - Added level 3 handling
...
#=============================================================
#<08-11-11 gsl> exp for pr2070506
   global mom_tool_axis mom_prev_tool_axis
   global mom_sys_control_out mom_sys_control_in

   set output_info 0

   foreach i { 1 } {  ;# Only check J component for A-axis machine
      if { [expr $mom_tool_axis($i) > 0.0] && ![expr $mom_prev_tool_axis($i) > 0.0] } {
         set output_info 1
      }
      if { $output_info } {
         MOM_output_literal "$mom_sys_control_out curr tool axis : $mom_tool_axis(0), $mom_tool_axis(1)  $mom_sys_control_in"
         MOM_output_literal "$mom_sys_control_out prev tool axis : $mom_prev_tool_axis(0), $mom_prev_tool_axis(1)  $mom_sys_control_in"
        # PAUSE "tool axis : $mom_tool_axis(0), $mom_tool_axis(1)"
return
      }
...
# - This command should be used in conjunction with
#   PB_CMD_after_rapid.
# - This can be used for rapid motion with or without work plane change.
#
# 01-26-10 gsl - Define non-existing mom_prev_out_angle_pos
#
   ...
   if { $mom_motion_event == "initial_move" || $mom_motion_event == "rapid_move" ||\
        $mom_motion_event == "first_move" || $mom_motion_type == "FROM" || $mom_motion_type == "APPROACH" } {

      if { ![info exists mom_prev_out_angle_pos(0)] } { set mom_prev_out_angle_pos(0) $mom_out_angle_pos(0) }
      if { ![info exists mom_prev_out_angle_pos(1)] } { set mom_prev_out_angle_pos(1) $mom_out_angle_pos(1) }
...
#=============================================================
proc PB_CMD_cancel_coord_rotation { } {
#=============================================================
   global pop_rotation_flag
   if {[info exists pop_rotation_flag] && $pop_rotation_flag == 1} {
      MOM_output_literal "G69"
   }
   if {[info exists pop_rotation_flag]} {
      unset pop_rotation_flag
   }
}
...
#=============================================================
#--------------------------------------------------------------------------
#Detect the Milling mode;
#Set the Var. pop_operation_type
#--------------------------------------------------------------------------
global mom_operation_type mom_tool_axis_type
global pop_operation_type

#detect the mutiaxis mode.


if { ($mom_operation_type == "Variable-axis Surface Contouring" && $mom_tool_axis_type !=0) || $mom_operation_type == "Sequential Mill Main Operation" } {
    set pop_operation_type "5axis"

} else {

    set pop_operation_type "non_5axis"
}
}
...
global pop_adjust_code

if { [info exists pop_operation_type ] && $pop_operation_type == "5axis" } {
return
}
...
#=============================================================
# Restore sequnece number back to orignal -
...
global mom_next_oper_has_tool_change
    ...
    MOM_force once G_cutcom G_adjust
    MOM_do_template absolute_mode_1
}


#=============================================================
proc PB_CMD_end_of_proc { } {
#=============================================================
  MOM_output_literal "T0 M6"
...
# commands PB_CMD_clamp_fifth_axis or PB_CMD_unclamp_fifth_axis.
# If you have a five axis machine tool and only want fourth or fifth axis auto
# clamping, you will need to comment out the line that outputs the unneeded code.
#
#
...
#
# For a four axis head machine, the retraction will be confined to
# a cylinder.  For a five axis dual-head machine, the retraction
# will be to a sphere.  And, for a dual-table machine, the retraction
# will be to a plane along the spindle axis.
#
#
...
proc PB_CMD_new_iks { } {
#=============================================================
#<gsl> Legacy not needed.
return

#
#  This custom command will allow you to use the inverse kimematics
#  solver for any post and initialize it with the new mom variables
#  needed.  Change the mom variables as needed.  A complete description
#  of all the mom variables used will be provided with the released
#  version of this command in PB 4.0.  The released version will also
#  have an automatic load feature.
#
#  In the start of program event dialog.  Select the custom command
#  PB_CMD_new_iks from the pull down dialog.  Place it on the start
#  of program program event marker.  If you use this custom command
#  with a linked post, place it in the start of operation marker.
#
uplevel #0 {
...
set mom_kin_4th_axis_point(0)            -150.0
set mom_kin_4th_axis_point(1)        -594.0
set mom_kin_4th_axis_point(2)        -50.0
...
set mom_kin_4th_axis_min_limit            -110.0
set mom_kin_4th_axis_max_limit            110.0

set mom_kin_5th_axis_min_limit            -360.0
...
#=============================================================
proc PB_CMD_oper_info { } {
#=============================================================
global mom_operation_name

   OPERATOR_MSG "OPERATION : $::mom_operation_name"

if 0 { ;# Commands only available during shopdoc
...
#=============================================================
#----------------------------------------------------------------
#Rotary the coordinate;
#CSYS_ROT Mode
#----------------------------------------------------------------

global pop_rotation_flag

#Return when Not need to rotation
if {![info exists pop_rotation_flag] || $pop_rotation_flag == 0} {
    return }
...
global mom_init_pos
global pop_A pop_B pop_C pop_X pop_Y pop_Z

#Tilt plane activate, defines the rotation plane;
#X, Y, Z :    Feature coordinate system origin;
#I, J, K :    Angle for determining the orientation of the feature coordinate system;
#G68 Xp x1 Yp y1 Zp z1 I i1 J j1 K k1 R α ;
#G68 Xp x2 Yp y2 Zp z2 I i2 J j2 K k2 R β ;
MOM_force once rotate_X rotate_Y rotate_Z
if { [info exists pop_A] && ![EQ_is_zero $pop_A] } {
      MOM_do_template coord_rot_i
}
if { [info exists pop_B] && ![EQ_is_zero $pop_B] } {
      MOM_do_template coord_rot_j
}
if { [info exists pop_C] && ![EQ_is_zero $pop_C] } {
      MOM_do_template coord_rot_k
}
...
#=============================================================
proc PB_CMD_plane_change_force { } {
#=============================================================
  ...
  VEC3_unitize mom_spindle_axis vec
  if { ![EQ_is_equal [expr abs($vec(0))] 1.0] && ![EQ_is_equal [expr abs($vec(1))] 1.0] && ![EQ_is_equal [expr abs($vec(2))] 1.0] } {
MOM_abort_event
  }
...
if {[info exists mom_next_oper_has_tool_change] && $mom_next_oper_has_tool_change == "YES"} {
   ...
   #<04-09-2014 gsl> Why next call is needed, since no KIN changes here?
    MOM_reload_kinematics
}
}
...
#
  ...
  # Convert the Unit
   ...
   global mom_kin_output_unit
   global pop_unit_convert_value

   set pop_unit_convert_value 1

   if { [string compare $mom_kin_output_unit $mom_part_unit] } {
     # Part and post have different unit
     # convert the unit
      switch $mom_kin_output_unit {
         MM {
            set pop_unit_convert_value 25.4
         }
         IN {
            set pop_unit_convert_value [expr 1/25.4]
         }
      }
   }
...
return
  ...
  # For simulation
   ...
   if { [info exists MCS_x_offset($mom_fixture_offset_value)] } {
      set mom_csys_origin(0) [expr [expr $MCS_x_offset($mom_fixture_offset_value)*$pop_unit_convert_value] - $mom_csys_origin(0)]
      set mom_csys_origin(1) [expr [expr $MCS_y_offset($mom_fixture_offset_value)*$pop_unit_convert_value] - $mom_csys_origin(1)]
      set mom_csys_origin(2) [expr [expr $MCS_z_offset($mom_fixture_offset_value)*$pop_unit_convert_value] - $mom_csys_origin(2)]
   }
  ######################

   if { $mom_kin_coordinate_system_type != "MAIN" && $mom_special_output == 3 } {
...
#MOM_output_to_listing_device "A = $pop_A B = $pop_B C = $pop_C X = $pop_X Y = $pop_Y Z = $pop_Z"
}
...
#=============================================================
#<02-18-08 gsl>
# Output simulated cycle on a non-principal plane
#

 ## Determine if the tool axis is along a principal direction
  global mom_spindle_axis
 ...
 ## Set this flag to indicate that cycles on non-Z principal plane are also simulated (1).
  if { [EQ_is_equal $vec(2) 1.0] } {
    set simulate_principal_cycle 0
  } else {
    set simulate_principal_cycle 1
  }
 ...
 ## Output simulated cycle or canned cycles
  if { $is_principal == 0 || $simulate_principal_cycle } {

    set mom_user_simulated_cycle 1
   ...
   # Disable workplane change for simulated cycle,
   # if tool axis is not along a principal axis. Restore it after a rapid move.
    if { $is_principal == 0 } {
      set mom_user_work_plane_change $mom_sys_work_plane_change
      set mom_user_spindle_first $spindle_first
      set mom_sys_work_plane_change 0
...
#=============================================================
  ...
  if {[info exists pop_operation_type] && $pop_operation_type == "5axis"} {
    set pop_adjust_code "43.4"
  } elseif {[info exists pop_operation_type] && $pop_operation_type == "non_5axis" } {
    set pop_adjust_code "43"

    if {![string compare  "5_axis_dual_head"  $mom_kin_machine_type]} {
       if {![EQ_is_zero $mom_out_angle_pos(1)] || ![EQ_is_zero $mom_out_angle_pos(0)] } {
         set pop_adjust_code "43.1"
       }
...
#=============================================================
proc PB_CMD_user_RAPID_SET { } {
#=============================================================
# <02-18-08 gsl>
# This command is provided to overwrite the system RAPID_SET
# in order to correct problem with workplane change that
# accounts +/- directions along the principal axis.
#
 # Only redefine RAPID_SET once!
  if { [llength [info commands POST_SYS_RAPID_SET]] == 0 } {
     rename RAPID_SET POST_SYS_RAPID_SET
  } else {
return
  }
...
return
 ...
 #-----------------------------------------------------------------------------
 # User can choose to disable work plane change for non-principal spindle axis
 #
  set disable_non_principal_spindle 0
  ...
  if { $mom_sys_work_plane_change } {
   ...
   # Force output in Initial Move and First Move.
    if { ![string match "*initial_move*" $mom_motion_event] && ![string match "*first_move*" $mom_motion_event] } {
...
#=============================================================
proc LINEARIZE_LOCK_MOTION-pb800 { } {
#=============================================================
# called by LOCK_AXIS_MOTION
#
#  This command linearizes the move between two positions that
