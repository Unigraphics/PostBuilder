...
proc PB_blk_SplitBlkTmplElem {SplitVar BLK_ELEM_OBJ_LIST BLK_OBJ_ATTR \
  ...
  if {$trim_char != -1}\
  {
   ...
   [expr $trim_char1 - 1]]
   set blk_elem_obj_attr(1) [string trim $temp_var " "]
  } else\
  {
   set blk_elem_obj_attr(0) "Text"
  set split_var [string trimright $split_var \}]
  set split_var [join [split $split_var \"] " "]
  set split_var [string trim $split_var " "]
...
proc PB_blk_AddToCommentList { COMMENT_BLK POST_OBJ } {
  ...
  set elem_mom_var [string trimleft $elem_mom_var "$comment_strt"]
  set elem_mom_var [string trimright $elem_mom_var "$comment_end"]
  ...
  if { [info exists word_mom_var_arr($elem_word_add)] } \
  {
   ...
   } else\
   {
    switch $elem_word_add\
    {
     ...
     default { set elem_word_desc "No Description" }
    }
   }
  } else \
  {
   set elem_word_desc "No Description"
  }
 }
...
#=======================================================================
proc PB_blk_CreateCmdBlkFromCmdBlkObj { CMD_BLK_LIST CMD_BLK_OBJ OBJ_INDEX} {
  upvar $CMD_BLK_LIST cmd_blk_list
  upvar $CMD_BLK_OBJ cmd_blk_obj
