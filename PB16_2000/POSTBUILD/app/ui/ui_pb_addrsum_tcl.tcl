...
proc UI_PB_ads__CreateFirstColAttr { page_obj grid no_row add_name add_desc } {
  ...
  if { ![winfo exists $name_frm] } {
   ...
   -cursor ""]
   bind $addn <Enter> "+%W config -highlightbackground navyBlue"
...
proc UI_PB_ads_EditAddress { page_obj add_name add_button_id } {
  ...
  "500x600+100+200" "UI_PB_ads_DisableAddrSumPageWidgets $page_obj" "" \
  "UI_PB_ads_ActivateAddrSumPageWidgets $page_obj $win_index"
  UI_PB_ads_UpdateAddressObjects
  $add_button_id config -background lightYellow -relief sunken
...
proc UI_PB_ads__SetPopupOptions {PAGE_OBJ MENU add_frm add_name attr} {
  ...
  foreach ELEMENT $options_list \
  {
   ...
   {
    set elmt "\"\""
    $menu add command -label [set $ELEMENT] -command \
...
proc UI_PB_ads__CreateSpecCharButton { PAGE_OBJ mid_frm } {
  ...
  $bb config -relief sunken -bd 1 -bg royalBlue -padx 5 -pady 5
...
proc UI_PB_ads__CreateSpecialCharsWin { page_obj } {
  ...
  "gPB(addrsum,other_trans,title,Label)" "+500+300" \
  ...
  set cb_arr(gPB(nav_button,apply,Label)) "UI_PB_ads_SpecApplyCallBack"
  set cb_arr(gPB(nav_button,cancel,Label))  "destroy $w"
  set cb_arr(gPB(nav_button,ok,Label))   "UI_PB_ads_SpecOkCallBack $w"
...
proc UI_PB_ads_ActivateAddrSumPageWidgets { page_obj win_index } {
  ...
  if { $gPB(toplevel_disable_$win_index) } \
  {
   ...
   {
    set name_frm $grid.name_$add_name.name
    bind $name_frm <Enter> \
    "+%W config -highlightbackground navyBlue"
    bind $name_frm <Leave> \
    "+%W config -highlightbackground $paOption(table_bg)"
    PB_enable_balloon $name_frm
    set add_frm $grid.add_$add_name
...
proc UI_PB_ads_DisableAddrSumPageWidgets { page_obj } {
  ...
  {
   PB_cancel_balloon
   bind $grid.name_$add_name.name <Enter> ""
   bind $grid.name_$add_name.name <Leave> ""
   bind $grid.add_$add_name.but <Enter> ""
   bind $grid.add_$add_name.but <3> ""
   bind $grid.trail_$add_name.but <Enter> ""
   bind $grid.trail_$add_name.but <3> ""
...
proc UI_PB_ads_SpecDefaultCallBack { } {
  ...
  for {set count 5} {$count < 10} {incr count} \
  {
...
proc UI_PB_ads_CreateSpecialChars { PAGE_OBJ page_frm } {
  ...
  foreach label $label_list \
  {
   ...
   {
    set options $option_1
    lappend options "gPB(other,opt_new,Label)"
   } else \
   {
...
proc UI_PB_ads__GetValuesforSpecOptions { OPTION_LAB OPT_VALUE } {
  ...
  } else \
  {
   switch $mom_sys_arr($option_lab) \
   {
    ...
    "\\012"  {set opt_value "gPB(other,opt_new,Label)"}
    default {
...
proc UI_PB_ads__SetSpecChar { frame count option_lab args } {
  ...
  switch $special_char($option_lab,label) \
  {
   "gPB(other,opt_none,Label)" \
   {
    ...
    $frame.$count.ent config -state disabled -bg lightBlue
   }
   ...
   {
    set special_char($option_lab,char) $mom_sys_arr($option_lab)
    $frame.$count.ent config -state normal -bg white
   }
...
proc UI_PB_ads_UpdateAddrSumPage { PAGE_OBJ } {
  ...
  set mom_var_list {"seqnum_start" "seqnum_incr" "seqnum_freq" \
   "\$mom_sys_gcodes_per_block" "\$mom_sys_mcodes_per_block" \
   "Word_Seperator" "End_of_Block" "Comment_Start" \
  "Comment_End" "\$mom_sys_opskip_block_leader"}
  foreach var $mom_var_list \
  {
