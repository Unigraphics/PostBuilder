...
proc UI_PB_tpth_ActivateTpthWidgets { page_obj seq_obj event_obj win_index } {
  ...
  if { $gPB(toplevel_disable_$win_index) } \
  {
   ...
   $bot_canvas itemconfigure $evt_text -fill white
...
proc UI_PB_tpth_EditPlaneCodes { page_obj event_obj member_obj } {
  ...
  "UI_PB_tpth_DialogCancel_CB $win $page_obj"
  set cb_arr(gPB(nav_button,ok,Label)) \
  "UI_PB_tpth_DialogOk_CB $win $page_obj $event_obj"
...
#=======================================================================
proc UI_PB_tpth_DialogOk_CB { win page_obj event_obj } {
  global mom_sys_arr
  set mom_var_list $Page::($page_obj,page_momvars)
...
#=======================================================================
proc UI_PB_tpth_CreateTable { page_obj event_obj no_rows code_edit } {
  global tixOption
  global paOption
  global mom_sys_arr
  global gPB
  ...
  set cb_arr(gPB(nav_button,cancel,Label)) \
  "UI_PB_tpth_SpindleRangeCancel $win"
  set cb_arr(gPB(nav_button,ok,Label)) \
...
#=======================================================================
proc UI_PB_tpth_SpindleRangeCancel { win } {
  destroy $win
 }
...
proc UI_PB_tpth_SpindleRangeOk { win page_obj event_obj } {
  ...
  for { set count 1 } { $count <= $no_rows } { incr count } \
  {
   ...
   {
    lappend add_mom_var "\$spindle_range($count,$elem)"
...
proc UI_PB_tpth_CreateRowElems { grid row_no code_edit } {
  global mom_sys_arr
  set table_elem [list code min max]
  foreach elem $table_elem \
  {
   set var "\$spindle_range($row_no,$elem)"
  ...
  set var "\$spindle_range($row_no,min)"
  ...
  set var "\$spindle_range($row_no,max)"
...
#=======================================================================
proc UI_PB_tpth_SpindleRange { page_obj event_obj member_obj args } {
  global mom_sys_arr
  global gPB
  set mom_var $group_member::($member_obj,mom_var)
  set no_rows $mom_sys_arr(\$mom_sys_spindle_ranges)
  if { $no_rows } \
  {
   for { set count 1 } { $count <= $no_rows } { incr count } \
   {
    set mom_sys_arr(\$spindle_range($count,code) 0
   }
  }
  switch $mom_sys_arr($mom_var) \
  {
   "None" \
   {
    set elem_expr ""
   ...
   "Separate_M_Code(41)" \
   {
    if { $no_rows == 0 } \
    {
     ...
     -message "$gPB(msg,nonzero_range)"
     return
    }
    UI_PB_tpth_CreateTable $page_obj $event_obj $no_rows 0
   ...
   "Range_Code_with_M_Code" \
   {
    ...
    set add_name "M_spindle"
    PB_int_RetAddrObjFromName add_name add_obj
    set search_var "mom_sys_spindle_direction_code"
    UI_PB_tpth_UpdateVarAddress add_obj search_var elem_expr elem_desc
   }
   "High_or_Low_Range_S_Code" \
   {
    set mom_sys_arr(\$mom_sys_spindle_ranges) 2
    set mom_sys_arr(\$spindle_range(1,code)) +1
    set mom_sys_arr(\$spindle_range(2,code)) -1
    set no_rows 2
    UI_PB_tpth_CreateTable $page_obj $event_obj $no_rows 1
   ...
   {
    set mom_sys_arr(\$mom_sys_spindle_ranges) 2
    set mom_sys_arr(\$spindle_range(1,code)) 0
    set mom_sys_arr(\$spindle_range(2,code)) 1
    set no_rows 2
    UI_PB_tpth_CreateTable $page_obj $event_obj $no_rows 1
    set elem_expr "\$mom_spindle_speed + \
...
proc UI_PB_tpth_EditRapidBlocks { page_obj event_obj member_obj } {
  ...
  {
   set blk_obj_attr(0) "rapid_traverse"
...
proc UI_PB_tpth_CreateHorzPane { PAGE_OBJ EVENT_OBJ } {
  ...
  if { $canvas_flag } \
  {
   ...
   pack $box_frm -side bottom -expand yes -fill both
  } else \
  {
...
proc UI_PB_tpth_ItemEndDrag1 {page_obj event_obj x y } {
  ...
  if { $Page::($page_obj,add_blk) != 0} \
  {
   ...
   {
    UI_PB_tpth_NewCommentPage page_obj event_obj add_to_evt_elem_obj \
    new_elem_obj
    } elseif { $blk_flag == "command"} \
   {
    UI_PB_tpth_NewCmdPage page_obj event_obj add_to_evt_elem_obj \
    new_elem_obj
...
proc UI_PB_tpth_DeletePostBlocks { EVENT_OBJ CYCLE_BLK_NAME } {
  ...
  foreach evt_elem_obj $event::($event_obj,evt_elem_list) \
  {
   ...
   {
    set ret_code [UI_PB_tpth_CheckCycleRefWord block_obj 1]
    if { $ret_code } \
    {
...
proc UI_PB_tpth_DeleteComEvtBlkElems { CYC_EVT_OBJ COM_EVT_NAME } {
  ...
  foreach cyc_evt_elem_obj $event::($cyc_evt_obj,evt_elem_list)\
  {
   ...
   if { [string compare $block::($cyc_blk_obj,blk_owner) $com_evt_name] == 0} \
   {
...
#=======================================================================
proc UI_PB_tpth_CheckCycleRefWord { BLOCK_OBJ check_flag } {
  upvar $BLOCK_OBJ block_obj
  if { $check_flag == 2 } \
  {
...
proc UI_PB_tpth_CreateEvtElemFrmEvtObj { EVT_ELEM_OBJ NEW_EVT_ELEM_OBJ \
  ...
  set blk_obj_attr(3) $block_type
  PB_blk_CreateBlkObj blk_obj_attr new_block_obj
  set block::($new_block_obj,blk_owner) $current_evt_name
  unset blk_obj_attr
...
proc UI_PB_tpth_UpdateDefsOfCycleEvent { COM_EVENT_OBJ CYCLE_EVT_OBJ } {
  ...
  foreach evt_elem_obj $event::($cycle_evt_obj,evt_elem_list) \
  {
   ...
   set ret [UI_PB_tpth_CheckCycleRefWord evt_blk 1]
   if { [string compare $block::($evt_blk,blk_owner) $com_evt_name] == 0 } \
   {
    ...
    } elseif { $ret } \
   {
    foreach cyc_evt_elem $def_cyc_evt_attr(2) \
    {
     if { [info exists event_element::($cyc_evt_elem,block_obj)] } \
     {
      ...
      set ret_code [UI_PB_tpth_CheckCycleRefWord cyc_blk_obj 1]
      if { $ret_code} \
      {
       set com_blk_elem_list ""
       foreach blk_elem $block::($evt_blk,elem_addr_list) \
       {
        if { [string compare $block_element::($blk_elem,owner) $com_evt_name] == 0 } \
        {
    ...
    foreach evt_elem_obj $event::($event_obj,evt_elem_list) \
    {
     ...
     } else \
     {
      foreach cyc_evt_elem $event::($cycle_evt_obj,evt_elem_list)\
      {
       ...
       set ret_code [UI_PB_tpth_CheckCycleRefWord cyc_evt_blk 1]
       if {$ret_code == 1}\
       {
        set cyc_elem_addr_list \
...
proc UI_PB_tpth_IconBindProcs { PAGE_OBJ EVENT_OBJ } {
  ...
  $event_obj %x %y"
  $bot_canvas bind nonmovable <3>     "UI_PB_tpth_NonMovableRightButton \
  $page_obj $event_obj %x %y"
 }
...
proc UI_PB_tpth_NonMovableRightButton { page_obj event_obj x y } {
  ...
  UI_PB_blk_NonMovableElemPopup page_obj act_blk_obj active_blk_elem $x $y
 }
...
proc UI_PB_tpth_BindRightButton { page_obj event_obj x y } {
  ...
  UI_PB_tpth_GetEvtElemAndBlkElem event_obj act_blk_elem act_evt_elem $x $y
  set act_blk_obj $event_element::($act_evt_elem,block_obj)
  UI_PB_blk_BlockPopupMenu page_obj act_blk_obj active_blk_elem $x $y
 }
...
#=======================================================================
proc UI_PB_tpth_EditCommentBlock { win page_obj cmd_blk_obj blk_elem_obj } {
  ...
  set cb_arr(gPB(nav_button,ok,Label)) \
  "UI_PB_cmd_tpthEditCommentOk_CB $win $page_obj \
  $cmd_blk_obj $blk_elem_obj"
  UI_PB_com_CreateButtonBox $box1_frm label_list cb_arr
  bind $win.top.ent <Return> "UI_PB_cmd_tpthEditCommentOk_CB $win $page_obj \
  $cmd_blk_obj $blk_elem_obj"
 }

#=======================================================================
proc UI_PB_tpth_EditCmdBlock { win page_obj cmd_blk_obj blk_elem_obj } {
  ...
  set cb_arr(gPB(nav_button,ok,Label)) \
  "UI_PB_cmd_tpthEditCmdBlkOk_CB $win $page_obj \
  $cmd_blk_obj $blk_elem_obj $cmd_page_obj"
  UI_PB_com_CreateButtonBox $box1_frm label_list cb_arr
 }

#=======================================================================
proc UI_PB_tpth_NewCommentPage { PAGE_OBJ EVENT_OBJ EVT_ELEM_OBJ \
  BLK_ELEM_OBJ } {
  upvar $PAGE_OBJ page_obj
  upvar $EVENT_OBJ event_obj
  upvar $EVT_ELEM_OBJ evt_elem_obj
  ...
  "gPB(nav_button,ok,Label)"}
  set cb_arr(gPB(nav_button,cancel,Label)) \
  "UI_PB_cmd_tpthCommentCancel_CB $win $page_obj \
  $event_obj $evt_elem_obj $blk_elem_obj"
  set cb_arr(gPB(nav_button,ok,Label)) \
  "UI_PB_cmd_tpthCommentOk_CB $win $page_obj \
...
#=======================================================================
proc UI_PB_tpth_NewCmdPage { PAGE_OBJ EVENT_OBJ EVT_ELEM_OBJ BLK_ELEM_OBJ } {
  upvar $PAGE_OBJ page_obj
  upvar $EVENT_OBJ event_obj
  upvar $EVT_ELEM_OBJ evt_elem_obj
  upvar $BLK_ELEM_OBJ blk_elem_obj
  ...
  "gPB(nav_button,ok,Label)"}
  set cb_arr(gPB(nav_button,cancel,Label)) \
  "UI_PB_cmd_tpthCmdBlkCancel_CB $win $page_obj \
  $event_obj $evt_elem_obj $blk_elem_obj $cmd_page_obj"
  set cb_arr(gPB(nav_button,ok,Label)) \
  "UI_PB_cmd_tpthCmdBlkOk_CB $win $page_obj $event_obj \
