...
proc UI_PB_mseq_SetPageAttributes { PAGE_OBJ } {
  ...
  set Page::($page_obj,w_cell)     52       
  set Page::($page_obj,w_divi)     6        
  set Page::($page_obj,x_orig)     100      
...
proc UI_PB_mseq_PopupSetOptions { page_obj X Y x y } {
  ...
  if {$popup_flag == 0} \
  {
   ...
   -command "UI_PB_mseq_NewAddress $page_obj $x $y"
   $menu add sep
   ...
   -command "UI_PB_mseq_EditAddress $page_obj $x $y"
   $menu add sep
   $menu add command -label "$gPB(wseq,popup_delete,Label)" \
   -command "UI_PB_mseq_DeleteAddress $page_obj $x $y"
   set Page::($page_obj,popup_flag) 0
  }
...
#=======================================================================
proc UI_PB_mseq_DeleteAddress { mseq_page_obj x y } {
  global gPB
  ...
  UI_PB_mseq_GetBlkElemObjFromCursorPos mseq_page_obj add_obj \
  $check_type $x $y
...
#=======================================================================
proc UI_PB_mseq_NewAddress { mseq_page_obj x y } {
  global gPB
  set canvas_frame $Page::($mseq_page_obj,canvas_frame)
  set bot_canvas $Page::($mseq_page_obj,bot_canvas)
  set x [$bot_canvas canvasx $x]
...
#=======================================================================
proc UI_PB_mseq_EditAddress { mseq_page_obj x y } {
  global gPB
  set canvas_frame $Page::($mseq_page_obj,canvas_frame)
  set bot_canvas $Page::($mseq_page_obj,bot_canvas)
  set x [$bot_canvas canvasx $x]
  ...
  "UI_PB_addr_AddRestoreCallBack $new_add_page"
  set cb_arr(gPB(nav_button,ok,Label)) \
  "UI_PB_add_MseqEditOkCallBack $mseq_page_obj \
