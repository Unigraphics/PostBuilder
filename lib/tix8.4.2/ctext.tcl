...
-linemap_select_bg]
	...
	if {$ar(-linemap) == 1} {
          #<05-08-06 peter> disabled the text widget below because it is not needed by PB.    
...
#<05-09-06 peter>add a switch to control the font
proc ctext::highlight {win start end {fnt ""}} {
	...
	#<2006-10-11 peter>
	set err ""
	catch { set org_font [lindex [$twin config -font] end] } err
	if {$err!=""} {
    #   set myfontname [font create myDefaultFont -family System -size 11 ]
    #   #option add *font myDefaultFont
	  # set org_font [ list ] 
	}
	set org_font [lrange $org_font 0 1]
	set org_font_normal [lappend org_font normal]
	set org_font_bold [lreplace $org_font end end bold]
	...
	while 1 {
		...
		if {[info exists highlightAr($word)] == 1} {
			...
			$twin tag configure $tagClass -foreground $color
                        ...
                        #<05-09-06 peter>
                        if {$fnt != ""} {
    			  $twin tag configure $tagClass -font $fnt    
                        }
	...
	foreach {ichar tagInfo} [array get highlightSpecialCharsAr] {
		...
		while 1 {
			...
			#<05-09-06 peter>
		       	$twin tag configure $tagClass -font $org_font_bold
	...
	foreach {tagClass tagInfo} [array get highlightRegexpAr] {
		...
		while 1 {
			...
			#<05-09-06 peter>
		       	$twin tag configure $tagClass -font $org_font_normal
