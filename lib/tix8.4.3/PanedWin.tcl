...
#
#
# 06-Mar-2008 gsl - Revised tixPanedWindow:BtnUp & tixPanedWindow:BtnMoveCompressed
#                   to tolerate when pane is grayed out.
#
...
proc tixPanedWindow:AddSeparator {w} {
    ...
    if {$tcl_platform(platform) eq "windows"} then {
      set relief "sunken" ;#09-20-05 peter-------was ridge-----------
      set thickness 2     ;#09-20-05 peter-------was 4---------------
    } else {
      set relief "sunken"
      set thickness 2
    }
...
proc tixPanedWindow:BtnMoveCompressed {w item {fake 0}} {
   ...
   #<03-06-08 gsl>
    if { [info exists data(curSize)] && [info exists data(beforeLimit)] && [info exists data(afterLimit)] } {
        if {$p == $data(curSize)} {
            set data(movePending) 0
    return
        }
...
proc tixPanedWindow:BtnUp {w item {fake 0}} {
   ...
   #<03-06-08 gsl> Clear data
    if { [info exists data(curSize)] } {
        unset data(curSize)
    }
}
