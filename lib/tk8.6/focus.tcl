...
#<01-11-06 peter> - add the condition to avoid the potential problem
#<Feb-27-2018 gsl> Added Peter's revision above from Tix8.4.2.
...
proc ::tk_focusFollowsMouse {} {
    ...
    if {"%d" eq "NotifyAncestor" || "%d" eq "NotifyNonlinear" \
        ...
        if {[tk::FocusOK %W]} {
#<01-11-06 peter>---add the condition to avoid the potential problem--------
#the function for Entry value validation may cause the problem
                set type [winfo class %W]
                if {![string match $type Entry] && \
                    ![string match $type Button]} {focus %W}
#-----------------------------------------------------------------------
                #focus %W
#<01-11-06 peter>----the following can be deleted
#<08-02-05 peter>---------------------make the cursor end----------------
              #  if ![string compare [winfo class %W] Entry] {%W icursor end}
#peter---------------------------------------------------------------------------
        }
    }
    }
